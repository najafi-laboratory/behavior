...
# recording parameters: easy-max deactivated; warmUp trials = 0; 100% for each difficulty
...
def audioAnalysis(file):
    ...
    peaks, _ = find_peaks(abs_audio_data, height=0.2)
    ...
    for trial in audioTrials:
        average = 0
        for value in range(4, len(trial) - 1):
            if trial[value] > 99:
                average += trial[value]
        if len(trial) > 5:
            average = round(average / (len(trial) - 5), 2)
