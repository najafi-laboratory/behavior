...
classdef PsychToolboxVideoPlayer < handle
    ...
    methods
        function obj = PsychToolboxVideoPlayer(MonitorID, ViewPortSize, ViewPortOffset, SyncPatchSize, SyncPatchYOffset)
            ...
            disp(['obj.TimerFPS:', num2str(obj.TimerFPS)]);
            ...
            disp(['CompensationFrames:', num2str(CompensationFrames)]);
                        ...
                        if obj.Videos{StimulusIndex}.nFrames == 1
                            ...
                            if obj.TimerMode == 1
                                ...
                                for iFrame = 1:obj.Videos{StimulusIndex}.nFrames
                                    ...
                                    % % modify delay of inter-frame-interval
                                    % % to stabilize sync patch (otherwise
                                    % % missing frames -> inconsistent sync
                                    % % patch pattern
                                    % IFI = 1 / obj.DetectedFrameRate;
                                    % MyDelay = IFI;
                                    % % disp('MyDelay:');
                                    % % disp(MyDelay);
                                    % pause(MyDelay);
        ...
        function playNextFrame(obj, e)
            ...
            Screen('Flip', obj.Window);
            %Screen('Flip', obj.Window, when, dontclear, dontsync, multiflip);
            %[VBLTimestamp StimulusOnsetTime FlipTimestamp Missed Beampos] = Screen('Flip', obj.Window, 0, 0, 1, 0);

            % % for debugging if there are sync patch/timing issues (maybe missed frames)
            % [VBLTimestamp StimulusOnsetTime FlipTimestamp Missed Beampos] = Screen('Flip', obj.Window);
            % global NumPosMissed;
            % if Missed > 0
            %     disp('Missed:');
            %     disp(Missed);
            %     NumPosMissed = NumPosMissed + 1;
            % end            
