%{ 
MODIFIED FROM GRATING FLASHES, MOST OF IT IS unused
...
% what is considered x and y is somewhat arbitrary, just needs to result in
% the right outcomes
XRes = 1080;
YRes = 1920;

ScreenPositionIndicator = gray(1:XRes, 1:YRes);

w = 53.136;  % width of screen, in cm
h = 29.889;  % height of screen, in cm

% make lines
horizLine = zeros(2, YRes);
vertLine = zeros(XRes, 2);

% screen resolution is even-numbered, so we need to draw marker on both
% sides of the horizontal and vertical nothing
vMid = XRes/2;
hMid = YRes/2;
ScreenPositionIndicator(vMid:vMid+1, 1:end) = horizLine;
ScreenPositionIndicator(1:end, hMid:hMid+1) = vertLine;

% add offset
DorsalOffset = 3.16; % vertical offset of monitor center dorsal to mouse eye, in cm

w_PixelPerCm = YRes/w;
h_PixelPerCm = XRes/h;

VertPixelOffset = round(h_PixelPerCm * DorsalOffset);

offsetLine = zeros(1, YRes);

ScreenPositionIndicator(vMid+VertPixelOffset, 1:end) = offsetLine;
...
gratingTexture = Screen('MakeTexture', win, ScreenPositionIndicator);
