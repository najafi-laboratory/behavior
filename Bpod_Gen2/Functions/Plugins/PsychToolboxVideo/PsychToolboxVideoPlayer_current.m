...
classdef PsychToolboxVideoPlayer < handle
    properties
        ...
        tic1 % have to throw tics into public property
    end
    properties (Access = private)
        ...
        Timer2 % Timer object that controls playback
        %tic1 % have to throw tics into public property
        toc1
    end
    methods
        function obj = PsychToolboxVideoPlayer(MonitorID, ViewPortSize, ViewPortOffset, SyncPatchSize, SyncPatchYOffset)
            ...
            % debug to check FPS
            disp(['PsychToolboxVideoPlayer.m: obj.TimerFPS:', num2str(obj.TimerFPS)]);
            ...
            % this timer is used for TimerMode = 1, it calls
            % @(x,y)obj.playNextFrame() to display a video using the input
            % timer period
            %obj.Timer = timer('TimerFcn','', 'Period', round(1/obj.TimerFPS*1000)/1000, 'ExecutionMode', 'fixedRate', 'Tag', 'PTV');
            % edit to reduce timer period -> since there is matlab
            % processing overhead, timer period must be less than frame
            % rate to deliver frames fast enough so that they aren't
            % dropped

            % add compensation frames as attempt to prevent frame dropping
            %CompensationFrames = obj.TimerFPS/2;
            %CompensationFrames = round(obj.TimerFPS/6);
            
            %CompensationFrames = round(4);
            CompensationFrames = 0;
            disp(['PsychToolboxVideoPlayer.m: CompensationFrames:', num2str(CompensationFrames)]);
            %obj.Timer = timer('TimerFcn','', 'ErrorFcn','disp("Video Stopped")', 'Period', round(1/(obj.TimerFPS+CompensationFrames)*1000)/1000, 'ExecutionMode', 'fixedRate', 'Tag', 'PTV', 'BusyMode', 'error');
            %obj.Timer = timer('TimerFcn','', 'ErrorFcn','disp("Video Stopped")', 'Period', round(1/(obj.TimerFPS+CompensationFrames)*1000)/1000, 'ExecutionMode', 'fixedRate', 'Tag', 'PTV', 'BusyMode', 'error');
            %obj.Timer = timer('TimerFcn','','Period', round(1/(obj.TimerFPS+CompensationFrames)*1000)/1000, 'ExecutionMode', 'fixedRate', 'Tag', 'PTV', 'BusyMode', 'queue');            
            %obj.Timer = timer('TimerFcn','', 'Period', round(1/obj.TimerFPS*1000)/1000, 'ExecutionMode', 'fixedRate', 'Tag', 'PTV');
            %obj.Timer = timer('TimerFcn','','Period', round(1/(obj.TimerFPS+CompensationFrames)*1000)/1000, 'ExecutionMode', 'fixedRate', 'Tag', 'PTV');
            %obj.Timer = timer('TimerFcn','','Period', round(1/(obj.TimerFPS+5)*1000)/1000, 'ExecutionMode', 'fixedRate', 'Tag', 'PTV');
            %obj.Timer = timer('TimerFcn','','Period', round(1/(obj.TimerFPS)*1000)/1000, 'ExecutionMode', 'fixedRate', 'Tag', 'PTV');
            
            % Query duration of one monitor refresh interval:
            obj.ifi = Screen('GetFlipInterval', obj.Window);
            %obj.Timer = timer('TimerFcn','','Period', ifi, 'ExecutionMode', 'fixedRate', 'Tag', 'PTV');
            %obj.Timer = timer('TimerFcn','','Period', obj.ifi, 'ExecutionMode', 'fixedRate', 'Tag', 'PTV', 'BusyMode', 'drop');

            % obj.Timer = timer('TimerFcn','','Period', obj.ifi, 'ExecutionMode', 'singleShot', 'Tag', 'PTV', 'BusyMode', 'drop');
            % obj.Timer2 = timer('TimerFcn','','Period', obj.ifi, 'ExecutionMode', 'singleShot', 'Tag', 'PTV', 'BusyMode', 'drop');
            obj.Timer = timer('TimerFcn','', 'ExecutionMode', 'singleShot', 'Tag', 'PTV', 'BusyMode', 'drop');
            obj.Timer2 = timer('TimerFcn','', 'ExecutionMode', 'singleShot', 'Tag', 'PTV', 'BusyMode', 'drop');            

            % debug to check ifi
            disp(['ifi:', num2str(obj.ifi)]);
        ...
        function play(obj, StimulusIndex)
            %obj.tic1 = tic; %% need to factor in processing overhead from whence play is called
            if StimulusIndex == 0
                Screen('DrawTexture', obj.Window, obj.BlankScreen);
                Screen('Flip', obj.Window);
            else
                    ...
                    case 0 % Play video
                        if obj.Videos{StimulusIndex}.nFrames == 1
                            ...
                            if obj.TimerMode == 1
                                ...
                                % start(obj.Timer);
                                set(obj.Timer, 'TimerFcn', @(x,y)obj.playNextFrame(true, obj.Timer, obj.Timer2));
                                set(obj.Timer2, 'TimerFcn', @(x,y)obj.playNextFrame(false, obj.Timer, obj.Timer2));
                                %obj.tic1 = tic;
                                start(obj.Timer);                                
                            else
        ...
        %     Screen('Flip', obj.Window);
        %     % for debug
        %     %[VBLTimestamp StimulusOnsetTime FlipTimestamp Missed Beampos] = Screen('Flip', obj.Window);
        % 
        ...
        % 
        %     obj.Timer.
        %     obj.Timer.Period = obj.ifi - toc;
        % end
        function playNextFrame(obj, isTimer, timer1, timer2) 
            if isTimer
                nextTimer = timer2;
            else
                nextTimer = timer1;
            ...
            % get time elapsed since previous run of this timer function
            ...
            obj.tic1 = tic; % start stopwatch to capture processing time of frame drawing function
            ...
            % can't set sub-millisecond delay for timer, set to zero
            ...
            if nextFrame > obj.Videos{obj.StimulusIndex}.nFrames
                ...
                stop(obj.Timer2);
                set(obj.Timer2, 'TimerFcn', '');
                % added reset to blankscreen since sometimes last frame is
                % dropped
                Screen('DrawTexture', obj.Window, obj.BlankScreen);
                Screen('Flip', obj.Window);
            else
