...
def plot_trial_outcome_vs_type_with_isi(session_props):
    ...
    contingency_type = session_props['Contingency']
    ...
    if warmup_end_idx >= 0:  # If there are warmup trials
        ...
        ax0.text(mid_x, 0.5, 'Warmup Trials', 
    ...
    ax0.set_yticks([1, 2])
    ax0.set_yticklabels(['Short ISI', 'Long ISI'])
    ...
    if warmup_end_idx >= 0:  # If there are warmup trials
        ...
        # Add text label for warmup area
        mid_x = warmup_end_x / 2
        ax3.text(mid_x, 0.5, 'Warmup Trials', 
                 horizontalalignment='center', 
                 verticalalignment='center',
    ...
    # count short vs long ISI trials
    short_isi_count = sum(1 for t_type in trial_types if t_type == 1)
    long_isi_count = len(trial_types) - short_isi_count
    isi_labels = ['Short ISI Trials', 'Long ISI Trials']
    isi_data = [short_isi_count, long_isi_count]
    isi_colors = ['#3498db', '#e74c3c']  # Blue and Red

    # plot
    ax7 = fig.add_subplot(gs[4, 1])
    create_donut_pie_chart(isi_data, isi_labels, ax7, 
                           colors=isi_colors, inner_radius=0.8, title= 'Short vs Long ISI Trials')
