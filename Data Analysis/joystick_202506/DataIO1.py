    ...
    for f in range(len(file_names)):
        ...
        for i in range(nTrials-Exluded):
            ...
            if outcome == 'DidNotPress1':
                ...
                if len(positions[late_index:-1]) > 0 :
                    if max(positions[late_index:-1]) > trial_target_thresh[-1]:
                        outcome = 'LatePress1'
                        latetime = times[np.where(positions[late_index:-1]> trial_target_thresh[-1])[0][0]+late_index]
