# -*- coding: utf-8 -*-
"""
Created on Wed Apr 30 13:51:00 2025
...
def plot_dist(axs, delay_data, color_tag, alpha = 0.2):
    ...
    bin_end = np.linspace(percentile_90, np.nanmax(delay_data), 10)
...
def run_grand_trajectory(axs, session_data , block_data , probe_ind , st = 0):
    ...
    plot_grand_trajectory(axs[0], dates1 , time_reward , np.array(trajectory_reward_short) ,probe_ind , title_str[probe_ind] ,'Short Rewarded Trials' , 1 , 1)
    plot_grand_trajectory(axs[1], dates1 , time_reward , np.array(trajectory_reward_long) , probe_ind  , title_str[probe_ind] ,'Long Rewarded Trials' , 0 , 1)        
    axs[0].set_xlabel('Time from Press2 (s)')
    axs[1].set_xlabel('Time from Press2 (s)')
...
def run_grand_trajectory_first_press(axs, session_data,probe_ind, st):
    ...
    for i in plotting_sessions:
       ...
       for j in range(len(outcome_sess)):
           outcome = outcome_sess[j]
           if j+probe_sign[probe_ind] > 0 and j+probe_sign[probe_ind]<len(outcome_sess):
               if outcome in ('Reward' ,'DidNotPress2', 'latePress2', 'EarlyPress2'):
    ...
    title_str = ['Probe-1' , 'Probe' , 'Probe+1']
    plot_grand_trajectory(axs[0], dates1 , time_reward , np.array(trajectory_reward_short) ,probe_ind , title_str[probe_ind] ,'All Short Trials' , 1 , 1, 1)
    plot_grand_trajectory(axs[1], dates1 , time_reward , np.array(trajectory_reward_long) , probe_ind  , title_str[probe_ind] ,'All Long Trials' , 0 , 1, 1) 
    axs[0].set_xlabel('Time from Press1 (s)')
    axs[1].set_xlabel('Time from Press1 (s)')
...
def run_grand_trajectory_first_press_rewarded(axs, session_data,probe_ind, st):
    ...
    title_str = ['Probe-1' , 'Probe' , 'Probe+1']
    plot_grand_trajectory(axs[0], dates1 , time_reward , np.array(trajectory_reward_short) ,probe_ind , title_str[probe_ind] ,'Short Rewarded Trials' , 1 , 1, 1)
    plot_grand_trajectory(axs[1], dates1 , time_reward , np.array(trajectory_reward_long) , probe_ind  , title_str[probe_ind] ,'Long Rewarded Trials' , 0 , 1, 1) 
    axs[0].set_xlabel('Time from Press1 (s)')
    axs[1].set_xlabel('Time from Press1 (s)')
...
def run_grand_dist(axs, session_data , block_data , st = 0):
    ...
    plot_dist(axs[0], np.array(short_delay_preprobe), 'gold', alpha = 1)
    plot_dist(axs[0], np.array(short_delay_probe), 'k', alpha = 1)
    plot_dist(axs[0], np.array(short_delay_postprobe), 'dodgerblue', alpha = 1)
    
    plot_dist(axs[1], np.array(long_delay_preprobe), 'gold', alpha = 1)
    plot_dist(axs[1], np.array(long_delay_probe), 'k', alpha = 1)
    plot_dist(axs[1], np.array(long_delay_postprobe), 'dodgerblue', alpha = 1)