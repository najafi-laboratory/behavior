...
def plot_psychometric_opto_avg(M, config, subjectIdx, sessionIdx, figure_id=None, show_plot=1):
    # figure meta
    rowspan, colspan = 2, 2
    ...
    if sessionIdx != -1:
        ...
        # Now you can access:
        # region_combined_dfs['LIntA'] → df for LIntA sessions
        # region_combined_dfs['mPFC'] → df for mPFC sessions        
    ...
    sem_greens = generate_green_colors(num_colors, shift=100)
    
    mean_blues = generate_blue_colors(num_colors)
    sem_blues = generate_blue_colors(num_colors, shift=100)    
    
    mean_purples = generate_purple_colors(num_colors)
    sem_purples = generate_purple_colors(num_colors, shift=100)  
    ...
    for region, data in region_results.items():
        ...
        if region in left_regions:
            ...
            control_mean_color = mean_blues[-1]
            control_sem_color = sem_blues[-1]
            
            mean_color = mean_blues[0]
            sem_color = sem_blues[0]           
            ...
            control_mean_color = mean_greens[-1]
            control_sem_color = sem_greens[-1]
            
            mean_color = mean_greens[0]
            sem_color = sem_greens[0]       
        else:
            ...
            control_mean_color = mean_purples[-1]
            control_sem_color = sem_purples[-1]
            
            mean_color = mean_purples[0]
            sem_color = sem_purples[0]            
        ...
        ax.fill_between(
            bin_isi_opto[0],
            bin_mean_opto[0] - bin_sem_opto[0],
            bin_mean_opto[0] + bin_sem_opto[0],
            color=sem_color,
            alpha=0.2
        )    
    ...
    if start_from=='start_date':
        ...
        if not is_avg:
            ...
            ax.set_title('Average Opto Psychometric Function ' + dates[0] + '-' + dates[-1])
    ...
    figure_id = f"{subject}_psychometric_opto_avg"
    file_ext = '.pdf'
