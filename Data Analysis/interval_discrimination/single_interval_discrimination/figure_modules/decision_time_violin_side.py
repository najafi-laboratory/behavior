...
def plot_decision_time_violin_side(M, config, subjectIdx, sessionIdx=-1, figure_id=None, show_plot=1, opto=False, side='both'):
    # figure meta
    rowspan, colspan = 2, 2
    ...
    # sns.stripplot(x='outcome', y='reaction_time', hue='condition', data=filtered_df, color='black', alpha=0.3, jitter=True)   
    sns.violinplot(x='trial_type', y='reaction_time', hue='condition', data=filtered_df, order=['Left', 'Right'])  # this sets the order *on the figure*)
        
    # Add labels and title
    plt.xlabel('Trial Side')                # or something like 'Choice Side'
    plt.ylabel('Reaction Time (s)')         # or 'Decision Time (s)'
    plt.title('Reaction Time by Side and Condition')
    plt.legend(title='Condition', loc='best')  # or loc='upper right', etc.
    ...
    figure_id = f"{subject}_decision_time_violin_side"
    file_ext = '.pdf'
