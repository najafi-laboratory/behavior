...
#     ax.set_ylim(np.min(y), np.max(y))
    ...
    # lc = LineCollection(segments, linewidths=linewidths, colors=colors, label=label)
    # ax.add_collection(lc)
    ...
    ax.add_collection(lc)
    # ax.legend(handles.append(legend_handle))    
...
def plot_decision_time_kernel_opto(M, config, subjectIdx, sessionIdx=-1, figure_id=None, show_plot=1, opto=False, side='both'):
    # figure meta
    rowspan, colspan = 2, 2
    ...
    if opto:
        ...
        opto_response_effect = control_response_curve - opto_response_curve
        if side == 'both':
            ...
            opto_response_curve_left, opto_percent_density_left = response_curve(opto_df[(opto_df['trial_side'] == 'left')], t_vals, bw_method=1.0)
            opto_response_effect_left = control_response_curve_left - opto_response_curve_left
        elif side == 'right':
            opto_response_curve_right, opto_percent_density_right = response_curve(opto_df[(opto_df['trial_side'] == 'right')], t_vals, bw_method=1.0)    
            opto_response_effect_right = control_response_curve_right - opto_response_curve_right
    ...
    plot_variable_linewidth(t_vals, control_response_curve, control_percent_density, ax, color='black', label='Control', handles=legend_handles)    
    ...
    if side == 'both':
        ...
        plot_variable_linewidth(t_vals, control_response_curve_left, control_percent_density_left, ax, color='blue', label='Control Left', handles=legend_handles)            
        plot_variable_linewidth(t_vals, control_response_curve_right, control_percent_density_right, ax, color='red', label='Control Right', handles=legend_handles)        
    elif side == 'left':
        # ax.plot(
        #     t_vals,
        #     control_response_curve_left,
        ...
        #     label='Control Left',
        #     markersize=2)        
        plot_variable_linewidth(t_vals, control_response_curve_left, control_percent_density_left, ax, color='blue', label='Contro Leftl', handles=legend_handles)           
        plot_variable_linewidth(t_vals, control_response_curve_left, control_percent_density_left, ax, color='blue', label='Contro Leftl', handles=legend_handles)           
    elif side == 'right':
        # ax.plot(
        #     t_vals,
    ...
    if opto:
        ...
        plot_variable_linewidth(t_vals, opto_response_curve, opto_percent_density, ax, color='violet', label='Opto', handles=legend_handles)           
        if side == 'both':  
            # ax.plot(
            #     t_vals,
            ...
            plot_variable_linewidth(t_vals, opto_response_effect, opto_percent_density_left, ax, color='violet', label='Control - Opto', handles=legend_handles)     
            ...
            plot_variable_linewidth(t_vals, opto_response_curve_left, opto_percent_density_left, ax, color='lightblue', label='Opto Left', handles=legend_handles)                           
        elif side == 'right':            
            # ax.plot(
            #     t_vals,
            #     opto_response_curve_right,
            ...
            #     label='Opto Right',
            #     markersize=2)    
            plot_variable_linewidth(t_vals, opto_response_curve_right, opto_percent_density_right, ax, color='lightcoral', label='Opto Right', handles=legend_handles)                             
    
    ax.hlines(
        [0.25, 0.5, 0.75], 0.0, max_time,
    ...
    ax2.set_yticks([0.25, 0.50, 0.75, 1])
    

    # ax.legend(loc='upper right', ncol=1, bbox_to_anchor=(1, 1))
    ax.set_title('Average Decision Time Curve ' + date_str)
    ...
    # labels = labels + [e.get_label() for e in extra]
    ...
    figure_id = f"{subject}_decision_time_curve_opto"
    file_ext = '.pdf'
