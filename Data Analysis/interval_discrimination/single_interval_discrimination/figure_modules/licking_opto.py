# -*- coding: utf-8 -*-
"""
Created on Mon Apr 14 11:32:36 2025
...
def plot_licking_opto(M, config, subjectIdx, figure_id=None, show_plot=1):
    # figure meta
    rowspan, colspan = 4, 8
    fig_size = get_figsize_from_pdf_spec(rowspan, colspan, config['pdf_spec']['pdf_pg_licking'])    
    ...
    for i in range(len(dates)):
        ...
        print(f"Plotting lick traces...{subject}...{date}")
        if debug:
        ...
        figure_id = f"{subject}_licking_opto_{dates[i]}"
        file_ext = '.pdf'
        ...
        #     'figure_id': figure_id,
        #     'path': out_path,
        #     'caption': f"Licking opto plot for {subject}",
        #     'subject': subject,
        #     'tags': ['performance', 'bias'],
        #     "layout": {
        #       "page": page_offset,
        #       "page_key": f"pdf_pg_licking_{dates[i]}",
        #       "grid_size": (4, 8),
        #       "row": 0,
        #       "col": 0,
        #       "rowspan": rowspan,
    ...
    fig, axs = plt.subplots(nrows=nrows, ncols=2, figsize=(40, 30), gridspec_kw={'hspace': 0.4} )     
    ...
    for i in range(len(dates)):
        ...
        outcome_time = outcomes_time[i]
        session_date = dates[i]
        print(f"Plotting session avg lick traces...{subject}...{date}")
    ...
    subject = config['list_config'][subjectIdx]['subject_name']
    output_dir = os.path.join(config['paths']['figure_dir_local'] + subject)
    figure_id = f"{subject}_avg_licks"
    file_ext = '.pdf'
    filename = figure_id + file_ext
    os.makedirs(output_dir, exist_ok=True)
    out_path = os.path.join(output_dir, filename)
    ...
    # # if figure_id is None:
    #     # figure_id = f"{subject}_avg_licking_opto{sessionIdx}"
    # figure_id = f"{subject}_avg_licking_opto"        
