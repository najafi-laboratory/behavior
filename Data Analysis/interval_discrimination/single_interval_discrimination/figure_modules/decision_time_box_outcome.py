...
def plot_decision_time_box_outcome(M, config, subjectIdx, sessionIdx=-1, figure_id=None, show_plot=1, opto=False, side='both'):
    # figure meta
    rowspan, colspan = 2, 2
    ...
    sns.boxplot(x='outcome', y='reaction_time', hue='condition', data=filtered_df)
    # sns.boxplot(x='outcome', y='reaction_time', hue='condition', data=filtered_df,
            # showcaps=False, showfliers=False)    
    sns.stripplot(x='outcome', y='reaction_time', hue='condition', data=filtered_df, 
              dodge=True, jitter=True, alpha=0.4)        
    # sns.stripplot(x='outcome', y='earliest_lick', data=control_df, color='black', alpha=0.3, jitter=True)
    ...
    figure_id = f"{subject}_decision_time_box_outcome"
    file_ext = '.pdf'
