...
def segment_trials_with_voltage_sync(dff, frame_times, trials_df, voltage_time, voltage_sync_signal, voltage_stim, threshold=0.5, ops=None, bruker_xml_path=None):
    ...
    trials_df = match_trials_with_scope_data(trials_df, scope_dffs)
...
def kmeans_cluster_neurons(neuron_averages, k=5, normalize=True, random_seed=43):
    print(f"[CLUSTERING] Performing k-means with k={k}.")

    np.random.seed(random_seed)  # <--- Add this for deterministic behavior

    print(f"[Q] Set random seed {random_seed}.")
...
def plot_cluster_heatmap(common_time, neuron_averages, cluster_df, time_window, plot_title, align_line_label, events_to_plot, trials_df, save_filename, custom_time_window=None):
    plt.figure(figsize=(12, 8))
    sorted_df = cluster_df.sort_values('cluster')
    plt.imshow(neuron_averages[sorted_df['neuron_idx']], aspect='auto', extent=[common_time[0], common_time[-1], 0, neuron_averages.shape[0]], origin='lower', cmap='viridis')
    plt.colorbar(label='dF/F')
    ...
    subject_id = trials_df.iloc[0]['mouse_id']
    session_date = trials_df.iloc[0]['date']

    unique_name=f"{save_filename}"
    
    save_current_figure_as_pdf(
...
def plot_cluster_averages(common_time, cluster_avg_traces, cluster_sem_traces, time_window, plot_title, align_line_label, events_to_plot, trials_df, save_filename, custom_time_window=None):
    plt.figure(figsize=(12, 8))
    colors = plt.cm.tab10(np.linspace(0, 1, len(cluster_avg_traces)))
...
def generate_full_report(trials_df, conditions, alignments, events_to_plot, k_clusters=5, time_window=[-10, 10], custom_time_windows=None):
    for condition_name, condition_func in conditions.items():
        ...
        for alignment_name, align_col in alignments.items():
            ...
            save_filename = f"Clustered_Heatmap_{condition_name}_{alignment_name}"
            ...
            save_filename = f"Cluster_Averages_{condition_name}_{alignment_name}"
...
def resample_to_common_time(common_time_ref, common_time_target, traces_target):
    resampled = np.zeros((traces_target.shape[0], len(common_time_ref)))
    for i in range(traces_target.shape[0]):
        resampled[i, :] = np.interp(common_time_ref, common_time_target, traces_target[i, :], left=np.nan, right=np.nan)
    return resampled

# def overlay_traces_by_cluster(common_time_left, common_time_right, neuron_traces_left, neuron_traces_right, title_prefix=""):
#     plt.figure(figsize=(14, 8))
#     colors = plt.cm.tab10(np.linspace(0, 1, len(neuron_traces_left)))

#     for (cluster_id, traces_left), (_, traces_right), color in zip(neuron_traces_left.items(), neuron_traces_right.items(), colors):
#         traces_right_resampled = resample_to_common_time(common_time_left, common_time_right, traces_right)

#         mean_trace_left = np.nanmean(traces_left, axis=0)
#         sem_trace_left = np.nanstd(traces_left, axis=0) / np.sqrt(traces_left.shape[0])

#         mean_trace_right = np.nanmean(traces_right_resampled, axis=0)
#         sem_trace_right = np.nanstd(traces_right_resampled, axis=0) / np.sqrt(traces_right_resampled.shape[0])

#         plt.plot(common_time_left, mean_trace_left, label=f"Cluster {cluster_id} Left", color=color, linestyle='-')
#         plt.plot(common_time_left, mean_trace_right, label=f"Cluster {cluster_id} Right", color=color, linestyle='--')
...
# def overlay_traces_by_cluster_individual(common_time_left, common_time_right, neuron_traces_left, neuron_traces_right, title_prefix=""):
#     colors = plt.cm.tab10(np.linspace(0, 1, len(neuron_traces_left)))
...
#         plt.plot(common_time_left, mean_trace_left, label=f"Cluster {cluster_id} Left", color=color, linestyle='-')
#         plt.fill_between(common_time_left, mean_trace_left - sem_trace_left, mean_trace_left + sem_trace_left, color=color, alpha=0.3)

#         plt.plot(common_time_left, mean_trace_right, label=f"Cluster {cluster_id} Right", color=color, linestyle='--')
#         plt.fill_between(common_time_left, mean_trace_right - sem_trace_right, mean_trace_right + sem_trace_right, color=color, alpha=0.3)

#         plt.axvline(x=0, color='purple', linestyle='--', linewidth=2)
#         plt.xlabel("Time (s)")
#         plt.ylabel("dF/F")
#         plt.title(f"{title_prefix} Cluster {cluster_id} Left vs Right")
#         plt.legend()
#         plt.grid(True)
#         plt.show()

def overlay_traces_by_cluster(common_time_left, common_time_right, neuron_traces_left, neuron_traces_right, df, save_filename, title_prefix=""):
    plt.figure(figsize=(14, 8))
    colors = plt.cm.tab10(np.linspace(0, 1, len(neuron_traces_left)))
    ...
    for (cluster_id, traces_left), (_, traces_right), color in zip(neuron_traces_left.items(), neuron_traces_right.items(), colors):
        ...
        plt.plot(common_time_left, mean_trace_left, label=f"Cluster {cluster_id} Left (n={traces_left.shape[0]})", color=color, linestyle='-')
        plt.plot(common_time_left, mean_trace_right, label=f"Cluster {cluster_id} Right (n={traces_right.shape[0]})", color=color, linestyle='--')
    ...
    subject_id = df.iloc[0]['mouse_id']
    session_date = df.iloc[0]['date']

    unique_name=f"{save_filename}_clusters_combine"
...
def overlay_traces_by_cluster_individual(common_time_left, common_time_right, neuron_traces_left, neuron_traces_right, df, save_filename, title_prefix=""):
    colors = plt.cm.tab10(np.linspace(0, 1, len(neuron_traces_left)))
    ...
    for (cluster_id, traces_left), (_, traces_right), color in zip(neuron_traces_left.items(), neuron_traces_right.items(), colors):
        ...
        plt.plot(common_time_left, mean_trace_left, label=f"Left (n={traces_left.shape[0]})", color=color, linestyle='-')
        plt.fill_between(common_time_left, mean_trace_left - sem_trace_left, mean_trace_left + sem_trace_left, color=color, alpha=0.3)

        plt.plot(common_time_left, mean_trace_right, label=f"Right (n={traces_right.shape[0]})", color=color, linestyle='--')
        plt.fill_between(common_time_left, mean_trace_right - sem_trace_right, mean_trace_right + sem_trace_right, color=color, alpha=0.3)
        ...
        unique_name=f"{save_filename}_cluster_{cluster_id}"
...
def plot_comparison_overlays(extracted, df, save_filename='', clusters_to_plot=None):
    for cluster_id in clusters_to_plot if clusters_to_plot else extracted[list(extracted.keys())[0]][1].keys():
        ...
        unique_name=f"{save_filename}_cluster_{cluster_id}_adaptation_with_flashes_fixed"
...
def plot_cluster_adaptation_heatmaps(common_time, aligned_trials, clusters_neuron_ids, cluster_label_prefix="Cluster", cmap='viridis', save=False, output_folder="plots"):
    """
    For each cluster, plot heatmap of per-trial average traces across session.
    
    Args:
        common_time (np.ndarray): shared time axis
        aligned_trials (pd.DataFrame): trials with aligned_scope_time and scope_dff
        clusters_neuron_ids (dict): cluster_id -> list of neuron indices
        cluster_label_prefix (str): prefix for cluster labels
        cmap (str): matplotlib colormap
        save (bool): whether to save plots
        output_folder (str): folder to save PDFs if save=True
    """
    ...
    for cluster_id, neuron_ids in clusters_neuron_ids.items():
        ...
        for idx, row in aligned_trials.iterrows():
            if row['scope_dff'] is None or row['aligned_scope_time'] is None:
                continue
            scope_time = row['aligned_scope_time']
            scope_dff = row['scope_dff']

            if idx < 10:
                print(f"[DEBUG] [Without flash] First aligned scope_time min/max: {np.nanmin(scope_time):.2f} / {np.nanmax(scope_time):.2f}")
                print(f"[DEBUG] [Without flash] Common time: {common_time[0]:.2f} -> {common_time[-1]:.2f}")


            # Select neurons in cluster
            cluster_traces = scope_dff[neuron_ids, :]
            mean_trace = np.nanmean(cluster_traces, axis=0)

            interp_trace = np.interp(common_time, scope_time, mean_trace, left=np.nan, right=np.nan)
            trial_traces.append(interp_trace)

        if len(trial_traces) == 0:
            print(f"[WARNING] No valid trials for Cluster {cluster_id}. Skipping.")
            continue
        ...
        # --- Plotting ---
        plt.figure(figsize=(10, 6))
        plt.imshow(trial_traces, aspect='auto', extent=[common_time[0], common_time[-1], 0, trial_traces.shape[0]], origin='lower', cmap=cmap)
        plt.colorbar(label='Mean dF/F across neurons')
        plt.axvline(0, color='purple', linestyle='--', linewidth=2)
        plt.xlabel('Time (s)')
        plt.ylabel('Trial #')
        plt.title(f"{cluster_label_prefix} {cluster_id} Adaptation Heatmap")
        plt.grid(False)

        if save:
            filename = os.path.join(output_folder, f"cluster_adaptation_heatmap_{cluster_id}.pdf")
...
def plot_cluster_adaptation_heatmaps_with_mean(common_time, aligned_trials, clusters_neuron_ids, cluster_label_prefix="Cluster", cmap='viridis', save=False, output_folder="plots"):
    """
    Plot heatmaps + overlaid smoothed trial-mean traces for each cluster.
    """
    if save and not os.path.exists(output_folder):
        os.makedirs(output_folder)
    ...
    for cluster_id, neuron_ids in clusters_neuron_ids.items():
        ...
        # Overlay smoothed trial mean
        trial_means = np.nanmean(trial_traces, axis=1)
        trial_means_smoothed = gaussian_filter1d(trial_means, sigma=2)
        plt.plot(common_time[np.argmax(~np.isnan(trial_traces[0])):np.argmax(~np.isnan(trial_traces[-1]))], trial_means_smoothed, color='black', linewidth=2, label='Smoothed Trial Avg')

        plt.xlabel('Time (s)')
        plt.ylabel('Trial #')
        plt.title(f"{cluster_label_prefix} {cluster_id} Adaptation Heatmap + Mean")
        plt.grid(False)
        plt.legend()

        if save:
            filename = os.path.join(output_folder, f"cluster_adaptation_with_mean_{cluster_id}.pdf")
            plt.savefig(filename, bbox_inches='tight')
            plt.close()
            print(f"[SAVED] {filename}")
        else:
...
# def plot_cluster_adaptation_heatmaps_with_flashes(common_time, aligned_trials, clusters_neuron_ids, cluster_label_prefix="Cluster", cmap='viridis', save=False, output_folder="plots"):
#     """
#     Plot cluster adaptation heatmaps, adding mean visual stimulus flash timing as blue bars.
#     """
#     if save and not os.path.exists(output_folder):
#         os.makedirs(output_folder)
...
#         for flashes in aligned_trials['aligned_flash_times']:
#             if isinstance(flashes, (list, tuple)) and len(flashes) >= 2:
#                 flash1, flash2 = flashes[0], flashes[1]
#                 if not np.isnan(flash1[0]) and not np.isnan(flash1[1]):
#                     flash_starts_1.append(flash1[0])
#                     flash_ends_1.append(flash1[1])
#                 if not np.isnan(flash2[0]) and not np.isnan(flash2[1]):
#                     flash_starts_2.append(flash2[0])
#                     flash_ends_2.append(flash2[1])

#         ymin, ymax = plt.ylim()

#         if flash_starts_1 and flash_ends_1:
#             plt.fill_betweenx([ymin, ymax], np.nanmean(flash_starts_1), np.nanmean(flash_ends_1), color='red', alpha=0.2)
#         if flash_starts_2 and flash_ends_2:
#             plt.fill_betweenx([ymin, ymax], np.nanmean(flash_starts_2), np.nanmean(flash_ends_2), color='red', alpha=0.2)

#         plt.xlabel('Time (s)')
#         plt.ylabel('Trial #')
#         plt.title(f"{cluster_label_prefix} {cluster_id} Adaptation Heatmap + Flash Timing")
#         plt.grid(False)

#         if save:
#             filename = os.path.join(output_folder, f"cluster_adaptation_with_flashes_{cluster_id}.pdf")
#             plt.savefig(filename, bbox_inches='tight')
#             plt.close()
#             print(f"[SAVED] {filename}")
#         else:
...
def plot_cluster_adaptation_heatmaps_with_flashes_fixed(aligned_trials, common_time, cluster_df, clusters_neuron_ids, save_folder="plots", save_pdf=True, save_filename=''):
    print("[PLOTTING] Cluster adaptation heatmaps with flashes (corrected alignment)")
    ...
    for cluster_id, neuron_ids in clusters_neuron_ids.items():
        ...
        for idx, row in aligned_trials.iterrows():
            scope_time = row['aligned_scope_time'] if 'aligned_scope_time' in row else row['scope_time']
            scope_dff = row['scope_dff']
            flashes = row.get('aligned_flash_times', None)

            if scope_time is None or scope_dff is None:
                continue

            if idx < 10:
                print(f"[DEBUG] First aligned scope_time min/max: {np.nanmin(scope_time):.2f} / {np.nanmax(scope_time):.2f}")
                print(f"[DEBUG] Common time: {common_time[0]:.2f} -> {common_time[-1]:.2f}")

            # Average over neurons in the cluster
            cluster_trace = np.nanmean(scope_dff[neuron_ids, :], axis=0)
            interp_trace = np.interp(common_time, scope_time, cluster_trace, left=np.nan, right=np.nan)
            avg_traces.append(interp_trace)

            if flashes is not None:
                trial_flashes.append(flashes)
        ...
        plt.figure(figsize=(12, 8))
        plt.imshow(avg_traces, aspect='auto', extent=[common_time[0], common_time[-1], 0, avg_traces.shape[0]], cmap='viridis', origin='lower')
        plt.colorbar(label='Mean dF/F (Cluster Avg)')
        plt.axvline(x=0, color='purple', linestyle='--', linewidth=2)

        # Now plot mean flashes
        first_flash_starts = []
        first_flash_ends = []
        second_flash_starts = []
        second_flash_ends = []

        for flashes in trial_flashes:
            if isinstance(flashes, list) and len(flashes) >= 2:
                flash1 = flashes[0]
                flash2 = flashes[1]
                if not np.isnan(flash1[0]) and not np.isnan(flash1[1]):
                    first_flash_starts.append(flash1[0])
        ...
        if first_flash_starts and first_flash_ends:
            plt.fill_betweenx([ymin, ymax], np.mean(first_flash_starts), np.mean(first_flash_ends), color='red', alpha=0.3)
        if second_flash_starts and second_flash_ends:
            plt.fill_betweenx([ymin, ymax], np.mean(second_flash_starts), np.mean(second_flash_ends), color='red', alpha=0.3)

        plt.title(f"Cluster {cluster_id} Across Adaptation Session with Flashes (n={len(neuron_ids)} neurons)")
        plt.xlabel("Time (s)")
        plt.ylabel("Trial #")
        plt.grid(True)

        if save_pdf:
            filename = os.path.join(save_folder, f"cluster_{cluster_id}_adaptation_with_flashes_fixed.pdf")
            plt.savefig(filename, bbox_inches='tight')
            print(f"[SAVED] {filename}")

        

        subject_id = aligned_trials.iloc[0]['mouse_id']
        session_date = aligned_trials.iloc[0]['date']
...
def match_trials_with_scope_data(trials_df, scope_dffs):
    if len(scope_dffs) < len(trials_df):
        trials_df = trials_df.iloc[:len(scope_dffs)].copy()
        print(f"[INFO] Dropped excess trials. Now {len(trials_df)} trials match {len(scope_dffs)} scope entries.")
    return trials_df

def save_current_figure_as_pdf(unique_name, subject_id, session_date, base_folder="D:\FIGS", verbose=True):
    """
    Save the current active matplotlib figure as a PDF.
    
    Args:
        unique_name (str): A unique filename for the figure.
        subject_id (str): Subject or mouse ID.
        session_date (str): Session date (format YYYYMMDD or similar).
        base_folder (str): Base folder path to save under.
        verbose (bool): Whether to print save path.
    """
    save_path = os.path.join(base_folder, subject_id, session_date)
    ...
    plt.savefig(f"{base_name}.png", format="png", bbox_inches="tight")
    ...
    if verbose:
        print(f"[SAVED] Figure saved to {full_path}")

def combine_pdfs_in_folder(folder_path, output_filename="combined_neural_report.pdf"):
    """
    Combines all PDF files in a folder into a single PDF.
    
    Args:
        folder_path (str): Path to the folder containing PDFs.
        output_filename (str): Output filename for the combined PDF.
    """
    pdfs = [f for f in os.listdir(folder_path) if f.endswith('.pdf')]
    pdfs.sort()  # Sort alphabetically (optional)
...
def neural_report(M, config, subjectIdx, sessionIdx=-1, figure_id=None, show_plot=1):
    ...
    # suite2p_path = 'D:/git/2p_imaging/passive_interval_oddball_202412/results/TS03/TS03_CRBL_20250425_2AFC'
    # xml_file = os.path.join(suite2p_path, 'TS03_CRBL_20250425_2AFC-169.xml')    
    
    # suite2p_path = 'D:/git/2p_imaging/passive_interval_oddball_202412/results/TS03/YH24LG_CRBL_crux1_20250425_2afc'
    # xml_file = os.path.join(suite2p_path, 'YH24LG_CRBL_crux1_20250425_2afc-178.xml')        
    ...
    # check/drop after segmentation so that traces are aligned with proper index
    trials_df = trials_df[trials_df['trial_stop'] <= 13].reset_index(drop=True)  # drop trials that are longer than should be
        # raw_voltages = {k: f['raw'][k][:] for k in f['raw'].keys()}
    ...
    overlay_traces_by_cluster(
        ...
        neuron_traces_right,
        trials_df,
        save_filename=f"Rewarded_stim_start_left_right_cluster",         
        title_prefix="Rewarded - Stim Start Aligned -"
    ) 

    overlay_traces_by_cluster_individual(
        common_time_left,  # or common_time_right (should be close)
        common_time_right,
        neuron_traces_left,
        neuron_traces_right,
    ...
    # --- Step 3: Cluster on Left Trials ---
    cluster_df_left = kmeans_cluster_neurons(neuron_averages_left, k=11)
    ...
    #     neuron_traces_right,
    #     title
    #     _prefix="Rewarded - Choice Start Aligned -"
    # )
    overlay_traces_by_cluster(
        common_time_left,  # or common_time_right (should be close)
        common_time_right,
        ...
        neuron_traces_right,
        trials_df,
        save_filename=f"Rewarded_choice_start_left_right_cluster",
    ...
    for condition_name, condition_filter in conditions.items():
        ...
        filtered_trials = trials_df[condition_filter].reset_index(drop=True)
    
        # Align
        aligned_trials = align_trials(filtered_trials, align_to_time_column='stim_start_bpod')
    
        # Compute neuron averages
        common_time, neuron_averages = compute_neuron_averages(aligned_trials)
    
        # Cluster (you could adjust k here if needed)
        cluster_df = kmeans_cluster_neurons(neuron_averages, k=5)
        clusters_neuron_ids = extract_cluster_neuron_ids(cluster_df)
    
        # Plot and Save Heatmaps
        plot_cluster_adaptation_heatmaps(
            common_time,
            aligned_trials,
            clusters_neuron_ids,
            cluster_label_prefix=f"Rewarded - Left - Choice Start Aligned - Cluster",
            save=False,
            output_folder=f"plots/adaptation_heatmaps_{condition_name}"
        )    
    ...
    for condition_name, condition_filter in conditions.items():
        ...
        # Align
        align_to_time_column='stim_start_bpod'
        aligned_trials = align_trials(filtered_trials, align_to_time_column=align_to_time_column)
        ...
        plot_cluster_adaptation_heatmaps_with_flashes_fixed(
            aligned_trials=aligned_trials, 
            common_time=common_time, 
            cluster_df=cluster_df, 
            clusters_neuron_ids=clusters_neuron_ids, 
            save_folder="plots_adaptation_with_flashes", 
            save_pdf=False,
            save_filename=f"{condition_name}_{align_to_time_column}_"
        )        
    
        # Assume you have common_time, aligned_trials, clusters_neuron_ids
        # plot_cluster_adaptation_heatmaps_with_mean(
        #     common_time_left,
        #     aligned_left,
        #     clusters_neuron_ids,
        #     cluster_label_prefix="Rewarded - Left - Choice Start Aligned - Cluster",
        #     save=False,
        #     output_folder="plots/adaptation_with_mean_left_rewarded"
        # )   
        # Assume you have common_time, aligned_trials, clusters_neuron_ids
        # plot_cluster_adaptation_heatmaps_with_flashes(
        #     common_time_left,
        #     aligned_left,
        #     clusters_neuron_ids,
        #     cluster_label_prefix="Rewarded Left Cluster",
        #     save=False,
        #     output_folder="plots/adaptation_with_flashes_left_rewarded"
        # )   
    ...
    plot_comparison_overlays(extracted_traces, left_trials_df, 'Left_Right_ROI_Cluster_Comparison')
    ...
    save_path = os.path.join(base_folder, subject_id, session_date)
    os.makedirs(save_path, exist_ok=True)
    report_path = os.path.normpath(save_path) 
    combine_pdfs_in_folder(report_path)
