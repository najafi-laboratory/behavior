...
def run(ax, subject_session_data, start_from='std'):
    ...
    bin_mean_fix, bin_sem_fix, bin_isi_fix, trials_per_bin_fix  = get_bin_stat(decision_fix)
    bin_mean_jitter, bin_sem_jitter, bin_isi_jitter, trials_per_bin_jitter = get_bin_stat(decision_jitter)
    bin_mean_chemo, bin_sem_chemo, bin_isi_chemo, trials_per_bin_chemo = get_bin_stat(decision_chemo)
    bin_mean_opto, bin_sem_opto, bin_isi_opto, trials_per_bin_opto = get_bin_stat(decision_opto)
    ax.plot(
        bin_isi_fix,
        trials_per_bin_fix,
    ...
    ax.vlines(750, 0.0, np.max(trials_per_bin_fix) + 10, linestyle=':', color='grey')
    ax.tick_params(tick1On=False)
    ax.tick_params(axis='x', rotation=45)    
    ax.spines['right'].set_visible(False)
    ax.spines['top'].set_visible(False)
    ax.set_xlim([-50,1600])
    ax.set_ylim([0,np.max(trials_per_bin_fix) + 10])
    ...
    if start_from=='start_date':
        ax.set_title('average isi distribution from ' + start_date)
    elif start_from=='non_naive':
        ax.set_title('average isi distribution non-naive')
    else:
        ax.set_title('average isi distribution')