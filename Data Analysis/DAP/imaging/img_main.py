...
def _load_npy_safe(path: str, memmap: bool) -> Optional[np.ndarray]:
    ...
    if memmap:
        # For memory mapping, pass the path directly (no file handle)
        arr = np.load(path, allow_pickle=True, mmap_mode='r')
        print(f"    memmap view: {arr.shape} {arr.dtype}")
        return arr
    else:
        # For in-memory loading, you can use either approach
...
def load_suite2p(cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    data["iscell"] = _load_npy_safe(os.path.join(folder, "iscell.npy"), memmap)
...
def subset_cells(data: Dict[str, Any], cfg: Dict[str, Any]) -> Dict[str, Any]:
    ...
    keep_all = cfg["selection"]["keep_all"]
    ...
    if iscell.ndim != 2 or iscell.shape[1] < 2:
        ...
        print("  Keep all rois")
        thresh = -100
        mask = (iscell[:, 0] > thresh) & (iscell[:, 1] > thresh)
    else:
    ...
    if not keep_all:        
        ...
        print("  keep_all is True, skipping filtering of F, Fneu, spks")
        for k in ("F", "Fneu", "spks"):
            if data.get(k) is not None:
    ...
    df_roi = pd.DataFrame({
        'idx': range(n_kept),
        's2p_idx': true_indices
    })
    data['df_rois'] = df_roi
    # roi_df.index = range(n_kept)  # Index is 0, 1, 2, ... n_kept-1
    
    print(f"Cell filtering complete")
    return data


def _memmap_subset_rows(
    src: np.ndarray,
    mask: np.ndarray,
    out_dir: str,
    name: str,
    ...
    """
    Create a memmapped subset of 'src' retaining rows where mask==True.

    - src: shape (n_rois, n_time) matrix (np.ndarray or np.memmap)
    - mask: boolean vector of length n_rois
    - out_dir: directory for the new memmap .npy file
    - name: base filename (e.g. 'F', 'Fneu', 'spks')
    - order: 'C' or 'F'
    - dtype: override dtype for output (defaults to src.dtype)
    - chunk_rows: number of rows to copy per chunk

    Returns (memmap_array, path).
    """
    os.makedirs(out_dir, exist_ok=True)
    idx = np.flatnonzero(mask)
    out_shape = (len(idx), src.shape[1])
    out_path = os.path.join(out_dir, f"{name}_subset.npy")
    out_mm = np.lib.format.open_memmap(
        out_path, mode="w+", dtype=(dtype or src.dtype), shape=out_shape, order=order
    )

    # Copy in row chunks to avoid large memory spikes
    for i0 in range(0, len(idx), chunk_rows):
        sel = idx[i0:i0 + chunk_rows]
        # This reads a small block of rows from src and writes to out_mm
        out_mm[i0:i0 + len(sel)] = src[sel]

    out_mm.flush()
    return out_mm, out_path
...
def classify_rois_scale_aware(data: Dict[str, Any], cfg: Dict[str, Any]) -> None:
    ...
    data['df_rois']['roi_labels'] = data['roi_labels']
...
data['imaging_fs'] = 29.760441593958042
...
def hierarchical_clustering(data, max_iterations=5, min_cluster_size_ratio=0.03, 
                          min_subcluster_size=5, confidence_threshold=0.6):
    """
    Perform hierarchical clustering with subclustering iterations.
    ...
    while processing_queue and len([item for item in processing_queue if item[0] < max_iterations]) > 0:
        ...
        clusters, probs, pca, um, hdb = cluster_data(
            Xr, min_cluster_size,
            feature_mode="autocorr",      # <-- key change
            circ_shift=True,
            umap_metric='cosine'
        )        
    ...
    while processing_queue and len([item for item in processing_queue if item[0] < max_iterations]) > 0:
        ...
        for cluster_id in unique_clusters:
            ...
            for roi_idx in cluster_roi_indices:
                if roi_idx not in results['roi_to_cluster'][iteration]:
                    results['roi_to_cluster'][iteration][roi_idx] = cluster_key
        # --- Rescue pass on noise ---
        noise_mask = (clusters == -1)
        n_noise = int(noise_mask.sum())
        if n_noise >= min_subcluster_size:  # only if there's enough to split
            noise_roi_indices = valid_roi_indices[noise_mask]
            X_noise = X[noise_roi_indices]

            # more permissive params
            resc_min_cluster_size = max(10, min_subcluster_size // 2)
            resc_min_samples = 2

            # re-cluster NOISE with morphology-aware features
            resc_clusters, resc_probs, _, _, _ = cluster_data(
                # NOTE: use the same robust preprocess for noise subset
                preprocess_data(X_noise, noise_roi_indices)[0],
                resc_min_cluster_size,
                feature_mode="autocorr",   # or "combo"
                circ_shift=True,
                umap_metric='cosine'
            )

            # gather rescued groups (ignore resc -1)
            for rid in np.unique(resc_clusters):
                m = (resc_clusters == rid)
                idxs = noise_roi_indices[m]
                if rid == -1 or len(idxs) < 5:
                    continue
                subkey = f"{parent_cluster}.noise->{iteration}.{int(rid)}"
                results['hierarchy'][iteration][subkey] = idxs
                results['cluster_info'][iteration][subkey] = {
                    'size': len(idxs),
                    'probabilities': resc_probs[m],
                    'parent': f"{parent_cluster}.noise",
                    'confidence_kept': int((resc_probs[m] >= confidence_threshold).sum())
                }
                # enqueue for further subclustering if big enough
                if len(idxs) >= min_subcluster_size * 2 and iteration + 1 < max_iterations:
                    processing_queue.append((iteration + 1, subkey, idxs, X[idxs]))
                for roi_idx in idxs:
                    results['roi_to_cluster'][iteration][roi_idx] = subkey
...
def hierarchical_clustering(
    ...
    """
    Hierarchical clustering with subclustering on *all* clusters (including HDBSCAN -1).
    """
    X = data['dFF_clean']  # (n_rois, n_time)
    original_n_rois = X.shape[0]
    original_indices = np.arange(original_n_rois)
    ...
    results = {
        'hierarchy': {},         # iteration -> cluster_key -> roi_indices
        'cluster_info': {},      # iteration -> cluster_key -> metadata
        'roi_to_cluster': {},    # iteration -> roi_index -> cluster_path
        'filtered_indices': None
    }

    # ---- helpers ----
    def _format_key(parent, cid):
        """Make a hierarchical key; map -1 to 'N' but don't treat it specially."""
        node = "N" if cid == -1 else str(cid)
        return node if parent == 'root' else f"{parent}.{node}"
    ...
    def cluster_data(Xr, min_cluster_size,
                     feature_mode="autocorr", circ_shift=True, umap_metric='cosine'):
        """
        feature_mode: 'raw' | 'autocorr' | 'fft' | 'diff' | 'combo'
        """
        Xf_in = _maybe_circ_shift(Xr, enable=circ_shift)
        ...
        n_rois, n_feats = Xf.shape
         
        # ---- PCA (robust to small clusters) ----
        max_pca = min(n_feats, max(1, n_rois - 1))   # <= min(n_samples, n_features)
        if max_pca < 2:
            # too small for PCA; pass features through
            Xp = Xf
            pca = None
        else:
            n_comp = min(50, max_pca)                # cap by rows & cols
            pca = PCA(n_components=n_comp, random_state=42, svd_solver="auto")
            Xp = pca.fit_transform(Xf)
        
        # optional whitening in PCA space (or raw feature space if PCA skipped)
        Xp = StandardScaler(with_mean=False, with_std=True).fit_transform(Xp)
        
        # ---- UMAP ----
        n_neighbors = min( max(5, n_rois // 3), max(2, n_rois - 1) )  # valid range
        um = umap.UMAP(
        ...
        # ---- HDBSCAN ----
        min_samples = max(2, min_cluster_size // 2)  # a bit looser for small groups
        hdb = hdbscan.HDBSCAN(
            min_cluster_size=min_cluster_size,
            min_samples=min_samples,
            cluster_selection_method='eom',
    ...
    while processing_queue and any(item[0] < max_iterations for item in processing_queue):
        iteration, parent_cluster, roi_indices, X_subset = processing_queue.pop(0)
        if iteration >= max_iterations:
            continue
        ...
        # cluster size schedule
        min_cluster_size = (max(30, int(min_cluster_size_ratio * n_valid_rois))
                            if iteration == 0 else min_subcluster_size)

        # clusters, probs, pca, um, hdb = cluster_data(
        #     Xr, min_cluster_size,
        #     feature_mode="autocorr",
        #     circ_shift=True,
        #     umap_metric='cosine'
        # )
        ...
        clusters, probs, pca, um, hdb = cluster_data(
            Xr, min_cluster_size,
            feature_mode=("raw" if iteration == 0 else "autocorr"),
            circ_shift=True,
            umap_metric='cosine'
        )        
        ...
        unique_clusters = np.unique(clusters)
        print(f"Found {len(unique_clusters)} cluster labels (including -1)")

        for cid in unique_clusters:
            m = (clusters == cid)
            idxs = valid_roi_indices[m]
            cprobs = probs[m]
            count = idxs.size

            # give -1 a normal hierarchical key ("N")
            cluster_key = _format_key(parent_cluster, cid)
            tag = "N" if cid == -1 else str(cid)
            print(f"  Node {cluster_key} [{tag}]: {count} ROIs")

            # enqueue EVERY node (including -1/"N") if it's large enough
            if count >= (min_subcluster_size * 2) and (iteration + 1) < max_iterations:
                processing_queue.append(
                    (iteration + 1, cluster_key, idxs, X[idxs])
                )

            # store
            results['hierarchy'][iteration][cluster_key] = idxs
            results['cluster_info'][iteration][cluster_key] = {
                'size': count,
                'probabilities': cprobs,
                'parent': parent_cluster,
                'confidence_kept': int(conf_keep[m].sum())
            }
            for roi_idx in idxs:
                if roi_idx not in results['roi_to_cluster'][iteration]:
                    results['roi_to_cluster'][iteration][roi_idx] = cluster_key
...
# Run the hierarchical clustering
results = hierarchical_clustering(data, max_iterations=10,
                                  min_cluster_size_ratio=0.03,
...
# BASIC WORKING PLUS PATH LENGTH - NOISY TRACE LONGER PATH
...
def hierarchical_clustering(
    ...
    def _path_length_feats(X, dt=1.0, smooth_win=1):
        """
        Total arclength of each trace.
        X : (n_rois, n_time)  -- use the *preprocessed* (z-scored/clipped) traces.
        dt: time step between samples (1.0 if working in frame units).
        smooth_win: optional tiny smoothing (e.g., 1=no smoothing, 3=light) applied
                    only for the arclength calc to avoid pixel jitter explosions.
        Returns: (n_rois, 1) column of total lengths for concatenation.
        """
        Xs = uniform_filter1d(X, size=max(1, smooth_win), axis=1, mode='nearest') if smooth_win > 1 else X
        dY = np.diff(Xs, axis=1)                     # (n_rois, n_time-1)
        seg_len = np.hypot(dt, dY)                   # sqrt( (dt)^2 + (dy)^2 ) per step
        total_len = seg_len.sum(axis=1)              # (n_rois,)
        return total_len[:, None].astype(np.float32) # (n_rois, 1)    
    ...
    def cluster_data(Xr, min_cluster_size,
                     feature_mode="autocorr", circ_shift=True, umap_metric='cosine',
                     path_weight=8.0, path_smooth=3, dt=1.0):
   
        """
        feature_mode: 'raw' | 'autocorr' | 'fft' | 'diff' | 'combo'
        """
        # --- base (your existing) features ---
        Xf_in = _maybe_circ_shift(Xr, enable=circ_shift)
        if feature_mode == "raw":
            Xb = Xf_in
        elif feature_mode == "autocorr":
            Xb = _autocorr_feats(Xf_in, max_lag=min(150, Xf_in.shape[1]//2))
        elif feature_mode == "fft":
            Xb = _fft_feats(Xf_in, keep=min(256, Xf_in.shape[1]//2))
        elif feature_mode == "diff":
            Xb = _diff_feats(Xf_in, k=1)
        elif feature_mode == "combo":
            A = _autocorr_feats(Xf_in, max_lag=min(120, Xf_in.shape[1]//2))
            F = _fft_feats(Xf_in, keep=min(128, Xf_in.shape[1]//2))
            Xb = np.concatenate([A, F], axis=1)
        else:
            raise ValueError("Unknown feature_mode")
        

        # --- NEW: path-length feature on *Xr* (not shifted) ---
        P = _path_length_feats(Xr, dt=dt, smooth_win=path_smooth)   # (n_rois, 1)
        
        # standardize blocks separately, then weight path length heavily
        Bb = StandardScaler(with_mean=False, with_std=True).fit_transform(Xb)
        Bp = (P - P.mean(0)) / (P.std(0) + 1e-8)                    # manual standardize 1D
        Xf = np.hstack([Bb, path_weight * Bp]).astype(np.float32)
    ...
    while processing_queue and any(item[0] < max_iterations for item in processing_queue):
        ...
        clusters, probs, pca, um, hdb = cluster_data(
            Xr, min_cluster_size,
            feature_mode=("raw" if iteration == 0 else "autocorr"),  # or "combo" at root
            circ_shift=True,
            umap_metric='cosine',
            path_weight=(12.0 if iteration == 0 else 8.0),  # heavy at root
            path_smooth=3,  # 1=no smoothing; 3 is a nice compromise
            dt=1.0          # or your true frame interval in seconds
        )        
...
# WORKING , PATH LENGTH, PLUS ADDITIONAL STATS
...
def hierarchical_clustering(
    ...
    """
    Hierarchical clustering with subclustering on *all* clusters (including HDBSCAN -1),
    augmented with path length + light 'visual' stats that separate good vs noisy traces.
    """
    X = data['dFF_clean']  # (n_rois, n_time)
    original_n_rois = X.shape[0]
    original_indices = np.arange(original_n_rois)
    ...
    def _path_length_feats(X, dt=1.0, smooth_win=3):
        Xs = uniform_filter1d(X, size=max(1, smooth_win), axis=1, mode='nearest') if smooth_win > 1 else X
        dY = np.diff(Xs, axis=1)
        seg_len = np.hypot(dt, dY)           # sqrt(dt^2 + dy^2)
        total_len = seg_len.sum(axis=1)
        return total_len[:, None].astype(np.float32)  # (n_rois, 1)

    def _quick_stats_feats(X, smooth_win=3):
        """
        Tiny, robust feature block that matches what we visually spot.
        Returns array shape (n_rois, 4): [deriv_rms, zc_rate, low_high_ratio, acf1]
        """
        Xs = uniform_filter1d(X, size=max(1, smooth_win), axis=1, mode='nearest')
        dX = np.diff(Xs, axis=1)

        # 1) jitter/energy
        deriv_rms = np.sqrt(np.mean(dX**2, axis=1))

        # 2) 'wiggle' (how often slope flips)
        zc_rate = np.mean(np.abs(np.diff(np.signbit(dX), axis=1)), axis=1)

        # 3) spectrum split: low-vs-high power ratio (more 'good' → higher ratio)
        X0 = X - X.mean(axis=1, keepdims=True)
        F = np.fft.rfft(X0, axis=1)
        P = (F.real**2 + F.imag**2) + 1e-12
        cut = max(2, int(0.15 * P.shape[1]))  # ~0–0.15 Nyquist = "low"
        low = P[:, 1:cut].sum(1)
        high = P[:, cut:].sum(1) + 1e-12
        low_high_ratio = low / high

        # 4) lag-1 autocorr (smoothness)
        Xzm = X0
        num = (Xzm[:, :-1] * Xzm[:, 1:]).sum(1)
        den = np.sqrt((Xzm[:, :-1]**2).sum(1) * (Xzm[:, 1:]**2).sum(1)) + 1e-12
        acf1 = num / den

        return np.column_stack([deriv_rms, zc_rate, low_high_ratio, acf1]).astype(np.float32)

    def cluster_data(
        Xr, min_cluster_size,
        feature_mode="autocorr", circ_shift=True, umap_metric='cosine',
        path_weight=8.0, path_smooth=3, dt=1.0,
        stats_weight=2.0, good_weight=6.0
    ):
        """
        feature_mode: 'raw' | 'autocorr' | 'fft' | 'diff' | 'combo'
        Adds: path length (heavy), 4 fast stats, and a 1D 'goodness' score.
        """
        ...
        # --- path length on original (unshifted) traces ---
        P = _path_length_feats(Xr, dt=dt, smooth_win=path_smooth)  # (n,1)

        # --- lightweight stats ---
        S = _quick_stats_feats(Xr, smooth_win=path_smooth)         # (n,4)

        # --- build a single scalar 'goodness' score and append it too ---
        # normalize blocks
        Bb = StandardScaler(with_mean=False, with_std=True).fit_transform(Xb)
        Pz = (P - P.mean(0)) / (P.std(0) + 1e-8)                    # 1D safe z
        Sz = StandardScaler(with_mean=True, with_std=True).fit_transform(S)

        # good score: +smooth/+lowfreq  -jitter  -path
        # weights chosen to bias strongly toward 'good signal' look
        g = (+1.0*Sz[:, 2]   # low_high_ratio (higher is better)
             +1.0*Sz[:, 3]   # acf1 (smoother is better)
             -0.8*Sz[:, 0]   # -deriv_rms
             -0.8*Sz[:, 1]   # -zc_rate
             -1.2*Pz[:, 0])  # -path length
        g = (g - g.mean()) / (g.std() + 1e-8)
        g = g[:, None].astype(np.float32)

        # final feature matrix
        Xf = np.hstack([Bb, path_weight * Pz, stats_weight * Sz, good_weight * g]).astype(np.float32)

        # --- PCA (safe for small n) ---
        n_rois, n_feats = Xf.shape
        max_pca = min(n_feats, max(1, n_rois - 1))
        if max_pca < 2:
            Xp, pca = Xf, None
        else:
            pca = PCA(n_components=min(50, max_pca), random_state=42, svd_solver="auto")
            Xp = pca.fit_transform(Xf)

        Xp = StandardScaler(with_mean=False, with_std=True).fit_transform(Xp)

        # --- UMAP ---
        n_neighbors = min(50, max(15, int(np.sqrt(n_rois))))
        um = umap.UMAP(
            n_neighbors=n_neighbors,
            min_dist=0.05,
            n_components=min(10, max(2, n_rois // 2)),
        ...
        # --- HDBSCAN ---
        min_samples = max(5, int(0.35 * min_cluster_size))
        hdb = hdbscan.HDBSCAN(
            min_cluster_size=min_cluster_size,
            min_samples=min_samples,
            cluster_selection_method='eom',
        ...
        # light rescue of -1 to nearest cluster if any valid exist
        if np.any(clusters == -1) and np.any(clusters >= 0):
            from sklearn.neighbors import KNeighborsClassifier
            keep = clusters >= 0
            knn = KNeighborsClassifier(n_neighbors=min(5, keep.sum()))
            knn.fit(Z[keep], clusters[keep])
            idx = np.where(clusters == -1)[0]
            if idx.size:
                clusters[idx] = knn.predict(Z[idx])
                probs[idx] = np.maximum(probs[idx], 0.51)

        return clusters, probs, pca, um, hdb

    # ---- BFS over hierarchy (unchanged) ----
    processing_queue = [(0, 'root', original_indices, X)]
    while processing_queue and any(item[0] < max_iterations for item in processing_queue):
        iteration, parent_cluster, roi_indices, X_subset = processing_queue.pop(0)
        if iteration >= max_iterations:
        ...
        clusters, probs, pca, um, hdb = cluster_data(
            ...
            circ_shift=True,
            umap_metric='cosine',
            path_weight=(12.0 if iteration == 0 else 8.0),
            path_smooth=3,
            dt=1.0,
            stats_weight=2.0,
            good_weight=6.0
        ...
        unique_clusters = np.unique(clusters)
        print(f"Found {len(unique_clusters)} cluster labels")
        ...
        for cid in unique_clusters:
            ...
            cluster_key = _format_key(parent_cluster, cid)
            print(f"  Node {cluster_key}: {count} ROIs")

            if count >= (min_subcluster_size * 2) and (iteration + 1) < max_iterations:
                processing_queue.append((iteration + 1, cluster_key, idxs, X[idxs]))
...
# WORKING PLUS PATH LENGTH AND HETEROGENEITY GATE
...
def hierarchical_clustering(
    ...
    """
    Hierarchical clustering with subclustering on *all* clusters (including -1/"N"),
    but only if the child still looks heterogeneous (heterogeneity gate).
    """
    X = data['dFF_clean']              # (n_rois, n_time)
    ...
    def _path_length_feats(X, dt=1.0, smooth_win=3):
        # total arclength per trace (sqrt(dt^2 + dy^2) summed)
        Xs = uniform_filter1d(X, size=max(1, smooth_win), axis=1, mode='nearest') if smooth_win > 1 else X
        dY = np.diff(Xs, axis=1)
        seg_len = np.hypot(dt, dY)
        return seg_len.sum(axis=1, keepdims=True).astype(np.float32)
    ...
    # gate: only re-split nodes that are still “broad” in feature space
    def should_split(Xf_std, is_noise_node, min_n, tight_thresh=0.30, noise_tight=0.34):
        n = Xf_std.shape[0]
        if n < max(12, min_n):
            return False
        Dmed = np.median(pairwise_distances(Xf_std, metric='cosine'))
        thr = (noise_tight if is_noise_node else tight_thresh)
        return Dmed >= thr

    # base featureizer (you already have these helpers in your codebase)
    def _maybe_circ_shift(Xr, enable=True):
        if not enable: return Xr
        # random circular shift per row (kills absolute timing)
        n, T = Xr.shape
        offs = np.random.randint(0, T, size=n)
        out = np.empty_like(Xr)
        for i, o in enumerate(offs): out[i] = np.roll(Xr[i], o)
        return out

    # ---------- one clustering pass ----------
    def cluster_data(
        Xr, min_cluster_size, depth,
        feature_mode="autocorr",
        circ_shift=True,
        umap_metric='cosine',
        path_weight_root=12.0,
        path_weight_deep=8.0,
        path_smooth=3,
        dt=1.0
    ):
        # base features (shape-aware)
        Xf_in = _maybe_circ_shift(Xr, enable=circ_shift)
        if feature_mode == "raw":
            Xb = Xf_in
        elif feature_mode == "autocorr":
        ...
        # path length on Xr (not shifted)
        P = _path_length_feats(Xr, dt=dt, smooth_win=path_smooth)
        Bb = StandardScaler(with_mean=False, with_std=True).fit_transform(Xb)
        Pn = (P - P.mean(0)) / (P.std(0) + 1e-8)
        w = path_weight_root if depth == 0 else path_weight_deep
        Xf = np.hstack([Bb, w * Pn]).astype(np.float32)

        # keep standardized feature space for heterogeneity check
        Xf_std = StandardScaler(with_mean=True, with_std=True).fit_transform(Xf)

        # PCA → UMAP
        n, d = Xf_std.shape
        max_pca = min(d, max(1, n - 1))
        if max_pca >= 2:
            pca = PCA(n_components=min(50, max_pca), random_state=42, svd_solver="auto")
            Xp = pca.fit_transform(Xf_std)
        else:
            pca, Xp = None, Xf_std

        # UMAP schedule
        if depth == 0:
            n_neighbors = min(50, max(30, int(np.sqrt(n))))
        else:
            n_neighbors = max(10, n // 3)
        n_neighbors = min(max(2, n_neighbors), max(2, n - 1))
        ...
        if depth == 0:
            ...
            method = 'leaf'
            cse = 0.04
            allow_sc = False  # avoids “everything -> one label”
        ...
        hdb = hdbscan.HDBSCAN(
            ...
            allow_single_cluster=allow_sc,
            prediction_data=True
        )
        clusters = hdb.fit_predict(Z)
        probs = hdb.probabilities_

        return clusters, probs, pca, um, hdb, Xf_std

    # ---------- BFS over hierarchy ----------
    processing_queue = [(0, 'root', original_indices, X)]

    while processing_queue and any(it < max_iterations for it, *_ in processing_queue):
        iteration, parent_cluster, roi_indices, X_subset = processing_queue.pop(0)
        if iteration >= max_iterations:
            continue
        ...
        Xr, valid_roi_indices, _ = preprocess_data(X_subset, roi_indices)
        if iteration == 0:
            results['filtered_indices'] = valid_roi_indices

        n_valid = Xr.shape[0]
        if n_valid < 10:
            print(f"Skipping: only {n_valid} valid ROIs")
            continue

        min_cluster_size = (max(30, int(min_cluster_size_ratio * n_valid))
                            if iteration == 0 else min_subcluster_size)

        clusters, probs, pca, umap_obj, hdb, Xf_std = cluster_data(
            Xr, min_cluster_size, depth=iteration,
            feature_mode=("raw" if iteration == 0 else "autocorr"),
            circ_shift=True,
            umap_metric='cosine',
            path_weight_root=12.0,
        ...
        unique_clusters = np.unique(clusters)
        print(f"Found {len(unique_clusters)} labels (incl. -1)")
        ...
        for cid in unique_clusters:
            ...
            cluster_key = _format_key(parent_cluster, cid)
            is_noise = (cid == -1)
            print(f"  Node {cluster_key} [{'N' if is_noise else cid}]: {count} ROIs")

            # enqueue child only if it still looks heterogeneous
            hetero = should_split(
                Xf_std[m],
                is_noise_node=is_noise,
                min_n=min_subcluster_size,
                tight_thresh=0.30,
                noise_tight=0.34
            )
            if hetero and (iteration + 1) < max_iterations and count >= min_subcluster_size:
                processing_queue.append((iteration + 1, cluster_key, idxs, X[idxs]))
            ...
            for r in idxs:
                if r not in results['roi_to_cluster'][iteration]:
                    results['roi_to_cluster'][iteration][r] = cluster_key
...
# WORKING PLUS PATH LENGTH, HETEROGENEITY GATE, AND STATS
...
# BASIC WORKING, HETERO GATE, MACHINE VISION RESNET
...
def hierarchical_clustering(
    ...
    """
    Hierarchical clustering with subclustering on *all* clusters (including -1/"N"),
    using a heterogeneity gate to keep splitting only when a node is still mixed.
    Root pass uses a visual CNN embedding (trace->image->ResNet18) + path length.
    """
    X = data['dFF_clean']              # (n_rois, n_time)
    original_indices = np.arange(X.shape[0])
    ...
    def _autocorr_feats(X, max_lag=120):
        """
        Row-wise normalized autocorrelation up to max_lag (exclude lag 0).
        X: (n, T)
        returns: (n, L) where L = min(max_lag, T-1)
        """
        ...
        # FFT-based autocorr for all rows at once
        fft_len = int(2 ** np.ceil(np.log2(2 * T - 1)))
        F = np.fft.rfft(Xz, n=fft_len, axis=1)
        ac = np.fft.irfft(F * np.conj(F), n=fft_len, axis=1)[:, :T]  # (n, T)
        ac0 = np.maximum(ac[:, [0]], 1e-12)  # avoid divide-by-zero
        acn = ac[:, 1:L+1] / ac0            # drop lag 0
        return acn.astype(np.float32)
    
    def _fft_feats(X, keep=128, log_power=True, l1_norm=True):
        """
        Row-wise rFFT power spectrum (DC removed), first `keep` bins.
        X: (n, T)
        returns: (n, K) where K = min(keep, floor(T/2))
        """
        X = np.asarray(X, dtype=np.float32)
        n, T = X.shape
        F = np.fft.rfft(X - X.mean(axis=1, keepdims=True), axis=1)
        P = (F.real**2 + F.imag**2).astype(np.float32)  # (n, floor(T/2)+1)
        if P.shape[1] <= 1:
            return np.zeros((n, 1), dtype=np.float32)
        P[:, 0] = 0.0  # drop DC
        K = int(min(keep, P.shape[1] - 1))
        out = P[:, 1:K+1]
        if log_power:
            out = np.log1p(out)
        if l1_norm:
    ...
    def _diff_feats(X, k=1, max_len=512):
        """
        k-th order finite difference along time; optionally downsample to <= max_len cols.
        X: (n, T)
        returns: (n, L) where L <= T-k and possibly downsampled
        """
        X = np.asarray(X, dtype=np.float32)
        D = np.diff(X, n=int(k), axis=1)
        if D.shape[1] <= 0:
            return np.zeros((X.shape[0], 1), dtype=np.float32)
        # light downsampling to keep dimensions reasonable
        step = max(1, D.shape[1] // max_len)
        if step > 1:
            D = D[:, ::step]
        return D.astype(np.float32)
    ...
    # --------- VISUAL EMBEDDING HELPERS (used only at root) ----------
    def _traces_to_imgs(X, H=224, W=224, pad=6, line_width=2):
        # local import to avoid hard dependency
        from PIL import Image, ImageDraw
        imgs = []
        t = X.shape[1]
        xs = np.linspace(pad, W - pad - 1, t)
        for y in X:
            lo, hi = np.nanpercentile(y, [1, 99])
            if not np.isfinite(lo) or not np.isfinite(hi) or hi <= lo:
                lo, hi = np.nanmin(y), np.nanmax(y) + 1e-8
            y01 = np.clip((y - lo) / (hi - lo + 1e-8), 0, 1)
            ys = (H - pad - 1) - (H - 2 * pad - 1) * y01  # up is up
            im = Image.new('L', (W, H), 255)              # white bg
            draw = ImageDraw.Draw(im)
            draw.line(list(zip(xs, ys)), fill=0, width=line_width, joint="curve")
    ...
    def _rolling_std_and_linelen(X, win):
        """
        Exact rolling std and line length (mean |Δ|) per window.
        Returns arrays of shape (n, T-win+1).
        """
        X = np.asarray(X, dtype=np.float32)
        n, T = X.shape
        if win >= T:
            # degenerate: one window covering whole trace
            std = X.std(axis=1, keepdims=True)
            ll  = np.mean(np.abs(np.diff(X, axis=1)), axis=1, keepdims=True) if T > 1 else np.zeros((n,1), np.float32)
            return std, ll
    
        # rolling std via cumsums
        S1 = np.cumsum(X, axis=1, dtype=np.float64)
        S2 = np.cumsum(X**2, axis=1, dtype=np.float64)
        S1 = np.hstack([np.zeros((n,1)), S1])  # pad for windowed diffs
        S2 = np.hstack([np.zeros((n,1)), S2])
        sumX  = S1[:, win:] - S1[:, :-win]
        sumX2 = S2[:, win:] - S2[:, :-win]
        mean  = sumX / win
        var   = np.maximum(sumX2 / win - mean**2, 0.0)
        std   = np.sqrt(var, dtype=np.float32)
    
        # rolling line length (mean |Δ|) via cumsums on |diff|
        dX   = np.abs(np.diff(X, axis=1))
        L    = win - 1
        if L < 1:
            ll = np.zeros_like(std, dtype=np.float32)
        else:
            CS  = np.hstack([np.zeros((n,1), dtype=np.float64), np.cumsum(dX, axis=1, dtype=np.float64)])
            sum_absdiff = CS[:, L:] - CS[:, :-L]             # (n, T-L)
            ll = (sum_absdiff / L).astype(np.float32)        # mean |Δ| per window
        return std.astype(np.float32), ll
    
    def _select_windows(X, win, top_k=1, min_separation=None, w_std=0.3, w_ll=0.7):
        """
        Pick top_k non-overlapping windows per row using a simple activity score:
          score = w_ll * line_length + w_std * std
        Returns starts: (n, top_k) int array clipped to valid range.
        """
        n, T = X.shape
        win = int(win)
        top_k = int(max(1, top_k))
        if T <= win:
            starts = np.zeros((n, top_k), dtype=np.int32)
            return starts
    
        std, ll = _rolling_std_and_linelen(X, win)
        score = w_ll * ll + w_std * std  # (n, T-win+1)
    
        starts = np.zeros((n, top_k), dtype=np.int32)
        width = score.shape[1]
        sep = int(min_separation if min_separation is not None else max(1, win//2))
    
        for i in range(n):
            s = score[i].copy()
            taken = 0
            while taken < top_k and s.size > 0:
                j = int(np.argmax(s))
                starts[i, taken] = j
                taken += 1
                # suppress neighbors to avoid overlap
                lo = max(0, j - sep)
                hi = min(width, j + sep + 1)
                s[lo:hi] = -np.inf
            # if fewer than top_k found (short trace), repeat the best
            if taken < top_k:
                starts[i, taken:] = starts[i, 0]
        return starts
    
    def _segments_to_imgs(X, starts, win, H=224, pad=6, line_width=3):
        """
        Render chosen segments only. No time downsample; width == win.
        Returns list of PIL RGB images in ROI-major, crop-minor order.
        """
        n, T = X.shape
        k = starts.shape[1]
        imgs = []
        for i in range(n):
            for c in range(k):
                ...
                y = X[i, s:e]
                if y.size < win:
                    # right-pad if we hit the end
                    y = np.pad(y, (0, win - y.size), mode='edge')
    
                # robust 1–99% scaling per segment
                lo, hi = np.nanpercentile(y, [1, 99])
                if not np.isfinite(lo) or not np.isfinite(hi) or hi <= lo:
                    lo, hi = float(np.nanmin(y)), float(np.nanmax(y) + 1e-8)
                y01 = np.clip((y - lo) / (hi - lo + 1e-8), 0, 1)
    
                W = win  # 1 sample -> 1 pixel column
                xs = np.linspace(pad, W - pad - 1, W)
                ys = (H - pad - 1) - (H - 2*pad - 1) * y01
    ...
    def _resnet18_embed(imgs, device=None, batch_size=128, out_h=224):
        """
        List[PIL.Image] -> (n, 512) ResNet18 embeddings.
        Preserves width (no resize), only ensures height==out_h so 1 sample ≈ 1 px.
        """
        ...
        # Resize shortest side -> out_h. Our images are H==out_h already, so width stays unchanged.
        tfm = T.Compose([
            T.Resize(out_h),
            T.ToTensor(),
        ...
        with torch.inference_mode():
            for i in range(0, len(imgs), batch_size):
                batch = torch.stack([tfm(im) for im in imgs[i:i+batch_size]]).to(device, non_blocking=True)
    ...
    def cluster_data(
        ...
        # visual settings
        vis_device='cpu',
        vis_img_size=224,     # kept for other visual modes
        cnn_weight=1.0,
        vis_batch_size=128,   # <-- NEW
        visseg_window=448,    # <-- NEW (samples per crop, width seen by ResNet)
        visseg_topk=2         # <-- NEW (crops per ROI)
    ):
        # base features (shape-aware)
        Xf_in = _maybe_circ_shift(Xr, enable=circ_shift)
        ...
        if feature_mode == "raw":
            ...
            F = _fft_feats(Xf_in, keep=min(128, Xf_in.shape[1] // 2))
            Xb = np.concatenate([A, F], axis=1)
        elif feature_mode == "visseg":
            # 1) pick informative windows (no resampling; 1 sample -> 1 pixel col)
            starts = _select_windows(
                Xr, win=visseg_window, top_k=visseg_topk,
                min_separation=visseg_window//2, w_std=0.3, w_ll=0.7
            )
            # 2) render only those segments
            imgs = _segments_to_imgs(Xr, starts, win=visseg_window, H=224, pad=6, line_width=3)
            # 3) embed with ResNet (keep width, only fix height)
            E_all = _resnet18_embed(imgs, device=vis_device, batch_size=vis_batch_size, out_h=224)
            # 4) pool crops per ROI
            E = E_all.reshape(Xr.shape[0], visseg_topk, -1).mean(axis=1)  # (n, 512)
            Xb = cnn_weight * E
        else:
            raise ValueError("Unknown feature_mode")
    
        # --- Path length on original traces ---
        P = _path_length_feats(Xr, dt=dt, smooth_win=path_smooth)
        ...
        # keep standardized space for heterogeneity gate
        Xf_std = StandardScaler(with_mean=True, with_std=True).fit_transform(Xf)
        ...
        Z = um_obj.fit_transform(Xp)
    
        # --- HDBSCAN ---
        if depth == 0:
            min_samples = 5; method = 'eom'; cse = 0.0; allow_sc = False
        else:
            min_samples = 1; method = 'leaf'; cse = 0.04; allow_sc = False
        hdb = hdbscan.HDBSCAN(
            min_cluster_size=min_cluster_size,
            min_samples=min_samples,
            cluster_selection_method=method,
        ...
        return clusters, probs, pca, um_obj, hdb, Xf_std    
    ...
    while processing_queue and any(it < max_iterations for it, *_ in processing_queue):
        ...
        # feature_mode=("visseg" if iteration == 0 else "autocorr"),
        clusters, probs, pca, umap_obj, hdb, Xf_std = cluster_data(
            Xr, min_cluster_size, depth=iteration,
            feature_mode="visseg",
            circ_shift=True,
            ...
            vis_img_size=224,      # kept for other visual modes
            cnn_weight=1.0,
            vis_batch_size=128,    # now accepted
            visseg_window=448,
            visseg_topk=2
        )
...
@lru_cache(maxsize=1)
def _get_resnet18_and_transform(device='cuda', out_h=224, warmup_W=448, warmup_B=16):
    """
    Returns a cached (model, transform). Performs a one-time warm-up
    with H=out_h and W=warmup_W to prime kernels & CUDA context.
    """
    import torch, torchvision
    from torchvision.models import ResNet18_Weights
    import torchvision.transforms as T
    ...
    tfm = T.Compose([
        T.Resize(out_h),  # fixes height; width untouched (your images already have desired width)
        T.ToTensor(),
        T.Normalize(mean=[0.485,0.456,0.406], std=[0.229,0.224,0.225]),
    ])
    ...
    # ---- one-time warm-up ----
    with torch.inference_mode():
        dummy = torch.zeros(warmup_B, 3, out_h, int(warmup_W), device=device)
        if device.startswith('cuda'):
            with torch.cuda.amp.autocast():
                _ = model(dummy)
...
import torch, os
torch.backends.cudnn.benchmark = True
os.environ.setdefault("TF32_OVERRIDE", "0")
...
def hierarchical_clustering(
    ...
    confidence_threshold=0.6,
    # NEW knobs
    noise_patience=2,              # stop splitting after this many trailing 'N'
    rescue_gamma_root=1.10,        # noise rescue radius @ root (× cluster radius)
    rescue_gamma_deep=1.25,        # noise rescue radius @ deeper levels
    hetero_thresh=0.30,            # median cosine distance threshold (normal)
    hetero_thresh_noise=0.34,       # threshold for noise nodes    
    debug=False,
    debug_dir="cluster_debug",
    debug_max_imgs=48,
    debug_trace_plots=12
):
    """
    Hierarchical clustering with subclustering on *all* clusters (including -1/"N"),
    using a heterogeneity gate to keep splitting only when a node is still mixed.
    ...
    def _save_img_grid(imgs, out_path, max_n=48, cols=8):
        """Save a grid of the first max_n PIL images."""
        if not imgs: return None
        n = min(max_n, len(imgs))
        rows = int(math.ceil(n / cols))
        W, H = imgs[0].size
        grid = Image.new('RGB', (cols*W, rows*H), (255,255,255))
        for i in range(n):
            r, c = divmod(i, cols)
            grid.paste(imgs[i], (c*W, r*H))
        grid.save(out_path)
        grid.close()  # free the big canvas memory sooner
        return out_path
    
    def _plot_traces_with_windows(X, starts, win, out_path, max_traces=12):
        """Plot a handful of traces with the chosen windows highlighted."""
        n, T = X.shape
        K = starts.shape[1]
        take = min(max_traces, n)
        idxs = np.linspace(0, n-1, take, dtype=int)
        fig, axes = plt.subplots(take, 1, figsize=(10, 1.8*take), sharex=True)
        if take == 1: axes = [axes]
        ts = np.arange(T)
        for ax, i in zip(axes, idxs):
            ax.plot(ts, X[i], lw=0.8)
            for k in range(K):
                s = int(starts[i, k]); e = min(T, s+win)
                ax.axvspan(s, e, color='orange', alpha=0.15)
            ax.set_ylabel('dF/F', rotation=0, ha='right', va='center')
            ax.set_title(f'ROI {i}')
        axes[-1].set_xlabel('sample')
        fig.tight_layout(); fig.savefig(out_path, dpi=150); plt.close(fig)
        return out_path
    
    def _save_umap_scatter(Z, labels, out_path):
        """Color by labels; -1 in light gray."""
        uniq = np.unique(labels)
        fig, ax = plt.subplots(figsize=(6,5))
        for k in uniq:
            m = (labels == k)
            c = '#aaaaaa' if k == -1 else None
            ax.scatter(Z[m,0], Z[m,1], s=6, alpha=0.8, label=str(k), c=c)
        ax.set_title('UMAP (color=cluster label)'); ax.set_xlabel('UMAP-1'); ax.set_ylabel('UMAP-2')
        if (uniq[uniq!=-1].size)>0: ax.legend(markerscale=3, loc='best', fontsize=8)
    ...
    def plot_preprocess_compare(
        X_raw, X_pre, out_path,
        max_traces=12, title="Preprocess comparison (raw vs MAD z-score clipped)"
    ):
        """
        X_raw: (n, T) raw traces BEFORE preprocess_data
        X_pre: (n', T) traces AFTER preprocess_data, filtered to row_ok
        out_path: file path to save the comparison plot
        """
        # pick indices to visualize from the aligned set (X_pre rows correspond to kept rows)
        n_pre, T = X_pre.shape
        take = min(max_traces, n_pre)
        idxs = np.linspace(0, n_pre-1, take, dtype=int)
    
        fig, axes = plt.subplots(take, 2, figsize=(12, 1.6 * take), sharex=True)
        if take == 1:
            axes = np.array([axes])  # shape (1,2)
    
        ts = np.arange(T)
    
        # Compute y-lims for raw side (robust, per-trace) to avoid deceptive scaling
        def robust_ylim(y, p_lo=1, p_hi=99):
            lo, hi = np.nanpercentile(y, [p_lo, p_hi])
            if not np.isfinite(lo) or not np.isfinite(hi) or hi <= lo:
                lo, hi = float(np.nanmin(y)), float(np.nanmax(y) + 1e-8)
            pad = 0.05 * (hi - lo + 1e-8)
        ...
        for r, i in enumerate(idxs):
            ax_raw, ax_pre = axes[r, 0], axes[r, 1]
            # raw trace corresponding to the kept row i
            y_raw = X_raw[i]
            y_pre = X_pre[i]
    
            ax_raw.plot(ts, y_raw, lw=0.8)
            yl = robust_ylim(y_raw)
            ax_raw.set_ylim(*yl)
            ax_raw.set_ylabel(f"ROI {i}", rotation=0, ha='right', va='center')
    
            ax_pre.plot(ts, y_pre, lw=0.8)
            # Show the clipping band on the preprocessed axis (±5 if you used clip(-5,5))
            ax_pre.axhline(5, ls='--', lw=0.7, alpha=0.6)
            ax_pre.axhline(-5, ls='--', lw=0.7, alpha=0.6)
            ax_pre.set_ylim(-6, 6)
    
            if r == 0:
                ax_raw.set_title("Raw")
                ax_pre.set_title("Preprocessed (median/MAD, clipped)")
    ...
    def _resnet18_embed(imgs, device=None, batch_size=128, out_h=224, tag=''):
        """
        List[PIL.Image] -> (n, 512) using cached ResNet18.
        Preserves width; only ensures height==out_h.
        """
        import torch
        if not imgs:
            return np.zeros((0, 512), dtype=np.float32)
        ...
        # Use cached model+transform; warm-up already done inside the getter.
        # If your visseg_window != 448, pass warmup_W once at top of the cell (see below).
        model, tfm = _get_resnet18_and_transform(device=device, out_h=out_h)
    
        t0 = time.perf_counter()
        tensors = [tfm(im) for im in imgs]   # CPU-side transform
        t1 = time.perf_counter()
    
        outs = []
        with torch.inference_mode():
            for i in range(0, len(tensors), batch_size):
                batch = torch.stack(tensors[i:i+batch_size]).to(device, non_blocking=True)
                if device.startswith('cuda'):
                    with torch.cuda.amp.autocast():
                        y = model(batch)
                    torch.cuda.synchronize()
        ...
        y = torch.cat(outs, dim=0).numpy().astype(np.float32)
        t2 = time.perf_counter()
        print(f"[resnet {tag}] n={len(imgs)} prep={t1-t0:.3f}s infer={t2-t1:.3f}s total={t2-t0:.3f}s", flush=True)
        return y


    # --- noise rescue ---
    def _rescue_noise(Z, labels, probs, gamma):
        lab = labels.copy(); pr = probs.copy()
        valid = np.where(lab >= 0)[0]
        if valid.size == 0: return lab, pr
        uniq = np.unique(lab[valid])
        cents = []; scales = []
        for k in uniq:
            m = (lab == k)
            C = Z[m].mean(0)
            d = np.linalg.norm(Z[m] - C, axis=1)
            cents.append(C)
            scales.append(np.median(d) + 1e-6)    # robust cluster radius
        cents = np.vstack(cents); scales = np.asarray(scales)
        noise_idx = np.where(lab == -1)[0]
        if noise_idx.size == 0: return lab, pr
        D = cdist(Z[noise_idx], cents)
        j = D.argmin(axis=1)
        dmin = D[np.arange(D.shape[0]), j]
        accept = dmin <= (gamma * scales[j])
        if accept.any():
            chosen = uniq[j[accept]]
            rows = noise_idx[accept]
            lab[rows] = chosen
            pr[rows] = np.maximum(pr[rows], 0.60)
        return lab, pr
    ...
    def cluster_data(
        ...
        cnn_weight=1.0,
        vis_batch_size=128,   # <-- NEW
        visseg_window=512,    # <-- NEW (samples per crop, width seen by ResNet)
        visseg_topk=2         # <-- NEW (crops per ROI)
    ):
        # base features (shape-aware)
        Xf_in = _maybe_circ_shift(Xr, enable=circ_shift)
        ...
        # --- noise rescue in UMAP space ---
        gamma  = rescue_gamma_root if depth == 0 else rescue_gamma_deep
        before_N = np.sum(clusters == -1)
        clusters_resc, probs_resc = _rescue_noise(Z, clusters, probs, gamma=gamma)
        rescued = int(before_N - np.sum(clusters_resc == -1))
        clusters, probs = clusters_resc, probs_resc
    
        # --- build debug payload ---
        dbg = {
            "E_stats": None,            # filled only in visseg mode
            "starts": None,
            "imgs": None,               # only for sampling / grid save
            "Z": Z,
            "rescued": rescued,
            "gamma": gamma,
            "n_noise_after": int(np.sum(clusters == -1)),
        }
    
        # Basic embedding diagnostics (only meaningful for visseg)
        if feature_mode == "visseg":
            # pooled embeddings per ROI: E (n,512)
            # norms/variance/pairwise distance median give a quick sanity check
            norms = np.linalg.norm(E, axis=1)
            med_pw = np.median(pairwise_distances(E, metric='cosine'))
            var_feat = E.var(axis=0).mean()
            dbg["E_stats"] = {
                "norm_mean": float(norms.mean()),
                "norm_std":  float(norms.std()),
                "med_pairwise_cos": float(med_pw),
                "mean_feature_var": float(var_feat),
                "shape": list(E.shape),
            }
            dbg["starts"] = starts
            dbg["imgs"]   = imgs  # WARNING: big; we’ll only grid-save a subset in the caller        
            
            
        
        return clusters, probs, pca, um_obj, hdb, Xf_std, dbg    
    ...
    while processing_queue and any(it < max_iterations for it, *_ in processing_queue):
        ...
        # check preproc images, resnet needs good trace img 
        if debug:
            print(Xr.shape[0])
            plot_preprocess_compare(
                Xr,   # align with X_pre rows
                X_subset,
                out_path="preprocess_compare.png",
                max_traces=16
            )        
            ...
            starts = _select_windows(
                Xr, win=512, top_k=2,
                min_separation=512//2, w_std=0.3, w_ll=0.7
            )             
                
            
            imgs_raw = _segments_to_imgs(X_subset, starts, 512, H=224, pad=6, line_width=2)
            imgs_pre = _segments_to_imgs(Xr,        starts, 512, H=224, pad=6, line_width=2)
            
            _save_img_grid(imgs_raw[:48], "segments_raw.png")
            _save_img_grid(imgs_pre[:48], "segments_pre.png")            
            # print(Xr.shape[0])
            # plot_preprocess_compare(
            #     Xr[valid_roi_indices],   # align with X_pre rows
            #     X_subset,
            #     out_path="preprocess_compare.png",
            #     max_traces=16
            # )        
            ...
            # imgs_raw = _segments_to_imgs(X_subset, starts, 512, H=224, pad=6, line_width=2)
            # imgs_pre = _segments_to_imgs(Xr[valid_roi_indices],        starts, 512, H=224, pad=6, line_width=2)
        ...
        # feature_mode=("visseg" if iteration == 0 else "autocorr"),
        clusters, probs, pca, umap_obj, hdb, Xf_std, dbg = cluster_data(
            Xr, min_cluster_size, depth=iteration,
            feature_mode="visseg",
            circ_shift=True,
            umap_metric='cosine',
            ...
            vis_img_size=224,      # kept for other visual modes
            cnn_weight=1.0,
            vis_batch_size=16,    # now accepted
            visseg_window=512,
            visseg_topk=2
        )
        ...
        if debug:
            base = _ensure_dir(os.path.join(debug_dir, f"iter{iteration}_{parent_cluster.replace('.', '-') or 'root'}"))
            # 1) show some of the *exact* images we fed ResNet
            if dbg.get("imgs"):
                _save_img_grid(dbg["imgs"], os.path.join(base, "resnet_inputs_grid.png"),
                               max_n=debug_max_imgs, cols=8)
            # 2) trace plots with selected windows highlighted
            if dbg.get("starts") is not None:
                _plot_traces_with_windows(Xr, dbg["starts"], win=512,
                                          out_path=os.path.join(base, "windows_on_traces.png"),
                                          max_traces=debug_trace_plots)
            # 3) UMAP scatter (colored by final labels after rescue)
            _save_umap_scatter(dbg["Z"], clusters, os.path.join(base, "umap_scatter.png"))
        
            # 4) print quick embedding stats + rescue summary
            if dbg.get("E_stats"):
                es = dbg["E_stats"]
                print(f"[vis] E shape={es['shape']} | ||E|| mean={es['norm_mean']:.3f}±{es['norm_std']:.3f} | "
                      f"median cos-dist={es['med_pairwise_cos']:.3f} | mean(feat var)={es['mean_feature_var']:.5f}")
            print(f"[rescue] gamma={dbg['gamma']} | rescued={dbg['rescued']} | noise_after={dbg['n_noise_after']}")
...
# e.g., width = visseg_window, batch = vis_batch_size
_ = _get_resnet18_and_transform(device='cuda', out_h=224, warmup_W=512, warmup_B=128)
# _ = _get_resnet18_and_transform(device='cuda', out_h=224, warmup_W=224, warmup_B=8)
...
def assign_cluster_indices(data, results):
    """
    Assign cluster indices to ROIs based on their hierarchical paths
    """
    n_rois = len(data['df_rois'])
    paths = []
    
    # Get all hierarchy paths for filtered ROIs
    for roi_idx in range(n_rois):
        path = get_roi_full_path(results, roi_idx)
        paths.append(path)
    
    # Get unique clusters (remove None values)
    unique_clusters = list(dict.fromkeys(paths))
    
    # Get the ROI indices in each cluster
    unique_cluster_rois = []
    for cluster in unique_clusters:
        indices = [i for i, x in enumerate(paths) if x == cluster]
        unique_cluster_rois.append(indices)
    
    # Initialize cluster_idx column with -1 (for non-clustered ROIs)
    if 'cluster_idx' not in data['df_rois'].columns:
        data['df_rois']['cluster_idx'] = -1
    
    # Assign group numbers (0, 1, 2, 3, etc.) to each cluster
    for group_num, roi_indices in enumerate(unique_cluster_rois):
        for roi_idx in roi_indices:
            data['df_rois'].loc[roi_idx, 'cluster_idx'] = group_num
    
    return unique_cluster_rois, unique_clusters

# Usage example:
unique_cluster_rois, unique_clusters = assign_cluster_indices(data, results)

# Print results
print(f"Found {len(unique_clusters)} unique clusters:")
for i, (cluster_path, roi_indices) in enumerate(zip(unique_clusters, unique_cluster_rois)):
    print(f"Cluster {i}: {cluster_path}")
...
def plot_cluster_traces(data, clusters, cluster_labels, start_time=0, n_samples=5, duration=10):
    ...
    # Get time segment
    # time = data['imaging_time']
    if 'imaging_time' in data.keys():
        time = data['imaging_time']
    else:
        fs = data['imaging_fs']
        n_samples = data['dFF_clean'].shape[1]
        time = np.arange(0, n_samples/fs, step=1/fs)
    ...
    for idx, cluster_rois in enumerate(clusters):
        ...
        # Set overall figure title and x-label
        fig.suptitle(f'{idx} : {cluster_label} (n={n_rois_in_cluster}, showing {sample_size} traces)', 
                     fontsize=14, y=0.98)
        axes[-1].set_xlabel('Time (s)')
        plt.tight_layout()
        plt.subplots_adjust(top=0.93)  # Make room for suptitle
        ...
        # out_dir = ov_cfg['save_dir']
        out_dir = 'D:/behavior/2p_imaging/processed/2afc/YH24LG/YH24LG_CRBL_lobulev_20250620_2afc-494/qc_results_new/deep_trace'
        os.makedirs(out_dir, exist_ok=True)
        trace_filename = str(idx) + '.png'
        save_path = os.path.join(out_dir, trace_filename)
        print(save_path)
        fig.savefig(save_path, dpi=140)
        print(f"Cluster saved: {save_path}")
        plt.close(fig)

plot_cluster_traces(data, unique_cluster_rois, unique_clusters, start_time=50, n_samples=100, duration=10)
...
# BASIC WORKING, HETERO GATE, MACHINE VISION RESNET
# TEST FIND GOOD SEGMENTS, NO DOWNSAMPLE
# low resolution, aliasing, skipped
...
def hierarchical_clustering(
    ...
    # def _resnet18_embed(imgs, device='cpu'):
    #     # local import + graceful fallback
    #     try:
    #         import torch, torchvision
    #         import torchvision.transforms as T
    #     except Exception:
    ...
    #         T.Normalize(mean=[0.485,0.456,0.406], std=[0.229,0.224,0.225])
    #     ])
    #     batch = torch.stack([tfm(im) for im in imgs]).to(device)
    #     with torch.no_grad():
    #         feats = model(batch).cpu().numpy().astype(np.float32)  # (n,512)
    #     return feats
    def _resnet18_embed(imgs, device='cuda', batch_size=128):
        import torch, torchvision
        from torchvision.models import ResNet18_Weights
        import torchvision.transforms as T
        model = torchvision.models.resnet18(weights=ResNet18_Weights.DEFAULT)
        ...
        for i in range(0, len(imgs), batch_size):
            ...
            if device.startswith('cuda'):
                ...
                y = model(batch)
            outs.append(y.float().cpu())
        return np.concatenate([o.numpy() for o in outs], axis=0).astype(np.float32)
    ...
    def cluster_data(
        ...
        if feature_mode == "raw":
            ...
            F = _fft_feats(Xf_in, keep=min(128, Xf_in.shape[1] // 2))
            Xb = np.concatenate([A, F], axis=1)
        elif feature_mode == "vis":
            # IMPORTANT: build from ORIGINAL (unshifted) traces
            imgs = _traces_to_imgs(Xr, H=vis_img_size, W=vis_img_size, pad=6, line_width=2)
            E = _resnet18_embed(imgs, device=vis_device)
            if E is None:
                # fallback to combo if torch/torchvision missing
                A = _autocorr_feats(Xf_in, max_lag=min(120, Xf_in.shape[1] // 2))
                F = _fft_feats(Xf_in, keep=min(128, Xf_in.shape[1] // 2))
                Xb = np.concatenate([A, F], axis=1)
            else:
    ...
    while processing_queue and any(it < max_iterations for it, *_ in processing_queue):
        ...
        clusters, probs, pca, umap_obj, hdb, Xf_std = cluster_data(
            Xr, min_cluster_size, depth=iteration,
            feature_mode=("vis" if iteration == 0 else "autocorr"),
            circ_shift=True,
            umap_metric='cosine',
            path_weight_root=12.0,
            path_weight_deep=8.0,
            path_smooth=3,
            dt=1.0,
            vis_device='cpu',      # 'cuda' if available
            vis_img_size=224,
            cnn_weight=1.0
        )
...
def hierarchical_clustering(
    ...
    def _stats_feats(X, smooth_win=5):
        """
        Lightweight per-trace stats that separate 'good' vs 'jittery' noise.
        Works with any fps (all ratios/relative).
        """
        # light smoothing just for feature extraction
        Xs = uniform_filter1d(X, size=smooth_win, axis=1, mode='nearest')
    
        # derivative-based features (noise has lots of high-freq slope)
        dXs = np.diff(Xs, axis=1)
        line_len = np.mean(np.abs(dXs), axis=1)                # average absolute slope
        deriv_rms = np.sqrt(np.mean(dXs**2, axis=1))           # derivative energy
        # fraction of zero-crossings of derivative (oscillation-y noise pops high)
        zc = np.mean(np.abs(np.diff(np.signbit(dXs), axis=1)), axis=1)
    
        # amplitude distribution
        std_ = X.std(1)
        iqr_ = np.subtract(*np.percentile(X, [75, 25], axis=1))
        skew_ = skew(X, axis=1, bias=False, nan_policy='omit')
        kurt_ = kurtosis(X, axis=1, fisher=True, bias=False, nan_policy='omit')
    
        # simple spectral split (low-vs-high power ratio, timing-agnostic)
        X0 = X - X.mean(axis=1, keepdims=True)
        F = np.fft.rfft(X0, axis=1)
        P = (F.real**2 + F.imag**2) + 1e-12
        cut = max(2, int(0.15 * P.shape[1]))  # 0.0–0.15 of Nyquist = "low"
        low = P[:, 1:cut].sum(1)
        high = P[:, cut:].sum(1)
        low_high_ratio = low / high
    
        return np.column_stack([std_, iqr_, skew_, kurt_, line_len, deriv_rms, zc, low_high_ratio])
    ...
    def cluster_data(Xr, min_cluster_size,
                     feature_mode="autocorr", circ_shift=True, umap_metric='cosine',
                     stats_weight=2.0, smooth_win=5):
        """
        feature_mode: 'raw' | 'autocorr' | 'fft' | 'diff' | 'combo'
        Adds small stats block to tighten 'good' clusters.
        """
        # ---- base (your existing) features ----
        Xf_in = _maybe_circ_shift(Xr, enable=circ_shift)
        if feature_mode == "raw":
        ...
        # ---- small stats block (computed on original traces) ----
        Xs = _stats_feats(Xr, smooth_win=smooth_win)
    
        # standardize each block separately, then weight stats to “count”
        Bb = StandardScaler(with_mean=False, with_std=True).fit_transform(Xb)
        Bs = StandardScaler(with_mean=True,  with_std=True).fit_transform(Xs)
        Xf = np.hstack([Bb, stats_weight * Bs]).astype(np.float32)
        ...
        # ---- UMAP (moderate neighborhood binds groups) ----
        n_neighbors = min(50, max(20, int(np.sqrt(n_rois))))
        um = umap.UMAP(
            n_neighbors=n_neighbors,
            min_dist=0.01,
            n_components=min(10, max(2, n_rois // 2)),
        ...
        # --- if all noise, relax once, then k-means fallback ---
        if np.all(clusters == -1):
            # relax density first
            hdb_relax = hdbscan.HDBSCAN(
                min_cluster_size=max(8, min_cluster_size // 2),
                min_samples=1,
                cluster_selection_method='eom',
                allow_single_cluster=True,
                prediction_data=True
            )
            clusters = hdb_relax.fit_predict(Z)
            probs = hdb_relax.probabilities_
        
            # still all noise? fallback to a coarse partition
            if np.all(clusters == -1):
                k = min(8, max(2, n_rois // 80))  # ~ up to 8 groups for ~700 ROIs
                km = MiniBatchKMeans(n_clusters=k, random_state=42, batch_size=min(512, n_rois))
                clusters = km.fit_predict(Z)
                probs = np.full(n_rois, 0.6, dtype=float)
        
        # --- rescue weak points to nearest centroid of real clusters ---
        valid = np.where(clusters >= 0)[0]
        if valid.size:
            klist = np.unique(clusters[valid])
            cents = np.stack([Z[clusters == k].mean(0) for k in klist])
            weak = np.where((clusters == -1) | (probs < 0.5))[0]
            if weak.size:
                D = cdist(Z[weak], cents, metric='euclidean')
                assign = klist[D.argmin(axis=1)]
                clusters[weak] = assign
                probs[weak] = np.maximum(probs[weak], 0.55)
        ...
        # ---- tiny rescue: attach low-conf/-1 to nearest cluster centroid in UMAP ----
        valid = np.where(clusters >= 0)[0]
        if valid.size:
            klist = np.unique(clusters[valid])
            cents = np.stack([Z[clusters == k].mean(0) for k in klist])
...
def plot_cluster_trace_segments(data, cluster_labels, n_clusters=None, segment_duration=10, 
    ...
    if 'imaging_time' in data.keys():
        ...
        fs = data['imaging_fs']
        n_samples = data['dFF_clean'].shape[1]
        time_vec = np.arange(0, n_samples/fs, step=1/fs)
