"""
eye blink conditioning Imaging Analyzer

Experiment-specific imaging analysis for eye blink conditioning experiments.
...
class EyeBlinkConditioningImagingAnalyzer:
    """
    Imaging analyzer for eye blink conditioning experiments.
    """
    ...
    def __init__(self, config_manager, subject_list, logger=None):
        ...
        self.logger.info("EBC_IMG_ANALYZER: eye blink conditioning imaging analyzer initialized")
    ...
    def _load_preprocessed_data(self, subject_id: str, suite2p_path: str, output_path: str) -> Dict[str, Any]:
        ...
        """
        try:
            self.logger.info(f"EBC_IMG_ANALYZER: Loading preprocessed data for {subject_id}")
            ...
            self.logger.info(f"EBC_IMG_ANALYZER: Successfully loaded preprocessed data for {subject_id}")
            ...
            self.logger.error(f"EBC_IMG_ANALYZER: Failed to load preprocessed data for {subject_id}: {e}")
            return {
                'success': False,
                'error': str(e)
            }
    ...
    def extract_trial_segments_simple(self, imaging_data: Dict[str, Any], df_trials: pd.DataFrame,
            ...
            """
            try:
                self.logger.info(f"EBC_IMG: Extracting trial segments aligned to '{alignment}'")
                ...
                # Log dropped trials
                if dropped_trials:
                    self.logger.warning(f"EBC_IMG: Dropped {len(dropped_trials)} trials due to insufficient data:")
                ...
                if temp_dff_segments:
                    ...
                    self.logger.info(f"EBC_IMG: Successfully extracted and trimmed segments for {n_valid}/{len(df_enhanced)} trials")
                    self.logger.info(f"EBC_IMG: Common DFF segment shape: {trimmed_dff_segments[0].shape}")
                    self.logger.info(f"EBC_IMG: Common time vector length: {len(common_dff_time_vector)}")
                ...
                self.logger.error(f"EBC_IMG: Failed to extract trial segments: {e}")
                return {'error': str(e)}
    ...
    def plot_trial_segments_check(self, trial_data: Dict[str, Any], subject_id: str) -> None:
        ...
        try:
            ...
            if 'error' in trial_data:
                self.logger.error(f"EBC_IMG: Cannot plot - trial extraction failed: {trial_data['error']}")
                return
            ...
            if 'df_trials_with_segments' in trial_data:
                ...
                if len(df_valid) == 0:
                    self.logger.error("EBC_IMG: No valid trials with segments")
                    return
            ...
            self.logger.info(f"EBC_IMG: Generated trial segments check plot")
            
        except Exception as e:
            self.logger.error(f"EBC_IMG: Failed to create trial segments check plot: {e}")
    ...
    def _plot_individual_trials_from_segments(self, trial_data: Dict[str, Any], subject_id: str) -> None:
        ...
        try:
            ...
            self.logger.info(f"EBC_IMG: Generated individual trial plots from segments for {n_trials_to_plot} trials")
            
        except Exception as e:
            self.logger.error(f"EBC_IMG: Failed to create individual trial plots from segments: {e}")
        ...
        try:
            ...
            if 'df_trials_with_segments' in trial_data:
                ...
                if not np.any(left_rewarded) or not np.any(right_rewarded):
                    self.logger.warning("EBC_IMG: Insufficient trials for condition comparison")
                    return
            ...
            # Log summary
            if 'df_trials_with_segments' in trial_data:
                self.logger.info(f"EBC_IMG: Condition comparison heatmap - Left: {len(left_trials)} trials, Right: {len(right_trials)} trials")
            else:
                self.logger.info(f"EBC_IMG: Condition comparison heatmap - Left: {np.sum(left_rewarded)} trials, Right: {np.sum(right_rewarded)} trials")
            
        except Exception as e:
            self.logger.error(f"EBC_IMG: Failed to create condition comparison heatmap: {e}")
    ...
    def trim_sequences_to_common_length(self, sequences: List[np.ndarray], data_type: str) -> np.ndarray:
        ...
        self.logger.debug(f"EBC_IMG: Trimmed {len(sequences)} {data_type} sequences to length {min_length}")
    ...
    def plot_comprehensive_alignment_check(self, imaging_data: Dict[str, Any], behavioral_data: Dict[str, Any], 
        ...
        try:
            ...
            self.logger.info(f"EBC_IMG: Creating comprehensive alignment check ({alignment_stage})")
            ...
            self.logger.error(f"EBC_IMG: Failed to create comprehensive alignment check: {e}")
    ...
    def _plot_session_overview_with_trials(self, imaging_data, df_trials, subject_id, alignment_stage):
        ...
        try:
            ...
            self.logger.error(f"EBC_IMG: Failed to create session overview plot: {e}")
    ...
    def _plot_individual_trials(self, imaging_data, df_trials, subject_id, alignment_stage):
        ...
        try:
            ...
            self.logger.error(f"EBC_IMG: Failed to create individual trial plots: {e}")
    ...
    def plot_responsive_rois_by_condition(self, trial_data: Dict[str, Any], subject_id: str) -> None:
        ...
        try:
            ...
            if 'trial_side' not in df_valid.columns or 'rewarded' not in df_valid.columns:
                self.logger.warning("EBC_IMG: Missing trial_side or rewarded columns")
                return
            ...
            if not np.any(left_rewarded) or not np.any(right_rewarded):
                self.logger.warning("EBC_IMG: Insufficient trials for left+rewarded or right+rewarded conditions")
                return
            ...
            self.logger.error(f"EBC_IMG: Failed to plot responsive ROIs by condition: {e}")
            return None
        ...
        try:
            ...
            if 'df_trials_with_segments' in trial_data:
                ...
                if len(selected_trials) == 0:
                    self.logger.warning(f"EBC_IMG: No trials found for trial_type '{trial_type}'")
                    return
            ...
            self.logger.info(f"EBC_IMG: All ROIs heatmap - {n_trials} trials, {n_rois} ROIs")
            self.logger.info(f"EBC_IMG: ROI sorting: {sort_method}, Trial type: {trial_type}")
            
        except Exception as e:
            self.logger.error(f"EBC_IMG: Failed to create all ROIs heatmap: {e}")
        ...
        try:
            ...
            if 'df_trials_with_segments' in trial_data:
                ...
                if trial_index >= len(df_valid) or trial_index < 0:
                    self.logger.error(f"EBC_IMG: Trial index {trial_index} out of range (0-{len(df_valid)-1})")
                    return
                ...
                if trial_index >= len(dff_segments) or trial_index < 0:
                    self.logger.error(f"EBC_IMG: Trial index {trial_index} out of range (0-{len(dff_segments)-1})")
                    return
            ...
            # Log information
            self.logger.info(f"EBC_IMG: Single trial raster plot - Trial {trial_index}, {n_rois} ROIs, Sort: {sort_method}")
            if sort_method != 'none':
                if sort_method == 'max_peak':
                    top_roi = roi_order[0]
                    self.logger.info(f"EBC_IMG: Most active ROI: {top_roi} (peak: {np.max(dff_segment[top_roi, :]):.3f})")
                else:
                    bottom_roi = roi_order[0]
                    self.logger.info(f"EBC_IMG: Least active ROI: {bottom_roi} (min: {np.min(dff_segment[bottom_roi, :]):.3f})")
            
        except Exception as e:
            self.logger.error(f"EBC_IMG: Failed to create single trial raster plot: {e}")
    ...
    def plot_trial_alignment_diagnostic(self, imaging_data: Dict[str, Any], behavioral_data: Dict[str, Any], 
        ...
        try:
            ...
            self.logger.info(f"EBC_IMG: Creating trial alignment diagnostic for {len(trial_indices)} trials")
            ...
            for trial_idx in trial_indices:
                if trial_idx >= len(df_trials):
                    self.logger.warning(f"EBC_IMG: Trial index {trial_idx} out of range, skipping")
                    continue
                ...
                if pd.isna(trial_start):
                    self.logger.warning(f"EBC_IMG: Trial {trial_idx} has no trial_start_timestamp, skipping")
                    continue
                ...
                if img_start_idx >= img_end_idx:
                    self.logger.warning(f"EBC_IMG: No imaging data for trial {trial_idx} window, skipping")
                    continue
                ...
                self.logger.info(f"EBC_IMG: Trial {trial_idx} (actual trial #{actual_trial_index}) diagnostic - {n_rois} ROIs, "
                ...
                if alignment_issues:
                    self.logger.warning(f"EBC_IMG: Trial {trial_idx} potential alignment issues: {', '.join(alignment_issues)}")
                else:
                    self.logger.info(f"EBC_IMG: Trial {trial_idx} alignment looks good")
            
            self.logger.info(f"EBC_IMG: Trial alignment diagnostic completed for {len(trial_indices)} trials")
            
        except Exception as e:
            self.logger.error(f"EBC_IMG: Failed to create trial alignment diagnostic: {e}")
    ...
    def plot_session_alignment_overview(self, imaging_data: Dict[str, Any], behavioral_data: Dict[str, Any], 
        ...
        try:
            ...
            self.logger.info(f"EBC_IMG: Creating session alignment overview for first {time_window}s")
            ...
            self.logger.info(f"EBC_IMG: Session overview shows {len(trials_in_window)} trials in {time_window}s window")
            
        except Exception as e:
            self.logger.error(f"EBC_IMG: Failed to create session alignment overview: {e}")
    ...
    def filter_trial_data_by_isi(
        ...
        try:
            ...
            if hasattr(self, 'logger') and self.logger:
                self.logger.error(f"EBC_IMG: filter_trial_data_by_isi failed: {e}")
    ...
    def plot_trial_segments_spike_trial_check(self, trial_data: Dict[str, Any], subject_id: str,
        ...
        try:
            ...
            if 'df_trials_with_segments' not in trial_data:
                self.logger.error("EBC_IMG: Expected DataFrame-based segmented trial_data")
                return
            ...
            plt.tight_layout()
            plt.show()
            self.logger.info("EBC_IMG: Generated DFF+spike trial raster check plot")

        except Exception as e:
            self.logger.error(f"EBC_IMG: Failed to create trial raster check plot: {e}")
    ...
    def plot_trial_segments_spike_check(self, trial_data: Dict[str, Any], subject_id: str,
        ...
        try:
            ...
            plt.tight_layout()
            plt.show()
            self.logger.info("EBC_IMG: Generated DFF+spike trial segments check plot")
    ...
    def _load_qc_table(self, output_path: str) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            self.logger.warning(f"EBC_IMG: Failed loading QC table: {e}")
        return None
    ...
    def _apply_qc_roi_filter(
        ...
        n_rois = imaging_data['dff_traces'].shape[0]
        if qc_df is None:
            self.logger.warning(f"EBC_IMG: QC table not found for {subject_id}; using all {n_rois} ROIs.")
            ...
            if 'roi_id' not in qc_df.columns or 'keep_dff' not in qc_df.columns:
                self.logger.warning("EBC_IMG: QC table missing roi_id / keep_dff; skipping QC filter.")
                mask = np.ones(n_rois, bool)
            else:
                df = qc_df.copy()
                ...
                lost = np.sum(~mask)
                self.logger.info(
                    f"EBC_IMG: QC filter kept {mask.sum()}/{n_rois} ROIs "
    ...
    def analyze_subject(self, subject_id: str, suite2p_path: str, output_path: str, force: bool = False) -> Dict[str, Any]:
        ...
        """
        try:
            self.logger.info(f"EBC_IMG_ANALYZER: Analyzing imaging sessions for subject {subject_id}")
            ...
            self.logger.info(f"EBC_IMG: Extracted {trial_data['dff_segments_array'].shape[0]} trials")
            self.logger.info(f"EBC_IMG: Segment shape: {trial_data['dff_segments_array'].shape}")
            ...
            self.logger.error(f"EBC_IMG_ANALYZER: Failed to analyze subject {subject_id}: {e}")
