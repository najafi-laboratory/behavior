...
class SingleIntervalDiscriminationVisualizer:
    """
    Experiment-specific visualizer for single interval discrimination data.
    ...
    def __init__(self, config_manager, subject_list, analysis_results, infrastructure, logger):
        ...
        self.logger.info("VZ-SID: Initializing SingleIntervalDiscriminationVisualizer...")
        ...
        self.logger.info("VZ-SID: SingleIntervalDiscriminationVisualizer initialized successfully")
    
    def generate_visualizations(self) -> Dict[str, Any]:
        """Generate SID-specific visualizations."""
        self.logger.info("VZ-SID: Starting single interval discrimination visualization...")
        
        visualization_results = {
            'visualization_type': 'single_interval_discrimination',
        ...
        self.logger.info(f"VZ-SID: Single interval discrimination visualization completed")
    ...
    def _visualize_subject(self, subject_id: str, subject_analysis: Dict[str, Any]):
        ...
        self.logger.info(f"VZ-SID: Generating plots for subject {subject_id}")
    ...
    def _plot_performance_over_sessions(self, subject_id: str, subject_analysis: Dict[str, Any], do_plot=False, show_plot=False):
        ...
        try:
            ...
            self.logger.info(f"VZ-SID: Generated performance plot for {subject_id}")
            
        except Exception as e:
            self.logger.error(f"VZ-SID: Failed to generate performance plot for {subject_id}: {e}")
    ...
    def _plot_session_overview_trial_type(self, subject_id: str, session_name: str, session_data: Dict[str, Any], do_plot=True, show_plot=None):
        ...
        try:
            if 'df_trials' not in session_data:
                self.logger.warning(f"VZ-SID: No trial data found for {subject_id} session {session_name}")
                return
            
            df_trials = session_data['df_trials']
            if df_trials.empty:
                self.logger.warning(f"VZ-SID: Empty trial data for {subject_id} session {session_name}")
                return
            ...
            self.logger.info(f"VZ-SID: Generated session overview trial type plot for {subject_id} session {session_name}")
            
        except Exception as e:
            self.logger.error(f"VZ-SID: Failed to generate session overview trial type plot for {subject_id} session {session_name}: {e}")
    ...
    def _plot_session_overview_rt(self, subject_id: str, session_name: str, session_data: Dict[str, Any], do_plot=True, show_plot=None):
        ...
        try:
            ...
            if 'RT' not in df_trials.columns:
                self.logger.warning(f"VZ-SID: No reaction time data found for {subject_id} session {session_name}")
                return
            ...
            self.logger.info(f"VZ-SID: Generated session overview RT plot for {subject_id} session {session_name}")
            
        except Exception as e:
            self.logger.error(f"VZ-SID: Failed to generate session overview RT plot for {subject_id} session {session_name}: {e}")
    ...
    def _plot_trial_outcomes_by_session(self, subject_id: str, subject_analysis: Dict[str, Any], do_plot=True, show_plot=None):
        ...
        try:
            if 'combined_sessions_df' not in subject_analysis:
                self.logger.warning(f"VZ-SID: No combined session data found for {subject_id}")
                return
            
            df_all_sessions = subject_analysis['combined_sessions_df'].copy()
            
            if df_all_sessions.empty:
                self.logger.warning(f"VZ-SID: Empty combined session data for {subject_id}")
                return
            ...
            if df_all_sessions.empty:
                self.logger.warning(f"VZ-SID: No trials remaining after filtering for {subject_id}")
                return
            ...
            self.logger.info(f"VZ-SID: Generated trial outcomes by session plot for {subject_id}")
            
        except Exception as e:
            self.logger.error(f"VZ-SID: Failed to generate trial outcomes by session plot for {subject_id}: {e}")
    ...
    def _visualize_group(self):
        ...
        self.logger.info("VZ-SID: Group visualization placeholder - will be implemented later")
    ...
    def _plot_rolling_performance_across_sessions(self, subject_id: str, subject_analysis: Dict[str, Any], do_plot=True, show_plot=None):
        ...
        try:
            ...
            self.logger.info(f"VZ-SID: Generated rolling performance plot for {subject_id}")
            
        except Exception as e:
            self.logger.error(f"VZ-SID: Failed to generate rolling performance plot for {subject_id}: {e}")
    ...
    def _plot_isi_pdf(self, subject_id: str, session_name: str, session_data: Dict[str, Any], session_summary: Dict[str, Any], do_plot=True, show_plot=None):
        ...
        try:
            ...
            if 'session_isi_pdf' not in session_summary:
                self.logger.info(f"VZ-SID: No ISI analysis found for {subject_id} session {session_name}")
                return
            ...
            if 'error' in session_isi_pdf:
                self.logger.warning(f"VZ-SID: ISI analysis error for {subject_id} session {session_name}: {session_isi_pdf['error']}")
                return
            
            pdf_data = session_isi_pdf.get('pdf_data', {})
            if not pdf_data:
                self.logger.warning(f"VZ-SID: No PDF data for {subject_id} session {session_name}")
                return
            ...
            self.logger.info(f"VZ-SID: Generated ISI PDF plot for {subject_id} session {session_name}")
            
        except Exception as e:
            self.logger.error(f"VZ-SID: Failed to generate ISI PDF plot for {subject_id} session {session_name}: {e}")
    ...
    def _plot_psychometric(self, subject_id: str, session_name: str, session_data: Dict[str, Any], session_summary: Dict[str, Any], do_plot=True, show_plot=None):
        ...
        try:
            ...
            if 'psychometric_analysis' not in session_summary:
                self.logger.info(f"VZ-SID: No psychometric analysis found for {subject_id} session {session_name}")
                return
            ...
            if 'error' in psychometric_analysis:
                self.logger.warning(f"VZ-SID: Psychometric analysis error for {subject_id} session {session_name}: {psychometric_analysis['error']}")
                return
            
            psychometric_curves = psychometric_analysis.get('psychometric_curves', {})
            if not psychometric_curves:
                self.logger.warning(f"VZ-SID: No psychometric curves for {subject_id} session {session_name}")
                return
            ...
            self.logger.info(f"VZ-SID: Generated psychometric plot for {subject_id} session {session_name}")
            
        except Exception as e:
            self.logger.error(f"VZ-SID: Failed to generate psychometric plot for {subject_id} session {session_name}: {e}")
    ...
    def _plot_rt_histogram(self, subject_id: str, session_name: str, session_data: Dict[str, Any], do_plot=True, show_plot=None):
        ...
        try:
            ...
            if 'RT' not in df_trials.columns:
                self.logger.warning(f"VZ-SID: No RT data found for {subject_id} session {session_name}")
                return
            ...
            if df_filtered.empty:
                self.logger.warning(f"VZ-SID: No trials remaining after filtering for {subject_id} session {session_name}")
                return
            ...
            if global_rt.empty:
                self.logger.warning(f"VZ-SID: No valid RT data for {subject_id} session {session_name}")
                plt.close(fig)
                return
            ...
            self.logger.info(f"VZ-SID: Generated RT histogram plot for {subject_id} session {session_name}")
            
        except Exception as e:
            self.logger.error(f"VZ-SID: Failed to generate RT histogram plot for {subject_id} session {session_name}: {e}")
    ...
    def _plot_rt_stats(self, subject_id: str, session_name: str, session_data: Dict[str, Any], session_summary: Dict[str, Any], do_plot=True, show_plot=None):
        ...
        try:
            ...
            if 'rt_analysis' not in session_summary:
                self.logger.info(f"VZ-SID: No RT analysis found for {subject_id} session {session_name}")
                return
            ...
            if 'error' in rt_analysis:
                self.logger.warning(f"VZ-SID: RT analysis error for {subject_id} session {session_name}: {rt_analysis['error']}")
                return
            
            rt_statistics = rt_analysis.get('rt_statistics', {})
            if not rt_statistics:
                self.logger.warning(f"VZ-SID: No RT statistics for {subject_id} session {session_name}")
                return
            ...
            self.logger.info(f"VZ-SID: Generated RT statistics plot for {subject_id} session {session_name}")
            
        except Exception as e:
            self.logger.error(f"VZ-SID: Failed to generate RT statistics plot for {subject_id} session {session_name}: {e}")
