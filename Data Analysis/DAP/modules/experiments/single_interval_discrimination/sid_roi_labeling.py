...
def plot_group_overview_v2(M: Dict[str, Any],
    ...
    Additions / config:
      cfg['trace_ylim']          : (ymin,ymax) or None
      cfg['show_onset_markers']  : bool, scatter detected onsets on rasters
      cfg['onset_marker_kwargs'] : dict for scatter
      cfg['zero_line']           : bool add y=0 dashed line to trace panels
      cfg['zero_line_kwargs']    : line kwargs
      cfg['vlim']                : (vmin,vmax) manual raster scaling
      cfg['figsize']             : figure size (default (7.2,8.0))
      cfg['sort_mode']           : 'index' | 'onset' | 'peak_both'
          'onset'    : first threshold crossing inside onset_window
          'peak_both': find absolute peak (|value|) inside peak_window
                       sort ascending by peak time, then descending by peak magnitude
                       (ROIs with no valid samples in window go to end).
      cfg['onset_window']        : (t0,t1) for onset detection
      cfg['onset_z']             : baseline z-threshold
      cfg['peak_window']         : (t0,t1) for peak_both (defaults to onset_window)
      cfg['smoothing_sigma']     : temporal Gaussian σ (samples) (default 0.8)
      cfg['events']              : list of event keys
      cfg['event_colors']        : mapping event->color

    Stored (for optional external diagnostics when using peak_both):
      cfg['peak_times'] (ordered) and cfg['peak_vals'] (ordered)

    Notes:
      * Baseline for onset detection = t < onset_window[0].
      * If peak_window yields no time samples, keeps original order.
      * Uses trial_start chunk data.
    """
    if cfg is None:
        cfg = {}
    ...
    peak_window = cfg.get('peak_window', onset_window)
    ...
    roi_mean_all   = np.nanmean(sub, axis=0)       # (nR, T)

    # Onset detection (only if needed)
    onsets = None
    if sort_mode == 'onset' or (show_onsets and sort_mode != 'peak_both'):
    ...
    elif sort_mode == 'peak_both':
        mask_pw = (t >= peak_window[0]) & (t <= peak_window[1])
        if mask_pw.any():
            seg = roi_mean_all[:, mask_pw]  # (nR, Tw)
            tw_t = t[mask_pw]
            peak_times = np.full(seg.shape[0], np.inf, float)
            peak_vals = np.zeros(seg.shape[0], float)
            with np.errstate(invalid='ignore'):
                for r in range(seg.shape[0]):
                    row = seg[r]
                    if np.any(np.isfinite(row)):
                        # absolute peak index
                        pi = int(np.nanargmax(np.abs(row)))
                        peak_times[r] = tw_t[pi]
                        peak_vals[r] = np.abs(row[pi])
            # lexsort: primary ascending peak time, secondary descending peak magnitude
            order = np.lexsort((-peak_vals, peak_times))
            cfg['peak_times'] = peak_times[order]
            cfg['peak_vals'] = peak_vals[order]
        # else keep original order if window empty

    roi_mean_short = roi_mean_short[order]
    roi_mean_long  = roi_mean_long[order]
    if onsets is not None and sort_mode != 'peak_both':
    ...
    # Color scale (shared for rasters)
    ...
    def _plot_raster(ax, mat, title, ev_stats, cond_label):
        ...
        if show_onsets and onsets_ordered is not None and sort_mode != 'peak_both':
            ok = np.isfinite(onsets_ordered)
            if np.any(ok):
                y_idx = np.flatnonzero(ok)
                ax.scatter(onsets_ordered[ok], y_idx + 0.5,
                           **onset_marker_kwargs)
        return im
    ...
    if sort_mode == 'onset':
        ...
        ax1.text(0.99, 0.98, 'sorted by peak (time→mag)', transform=ax1.transAxes,
                 ha='right', va='top', fontsize=7, color='k')
    plt.tight_layout()
...
# def plot_all_groups_v3(M: Dict[str, Any],
#                        catalog: Dict[str, Any],
#                        groups: Optional[List[str]] = None,
#                        cfg: Optional[Dict[str, Any]] = None,
#                        window_cfg: Optional[Dict[str, Tuple[float,float]]] = None,
#                        compute_global_vlim: bool = True):
#     """
#     Enhanced variant of plot_all_groups_v2 that sorts ROIs within each component
#     using an onset search restricted to a per‑chunk window.

#     Motivation
#     ----------
#     The original onset sorting used a single global onset window so early
#     transients in other epochs could shift ordering. Here, each group (component)
#     is sorted by onset time computed ONLY inside its chunk‑specific window,
#     typically matching the detection window used in
#     run_time_domain_analysis_with_windows (e.g., brief flash windows vs longer ISI).

#     Parameters
#     ----------
#     M : session dict (must contain 'trial_start' chunk)
#     catalog : component catalog (incidence + comp_labels)
#     groups : optional subset of catalog['comp_labels']; None -> all
#     cfg : dict (optional)
#         Keys (subset of plot_group_overview_v2):
#           events, event_colors, smoothing_sigma, onset_z,
#           trace_ylim, vlim, figsize, etc.
#           sort_mode ignored (always uses windowed onset; falls back to index if disabled)
#     window_cfg : dict | None
#         Mapping chunk_prefix -> (t_start, t_end) (seconds, trial_start timeline)
#         Example:
#           {
#             'start_flash_1': (-0.05, 0.15),
#             'end_flash_1':   (-0.05, 0.10),
#             'isi':           (0.0, 2.3),
#             'start_flash_2': (-0.05, 0.15),
#             'end_flash_2':   (-0.05, 0.10),
#             'choice_start':  (-0.2, 0.10),
#             'lick_start':    (-0.1, 0.50)
#           }
#         Missing keys -> fallback to cfg['onset_window'] or full axis.
#     compute_global_vlim : bool
#         If True and no cfg['vlim'] provided, derives robust symmetric vlim across
#         requested groups (same as v2).

#     Behavior
#     --------
#     1. For each component label (prefix = text before first ':'), obtain ROI indices.
#     2. Compute per‑ROI mean trace across trials on the trial_start axis.
#     3. Restrict onset search to window_cfg[prefix] (clipped to available time) and
#        compute first threshold crossing:
#           baseline = t < win_start
#           onset    = first time in [win_start, win_end] exceeding
#                      baseline_mean + onset_z * baseline_std
#     4. Sort ROI indices by onset (NaN / no onset -> end).
#     5. Call plot_group_overview_v2 with the pre‑sorted ROI list (sort_mode='index')
#        so only the windowed ordering is used.

#     Notes
#     -----
#     * Does NOT modify underlying catalog membership.
#     * Provide smaller windows for brief flashes to align peak timing visually.
#     * Falls back gracefully if window empty or no ROI crossings -> original order.
...
#     if groups is None:
#         groups = labels
#     # Prepare global vlim if requested (reuse v2 logic)
#     if compute_global_vlim and 'vlim' not in cfg:
#         mats = []
#         ch_full = M['chunks']['trial_start']['data']
#         is_short = M['is_short']
#         for lbl in groups:
...
#     # Core data
#     X = M['chunks']['trial_start']['data']  # (trials, rois, T)
#     t = M['chunks']['trial_start']['time']
#     onset_z = cfg.get('onset_z', 1.0)
#     default_onset_window = cfg.get('onset_window', (float(t[0]), float(t[-1])))

#     def _compute_onsets(mean_traces: np.ndarray, win: Tuple[float,float]) -> np.ndarray:
#         """Return per-ROI onset times limited to win (NaN if none)."""
#         win_start, win_end = win
#         # Clip window to available time
#         win_start = max(win_start, float(t[0]))
#         win_end = min(win_end, float(t[-1]))
#         if win_end <= win_start:
#             return np.full(mean_traces.shape[0], np.nan)
#         mask_win = (t >= win_start) & (t <= win_end)
#         if not mask_win.any():
#             return np.full(mean_traces.shape[0], np.nan)
#         mask_base = t < win_start
#         # Baseline stats
#         if mask_base.any():
#             base_mean = np.nanmean(mean_traces[:, mask_base], axis=1)
#             base_std = np.nanstd(mean_traces[:, mask_base], axis=1) + 1e-9
#         else:
#             # If no pre-window baseline, use entire trace (less ideal but safe fallback)
#             base_mean = np.nanmean(mean_traces, axis=1)
#             base_std = np.nanstd(mean_traces, axis=1) + 1e-9
#         thr = base_mean + onset_z * base_std
#         win_times = t[mask_win]
#         onsets = np.full(mean_traces.shape[0], np.nan)
#         for i in range(mean_traces.shape[0]):
#             seg = mean_traces[i, mask_win]
#             hits = np.where(seg > thr[i])[0]
#             if hits.size:
#                 onsets[i] = win_times[hits[0]]
#         return onsets

#     for lbl in groups:
#         if lbl not in labels:
#             print(f"[plot_all_groups_v3] skip missing label '{lbl}'")
#             continue
#         comp_idx = labels.index(lbl)
#         roi_idx = np.flatnonzero(inc[:, comp_idx])
#         if roi_idx.size == 0:
#             print(f"[plot_all_groups_v3] '{lbl}' empty ROI set")
#             continue
#         # Mean across trials per ROI
#         roi_mean_all = np.nanmean(X[:, roi_idx, :], axis=0)  # (nR, T)
#         prefix = lbl.split(':', 1)[0]
#         win = window_cfg.get(prefix, default_onset_window)
#         onsets = _compute_onsets(roi_mean_all, win)
#         # Order (NaNs / inf -> end)
#         order = np.argsort(np.where(np.isfinite(onsets), onsets, np.inf))
#         roi_idx_sorted = roi_idx[order]

#         # Pass pre-sorted ROI indices; disable internal onset sorting
#         local_cfg = dict(cfg)
#         local_cfg['sort_mode'] = 'index'          # enforce given order
#         local_cfg['onset_window'] = win           # for annotation (if later extended)
#         # Optional: store computed onsets for reference (not plotted now)
#         # local_cfg['computed_onsets'] = onsets[order]

#         plot_group_overview_v2(M, roi_idx_sorted, lbl, cfg=local_cfg)
...
def plot_all_groups_v3(M: Dict[str, Any],
                       ...
                       compute_global_vlim: bool = True,
                       *,
                       window_relative: bool = True,
                       event_anchor_map: Optional[Dict[str,str]] = None):
    """
    Windowed onset‑sort variant of plot_all_groups_v2.

    New parameters
    --------------
    window_relative : bool (default True)
        If True, window_cfg entries are interpreted as relative (t0,t1) around each
        group’s anchor event (e.g. start_flash_1). They are converted to absolute
        trial_start axis windows using the median anchor event time across trials.
        If False, window_cfg entries are treated as absolute (as in earlier version).
    event_anchor_map : dict | None
        Mapping group prefix -> event key in M used as anchor. Default mapping:
          start_flash_1 -> F1_on
          end_flash_1   -> F1_off
          isi           -> F1_off
          start_flash_2 -> F2_on
    ...
    # Precompute global vlim if requested (unchanged)
    if compute_global_vlim and 'vlim' not in cfg:
        mats = []
        ch_full = M['chunks']['trial_start']['data']
        is_short = M['is_short']
    ...
    def _absolute_window(prefix: str) -> Tuple[float,float]:
        """
        Convert (relative) window_cfg[prefix] to absolute window on trial_start axis
        using median anchor event time.
        """
        rel_win = window_cfg.get(prefix, default_onset_window)
        if not window_relative:
            return rel_win  # treat as absolute
        anchor_key = event_anchor_map.get(prefix, None)
        if anchor_key is None or anchor_key not in M:
            return rel_win  # fallback
        ev_times = np.asarray(M[anchor_key])
        ev_times = ev_times[np.isfinite(ev_times)]
        if ev_times.size == 0:
            return rel_win
        med = float(np.median(ev_times))
        return (med + rel_win[0], med + rel_win[1])

    def _compute_onsets(mean_traces: np.ndarray, abs_win: Tuple[float,float]) -> np.ndarray:
        """Return per-ROI onset times limited to absolute window abs_win (NaN if none)."""
        w0, w1 = abs_win
        w0 = max(w0, float(t[0]))
        w1 = min(w1, float(t[-1]))
        if w1 <= w0:
    ...
    for lbl in groups:
        ...
        roi_mean_all = np.nanmean(X[:, roi_idx, :], axis=0)  # (nR, T)
        prefix = lbl.split(':', 1)[0]
        abs_win = _absolute_window(prefix)
        onsets = _compute_onsets(roi_mean_all, abs_win)

        # Order: finite onsets first
        order = np.argsort(np.where(np.isfinite(onsets), onsets, np.inf))
        roi_idx_sorted = roi_idx[order]

        # Pass pre-sorted ROI indices; disable internal onset sorting
        local_cfg = dict(cfg)
        local_cfg['sort_mode'] = 'index'
        local_cfg['onset_window'] = abs_win  # absolute window used
        # For reference: store relative window in cfg if needed later
        local_cfg['relative_onset_window'] = window_cfg.get(prefix, default_onset_window) if window_relative else None
...
# def plot_chunk_components_posneg_single_isi(M: Dict[str, Any],
#                                             chunk: str,
...
#                                             events: Optional[List[str]] = None,
#                                             event_colors: Optional[Dict[str,str]] = None,
#                                             vlim_mode: str = 'global',   # 'global' or 'per_component'
#                                             vlim_quantile: float = 0.98,
#                                             sort_rois: str = 'mean',     # 'mean' | 'none'
#                                             pos_color: str = 'tab:blue',
#                                             neg_color: str = 'tab:red',
#                                             cmap: str = 'coolwarm',
#                                             figsize_per_component: Tuple[float,float] = (6.5, 3.6),
#                                             show_negative_empty: bool = True,
#                                             zero_line: bool = True,
#                                             zero_line_kwargs: Optional[Dict[str,Any]] = None,
#                                             title: Optional[str] = None
#                                             ) -> Optional[plt.Figure]:
#     """
#     Plot all components of a chunk (signed_groups) for a SINGLE ISI value.

#     For each component (in ascending comp index) produce 4 stacked subplots:
#         1. Positive ROI raster (ROI x time, mean across selected trials)
#         2. Positive mean trace (avg across POS ROIs & trials)
#         3. Negative ROI raster
#         4. Negative mean trace

#     Parameters
#     ----------
#     M : session dict (build_M_from_chunks output)
#     chunk : str
#         Chunk key (must exist in M['chunks'])
#     signed_groups : list[dict]
#         Signed groups for this chunk (fields: comp, roi_idx, positive_mask, negative_mask)
#     isi_value : float
#         Target ISI; trials selected where |M['isi'] - isi_value| <= isi_tol
#     isi_tol : float
#         Tolerance for ISI matching.
#     smoothing_sigma : float
#         Temporal Gaussian smoothing (samples) applied AFTER trial averaging (0 disables).
#     events : list[str] | None
#         Event keys to draw vertical lines (mean ± sd over selected trials).
#     event_colors : dict | None
#         Mapping event -> color.
#     vlim_mode : 'global' | 'per_component'
#         Color scale symmetric (quantile) across all rasters or per component.
#     vlim_quantile : float
#         Robust abs quantile for symmetric vlim.
#     sort_rois : 'mean' | 'none'
#         If 'mean', order ROIs within each (pos/neg) subset by decreasing mean activity (selected trials).
#     show_negative_empty : bool
#         If True still allocate negative panels with "no NEG ROIs" note when negative subset empty.
#     zero_line : bool
#         Draw horizontal y=0 line on trace panels.
#     zero_line_kwargs : dict
#         Matplotlib line kwargs override.
#     figsize_per_component : (w,h)
#         Per-component base size; total figure scales by n_components vertically.
#     title : str | None
#         Figure suptitle.
...
#         print(f"[plot_chunk_components_posneg_single_isi] chunk '{chunk}' missing.")
#         return None
#     if zero_line_kwargs is None:
#         zero_line_kwargs = dict(color='k', lw=0.6, ls='--', alpha=0.8)
#     if events is None:
#         events = ['F1_on','F1_off','F2_on','F2_off','choice_start','lick_start']
#     if event_colors is None:
#         event_colors = {
...
#     # Select trials with desired ISI
#     isi = np.asarray(M['isi'])
#     trial_mask = np.abs(isi - isi_value) <= isi_tol
#     if trial_mask.sum() == 0:
#         print(f"[plot_chunk_components_posneg_single_isi] no trials match ISI={isi_value} (tol={isi_tol}).")
#         return None

#     ch = M['chunks'][chunk]
#     X = ch['data']              # (trials, rois, time)
#     t = ch['time']
#     sel_X = X[trial_mask]       # (sel_trials, rois, time)

#     # Event stats (mean ± sd over selected trials)
#     event_stats = {}
#     for ev in events:
#         if ev in M:
#             arr = np.asarray(M[ev])[trial_mask]
#             arr = arr[np.isfinite(arr)]
#             if arr.size:
#                 event_stats[ev] = (float(np.mean(arr)), float(np.std(arr)))
#     # Prepare smoothing
#     if smoothing_sigma and smoothing_sigma > 0:
#         from scipy.ndimage import gaussian_filter1d
#         def _smooth(arr: np.ndarray) -> np.ndarray:
#             return gaussian_filter1d(arr, axis=-1, sigma=smoothing_sigma)
#     else:
#         def _smooth(arr: np.ndarray) -> np.ndarray:
#             return arr

#     # Gather rasters first if global vlim needed
#     comp_list = sorted({g['comp'] for g in signed_groups})
#     pos_rasters = {}
#     neg_rasters = {}
#     for c in comp_list:
#         g = next((gg for gg in signed_groups if gg['comp'] == c), None)
#         if g is None:
#             continue
#         roi_idx = g['roi_idx']
...
#         def build_raster(idxs):
#             if idxs.size == 0:
#                 return np.empty((0, t.size), dtype=float)
#             # Mean across trials per ROI
#             rmat = np.nanmean(sel_X[:, idxs, :], axis=0)  # (nR, T)
#             if smoothing_sigma and smoothing_sigma > 0:
#                 rmat = gaussian_filter1d(rmat, sigma=smoothing_sigma, axis=1)
#             if sort_rois == 'mean' and rmat.shape[0] > 1:
#                 order = np.argsort(-np.nanmean(np.abs(rmat), axis=1))
#                 rmat = rmat[order]
#             return rmat

#         pos_rasters[c] = build_raster(pos_idx)
#         neg_rasters[c] = build_raster(neg_idx)

#     # Global vlim if requested
#     if vlim_mode == 'global':
#         all_vals = []
#         for c in comp_list:
#             if pos_rasters[c].size:
#                 all_vals.append(pos_rasters[c].ravel())
#             if neg_rasters[c].size:
#                 all_vals.append(neg_rasters[c].ravel())
#         if all_vals:
#             concat = np.concatenate(all_vals)
#             finite = concat[np.isfinite(concat)]
#             if finite.size:
#                 lim = np.quantile(np.abs(finite), vlim_quantile)
#                 if lim <= 0:
#                     lim = np.max(np.abs(finite)) or 1.0
#             else:
#                 lim = 1.0
#         else:
#             lim = 1.0
...
#     nC = len(comp_list)
#     if nC == 0:
#         print("[plot_chunk_components_posneg_single_isi] no components in signed_groups.")
#         return None
#     fig_w, per_h = figsize_per_component
#     fig_h = nC * per_h * 4 / 3.6  # scale modestly
#     fig, axes = plt.subplots(nC * 4, 1, sharex=True, figsize=(fig_w, fig_h))
#     if nC == 1:
#         axes = np.array(axes)
...
#         vals = []
#         if pos_rasters[c].size:
#             vals.append(pos_rasters[c].ravel())
#         if neg_rasters[c].size:
#             vals.append(neg_rasters[c].ravel())
#         if not vals:
#             return (-1, 1)
...
#         # Positive raster
#         if r_pos.size:
#             im = ax_pr.imshow(r_pos, aspect='auto', origin='lower',
#                               extent=(t[0], t[-1], 0, r_pos.shape[0]),
#                               cmap=cmap, vmin=vmin, vmax=vmax)
#         else:
#             ax_pr.text(0.5, 0.5, "no POS ROIs", ha='center', va='center', transform=ax_pr.transAxes, fontsize=8)
#         ax_pr.set_ylabel(f"POS\nn={r_pos.shape[0]}", fontsize=7)
#         ax_pr.set_title(f"{chunk} | comp {c} | ISI={isi_value:g}", fontsize=9)
#         draw_events(ax_pr)

#         # Positive trace
#         if r_pos.size:
#             pos_trace = np.nanmean(r_pos, axis=0)
#             ax_pt.plot(t, pos_trace, color=pos_color, lw=1.3)
#         else:
#             ax_pt.text(0.5, 0.5, "-", ha='center', va='center', transform=ax_pt.transAxes, fontsize=8)
#         draw_events(ax_pt)
#         if zero_line:
#             ax_pt.axhline(0, **zero_line_kwargs)
#         ax_pt.set_ylabel("POS\nmean", fontsize=7)
#         ax_pt.grid(alpha=0.25, linewidth=0.4)

#         # Negative raster
#         if r_neg.size:
#             ax_nr.imshow(r_neg, aspect='auto', origin='lower',
#                          extent=(t[0], t[-1], 0, r_neg.shape[0]),
#                          cmap=cmap, vmin=vmin, vmax=vmax)
#         else:
#             if show_negative_empty:
#                 ax_nr.text(0.5, 0.5, "no NEG ROIs", ha='center', va='center', transform=ax_nr.transAxes, fontsize=8)
#         ax_nr.set_ylabel(f"NEG\nn={r_neg.shape[0]}", fontsize=7)
#         draw_events(ax_nr)

#         # Negative trace
#         if r_neg.size:
#             neg_trace = np.nanmean(r_neg, axis=0)
#             ax_nt.plot(t, neg_trace, color=neg_color, lw=1.3)
#         else:
#             if show_negative_empty:
#                 ax_nt.text(0.5, 0.5, "-", ha='center', va='center', transform=ax_nt.transAxes, fontsize=8)
#         draw_events(ax_nt)
#         if zero_line:
#             ax_nt.axhline(0, **zero_line_kwargs)
#         ax_nt.set_ylabel("NEG\nmean", fontsize=7)
#         ax_nt.grid(alpha=0.25, linewidth=0.4)

#     axes[-1].set_xlabel("Time (s)", fontsize=9)
#     if global_vlim:
#         cax = fig.add_axes([0.92, 0.15, 0.015, 0.7])
#         # Reuse last im if exists
#         try:
#             fig.colorbar(im, cax=cax, label="Mean z-dF/F")
#         except:
#             pass
#     if title:
...
def plot_chunk_components_posneg_single_isi(M: Dict[str, Any],
                                            ...
                                            *,
                                            ...
                                            zero_line_kwargs: Optional[Dict[str,Any]] = None,
                                            title: Optional[str] = None,
                                            use_full_trial: bool = False  # NEW: use full trial_start traces instead of chunk window
                                            ) -> Optional[plt.Figure]:
    """
    Plot all components (signed groups) for a SINGLE ISI value.

    If use_full_trial=True the rasters / mean traces are taken from the
    'trial_start' chunk (global full-trial timeline) instead of the
    specified chunk window. ROI memberships still come from `chunk`.
    Shared colorbar fixed (was previously incomplete).
    ...
    # Data source (window vs full trial)
    src_chunk_key = 'trial_start' if use_full_trial else chunk
    base_ch = M['chunks'][src_chunk_key]
    X_full = base_ch['data']          # (trials, rois, time)
    t = base_ch['time']
    sel_X = X_full[trial_mask]        # (sel_trials, rois, time)
    ...
    # Build rasters (mean across selected trials per ROI)
    pos_rasters = {}
    neg_rasters = {}
    for c in comp_list:
        g = next((gg for gg in signed_groups if gg['comp'] == c), None)
        ...
        def build_raster(idxs: np.ndarray):
            if idxs.size == 0:
                return np.empty((0, t.size), dtype=float)
            rmat = np.nanmean(sel_X[:, idxs, :], axis=0)  # (nR, time)
            if smoothing_sigma and smoothing_sigma > 0 and rmat.size:
                rmat = _smooth(rmat)
            if sort_rois == 'mean' and rmat.shape[0] > 1:
                order = np.argsort(np.nanmean(rmat, axis=1))[::-1]
                rmat = rmat[order]
            return rmat
    ...
    # Global vlim
    def _robust_lim(vals: np.ndarray, q=0.98):
    ...
    if vlim_mode == 'global':
        ...
        for c in comp_list:
            if pos_rasters[c].size:
                acc.append(pos_rasters[c].ravel())
            if neg_rasters[c].size:
                acc.append(neg_rasters[c].ravel())
        if acc:
            vmin_global, vmax_global = _robust_lim(np.concatenate(acc), vlim_quantile)
        else:
    ...
    nC = len(comp_list)
    fig_w, per_h = figsize_per_component
    fig_h = nC * per_h * 4 / 3.6
    fig, axes = plt.subplots(nC * 4, 1, sharex=True, figsize=(fig_w, fig_h))
    if nC == 1:
        axes = np.array(axes)

    ims = []  # collect first raster for colorbar

    def component_vlim(c):
        if vlim_mode == 'global':
            return vmin_global, vmax_global
        loc = []
        if pos_rasters[c].size:
            loc.append(pos_rasters[c].ravel())
        if neg_rasters[c].size:
            loc.append(neg_rasters[c].ravel())
        if not loc:
            return (-1, 1)
        return _robust_lim(np.concatenate(loc), vlim_quantile)
    ...
    for i, c in enumerate(comp_list):
        r_pos = pos_rasters[c]
        r_neg = neg_rasters[c]
        vmin_c, vmax_c = component_vlim(c)
        base = i * 4
        ax_pr, ax_pt, ax_nr, ax_nt = axes[base:base+4]
        ...
        if r_pos.size:
            ...
            ax_pr.text(0.5, 0.5, "no POS ROIs", ha='center', va='center', transform=ax_pr.transAxes, fontsize=8)
        ax_pr.set_ylabel(f"POS\nn={r_pos.shape[0]}", fontsize=7)
        ax_pr.set_title(f"{chunk}{' (full)' if use_full_trial else ''} | comp {c} | ISI={isi_value:g}", fontsize=9)
        draw_events(ax_pr)

        # Positive mean trace
        if r_pos.size:
            pos_mean = np.nanmean(r_pos, axis=0)
            ax_pt.plot(t, pos_mean, color=pos_color, lw=1.2)
        else:
            ax_pt.text(0.5, 0.5, "-", ha='center', va='center', transform=ax_pt.transAxes, fontsize=8)
        draw_events(ax_pt)
        if zero_line:
        ...
        # Negative raster
        if r_neg.size:
            imn = ax_nr.imshow(r_neg, aspect='auto', origin='lower',
                               extent=(t[0], t[-1], 0, r_neg.shape[0]),
                               cmap=cmap, vmin=vmin_c, vmax=vmax_c)
            if not ims:
                ims.append(imn)
        ...
        # Negative mean trace
        if r_neg.size:
            neg_mean = np.nanmean(r_neg, axis=0)
            ax_nt.plot(t, neg_mean, color=neg_color, lw=1.2)
        else:
            if show_negative_empty:
                ax_nt.text(0.5, 0.5, "-", ha='center', va='center', transform=ax_nt.transAxes, fontsize=8)
        draw_events(ax_nt)
    ...
    axes[-1].set_xlabel("Time (s)" + (" (full trial)" if use_full_trial else ""), fontsize=9)

    # Shared colorbar
    if ims:
        # Use a single colorbar for all rasters (consistent scale if global; still OK per-component)
        cbar = fig.colorbar(ims[0], ax=axes, fraction=0.015, pad=0.012)
        cbar.set_label("Mean z-dF/F", fontsize=8)
...
def run_time_domain_analysis_with_windows(M: Dict[str, Any],
                                          chunk_keys: List[str],
                                          ranks: List[int],
                                          min_abs_corr: float = 0.65,
                                          *,
    ...
    """
    Window-aware variant of run_time_domain_analysis.

    Purpose
    -------
    Reduce cross‑alignment / overlapping detections by constraining temporal
    energy of each chunk to a user-specified (start, end) window (in seconds,
    same time base as M['chunks'][chunk]['time']). Outside the window data
    are zeroed (default) so component shapes remain in original time frame
    and downstream catalog integration stays compatible.

    Parameters
    ----------
    M : session dict (as before)
    chunk_keys : list of chunk names to process
    ranks : candidate CP ranks (forwarded)
    min_abs_corr : stability pruning threshold (forwarded to original function)
    window_cfg : dict {chunk: (t_start, t_end)}. Chunks absent -> full span.
    zero_outside : if True (default) set data outside window to 0.
                   (If False future extension could crop; not implemented here.)
    verbose : print per-chunk summary.

    Returns
    -------
    time_results (dict):
        Same keys / fields as original run_time_domain_analysis PLUS:
          'time_window' : (t_start, t_end) or None
          'time_mask'   : boolean mask over original time vector
          'window_applied' : bool
    """
    if window_cfg is None:
        window_cfg = {}

    # Ensure original function exists
    if 'run_time_domain_analysis' not in globals():
        raise RuntimeError("Original run_time_domain_analysis not found.")

    time_results: Dict[str, Dict[str, Any]] = {}
    for ch in chunk_keys:
        chunk_obj = M['chunks'][ch]
        t = chunk_obj['time']
        data = chunk_obj['data']  # (trials, rois, time)
        win = window_cfg.get(ch, None)

        if win is not None:
            t0, t1 = win
            mask = (t >= t0) & (t <= t1)
            if not mask.any():
                raise ValueError(f"[{ch}] window {win} selects no time points.")
            # Copy & zero outside
            if zero_outside:
                data_mod = data.copy()
                data_mod[:, :, ~mask] = 0.0
                # Build temporary shallow copy of M with modified chunk
                M_tmp = dict(M)
                M_tmp['chunks'] = dict(M['chunks'])
                M_tmp['chunks'][ch] = dict(chunk_obj)
                M_tmp['chunks'][ch]['data'] = data_mod
            else:
                # (future: implement cropping + padding restoration)
                raise NotImplementedError("Cropping mode not implemented; use zero_outside=True.")
        else:
            mask = np.ones_like(t, dtype=bool)
            M_tmp = M

        # Run original analysis on just this chunk
        base_res = run_time_domain_analysis(M_tmp, [ch], ranks, min_abs_corr)
        res = base_res[ch]
        # Attach window metadata
        res['time_window'] = win
        res['time_mask'] = mask
        res['window_applied'] = (win is not None)
        time_results[ch] = res

        if verbose:
            if win is None:
                print(f"[time-win] {ch}: no window (full span {t[0]:.3f}->{t[-1]:.3f}s) rank={res['rank']}")
            else:
                kept = mask.sum()
                print(f"[time-win] {ch}: window {win} kept {kept}/{t.size} time bins rank={res['rank']}")

    return time_results

# Example:
# time_results_w = run_time_domain_analysis_with_windows(
#     M,
#     chunk_keys=['start_flash_1','end_flash_1','isi','start_flash_2','end_flash_2','choice_start','lick_start'],
#     ranks=[6,8,10],
#     min_abs_corr=0.65,
#     window_cfg={
#         'start_flash_1': (-0.05, 0.45),
#         'end_flash_1':   (-0.05, 0.55),
#         'isi':           (0.0, 2.3),
#         'start_flash_2': (-0.05, 0.55),
#
...
def integrate_signed_groups_into_catalog(catalog: Dict[str, Any],
                                         ...
                                         keep_original: bool = True  # NEW
                                         ) -> Dict[str, Any]:
    """
    Augment catalog incidence matrix with separate positive / negative columns
    for each signed component.

    Set keep_original=False to return a catalog containing ONLY the newly added
    signed columns (original component columns discarded).
    """
    if catalog is None or 'incidence' not in catalog:
        raise ValueError("Base catalog must exist (run build_component_catalog first).")

    inc = catalog['incidence']            # (n_rois, n_components_orig)
    labels = list(catalog['comp_labels']) # original labels
    n_rois = inc.shape[0]
    ...
    for chunk, groups in signed_groups_by_chunk.items():
        for g in groups:
            ...
            # Positive
            lbl_pos = f"{label_prefix}{chunk}:comp{c}_pos"
            if (not skip_existing) or (lbl_pos not in existing):
                col = np.zeros(n_rois, dtype=bool)
                col[pos_idx] = True
                new_cols.append(col)
                new_labels.append(lbl_pos)
                existing.add(lbl_pos)

            # Negative (if non-empty)
            if neg_idx.size:
                lbl_neg = f"{label_prefix}{chunk}:comp{c}_neg"
                if (not skip_existing) or (lbl_neg not in existing):
                    col = np.zeros(n_rois, dtype=bool)
                    col[neg_idx] = True
                    new_cols.append(col)
                    new_labels.append(lbl_neg)
                    existing.add(lbl_neg)

    if new_cols:
        new_block = np.column_stack(new_cols)
        if keep_original:
            catalog['incidence'] = np.column_stack([inc, new_block])
            catalog['comp_labels'] = labels + new_labels
        else:
            catalog['incidence'] = new_block
            catalog['comp_labels'] = new_labels
    else:
        if not keep_original:
            # No new columns but caller asked to drop originals -> empty catalog
            catalog['incidence'] = np.zeros((n_rois, 0), dtype=bool)
            catalog['comp_labels'] = []

    return catalog


# def build_signed_only_catalog(M: Dict[str, Any],
#                               signed_groups_by_chunk: Dict[str, List[Dict[str, Any]]],
#                               *,
#                               base_catalog: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
#     """
#     Convenience: start from an existing catalog (for ROI count) but return a
#     catalog containing ONLY signed pos/neg columns.
#     """
#     if base_catalog is None:
#         raise ValueError("Provide base_catalog (output of build_component_catalog).")
#     signed_only = dict(incidence=base_catalog['incidence'],
#                        comp_labels=list(base_catalog['comp_labels']))
#     signed_only = integrate_signed_groups_into_catalog(signed_only, M,
...
def build_signed_only_catalog(M: Dict[str, Any],
    ...
    """
    Return a catalog containing ONLY signed pos/neg columns.

    Important:
      Pass the ORIGINAL (pre‑signed) base catalog (output of build_component_catalog),
      or the function will see existing *_pos/_neg labels and skip them.

    Parameters
    ----------
    base_catalog : original catalog (component incidence only)
    skip_existing : set False so columns are always added.

    """
    if base_catalog is None:
        raise ValueError("Provide base_catalog (output of build_component_catalog before signed integration).")
    n_rois = base_catalog['incidence'].shape[0]
    empty = dict(incidence=np.zeros((n_rois, 0), dtype=bool),
                 comp_labels=[])
    signed_only = integrate_signed_groups_into_catalog(
        empty, M,
        signed_groups_by_chunk,
        label_prefix='',
...
# After calling integrate_signed_groups_into_catalog(...)
def rebuild_catalog_entries(cat):
    inc = cat['incidence']
    labels = cat['comp_labels']
    entries = []
    for j,lbl in enumerate(labels):
        roi_idx = np.flatnonzero(inc[:, j])
        entries.append(dict(label=lbl, rois=roi_idx, n_rois=roi_idx.size))
    cat['catalog'] = entries
...
def plot_intersection_roi_rasters(M: Dict[str, Any],
                                  parts: Dict[str, Any],
                                  group_order: List[str],
                                  *,
                                  categories: Tuple[str, ...] = ('non','within','across'),
                                  min_group_size: Optional[int] = None,
                                  smoothing_sigma: float = 0.0,
                                  abbreviate: Optional[Dict[str,str]] = None,
                                  cmap: str = 'coolwarm',
                                  vlim_quant: float = 0.98,
                                  pos_neg_order: Tuple[str,...] = ('pos','neg','ambig'),
                                  show_colorbar: bool = True,
                                  figsize_per_roi: float = 0.018,
                                  max_fig_height: float = 14.0,
                                  group_label_fontsize: int = 7,
                                  event_lines: Optional[List[str]] = None,
                                  event_colors: Optional[Dict[str,str]] = None,
                                  verbose: bool = True,
                                  # NEW:
                                  short_isis: Optional[List[float]] = None,
                                  long_isis: Optional[List[float]] = None,
                                  isi_match_tol: float = 1e-6,
                                  split_condition_figs: bool = False,
                                  shared_vlim: bool = True,
                                  normalize_rois: bool = False,
                                  event_stat: str = 'median',
                                  event_spread: Optional[str] = 'std',
                                  # NEW ORDERING / SORTING:
                                  group_time_order: bool = True,
                                  roi_sort_mode: str = 'onset',
                                  onset_windows: Optional[Dict[str, Tuple[float,float]]] = None,
                                  onset_z: float = 1.0
                                  ) -> Tuple[Union[plt.Figure, Dict[str,plt.Figure]], Dict[str, Any]]:
    """
    Enhanced: condition-specific ISI filtering & event medians.

    short_isis / long_isis : lists of ISI values to KEEP for each condition (None = all of that condition).
    isi_match_tol          : tolerance for float matching (abs(isi - target) <= tol).
    split_condition_figs   : if True returns dict {'short':fig_short,'long':fig_long} (separate panels).
    shared_vlim            : if True both conditions share symmetric vlim; else separate scaling.
    normalize_rois         : if True z-score each ROI across time within condition before averaging trials.
    event_stat             : 'median' or 'mean' for event time per condition.
    event_spread           : spread shading: 'std', 'iqr', or None.
    """
    import numpy as np
    import matplotlib.pyplot as plt

    if event_lines is None:
        event_lines = ['F1_on','F1_off','F2_on','F2_off','choice_start','lick_start']
    if event_colors is None:
        event_colors = {
            'F1_on':'#1f77b4','F1_off':'#1f77b4',
            'F2_on':'#ff7f0e','F2_off':'#ff7f0e',
    ...
    # --- Build base matrices (re-use existing logic) ---
    # Reuse original function's grouping / ordering by calling an internal helper (extract rows without trial averaging now)
    # We replicate minimal necessary earlier code (to avoid refactor of existing).
    cat_map = {
        'non':    parts['non_intersectional'],
        'within': parts['within_group_intersectional'],
        'across': parts['across_group_intersectional']
    }
    group_rank = {g: i for i, g in enumerate(group_order)} if group_order else {}

    # Helper parsers as before (simplified)
    def _parse_label_non(lbl: str):
        base, _, tail = lbl.partition(':')
        grp = base
        comp_idx = -1
        sign = ''
        if 'comp' in tail:
            after = tail.split('comp',1)[1]
            num = ''.join(ch for ch in after if ch.isdigit())
            comp_idx = int(num) if num else -1
            if after.endswith('_pos'): sign='pos'
            elif after.endswith('_neg'): sign='neg'
            elif after.endswith('_ambig'): sign='ambig'
        return (group_rank.get(grp,1e9), comp_idx, sign, grp)

    def _parse_label_within(lbl: str):
        base, _, tail = lbl.partition(':')
        grp = base
        sign=''
        if lbl.endswith('_pos'): sign='pos'
        elif lbl.endswith('_neg'): sign='neg'
        elif lbl.endswith('_ambig'): sign='ambig'
        core = tail.rsplit('_',1)[0] if sign else tail
        comp_part = core
        if comp_part.startswith('c_'): comp_part = comp_part[2:]
        comps = tuple(int(x) for x in comp_part.split('_') if x.isdigit())
        return (group_rank.get(grp,1e9), comps, sign, grp)

    def _parse_label_across(lbl: str):
        sign=''
        if lbl.endswith('_pos'): sign='pos'
        elif lbl.endswith('_neg'): sign='neg'
        elif lbl.endswith('_ambig'): sign='ambig'
        base = lbl.rsplit('_',1)[0] if sign else lbl
        left, _, right = base.partition(':c_')
        groups = left.split('+')
        granks = tuple(group_rank.get(g,1e9) for g in groups)
        segs = right.split('|') if right else []
        comp_sets=[]
        for seg in segs:
            if '-' not in seg:
                comp_sets.append(tuple())
                continue
            _, nums = seg.split('-',1)
            comp_sets.append(tuple(int(x) for x in nums.split('_') if x.isdigit()))
        return (granks, tuple(comp_sets), sign, left)

    # Collect group entries (same as before but store ROI list)
    group_entries = []
    for cat_key in categories:
        cat = cat_map[cat_key]
        incC = cat['incidence']
        labelsC = cat['comp_labels']
        for j,lbl in enumerate(labelsC):
            roi_idx = np.flatnonzero(incC[:, j])
            if roi_idx.size == 0:
                continue
            if min_group_size and roi_idx.size < min_group_size:
                continue
            if cat_key == 'non':
                grk, comp_idx, sign, grp = _parse_label_non(lbl)
                base_rank = grk
                order_key = (base_rank, comp_idx, pos_neg_order.index(sign) if sign in pos_neg_order else 0)
            elif cat_key == 'within':
                grk, comps, sign, grp = _parse_label_within(lbl)
                base_rank = grk
                order_key = (base_rank, comps, pos_neg_order.index(sign) if sign in pos_neg_order else 0)
            else:
                grks, comp_sets, sign, grp_concat = _parse_label_across(lbl)
                base_rank = min(grks) if len(grks) else 1e9
                order_key = (base_rank, grks, pos_neg_order.index(sign) if sign in pos_neg_order else 0)

            group_entries.append(dict(
                label=lbl,
    ...
    # --- NEW ORDERING BLOCK (replace previous group_time_order logic) ---
    # Desired top→bottom base group order (override group_order if provided list differs)
    desired_order = group_order  # e.g. ['start_flash_1','end_flash_1','isi','start_flash_2','end_flash_2','choice_start','lick_start']
    order_index = {g:i for i,g in enumerate(desired_order)}

    def primary_group(label: str) -> str:
        # non / within: prefix before ':'
        prefix = label.split(':',1)[0]
        if '+' not in prefix:
            return prefix
        # across: composite "g1+g2+..."; choose earliest in desired_order
        parts = prefix.split('+')
        parts_sorted = sorted(parts, key=lambda x: order_index.get(x, 1e9))
        return parts_sorted[0] if parts_sorted else prefix

    cat_priority = {'non':0,'within':1,'across':2}
    sign_priority = {'pos':0,'neg':1,'ambig':2}

    for ge in group_entries:
        lbl = ge['label']
        prim = primary_group(lbl)
        ge['primary_group'] = prim
        ge['primary_rank'] = order_index.get(prim, 1e9)
        # infer category & sign
        if '_ambig' in lbl: sgn='ambig'
        elif '_neg' in lbl: sgn='neg'
        elif '_pos' in lbl: sgn='pos'
        else: sgn='pos'
        ge['sign_order'] = sign_priority.get(sgn, 99)
        # recover category tag
        if '+':  # quick check; refine
            if '+' in lbl.split(':',1)[0]:
                cat='across'
            else:
                # within groups have ":c_"
                cat='within' if ':c_' in lbl and '+c_' not in lbl else 'non'
        if 'category' in ge:
            cat = ge.get('category', cat)
        ge['cat'] = cat
        ge['cat_order'] = cat_priority.get(cat, 99)

    # Group into blocks per base group in desired order
    # Sort within each block by (cat_order, sign_order, label)
    grouped = []
    for g in desired_order:
        block = [ge for ge in group_entries if ge['primary_group'] == g]
        block.sort(key=lambda d: (d['cat_order'], d['sign_order'], d['label']))
        grouped.extend(block)

    # Append any leftover (unexpected prefixes) at end
    leftovers = [ge for ge in group_entries if ge['primary_group'] not in desired_order]
    if leftovers:
        leftovers.sort(key=lambda d: (d['primary_group'], d['cat_order'], d['sign_order'], d['label']))
        grouped.extend(leftovers)

    group_entries = grouped

    # --- NEW: ROI onset sorting inside each group ---
    if roi_sort_mode == 'onset':
        X_all = M['chunks']['trial_start']['data']  # (trials, rois, T)
        t_all = M['chunks']['trial_start']['time']
        full_range = (float(t_all[0]), float(t_all[-1]))
        ow = onset_windows or {}
        for ge in group_entries:
            prefix = ge['label'].split(':',1)[0]
            win = ow.get(prefix, full_range)
            mask_win = (t_all >= win[0]) & (t_all <= win[1])
            if not mask_win.any():
                continue
            mask_base = t_all < win[0]
            roi_list = ge['roi_idx']
            onsets = []
            for r in roi_list:
                trace = np.nanmean(X_all[:, r, :], axis=0)
                base_mean = np.nanmean(trace[mask_base]) if mask_base.any() else np.nanmean(trace)
                base_std  = np.nanstd(trace[mask_base]) + 1e-9 if mask_base.any() else (np.nanstd(trace)+1e-9)
                thr = base_mean + onset_z * base_std
                seg = trace[mask_win]
                hit = np.where(seg > thr)[0]
                if hit.size:
                    onsets.append(t_all[mask_win][hit[0]])
                else:
                    onsets.append(np.inf)
            # Sort ROI indices by onset (stable: np.inf go last)
            order = np.argsort(onsets)
            ge['roi_idx'] = roi_list[order]
            ge['onset_times'] = np.array(onsets)[order]

    # Trial selection masks (with ISI filters)
    isi = np.asarray(M['isi'])
    is_short = np.asarray(M['is_short']).astype(bool)
    def _filter_mask(base_mask: np.ndarray, isi_list: Optional[List[float]]) -> np.ndarray:
        if isi_list is None or len(isi_list)==0:
            return base_mask
        keep = np.zeros_like(base_mask)
        for v in isi_list:
            keep |= np.abs(isi - v) <= isi_match_tol
        return base_mask & keep

    short_mask = _filter_mask(is_short, short_isis)
    long_mask  = _filter_mask(~is_short, long_isis)

    # Provide counts summary
    if verbose:
        def _fmt_list(lst):
            if not lst: return "ALL"
            return ",".join(str(x) for x in lst)
        print(f"[intersection rasters] short trials kept: {short_mask.sum()} (ISI filter={_fmt_list(short_isis)})")
        print(f"[intersection rasters] long trials kept : {long_mask.sum()} (ISI filter={_fmt_list(long_isis)})")
    ...
    if smoothing_sigma > 0:
        from scipy.ndimage import gaussian_filter1d
    # Build per-condition ROI x time matrices (stack all ROIs in group order)
    row_meta = []
    def _build_condition_matrix(mask: np.ndarray) -> np.ndarray:
        rows = []
        for ge in group_entries:
            for r in ge['roi_idx']:
                # average across selected trials for this single ROI
                tr_sel = X[mask, r, :]
                if tr_sel.size == 0:
                    mean_trace = np.full(t.shape, np.nan)
                else:
                    mean_trace = np.nanmean(tr_sel, axis=0)
                rows.append(mean_trace)
                row_meta.append(dict(group_label=ge['label'], roi=int(r)))
        mat = np.vstack(rows)
        if normalize_rois:
            ...
            mat = (mat - m)/s
        if smoothing_sigma > 0:
            mat = gaussian_filter1d(mat, sigma=smoothing_sigma, axis=1)
        return mat

    short_mat = _build_condition_matrix(short_mask)
    long_mat  = _build_condition_matrix(long_mask)

    # Color scaling
    if shared_vlim:
        vals = np.concatenate([short_mat.ravel(), long_mat.ravel()])
        finite = vals[np.isfinite(vals)]
        if finite.size:
            lim = np.quantile(np.abs(finite), vlim_quant)
            if lim <= 0: lim = np.max(np.abs(finite)) or 1.0
        else:
            lim = 1.0
        vlims = dict(short=(-lim, lim), long=(-lim, lim))
    else:
        vlims = {}
        for name, mat in [('short', short_mat), ('long', long_mat)]:
    ...
    # Event stats per condition
    def _event_stats(ev, mask):
        if ev not in M: return (np.nan, np.nan)
        arr = np.asarray(M[ev])[mask]
        arr = arr[np.isfinite(arr)]
        if arr.size == 0:
            return (np.nan, np.nan)
        if event_stat == 'median':
            center = float(np.median(arr))
        else:
            center = float(np.mean(arr))
        if event_spread == 'std':
            spread = float(np.std(arr))
        elif event_spread == 'iqr':
            q75, q25 = np.percentile(arr, [75,25])
            spread = float(0.5*(q75 - q25))
        else:
            spread = np.nan
        return (center, spread)

    ev_stats_short = {ev: _event_stats(ev, short_mask) for ev in event_lines}
    ev_stats_long  = {ev: _event_stats(ev, long_mask)  for ev in event_lines}

    def _plot_panel(mat, title, ev_stats, vmin, vmax):
        n_rows = mat.shape[0]
        fig_h = min(max(3.0, n_rows * figsize_per_roi + 1.8), max_fig_height)
        fig, ax = plt.subplots(1,1, figsize=(9.0, fig_h))
        im = ax.imshow(mat, aspect='auto', origin='lower',
                       extent=(t[0], t[-1], 0, n_rows),
                       cmap=cmap, vmin=vmin, vmax=vmax)
        ax.set_title(title, fontsize=10)
        ax.set_ylabel("ROIs")
        # horizontal group separators + y tick labels (center per group)
        y_cursor = 0
        yticks = []
        ylabels = []
        for ge in group_entries:
            size = ge['roi_idx'].size
            y0, y1 = y_cursor, y_cursor + size
            y_mid = y0 + size / 2
            lab = ge['label']
            if abbreviate:
                for full, short in abbreviate.items():
                    lab = lab.replace(full, short)
            yticks.append(y_mid)
            ylabels.append(lab)
            ax.axhline(y1, color='k', lw=0.3, alpha=0.4)
            y_cursor = y1
        # lighten tick density if huge
        max_labels = int(fig_h * 3.3)
        if len(yticks) > max_labels:
            step = int(np.ceil(len(yticks)/max_labels))
            yticks_disp = yticks[::step]; ylabels_disp = ylabels[::step]
        else:
            yticks_disp, ylabels_disp = yticks, ylabels
        ax.set_yticks(yticks_disp)
        ax.set_yticklabels(ylabels_disp, fontsize=group_label_fontsize)
        ax.set_xlim(t[0], t[-1])
        ...
        if show_colorbar:
            ...
            cbar.set_label("Mean z-dF/F" + (" (per-ROI norm)" if normalize_rois else ""), fontsize=8)
        ax.set_xlabel("Time (s)")
        return fig, ax

    if split_condition_figs:
        fig_short, _ = _plot_panel(short_mat, "Short trials", ev_stats_short, *vlims['short'])
        fig_long, _  = _plot_panel(long_mat,  "Long trials",  ev_stats_long,  *vlims['long'])
        figs = {'short': fig_short, 'long': fig_long}
        meta = dict(
            short_rows=short_mat.shape[0],
            long_rows=long_mat.shape[0],
            vlims=vlims,
            short_trials=int(short_mask.sum()),
            long_trials=int(long_mask.sum()),
        ...
        # Original two-panel combined figure (condition-specific events & vlims)
        n_rows = short_mat.shape[0]
        fig_h = min(max(4.0, n_rows * figsize_per_roi + 2.2), max_fig_height)
        fig, axes = plt.subplots(2,1, sharex=True, figsize=(9.0, fig_h), gridspec_kw={'hspace':0.08})
        for mat, ax, title, evs, key in [
            (short_mat, axes[0], "Short trials", ev_stats_short, 'short'),
            (long_mat, axes[1],  "Long trials",  ev_stats_long,  'long')
        ]:
            im = ax.imshow(mat, aspect='auto', origin='lower',
                           extent=(t[0], t[-1], 0, mat.shape[0]),
                           cmap=cmap, vmin=vlims[key][0], vmax=vlims[key][1])
            ax.set_title(title, fontsize=10)
            ax.set_ylabel("ROIs")
            # separators & labels
            y_cursor=0
            ...
            for ge in group_entries:
                ...
                if abbreviate:
                    for full, short in abbreviate.items():
                        lab = lab.replace(full, short)
                yticks.append(y_mid); ylabels.append(lab)
                ax.axhline(y1, color='k', lw=0.3, alpha=0.4)
                y_cursor=y1
            ax.set_yticks(yticks)
            ax.set_yticklabels(ylabels, fontsize=group_label_fontsize)
            ax.set_ylim(0, mat.shape[0])
            for ev,(mu,sd) in evs.items():
                if not np.isfinite(mu): continue
                col = event_colors.get(ev,'k')
                ax.axvline(mu, color=col, lw=0.9)
                if np.isfinite(sd) and sd>0:
        ...
        if show_colorbar:
            ...
            cbar.set_label("Mean z-dF/F" + (" (per-ROI norm)" if normalize_rois else ""), fontsize=8)
        if verbose:
            print(f"[intersection rasters] vlims short={vlims['short']} long={vlims['long']}")
        meta = dict(
            rows=n_rows,
            vlims=vlims,
            short_trials=int(short_mask.sum()),
...
def plot_intersection_roi_rasters_condition(M: Dict[str, Any],
                                            parts: Dict[str, Any],
                                            group_order: List[str],
                                            *,
                                            condition: str,
                                            isis: Optional[List[float]] = None,
                                            **kwargs):
    """
    Convenience wrapper: single-condition raster (short or long) with ISI filtering.

    condition : 'short' | 'long'
    isis      : list of ISI values to keep (None = all of that condition)
    Other kwargs forwarded to plot_intersection_roi_rasters (split_condition_figs forced True).
    """
    if condition not in ('short','long'):
        raise ValueError("condition must be 'short' or 'long'")
    if condition == 'short':
        figs, meta = plot_intersection_roi_rasters(M, parts, group_order,
                                                   split_condition_figs=True,
                                                   short_isis=isis,
                                                   long_isis=None,
                                                   **kwargs)
        return figs['short'], meta
    else:
        figs, meta = plot_intersection_roi_rasters(M, parts, group_order,
                                                   split_condition_figs=True,
                                                   short_isis=None,
...
#                                   event_lines: Optional[List[str]] = None,
#                                   event_colors: Optional[Dict[str,str]] = None,
#                                   verbose: bool = True) -> Tuple[plt.Figure, Dict[str, Any]]:
#     """
#     Two stacked rasters (Short trials, Long trials):
#       Rows = individual ROIs grouped & ordered:
#          1. Non-intersectional components
#          2. Within-group fused subsets
#          3. Across-group fused subsets
#       Inside each category groups are ordered by trial progression (group_order).
#       Within each group ROIs are sorted by their index (stable & reproducible).
    
#     Signals: trial_start chunk data. Each pixel = mean across selected trials (short or long) for one ROI.
    
#     Parameters
#     ----------
#     parts : output of build_intersection_partitions
#     group_order : canonical temporal order of base groups
#     categories : subset ('non','within','across') to plot
#     min_group_size : drop groups smaller than this (None = keep all)
#     smoothing_sigma : temporal Gaussian smoothing (samples) AFTER trial averaging
#     abbreviate : optional mapping full_group_name -> short token for labels
#     vlim_quant : robust symmetric limit quantile over all displayed values
#     pos_neg_order : ordering of sign suffixes when present
#     figsize_per_roi : vertical size scaling per ROI (in inches)
#     max_fig_height : clamp total figure height
#     Returns
#     -------
#     fig, info dict (row_metadata list with fields: group_label, roi)
#     """
#     if event_lines is None:
#         event_lines = ['F1_on','F1_off','F2_on','F2_off','choice_start','lick_start']
#     if event_colors is None:
...
#         'across': parts['across_group_intersectional']
#     }
#     # ADD: build group_rank mapping (fix NameError)
#     group_rank = {g: i for i, g in enumerate(group_order)} if group_order else {}
#     missing = [c for c in categories if c not in cat_map]
#     if missing:
#         raise ValueError(f"Unknown category keys: {missing}")
#     # Data source
#     X = M['chunks']['trial_start']['data']   # (trials, rois, T)
#     t = M['chunks']['trial_start']['time']
#     is_short = M['is_short'].astype(bool)
#     # Precompute mean traces per ROI per condition
#     # (Doing group-wise slicing still cheap; but we build on demand to save memory if large)
#     # --- Helpers ---
#     def _parse_label_non(lbl: str) -> Tuple[int,int,str,str]:
#         # "<group>:compK[_pos|_neg]" or "<group>:K" fallback
#         # return (group_rank, comp_idx, sign_suffix, base_group)
#         base, _, tail = lbl.partition(':')
#         grp = base
#         comp_idx = -1
#         sign = ''
...
#             # tail like "comp7_pos"
#             after = tail.split('comp',1)[1]
#             # split on '_' first token numeric
#             num = ''.join(ch for ch in after if ch.isdigit())
#             if num != '':
#                 comp_idx = int(num)
#             if after.endswith('_pos'): sign='pos'
...
#             elif after.endswith('_ambig'): sign='ambig'
#         return (group_rank.get(grp, 1e9), comp_idx, sign, grp)
#     def _parse_label_within(lbl: str) -> Tuple[int, Tuple[int,...], str, str]:
#         # "<group>:c_i_j_k_pos"
#         base, _, tail = lbl.partition(':')
#         grp = base
#         sign = ''
...
#         core = tail.split(':')[-1]
#         # strip sign suffix
#         core_no = core.rsplit('_',1)[0] if sign else core
#         # remove leading "c_"
#         comp_part = core_no.split(':')[-1]
#         if comp_part.startswith('c_'):
#             comp_part = comp_part[2:]
#         comps = tuple(int(x) for x in comp_part.split('_') if x.isdigit())
#         return (group_rank.get(grp,1e9), comps, sign, grp)
#     def _parse_label_across(lbl: str) -> Tuple[Tuple[int,...], Tuple[Tuple[int,...],...], str, str]:
#         # "g1+g2+...:c_g1-a_b|g2-c|..."
#         sign=''
#         if lbl.endswith('_pos'): sign='pos'
#         elif lbl.endswith('_neg'): sign='neg'
...
#         left, _, right = base.partition(':c_')
#         groups = left.split('+')
#         group_ranks = tuple(group_rank.get(g,1e9) for g in groups)
#         segs = right.split('|')
#         comp_sets=[]
#         for seg in segs:
#             # seg: gX-idx_idx_idx
...
#                 comp_sets.append(tuple())
#                 continue
#             gpart, cpart = seg.split('-',1)
#             nums = tuple(int(x) for x in cpart.split('_') if x.isdigit())
#             comp_sets.append(nums)
#         return (group_ranks, tuple(comp_sets), sign, left)
#     # Collect groups
#     group_entries = []  # list of dicts: {category,label,roi_idx_list, order_key, size}
#     for cat_key in categories:
#         cat = cat_map[cat_key]
#         inc = cat['incidence']
#         labels = cat['comp_labels']
...
#             if cat_key == 'non':
#                 grk, comp_idx, sign, grp = _parse_label_non(lbl)
#                 order_key = (0, grk, comp_idx, pos_neg_order.index(sign) if sign in pos_neg_order else 0)
#             elif cat_key == 'within':
#                 grk, comps, sign, grp = _parse_label_within(lbl)
#                 order_key = (1, grk, comps, pos_neg_order.index(sign) if sign in pos_neg_order else 0)
#             else:  # across
#                 grks, comp_sets, sign, grp_concat = _parse_label_across(lbl)
#                 order_key = (2, grks, comp_sets, pos_neg_order.index(sign) if sign in pos_neg_order else 0)
#             group_entries.append(dict(
#                 category=cat_key,
#                 label=lbl,
#                 roi_idx=np.sort(roi_idx),
...
#             ))
#     if not group_entries:
#         raise ValueError("No groups pass filters.")
#     # Sort all groups by order_key
#     group_entries.sort(key=lambda d: d['order_key'])
#     # Build stacked matrices
#     rows_meta = []
#     short_rows = []
#     long_rows = []
#     # Trial masks
#     tr_short = np.where(is_short)[0]
#     tr_long  = np.where(~is_short)[0]
#     # Pre-smoothing helper
#     from scipy.ndimage import gaussian_filter1d
#     for ge in group_entries:
#         roi_list = ge['roi_idx']
#         for r in roi_list:
#             # mean over trials condition -> (T,)
#             if tr_short.size:
#                 ts = np.nanmean(X[tr_short, r, :], axis=0)
#             else:
#                 ts = np.full_like(t, np.nan)
#             if tr_long.size:
#                 tl = np.nanmean(X[tr_long, r, :], axis=0)
#             else:
#                 tl = np.full_like(t, np.nan)
#             if smoothing_sigma > 0:
#                 ts = gaussian_filter1d(ts, sigma=smoothing_sigma)
#                 tl = gaussian_filter1d(tl, sigma=smoothing_sigma)
#             short_rows.append(ts)
#             long_rows.append(tl)
#             rows_meta.append(dict(group_label=ge['label'], roi=int(r)))
#     short_mat = np.vstack(short_rows)  # (R, T)
#     long_mat  = np.vstack(long_rows)
#     # Color scale
#     vals = np.concatenate([short_mat.ravel(), long_mat.ravel()])
#     finite = vals[np.isfinite(vals)]
#     if finite.size == 0:
...
#     else:
#         lim = np.quantile(np.abs(finite), vlim_quant)
#         if lim <= 0: lim = np.max(np.abs(finite))
#         vmin, vmax = -lim, lim
#     # Figure sizing
#     n_rows = short_mat.shape[0]
#     fig_h = min(max(4.0, n_rows * figsize_per_roi + 2.2), max_fig_height)
#     fig_w = 8.0
#     fig, axes = plt.subplots(2, 1, sharex=True, figsize=(fig_w, fig_h),
#                              gridspec_kw={'height_ratios':[1,1], 'hspace':0.08})
#     ax_short, ax_long = axes
#     extent=(t[0], t[-1], 0, n_rows)
#     im1 = ax_short.imshow(short_mat, aspect='auto', origin='lower',
#                           extent=extent, cmap=cmap, vmin=vmin, vmax=vmax)
#     im2 = ax_long.imshow(long_mat, aspect='auto', origin='lower',
#                          extent=extent, cmap=cmap, vmin=vmin, vmax=vmax)
#     ax_short.set_title("Short trials", fontsize=10)
#     ax_long.set_title("Long trials", fontsize=10)
#     ax_long.set_xlabel("Time (s)")
#     ax_short.set_ylabel("ROIs")
#     ax_long.set_ylabel("ROIs")
#     # Group boundaries & y tick labels at centers
#     y_cursor = 0
#     yticks = []
#     ylabels = []
#     for ge in group_entries:
...
#         # label center
#         y_mid = y0 + size/2
#         # Abbreviate base group inside label
#         base_label = ge['label']
#         if abbreviate:
#             # Replace group tokens only at start or before '+' / ':'
#             for full, short in abbreviate.items():
#                 base_label = base_label.replace(full, short)
#         yticks.append(y_mid)
#         ylabels.append(base_label)
#         # horizontal line
#         for ax in axes:
#             ax.axhline(y1, color='k', lw=0.3, alpha=0.4)
#         y_cursor = y1
#     # Reduce label clutter if too many
#     max_labels = int(fig_h * 3.2)  # heuristic
#     if len(yticks) > max_labels:
#         step = int(np.ceil(len(yticks)/max_labels))
#         yticks_disp = yticks[::step]
#         ylabels_disp= ylabels[::step]
#     else:
#         yticks_disp = yticks
#         ylabels_disp= ylabels
#     for ax in axes:
#         ax.set_yticks(yticks_disp)
#         ax.set_yticklabels(ylabels_disp, fontsize=group_label_fontsize)
#         ax.set_ylim(0, n_rows)
#     # Event markers (mean ± SD across all trials)
...
#                 ax.axvspan(mu - sd, mu + sd, color=col, alpha=0.05)
#     if show_colorbar:
#         cbar = fig.colorbar(im1, ax=axes.ravel().tolist(), fraction=0.015, pad=0.012)
#         cbar.set_label("Mean z-dF/F", fontsize=9)
#     if verbose:
#         print(f"[intersection rasters] rows={n_rows} groups={len(group_entries)} vlim=({vmin:.3g},{vmax:.3g})")
#     return fig, dict(row_metadata=rows_meta,
#                      vlim=(vmin,vmax),
#                      n_groups=len(group_entries),
#                      n_rows=n_rows,
#                      group_entries=group_entries)

# Example usage (after parts already built):
# fig, info = plot_intersection_roi_rasters(
#     M, parts,
#     group_order=['trial_start','isi','start_flash_1','end_flash_1','start_flash_2','end_flash_2','choice_start','lick_start'],
#     min_group_size=None,
#     smoothing_sigma=0.6,
#     abbreviate={'trial_start':'TS','start_flash_1':'F1on','end_flash_1':'F1off',
#                 'start_flash_2':'F2on','end_flash_2':'F2off','choice_start':'Choice','lick_start':'Lick'},
#     vlim_quant=0.98
# )
# fig.tight_layout()
# plt.show()
...
def save_catalog_for_fov(catalog: Dict[str, Any],
                         out_path: Union[str, Path],
                         *,
                         roi_index_order: Optional[np.ndarray] = None,
                         suite2p_source: Optional[Dict[str, Any]] = None,
                         format: str = 'npz',
                         compress: bool = True,
                         extra_meta: Optional[Dict[str, Any]] = None) -> Path:
    """
    Persist catalog so another script (e.g. FOV mask renderer) can load component
    memberships without re‑running CP.

    Stored fields
    -------------
    incidence      : uint8 (n_rois, n_components)
    comp_labels    : object array (n_components,)
    roi_index_order: (n_rois,) original Suite2p ROI order (if provided; else arange)
    meta_json      : JSON string (optional metadata)

    suite2p_source (optional):
      Provide e.g. {'stat_path': '.../stat.npy', 'ops_path': '.../ops.npy'}
      Only references (paths) are stored, not large arrays.

    format: 'npz' (recommended) or 'json'
    """
    ...
    if roi_index_order is None:
        roi_index_order = np.arange(n_rois, dtype=int)

    meta = dict(
        n_rois=int(n_rois),
        n_components=int(inc.shape[1]),
        suite2p=suite2p_source or {},
    )
    if extra_meta:
    ...
    if format.lower() == 'npz':
        ...
        saver(out_path,
              incidence=inc.astype(np.uint8),
              comp_labels=np.asarray(labels, dtype=object),
              roi_index_order=roi_index_order.astype(int),
              meta_json=json.dumps(meta))
    elif format.lower() == 'json':
        payload = dict(
            incidence=inc.astype(int).tolist(),  # beware size for large ROI counts
            comp_labels=list(labels),
            roi_index_order=roi_index_order.astype(int).tolist(),
            meta=meta
        )
        out_path.write_text(json.dumps(payload))
    else:
        raise ValueError("format must be 'npz' or 'json'")
    return out_path
...
# Example:
# save_catalog_for_fov(catalog, "catalog_fov.npz",
#                      suite2p_source={'stat_path': '.../stat.npy',
#                                      'ops_path': '.../ops.npy'})
# fov_cat = load_catalog_for_fov("catalog_fov.npz")
# ...existing
...
# ======================= FOV / SUITE2P ROI MASK UTILITIES =======================

def load_suite2p_fov(stat_path: Union[str, Path],
                     ops_path: Optional[Union[str, Path]] = None,
                     *,
                     load_masks: bool = True,
                     verbose: bool = True) -> Dict[str, Any]:
    """
    Load Suite2p stat / ops and build per-ROI binary (or weighted) masks.

    Returns dict:
      stat            : list[dict]
      ops             : dict
      Ly, Lx          : image dimensions
      roi_masks_bool  : (N, Ly, Lx) bool (if load_masks)
      roi_masks_float : (N, Ly, Lx) float32 weights (0 if not available)
      centroids       : (N,2) y,x float
      order           : np.arange(N) (ROI index order)
    """
    from pathlib import Path
    ...
    if verbose:
        print(f"[suite2p] loading stat: {stat_path}")
    stat = np.load(stat_path, allow_pickle=True).tolist()
    if not isinstance(stat, list):
        raise ValueError("stat.npy did not load as list.")
    if verbose:
        print(f"[suite2p] loading ops:  {ops_path}")
    ...
    N = len(stat)
    if Ly == 0 or Lx == 0:
        # fallback try shape from first stat entry
        if 'ypix' in stat[0] and 'xpix' in stat[0]:
            Ly = int(np.max([np.max(s['ypix']) for s in stat]) + 2)
            Lx = int(np.max([np.max(s['xpix']) for s in stat]) + 2)
        else:
            raise ValueError("Cannot infer Ly/Lx.")

    if not load_masks:
        roi_masks_bool = None
        roi_masks_float = None
    else:
        roi_masks_bool = np.zeros((N, Ly, Lx), bool)
        roi_masks_float = np.zeros((N, Ly, Lx), np.float32)

        for i, s in enumerate(stat):
            ypix = np.asarray(s.get('ypix', []), int)
            xpix = np.asarray(s.get('xpix', []), int)
            if ypix.size == 0:
                continue
            lam = np.asarray(s.get('lam', np.ones_like(ypix)), float)
            roi_masks_bool[i, ypix, xpix] = True
            # normalize weights locally (avoid div0)
            if lam.sum() > 0:
                roi_masks_float[i, ypix, xpix] = (lam / lam.sum()).astype(np.float32)
            else:
                roi_masks_float[i, ypix, xpix] = (lam.astype(np.float32))

    centroids = np.zeros((N, 2), float)
    for i, s in enumerate(stat):
        ypix = np.asarray(s.get('ypix', []), float)
        xpix = np.asarray(s.get('xpix', []), float)
        if ypix.size:
            centroids[i] = [np.mean(ypix), np.mean(xpix)]
        else:
    ...
    return dict(
        ...
        Ly=Ly,
        Lx=Lx,
        roi_masks_bool=roi_masks_bool,
        roi_masks_float=roi_masks_float,
        centroids=centroids,
        order=np.arange(N, dtype=int)
    )


def _color_to_rgb(color, default=(1.0, 0.0, 0.0)):
    """
    Accept matplotlib color spec or tuple; return (r,g,b).
    """
...
def make_fov_background(ops: Dict[str, Any],
                        prefer_mean_img: bool = True) -> np.ndarray:
    """
    Build grayscale background from ops (meanImg / meanImgE / reference).
    Returns float32 (Ly, Lx) scaled 0..1.
    """
    candidates = []
    if prefer_mean_img:
        for k in ('meanImg', 'meanImg_chan2'):
            if k in ops and ops[k] is not None:
                candidates.append(np.asarray(ops[k], float))
    for k in ('meanImgE', 'refImg'):
        if k in ops and ops[k] is not None:
            candidates.append(np.asarray(ops[k], float))
    if not candidates:
        raise ValueError("No suitable image in ops (meanImg / meanImgE / refImg).")
    img = candidates[0]
    # robust scale
    ...
    if finite.size:
        ...
        img = np.zeros_like(img, float)
    return img.astype(np.float32)


def render_roi_set_on_fov(fov: Dict[str, Any],
                          roi_indices: np.ndarray,
                          *,
                          mode: str = 'outline',          # 'outline' | 'fill' | 'alpha'
                          color: Union[str, Tuple[float, float, float]] = 'tab:red',
                          base_img: Optional[np.ndarray] = None,
                          outline_width: int = 1,
                          alpha: float = 0.55) -> np.ndarray:
    """
    Return RGB image (Ly, Lx, 3) with ROI set rendered on optional base image.

    mode:
      outline : draw ROI boundary pixels
      fill    : fill ROI area with color (alpha blend)
      alpha   : same as fill but keeps interior semitransparent

    If base_img None -> build from ops mean image (grayscale).
    """
    Ly = fov['Ly']; Lx = fov['Lx']
    if base_img is None:
        base_img = make_fov_background(fov['ops'])
    if base_img.ndim == 2:
        rgb = np.repeat(base_img[..., None], 3, axis=2)
    else:
        rgb = base_img.copy()

    roi_indices = np.asarray(roi_indices, int)
    mask_union = np.zeros((Ly, Lx), bool)
    for r in roi_indices:
        if r < 0 or r >= fov['roi_masks_bool'].shape[0]:
            continue
        mask_union |= fov['roi_masks_bool'][r]

    if not mask_union.any():
        return rgb

    col = np.array(_color_to_rgb(color))[None, None, :]
    if mode in ('fill', 'alpha'):
        # alpha blend
        blend = alpha if mode == 'alpha' else 1.0
        rgb[mask_union] = (1 - blend) * rgb[mask_union] + blend * col
    elif mode == 'outline':
        from scipy.ndimage import binary_dilation
        edge = mask_union ^ binary_dilation(mask_union, iterations=outline_width)
        rgb[edge] = col
    else:
        raise ValueError("mode must be outline|fill|alpha")
    return np.clip(rgb, 0, 1)


def get_catalog_group_indices(catalog: Dict[str, Any],
                              label: str) -> np.ndarray:
    """
    Return ROI indices for catalog column 'label'.
    """
    labels = catalog['comp_labels']
    try:
        j = labels.index(label)
    except ValueError:
        raise KeyError(f"Label '{label}' not in catalog.")
    return np.flatnonzero(catalog['incidence'][:, j])
...
def _split_signed_label(label: str) -> Tuple[str, Optional[str]]:
    """
    Return (base_label, sign) where sign in {'pos','neg','ambig',None}.
    Examples:
      'choice_start:comp2_pos' -> ('choice_start:comp2', 'pos')
      'isi:comp7'              -> ('isi:comp7', None)
    """
    if label.endswith(('_pos','_neg','_ambig')):
        for s in ('_pos','_neg','_ambig'):
            if label.endswith(s):
                return label[:-len(s)], s[1:]
    return label, None


def render_signed_component_on_fov(fov: Dict[str, Any],
                                   catalog: Dict[str, Any],
                                   base_label: str,
                                   *,
                                   mode: str = 'outline',
                                   pos_color: Union[str, Tuple[float,float,float]] = 'tab:blue',
                                   neg_color: Union[str, Tuple[float,float,float]] = 'tab:red',
                                   alpha: float = 0.55,
                                   outline_width: int = 1,
                                   annotate: bool = True,
                                   annotate_loc: str = 'ul',
                                   annotate_fmt: str = "{label}  +{n_pos} / -{n_neg}",
                                   prefer_fill_for_overlap: bool = False,
                                   base_img: Optional[np.ndarray] = None) -> np.ndarray:
    """
    Render positive and negative ROI subsets for a signed component on ONE image.

    base_label: label WITHOUT _pos / _neg suffix (e.g. 'choice_start:comp2')
                If only one side exists, falls back to that side.

    prefer_fill_for_overlap:
        If True and mode=='outline', draws POS first then NEG outlining;
        if mode in ('fill','alpha') overlays POS then NEG (NEG on top).
    """
    labels = catalog['comp_labels']
    label_pos = f"{base_label}_pos"
    label_neg = f"{base_label}_neg"
    has_pos = label_pos in labels
    has_neg = label_neg in labels
    if not (has_pos or has_neg):
        raise ValueError(f"Signed labels not found for base '{base_label}'")
    ...
    def _render_side(side_label, color, current):
        idx = get_catalog_group_indices(catalog, side_label)
        return render_roi_set_on_fov(
            fov, idx,
            mode=mode,
            color=color,
            base_img=current,
    ...
    # Order: draw POS then NEG (NEG on top) for visibility
    if has_pos:
        rgb = _render_side(label_pos, pos_color, rgb)
    if has_neg:
        rgb = _render_side(label_neg, neg_color, rgb)

    if annotate:
        n_pos = get_catalog_group_indices(catalog, label_pos).size if has_pos else 0
        n_neg = get_catalog_group_indices(catalog, label_neg).size if has_neg else 0
        txt = annotate_fmt.format(label=base_label, n_pos=n_pos, n_neg=n_neg)
        rgb = _annotate_rgb_image(rgb, txt, loc=annotate_loc)
    return rgb


def batch_render_signed_components(fov: Dict[str, Any],
                                   catalog: Dict[str, Any],
    ...
    """
    Write one PNG per signed component (merging *_pos & *_neg onto one image).
    Skips base components with neither side present.
    """
    from pathlib import Path
    import matplotlib.pyplot as plt
    labels = catalog['comp_labels']
    # Collect unique base labels that have at least one signed suffix
    bases = {}
    for lbl in labels:
        base, sign = _split_signed_label(lbl)
        if sign in ('pos','neg'):
            bases.setdefault(base, set()).add(sign)
    base_list = sorted(bases.keys())
    if max_components is not None:
        base_list = base_list[:max_components]

    out_dir = Path(out_dir)
    out_dir.mkdir(parents=True, exist_ok=True)
    saved = []
    for k, base in enumerate(base_list):
        rgb = render_signed_component_on_fov(
            fov, catalog, base,
            mode=mode,
            pos_color=pos_color,
            neg_color=neg_color,
            ...
            annotate_loc=annotate_loc
        )
        plt.figure(figsize=(4,4), dpi=dpi)
        plt.imshow(rgb)
        plt.axis('off')
        out_path = out_dir / f"signed_{k:03d}_{base.replace(':','_')}.png"
        plt.savefig(out_path, bbox_inches='tight', pad_inches=0)
        plt.close()
        saved.append(out_path)
        if verbose and (k+1) % 25 == 0:
            print(f"[signed render] {k+1}/{len(base_list)}")
    if verbose:
        print(f"[signed render] saved {len(saved)} images to {out_dir}")
    return saved

# --- OPTIONAL: extend existing batch_render_catalog_groups with combine_signed flag ---

def batch_render_catalog_groups(fov: Dict[str, Any],
                                catalog: Dict[str, Any],
                                out_dir: Union[str, Path],
                                *,
                                mode: str = 'outline',
                                ...
                                verbose: bool = True,
                                annotate: bool = True,
                                annotate_fmt: str = "{label} (n={n})",
                                annotate_loc: str = 'ul',
                                combine_signed: bool = False,          # NEW
                                pos_color: str = 'tab:blue',
                                neg_color: str = 'tab:red') -> List[Path]:
    """
    Save one PNG per catalog group into out_dir.
    If combine_signed=True merges *_pos / *_neg of the same base into one image (POS blue, NEG red).
    """
    from pathlib import Path
    import matplotlib.pyplot as plt
    out_dir = Path(out_dir)
    ...
    if combine_signed:
        # Delegate to dedicated signed batch renderer
        return batch_render_signed_components(
            fov, catalog, out_dir,
            mode=mode,
            pos_color=pos_color,
            neg_color=neg_color,
            ...
            dpi=dpi,
            annotate=annotate,
            annotate_fmt="{label} +{n_pos}/-{n_neg}",
            annotate_loc=annotate_loc,
            max_components=max_groups,
            verbose=verbose
        )
    ...
    if color_cycle is None or len(color_cycle) == 0:
        color_cycle = ['tab:red', 'tab:blue', 'tab:orange', 'tab:green',
                       'tab:purple', 'gold', 'deepskyblue', 'tab:pink']

    saved = []
    base_img = make_fov_background(fov['ops'])
    for k, lbl in enumerate(labels):
        clr = color_cycle[k % len(color_cycle)]
        idx = get_catalog_group_indices(catalog, lbl)
        rgb = render_roi_set_on_fov(fov, idx,
                                    mode=mode,
                                    color=clr,
                                    base_img=base_img,
                                    outline_width=outline_width,
                                    alpha=alpha)
        if annotate:
            txt = annotate_fmt.format(label=lbl, n=idx.size)
            rgb = _annotate_rgb_image(rgb, txt, loc=annotate_loc)
        fig = plt.figure(figsize=(4, 4), dpi=dpi)
        plt.imshow(rgb)
        plt.axis('off')
        out_path = out_dir / f"group_{k:03d}_{lbl.replace(':','_')}.png"
        # fig.savefig(out_path, bbox_inches='tight', pad_inches=0)
        # plt.close(fig)
        saved.append(out_path)
        if verbose and (k + 1) % 25 == 0:
            print(f"[render] {k+1}/{len(labels)} groups")
    if verbose:
        print(f"[render] saved {len(saved)} masks to {out_dir}")
    return saved
# ...existing code...
...
def _annotate_rgb_image(img: np.ndarray,
                        text: str,
                        *,
                        loc: str = 'ul',
                        pad: int = 4,
                        facecolor: Tuple[float,float,float,float] = (0,0,0,0.55),
                        textcolor: str = 'white',
                        fontsize: int = 10,
                        fontweight: str = 'bold') -> np.ndarray:
    """
    Light‑weight in‑place text annotation (matplotlib free version).
    loc: 'ul' (upper-left), 'ur', 'll', 'lr'
    """
    try:
        import matplotlib.pyplot as plt
        ...
        ax.imshow(img)
        ax.axis('off')
        # background box alignment
        ha = 'left' if loc.endswith('l') else 'right'
        va = 'top' if loc.startswith('u') else 'bottom'
        x = 0.01 if ha == 'left' else 0.99
        y = 0.02 if va == 'top' else 0.98
        ax.text(x, y, text,
                ha=ha, va=va,
                color=textcolor,
                fontsize=fontsize,
                fontweight=fontweight,
                bbox=dict(facecolor=facecolor, edgecolor='none', pad=pad))
        buf = BytesIO()
        fig.savefig(buf, format='png', bbox_inches='tight', pad_inches=0)
        plt.close(fig)
        buf.seek(0)
        import PIL.Image as Image
        annotated = np.array(Image.open(buf).convert('RGB')) / 255.0
        # Resize back (tight bbox may crop edges slightly)
        if annotated.shape[0] != h or annotated.shape[1] != w:
            annotated = np.array(Image.fromarray((annotated*255).astype(np.uint8)).resize((w, h), Image.BILINEAR))/255.0
        return annotated
    except Exception:
        # Fallback: return original if minimal deps missing
        return img

def render_catalog_group(fov: Dict[str, Any],
                         catalog: Dict[str, Any],
                         label: str,
                         *,
                         mode: str = 'outline',
                         ...
                         annotate: bool = False,
                         annotate_fmt: str = "{label} (n={n})",
                         annotate_loc: str = 'ul') -> np.ndarray:
    """
    Convenience one-liner to render a single catalog label.
    Set annotate=True to burn group/subgroup text onto image.
    """
    idx = get_catalog_group_indices(catalog, label)
    rgb = render_roi_set_on_fov(fov, idx,
                                mode=mode,
                                color=color,
                                base_img=base_img,
                                ...
                                alpha=alpha)
    if annotate:
        txt = annotate_fmt.format(label=label, n=idx.size)
        rgb = _annotate_rgb_image(rgb, txt, loc=annotate_loc)
    return rgb
...
#                                 annotate: bool = True,
#                                 annotate_fmt: str = "{label} (n={n})",
#                                 annotate_loc: str = 'ul') -> List[Path]:
#     """
#     Save one PNG per catalog group into out_dir.
#     Now supports in-image annotation of group/subgroup.
#     """
#     from pathlib import Path
#     import matplotlib.pyplot as plt
#     out_dir = Path(out_dir)
...
def render_composite_overlay(fov: Dict[str, Any],
                             catalog: Dict[str, Any],
                             ...
                             *,
                             ...
                             annotate_loc: str = 'ul') -> np.ndarray:
    """
    Render multiple groups on a single RGB image (different colors).
    If annotate: add list of group labels + sizes.
    """
    base = make_fov_background(fov['ops'])
    ...
    for i, lbl in enumerate(group_labels):
        ...
        rgb = render_roi_set_on_fov(fov, idx,
                                    ...
                                    outline_width=outline_width,
                                    alpha=alpha)
        lines.append(f"{lbl} (n={idx.size})")
    if annotate:
        rgb = _annotate_rgb_image(rgb, "\n".join(lines), loc=annotate_loc)
    return rgb
# ...existing code...
...
CHUNK_KEYS        = ['start_flash_1','end_flash_1','isi','start_flash_2',
                     'end_flash_2','choice_start','lick_start']
...
window_cfg={
    'start_flash_1': (-0.05, 0.15),
    'end_flash_1':   (-0.05, 0.100),
    'isi':           (0.0, 2.3),
    'start_flash_2': (-0.05, 0.15),
    'end_flash_2':   (-0.05, 0.100),
    'choice_start':  (-0.2, 0.1),
    'lick_start':    (-0.1, 0.50)
}
...
if DEBUG_PRINTS:
    for ch in CHUNK_KEYS:
        print(f"[time CP] {ch} final components={time_results_w[ch]['final_A'].shape[1]} "
              f"stability={np.round(time_results_w[ch]['stability_vec'],3)}")

# (Optional) Inspect normalized temporal factor shapes
for ch in CHUNK_KEYS:
    plot_component_temporals(time_results_w[ch], f"{ch} temporal factors")
...
# %%
catalog = build_component_catalog(M, time_results_w)
summarize_component_overlaps(catalog)
...
# %%
# catalog = integrate_phase_components_into_catalog(catalog, res_phase, signed_phase)
...
# %%
combined_results = dict(time_results_w)
# %%
combined_results['isi_phase'] = {'final_A': res_phase['factors']['A']}
signed_all = build_signed_groups_for_chunks(M, combined_results,
                                            q=SIGNED_Q,
...
# catalog = integrate_signed_groups_into_catalog(catalog, M, signed_all,
#                                                label_prefix='', skip_existing=True)
# Keep both (current behavior)
catalog = integrate_signed_groups_into_catalog(catalog, M, signed_all,
                                               label_prefix='', skip_existing=True, 
                                               keep_original=True)

rebuild_catalog_entries(catalog)

# %%
# Or signed-only:
# signed_only_catalog = build_signed_only_catalog(M, signed_all, 
#                                                 base_catalog=catalog,
#                                                 skip_existing=False)

signed_only_catalog = integrate_signed_groups_into_catalog(catalog, M, signed_all,
                                               label_prefix='', skip_existing=True, 
                                               keep_original=False)
rebuild_catalog_entries(signed_only_catalog)

# %%


# signed_all = {}
# signed_all = signed_only_catalog
base_path = 'D:/behavior/2p_imaging/processed/2afc/YH24LG/YH24LG_CRBL_lobulev_20250620_2afc-494/'
suite2p_source={'stat_path': base_path + 'qc_results/stat.npy',
                                     'ops_path': base_path + 'ops.npy'}
save_catalog_for_fov(signed_only_catalog, base_path + "catalog_fov.npz",
...
fov = load_suite2p_fov(base_path + 'qc_results/stat.npy', base_path + 'ops.npy')

# %%
rgb = render_catalog_group(fov, catalog, 'choice_start:comp2_pos', annotate=True)
plt.imshow(rgb); plt.show()

batch_render_catalog_groups(fov, catalog, "fov_groups_out",
                            annotate=True, annotate_fmt="{label}  n={n}")

rgb_multi = render_composite_overlay(fov, catalog,
                                     ['isi:comp0_pos','choice_start:comp1_pos'],
                                     annotate=True)

# %%
# Single signed component (both sides) on FOV:
rgb = render_signed_component_on_fov(fov, catalog, "choice_start:comp2")
plt.imshow(rgb); 
plt.show()

# Batch all signed components (merged):
batch_render_signed_components(fov, catalog, "fov_signed_out")


# %%
# Or via extended batch function:
batch_render_catalog_groups(fov, catalog, "fov_signed_out", combine_signed=True)
...
#     onset_z=1.0,
#     smoothing_sigma=0.8,
#     events=list(['start_flash_2']),
#     show_onset_markers=True,
#     onset_marker_kwargs=dict(marker='o', s=8, color='k', alpha=0.7),
#     zero_line=True,
#     zero_line_kwargs=dict(color='k', lw=0.7, ls='--', alpha=0.9)
# )
plot_all_groups_v3(M, catalog, cfg=cfg, 
                   window_cfg=window_cfg,
                   compute_global_vlim=True)
...
# Assume:
# M : session dict from build_M_from_chunks
# time_results[chunk] : output of run_time_domain_analysis (contains final_A)
# Choose a chunk, e.g. 'isi' or 'choice_start'
chunk = 'isi'
# A = time_results[chunk]['final_A']        # (n_rois, n_components)

# # 1. Build signed groups (pos/neg ROI subsets per component)
# signed_groups = extract_signed_roi_groups(A, q=0.10)

# # 2. (Optional) orient so positive subset has larger mean trace
# orient_signed_groups(A, signed_groups, M=M, chunk=chunk, method='trace_mean')
# finalize_signed_orientation(M, {'factors': {'A': A}}, signed_groups, chunk)

# 3. Pick an ISI of interest (exact value present in M['unique_isis'])
target_isi = float(M['unique_isis'][0])

signed_groups = signed_all[chunk]



# 4. Plot all components (each gets 4 stacked panels)
fig = plot_chunk_components_posneg_single_isi(
    M,
    chunk=chunk,
    signed_groups=signed_groups,
    isi_value=target_isi,
    isi_tol=1e-6,              # widen if your ISIs have fp noise
    smoothing_sigma=0.8,
    events=['F1_on','F1_off','F2_on','F2_off','choice_start','lick_start'],
    vlim_mode='global',        # or 'per_component'
    sort_rois='mean',          # or 'none'
    pos_color='tab:blue',
    neg_color='tab:red',
    cmap='coolwarm',
    title=f"{chunk} components @ ISI={target_isi:g}s",
    use_full_trial=True
)

# # 5. Save (optional)
# if fig:
#     fig.savefig(f"{chunk}_components_ISI{target_isi:.3f}.png", dpi=150)

# %%



# loop chunks and isis
for chunk in CHUNK_KEYS:
    signed_groups = signed_all[chunk]
    for isi_val in M['unique_isis']:
        fig = plot_chunk_components_posneg_single_isi(
            M, chunk, signed_groups,
            isi_value=float(isi_val),
            isi_tol=1e-6,
            smoothing_sigma=0.8,
            events=['F1_on','F1_off','F2_on','F2_off','choice_start','lick_start'],
            vlim_mode='global',        # or 'per_component'
            ...
            neg_color='tab:red',
            cmap='coolwarm',
            title=f"{chunk} components @ ISI={isi_val:g}s",
            use_full_trial=True        
        )
...
#     if fig:
#         fig.savefig(f"{chunk}_comp_posneg_ISI{isi_val:.3f}.png", dpi=120)
#         plt.close(fig)

# # --- Using per-component color scaling & disabling negative empty panels ---
# plot_chunk_components_posneg_single_isi(
#     M, chunk, signed_groups,
#     isi_value=float(M['unique_isis'][0]),
#     vlim_mode='per_component',
#     show_negative_empty=False
# )

# # --- If your ISIs have slight jitter, relax tolerance ---
# plot_chunk_components_posneg_single_isi(
#     M, chunk, signed_groups,
#     isi_value=0.85,
#     isi_tol=0.002   # accept ±2 ms
# )

# # --- Skip orientation (raw loadings), and no smoothing ---
# raw_signed = extract_signed_roi_groups(time_results[chunk]['final_A'], q=0.10)
# plot_chunk_components_posneg_single_isi(
#     M, chunk, raw_signed,
#     isi_value=float(M['unique_isis'][1]),
#     smoothing_sigma=0.0,
#     sort_rois='none'
# )
...
# purge orig
signed_labels = [l for l in catalog['comp_labels'] if '_pos' in l or '_neg' in l]
keep_idx = [i for i,l in enumerate(catalog['comp_labels']) if l in signed_labels]
catalog['incidence'] = catalog['incidence'][:, keep_idx]
catalog['comp_labels'] = signed_labels
...
figs, meta = plot_intersection_roi_rasters(
    M, parts,
    group_order=['trial_start','isi','start_flash_1','end_flash_1','start_flash_2','end_flash_2','choice_start','lick_start'],
    short_isis=[0.7],     # keep only these short ISIs
    long_isis=[1.7],  # keep only these long ISIs
    split_condition_figs=True,
    smoothing_sigma=0.6,
    normalize_rois=False,
    shared_vlim=False,        # independent scaling
    event_stat='median',
    event_spread='iqr',
    onset_windows=onset_windows,
    group_time_order=False,
    roi_sort_mode='onset'    
)
...
# %%
fig_long, meta_long = plot_intersection_roi_rasters_condition(
    M, parts,
    group_order=[...],
    condition='long',
    isis=[1.7,1.850],
