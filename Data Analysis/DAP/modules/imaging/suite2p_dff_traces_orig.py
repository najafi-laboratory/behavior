...
class Suite2pDffTraces:
    ...
    #                normalize: bool = True, sig_baseline: float = 600) -> np.ndarray:
    #     """
    ...
    #                 mean_val = np.nanmean(dff[j, :])
    #                 std_val = np.nanstd(dff[j, :])
    #                 dff[j, :] = (dff[j, :] - mean_val) / (std_val + 1e-5)
    ...
    def plot_filtering_comparison_multi_window(self, original_dff: np.ndarray, filtered_dff: np.ndarray, 
                                            subject_id: str, roi_indices: List[int] = None, 
                                            fs: float = 30.0, windows: List[Tuple[float, float]] = None) -> None:
        """
    ...
    def plot_power_spectrum_comparison(self, original_dff: np.ndarray, filtered_dff: np.ndarray, 
                                    subject_id: str, fs: float = 30.0, roi_idx: int = 0) -> None:
        """
        ...
        try:
            ...
            self.logger.info(f"S2P_DFF: Power spectrum analysis completed for {subject_id}")
        ...
        try:
            ...
            if not have_dff:
                ...
                if show_filtering_plots:
                    ...
                    self.plot_filtering_comparison_multi_window(dff_original, dff_filtered, subject_id, fs=30.0)                
                    
                    # Also show power spectrum for first ROI
                    self.plot_power_spectrum_comparison(dff_original, dff_filtered, subject_id, fs=30.0, roi_idx=0)
