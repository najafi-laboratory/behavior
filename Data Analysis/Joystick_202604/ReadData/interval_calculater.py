...
def calculate_interal(block_data, session_data):
    session_vis1_push1 = []
    session_vis2_push2 = []
    session_push1_push2 = []
    ...
    for i in range(len(block_data['start'])):
        ...
        print('interval computation for ', dates[i])
        trial_start = block_data['start'][i]
        trial_end = block_data['end'][i]
        trial_num_blocks = block_data['NumBlocks'][i]
        push_data = session_data['events_timing'][i]
        vis1_push1 = np.concatenate(np.array(push_data['push1']))/1000 - np.array([push_data['vis1'][t][0] for t in range(len(push_data['vis1']))])/1000
        vis2_push2 = np.concatenate(np.array(push_data['push2']))/1000 - np.array([push_data['vis2'][t][0] for t in range(len(push_data['vis2']))])/1000
        wait2_push2 = np.concatenate(np.array(push_data['push2']))/1000 - np.array([push_data['wait2'][t][0] for t in range(len(push_data['wait2']))])/1000
        wait2_push2[wait2_push2 < 0] = np.nan
        push1_push2 = np.concatenate(np.array(push_data['push2']))/1000 - np.concatenate(np.array(push_data['push1']))/1000
        retract1_init_push2 = np.concatenate(np.array(push_data['push2']))/1000 - np.array([push_data['retract1_init'][t][0] for t in range(len(push_data['retract1_init']))])/1000
        retract1_end_push2 = np.concatenate(np.array(push_data['push2']))/1000 - np.array([push_data['retract1'][t][0] for t in range(len(push_data['retract1']))])/1000
        vis1_push2 = np.concatenate(np.array(push_data['push2']))/1000 - np.array([push_data['vis1'][t][0] for t in range(len(push_data['vis1']))])/1000
        trial_vis1_push1 = []
        trial_vis2_push2 = []
        trial_push1_push2 = []
