...
def read_block(
    ...
    for i in range(0 , len(session_id)):
        ...
        for j in range(0 , trial_num_blocks):
            ...
            trial_opto_tag.append(opto[trial_start[j] : trial_end[j]])
            ...
            if len(delay[trial_start[j] : trial_end[j]]) == len(delay[trial_start[j]+1 : trial_end[j]+1]):
                trial_next_delay.append(delay[trial_start[j]+1 : trial_end[j]+1])
                trial_next_outcome.append(outcome[trial_start[j]+1 : trial_end[j]+1])
            else:
                trial_next_delay.append(np.append(delay[trial_start[j]+1 : trial_end[j]+1] , np.nan))
                trial_next_outcome.append(np.append(outcome[trial_start[j]+1 : trial_end[j]+1] , np.nan))
            ...
            if session_data['isSelfTimedMode'][i][0] == 1:
                ...
                prepress2delay = np.array(predelay2[trial_start[j] : trial_end[j]]) + 0.06
                trial_lowerband.append(prepress2delay)
                trial_upperband.append(prepress2delay + window + 0.06)
        ...
        session_next_delay.append(trial_next_delay)
        session_next_outcome.append(trial_next_outcome)
    ...
    block_data = {
            ...
            'end': session_end ,
            'delay': session_delay ,
            'next_delay': session_next_delay ,
            'next_outcome': session_next_outcome ,
            ...
            'Opto' : session_opto,
            'OptoTag' : session_opto_tag, # opoto tag of each trial of the block
