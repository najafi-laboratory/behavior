...
from ReadData import event_time
...
def read_trials(session_data_path, subject, file_names):
    ...
    for f in range(len(file_names)):
        ...
        event_times = event_time.read_event_timing(raw_data, Exluded)
        session_event_time.append(event_times)
        ...
        if 'ProbeTrial' in raw_data.keys():
            ...
            trial_probe_present = np.sum(probe)
        else:
            probe = np.zeros(nTrials-Exluded)
            trial_effective_probe = np.zeros(nTrials-Exluded)
            trial_probe_present = 0
        # trial target
        ...
        for i in range(nTrials-Exluded):
            ...
            if 'Port2In' in trial_events.keys():
                ...
                trial_licking.append(lick_all)
                trial_lick_recoding = trial_lick_recoding + 1
                
            else:
                trial_licking.append([np.nan])
            ...
            ################## late press
            latetime = 0
            if outcome == 'LatePress2':
                latetime = trial_states['LatePress2'][1] 
            if outcome == 'DidNotPress1':
                late_index = np.where(times > trial_states['DidNotPress1'][1])[0][0]
                late_index_stop = np.where(times > times[late_index] + 2.5)[0]
            ...
            if (not isSelfTimedMode) and (probe[i] == 1) and (outcome in ['Reward' , 'DidNotPress2' , 'LatePress2']):
                trial_effective_probe[i] = 1
            if (isSelfTimedMode) and (probe[i] == 1) and (outcome in ['Reward']):
                trial_effective_probe[i] = 1
        ...
        print('number of trials with lick: ', trial_lick_recoding)
        print('number of probe: ', trial_probe_present)
    ...
    data = {
        ...
        'amp1' : session_amp1,
        'amp2' : session_amp2,
        'events_timing': session_event_time
    }
    return data
