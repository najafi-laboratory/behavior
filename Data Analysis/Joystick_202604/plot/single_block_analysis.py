...
def run_probe(axs , session_data, st):
    ...
    all_effective_probe = []
    for sess in plotting_sessions:
        probe = session_data['session_probe_raw'][sess]
        effective_probe = session_data['session_probe'][sess]
        all_probe.append(np.sum(probe))
        all_effective_probe.append(np.sum(effective_probe))
        
    session_id = np.arange(len(plotting_sessions)) + 1
    
    axs.scatter(session_id , all_effective_probe , color = 'gray' , s = 5)
    axs.scatter(session_id + 0.2, all_probe , color = 'k' , s = 5)
    
    axs.plot(session_id , all_effective_probe , color = 'gray' , label = 'number of effective probe trials')
    axs.plot(session_id + 0.2, all_probe , color = 'k' , label = 'number of probe trials')
    ...
    for i in plotting_sessions:
        if isSelfTime[i][0] == 1:
            ...
            dates_label.append(dates[i] + '-VG')
    axs.set_xticklabels(dates_label, rotation='vertical')
    axs.set_title('Probe trial count across sessions')
    axs.legend(loc='upper left', bbox_to_anchor=(1,1), ncol=1)
