# -*- coding: utf-8 -*-
"""
Created on Thu Sep 11 14:39:19 2025
...
def plot_brokenaxis(axs, array1, array2, array3, array4, title, pre, post, ylabel, sem1 = np.nan, sem2 = np.nan, sem3 = np.nan, sem4 = np.nan, sem_plot = 0):
    ...
    if title == 'All transitions short (control vs opto) (All)' or title == 'All transitions long (control vs opto) (All)':
        c1 = 'k'
        c2 = 'dodgerblue'
    else:
        c1 = 'y'
        c2 = 'b'
    axs.plot(x1, array1, label='short', color=c1)
    axs.plot(x2, array2, color=c1)
    axs.plot(x1, array3, label='long', color=c2)
    axs.plot(x2, array4, color=c2)
    if sem_plot:
        axs.fill_between(x1, array1-sem1, array1+sem1, color=c1, alpha=0.3)
        axs.fill_between(x2, array2-sem2, array2+sem2, color=c1, alpha=0.3)
        axs.fill_between(x1, array3-sem3, array3+sem3, color=c2, alpha=0.3)
        axs.fill_between(x2, array4-sem4, array4+sem4, color=c2, alpha=0.3)
    axs.axvline(x = pre, color = 'grey', linewidth = 1, linestyle='--')
    axs.axvline(x = x2[-1]-post, color = 'grey', linewidth = 1, linestyle='--')
    axs.axvline(x = 25, color = 'dodgerblue', linewidth = 1, linestyle='--' , alpha = 0.3)
...
def rest_block_array(interval_data, block_data, refrence, st, pre, post, first_exclude = 1, opto_tag = 0):
    short_initial = []
    long_initial = []
    short_end = []
    long_end = []
...
def run_all_blocks(axs1,axs2,axs3,axs4, session_data , block_data, interval_data, st_seperate = 1):
    delay_vector = block_data['delay_all']
    ST , VG = IsSelfTimed(session_data)
    all_sessions = np.arange(len(delay_vector))
    if st_seperate == 1:
    ...
    for st in range(len(all_st)):
        ...
        for refrence in interval_data.keys():
            short_initial, short_end, long_initial, long_end = rest_block_array(interval_data,block_data, refrence, all_st[st], pre, post, first_exclude = 0, opto_tag = 0)
            short_initial_opto, short_end_opto, long_initial_opto, long_end_opto = rest_block_array(interval_data, block_data, refrence, all_st[st], pre, post, first_exclude = 0, opto_tag = 1)
            y_label = refrence + ' (s)'
            ...
            plot_brokenaxis(axs3[num][st], output1, output2, output3, output4, title, pre, post, y_label, sem1 = sem_output1, sem2 = sem_output2, sem3 = sem_output3, sem4 = sem_output4,sem_plot = 1)
            ...
            plot_brokenaxis(axs4[num][st], output1, output2, output3, output4, title, pre, post, y_label, sem1 = sem_output1, sem2 = sem_output2, sem3 = sem_output3, sem4 = sem_output4,sem_plot = 1)
            num = num + 1
