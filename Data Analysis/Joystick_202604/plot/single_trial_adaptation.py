...
def create_label(delay_vector , outcomes):
    ...
    for i in range(len(outcomes)):
        ...
        #condition_early = np.percentile(delay_data, 20)
        # condition_late = np.percentile(delay_data, 65)
        condition_early = 0.25
        condition_late = 0.8
        #print(condition)
...
def plot_dist(axs, delay_data, color_tag, alpha = 0.8, cum = 0):
    ...
    plt.axvline(x = 0.25, color = color_tag,alpha = alpha, linewidth = 1, linestyle='--')
    plt.axvline(x = 0.8, color = color_tag,alpha = alpha, linewidth = 1, linestyle=':')
    axs.spines['right'].set_visible(False)
    axs.spines['top'].set_visible(False)
    axs.set_xlabel('Delay (s)')
...
def run_cum_dist(axs, session_data, delay_data, st):
    st_title = ['(All)' , '(Self Timed)' , '(Visually Guided)']
    delay_temp = delay_data['delay_all']
    num_session = len(delay_temp)
    ST , VG = IsSelfTimed(session_data)
    ...
    for i in range(len(delay)):
        color_tag = cmap(norm(len(delay)+1-i))
        #print(np.array(np.nancumsum(delay[i])))
        plot_dist(axs, np.array(delay[i]), color_tag, cum = 1)
    delay_all = [item for sublist in delay for item in sublist]
    #print(np.array(delay_all).shape)
    
    plot_dist(axs, np.array(delay_all), 'k', alpha = 1, cum = 1)
    axs.set_title('Distribution of Inter Press Interval '+ st_title[st])
...
def run_count_scatter(axs, session_data, delay_data):
    delay_vector = delay_data['delay_all']
    outcomes = copy.deepcopy(session_data['outcomes'])
    raw_data = session_data['raw']
    #outcome_array = ' '.join([item for sublist in outcomes for item in sublist])
    ...
    all_labels = ['Short' , 'Long' , 'DidNotPress1' , 'DidNotPress2' ,'Other']
    label_color = ['dodgerblue' , 'gold', '#FFB74D','#FB8C00', 'grey']
    all_outcomes1 = ['Reward' , 'DidNotPress1' , 'DidNotPress2' , 'EarlyPress' , 'EarlyPress1' , 'EarlyPress2' ,'VisStimInterruptDetect1' , 'VisStimInterruptDetect2' , 'VisStimInterruptGray1' , 'VisStimInterruptGray2' , 'LatePress1' , 'LatePress2' , 'Other', 'Warmup', 'Assisted']
    all_outcomes = ['Reward' , 'DidNotPress1' , 'DidNotPress2' , 'EarlyPress2', 'LatePress2']
    outcome_color = ['#4CAF50','#FFB74D','#FB8C00','r','#64B5F6','#1976D2','#967bb6','#9932CC','#800080','#2E003E','pink','deeppink','cyan','grey', 'darkgray']
    #print(session_data['outcomes'][0][0:3])
    # if press_type in all_labels:
    #     ref_array = copy.deepcopy(labels)
    ...
    color_tag = ['y','b']
    size = 8
    x_values = np.arange(len(all_outcomes))
    for st in range(3):
        outcome_short = np.zeros([len(all_st[st]), len(all_outcomes)])
        outcome_long = np.zeros([len(all_st[st]), len(all_outcomes)])
        num = 0
        for sess in all_st[st]:
            ...
            count_outcome_short = current_trial(session_data['outcomes'], index_all, trial_type_all, [sess], 1)
            final_count_outcome_short = count_label(all_outcomes , count_outcome_short)
            count_outcome_long = current_trial(session_data['outcomes'], index_all, trial_type_all, [sess], 2)
            final_count_outcome_long = count_label(all_outcomes , count_outcome_long)
            #final_count_outcome_long = session_data['outcomes'][sess]
            for outcome_indx in range(len(all_outcomes)):
                outcome_short[num , outcome_indx] = final_count_outcome_short[outcome_indx]
                outcome_long[num , outcome_indx] = final_count_outcome_long[outcome_indx]
            
            num = num + 1 
            
        for p in range(len(all_outcomes)):
            axs[st].scatter(x_values[p]-0.1, np.nanmean(outcome_short[:,p]), color = color_tag[0], s = size)
            sem_control = np.nanstd(outcome_short[:,p])/np.sqrt(np.count_nonzero(~np.isnan(outcome_short[:,p])))
            axs[st].errorbar(x = x_values[p]-0.1 , y = np.nanmean(outcome_short[:,p]) , yerr = sem_control , ecolor = color_tag[0])
            
            axs[st].scatter(x_values[p]+0.1, np.nanmean(outcome_long[:,p]), color = color_tag[1], s = size)
            sem_opto = np.nanstd(outcome_long[:,p])/np.sqrt(np.count_nonzero(~np.isnan(outcome_long[:,p])))
            #axs[st].plot([0, 1], [np.nanmean(control_outcome[:,p]),np.nanmean(opto_outcome[:,p])], color = color_tag[p], label = out_label[p])
            axs[st].errorbar(x = x_values[p]+0.1 , y = np.nanmean(outcome_long[:,p]) , yerr = sem_opto , ecolor = color_tag[1])
            
            x_labels = all_outcomes
            
            axs[st].set_ylim([0, 0.6])
            
            axs[st].set_xticks(x_values)
            axs[st].set_xticklabels(x_labels, rotation='vertical')
            title = 'Outcome ' + st_label[st]
