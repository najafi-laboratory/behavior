# -*- coding: utf-8 -*-
"""
Created on Sat Jun 28 17:12:16 2025
...
def create_opto_label(opto , outcomes, labels_all , outcome_ref = 'EarlyPress2'):
    label = copy.deepcopy(outcomes)
    for i in range(len(outcomes)):
        outcome = outcomes[i]
        opto_tag = opto[i]
        for j in range(len(outcome)):
            if outcome[j] == outcome_ref:
                if opto_tag[j] == 1:
                    label[i][j] = labels_all[0]
                else:
                    #print(labels_all[1])
                    label[i][j] = labels_all[1]
            else:
                label[i][j] = labels_all[2]
    return label
...
def run_opto_delay(axs, session_data, delay_data, press_type, color_tag = 'k'):
    ...
    all_labels = ['Opto_EarlyPress' , 'Control_EarlyPress' , 'Control']
    labels = create_opto_label(opto , outcomes, all_labels, 'EarlyPress2')
...
def run_opto_outcome(axs, session_data, delay_data):
    ...
    outcome_color = ['#4CAF50','#FFB74D','#FB8C00','r','#64B5F6','#1976D2','#967bb6','#9932CC','#800080','#2E003E','pink','deeppink','cyan','grey', 'darkgray']
    st = 0
    index , values , trial_type = devide(ref_array, all_labels[0], delay_vector, raw_data)
    index_c , values_c , trial_type_c = devide(ref_array, all_labels[1], delay_vector, raw_data)
    index_cn , values_cn , trial_type_cn = devide(ref_array, all_labels[2], delay_vector, raw_data)
    ST , VG = IsSelfTimed(session_data)
    all_sessions = np.arange(len(delay_vector))
    all_st = [all_sessions , np.array(ST) , np.array(VG)]
    x_values = [[0, 1 , 2] , [4 , 5 , 6] , [8 , 9 , 10]]
    ...
    plot_layout(axs[0], title)
    axs[0].legend()
    return opto_outcome, control_outcome, control_n_outcome
def run_opto_outcome_new(axs, opto_outcome, control_outcome, control_n_outcome):
    ...
    size = 12
    for p in range(3):
        axs[0].scatter(x_num[p]-0.1, np.nanmean(control_outcome[:,p]), color = 'k', s = size)
        sem_control = np.nanstd(control_outcome[:,p])/np.sqrt(np.count_nonzero(~np.isnan(control_outcome[:,p])))
        axs[0].errorbar(x = x_num[p]-0.1 , y = np.nanmean(control_outcome[:,p]) , yerr = sem_control , ecolor = 'k')
        
        axs[0].scatter(x_num[p], np.nanmean(opto_outcome[:,p]), color = 'dodgerblue', s = size)
        sem_opto = np.nanstd(opto_outcome[:,p])/np.sqrt(np.count_nonzero(~np.isnan(opto_outcome[:,p])))
        #axs[0].plot([0, 1], [np.nanmean(control_outcome[:,p]),np.nanmean(opto_outcome[:,p])], color = color_tag[p], label = out_label[p])
        axs[0].errorbar(x = x_num[p] , y = np.nanmean(opto_outcome[:,p]) , yerr = sem_opto , ecolor = 'dodgerblue')
        
        axs[0].scatter(x_num[p]+0.1, np.nanmean(control_n_outcome[:,p]), color = 'gray', s = size)
        sem_control_n = np.nanstd(control_n_outcome[:,p])/np.sqrt(np.count_nonzero(~np.isnan(control_n_outcome[:,p])))
        #axs[0].plot([1, -0.1], [np.nanmean(opto_outcome[:,p]),np.nanmean(control_n_outcome[:,p])], color = color_tag[p], linestyle = '--')
        axs[0].errorbar(x = x_num[p]+0.1 , y = np.nanmean(control_n_outcome[:,p]) , yerr = sem_control_n , ecolor = 'gray')
        #axs[0].scatter(4, np.nanmean(opto_outcome[:,p]), color = color_tag[p], s = size)
        #axs[0].errorbar(x = 4 , y = np.nanmean(opto_outcome[:,p]) , yerr = sem_opto , ecolor = color_tag[p])
    
            
    x_labels = ['Reward','Early' , 'No press']
    
    axs[0].set_xlim([-1, 3])
    
    axs[0].set_xticks(x_num)
    axs[0].set_xticklabels(x_labels, rotation='vertical')
    title = 'Outcome for Post Rewarded Trial'
    plot_layout(axs[0], title)
    axs[0].legend()