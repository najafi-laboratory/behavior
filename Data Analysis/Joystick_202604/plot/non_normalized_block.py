# -*- coding: utf-8 -*-
"""
Created on Fri Dec 20 16:54:50 2024
...
def plot_curves_indvi(axs,dates ,df, color , num , max_num):
    ...
    if color ==1 :
        ...
        for i in range(len(block_num)):
            #print(df_sorted[~df_sorted['block_id'].isin([block_num[i]])]['time'])
...
def plot_average_indvi(axs,dates ,df, color , num , max_num):
    ...
    average = filtered_df['delay'].rolling(w).mean()
    x_value = np.arange(0,len(average))
    std = filtered_df['delay'].rolling(w).std()
    x_value_std = np.arange(0,len(std))
    limit = min(len(average) , len(std))
    ...
    if color ==1 :
        ...
        axs.fill_between(x_value_std[:limit] , average[-limit:]-std[-limit:] , average[-limit:]+std[-limit:], color = 'y' , alpha = 0.1)
    else:
        axs.plot(x_value , average, color= 'b', label =  'long')
        axs.fill_between(x_value_std[:limit] , average[-limit:]-std[-limit:] , average[-limit:]+std[-limit:], color = 'b' , alpha = 0.1)
        axs.legend()

    
    

def plot_num_indvi(axs ,dates ,number, color , num , max_num):
    #colors = ['#4CAF50','#FFB74D','pink','r','#64B5F6','#1976D2']
    
    
        
    if color ==1 :
        axs.plot(np.arange(1 , len(number)+1) , number, color= 'y', label = 'short')
        axs.tick_params(tick1On=False)
        axs.spines['right'].set_visible(False)
        axs.spines['top'].set_visible(False)
        axs.set_xlabel('block number')
        axs.set_ylabel('number of trials')
        axs.set_title('blok length over session ' + dates[num])  
    else:
        axs.plot(np.arange(1 , len(number)+1) , number, color= 'b', label =  'long')
        axs.legend()
...
def run_indvi(axs,axs1,axs2,session_data , block_data):
    ...
    for k in range(num_session//max_num + 1):
        ...
        for i in range(start , end):
            ...
            plot_num_indvi(axs2[(i-start)-(i-start)//num_plotting], date ,len_block_short,1 , (i-start)-(i-start)//num_plotting , num_plotting)
            plot_num_indvi(axs2[(i-start)-(i-start)//num_plotting], date ,len_block_long,2 , (i-start)-(i-start)//num_plotting , num_plotting)