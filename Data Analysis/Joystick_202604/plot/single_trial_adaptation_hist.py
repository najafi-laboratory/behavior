...
def plot_arrow(axs, delays, next_delays, outcome_array, opto, session_opto_type, bins_all, short_low_avg, short_up_avg, title = 'short'):
    ...
    plot_layout(axs, 'Delay adaptation' + ' ' + title)
    axs.set_xlabel('Delay (s)')
    axs.set_ylabel('Probability')
...
def plot_scatter_outcome(axs, count_list, outcome_labels, shape, color_tag):
    ...
    axs.scatter(np.arange(len(outcome_labels)), mean_all, color=color_tag,s=5, marker = shape)
    axs.errorbar(np.arange(len(outcome_labels)), mean_all, yerr=std_all, fmt='none', ecolor=color_tag, elinewidth=1.3)
...
def plot_outcome(axs, outcome_array, outcome_next, opto, session_opto_type, title = 'short'):
    outcome_color = ['blue', 'pink', 'green']
    possible_outcome = ['EarlyPress2', 'LatePress2','Reward']
    possible_opto = ['EarlyPressITI', 'LatePressITI','RewardITI']
    ...
    for i in range(len(outcome_array)):
        control_indx = list(set(np.where(opto_effective_tag[i] == 0)[0]))
        ...
        control_indx = np.where(opto_effective_tag[i] == 0)[0]
        opto_indx = np.where(opto_effective_tag[i] == 1)[0]
    ...
    if title == 'short':
        ...
        temprary = 'b'
    plot_scatter_outcome(axs[0], control_array, outcome_labels, 'o', temprary)
    plot_scatter_outcome(axs[0], opto_array, outcome_labels, '^', 'dodgerblue')
    plot_scatter_outcome(axs[1], control_rew, outcome_labels, 'o', 'g')
    plot_scatter_outcome(axs[1], opto_rew, outcome_labels, '^', 'g')
    plot_scatter_outcome(axs[1], control_ep, outcome_labels, 'o', 'b')
    plot_scatter_outcome(axs[1], opto_ep, outcome_labels, '^', 'b')
    plot_scatter_outcome(axs[1], control_lp, outcome_labels, 'o', 'pink')
    plot_scatter_outcome(axs[1], opto_lp, outcome_labels, '^', 'pink')
    plot_layout(axs[0], 'outcome for tr opto vs control'+ ' ' + title)
    plot_layout(axs[1], 'outcome for tr+1 opto vs tr+1 control'+ ' ' + title)
    
    
def plot_outcome_selected(axs, outcome_array, outcome_next, opto, session_opto_type, title = 'short'):
    outcome_labels = ['Reward', 'EarlyPress2', 'LatePress2']
    control_array = []
    opto_array = []
    control_rew = []
    opto_rew = []
    ...
    for i in range(len(outcome_array)):
        opto_tag = np.asarray(opto_effective_tag[i])
        outcome1  = np.asarray(outcome_array[i])   # strings are fine
        mask1 = (opto_tag == 0) & np.isin(outcome1, outcome_labels)
        control_indx = np.where(mask1)[0].tolist()
        
        mask2 = (opto_tag == 1) & np.isin(outcome1, outcome_labels)
        opto_indx = np.where(mask2)[0].tolist()
        if len(control_indx)>0:
            control_array.append(count_outcome(outcome_array[i][control_indx], outcome_labels))
        else:
            control_array.append(nan_array)
        ...
        if len(opto_lp_indx)>0:
            ...
            opto_lp.append(nan_array)
    plot_scatter_outcome(axs[0], control_array, outcome_labels, 'o', 'r')
    plot_scatter_outcome(axs[0], opto_array, outcome_labels, '^', 'dodgerblue')
    plot_scatter_outcome(axs[1], control_rew, outcome_labels, 'o', 'g')
    plot_scatter_outcome(axs[1], opto_rew, outcome_labels, '^', 'g')
    plot_scatter_outcome(axs[1], control_ep, outcome_labels, 'o', 'b')
...
def run(axs, session_data, delay_data,bin_size_all, st, block_type, outcome_plot = 1, label = 0):
    ...
    delay_temp = delay_data['delay_all']
    #print(selected_sessions, len(delay_temp))
    ...
    if block_type == 'short':
        ...
        plot_arrow(axs[0], delay_short, delay_short_next, outcome_short, opto_short, session_opto_type, bins_short, short_low_avg, short_up_avg, block_type)
        ...
        if outcome_plot == 1:
            plot_outcome(axs[2:4], outcome_short, outcome_short_next, opto_short, session_opto_type, block_type)
    elif block_type == 'long':
        bins_long = find_bin(short_low_avg, short_up_avg, delay_all, bin_size = bin_size_all[0])
        plot_arrow(axs[0], delay_long, delay_long_next, outcome_long, opto_long, session_opto_type, bins_long, long_low_avg, long_up_avg, block_type)
        ...
        if outcome_plot == 1:
            plot_outcome(axs[2:4], outcome_long, outcome_long_next, opto_long, session_opto_type, block_type)
            
def run_filtered_outcome(axs, session_data, delay_data, st, block_type, label = 0):
    # finding the v isually guided or self timed sessions
    num_session = len(session_data['IDs'])
    ST , VG = IsSelfTimed(session_data)
    if st == 1:
    ...
    # mega session
    if block_type == 'short':
        plot_outcome_selected(axs, outcome_short, outcome_short_next, opto_short, session_opto_type, block_type)
    elif block_type == 'long':
        plot_outcome_selected(axs, outcome_long, outcome_long_next, opto_long, session_opto_type, block_type)
