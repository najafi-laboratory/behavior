...
def mean_cal(arrays):
    ...
    for arr in arrays:
        # Convert to numpy array and ensure it is float to support NaN
        np_arr = np.array(arr, dtype=float)
        
        # --- NEW LINE: Replace negatives with NaN ---
        np_arr[np_arr < 0] = np.nan
        np_arr[np_arr > 4] = np.nan
        
        processed_arrays.append(np_arr)

    # Calculate Mean
    output1 = np.nanmean(processed_arrays[0], axis=0)
    output2 = np.nanmean(processed_arrays[1], axis=0)
    output3 = np.nanmean(processed_arrays[2], axis=0)
    output4 = np.nanmean(processed_arrays[3], axis=0)

    # Calculate SEM (Standard Error of the Mean)
    # Helper to calculate SEM safely for each processed array
    def get_sem(data):
        std = np.nanstd(data, axis=0)
        # Count non-NaN values along the axis
        n = np.count_nonzero(~np.isnan(data), axis=0)
        # Avoid division by zero if a column is all NaN
        n[n == 0] = 1 
        return std / np.sqrt(n)

    sem_output1 = get_sem(processed_arrays[0])
    sem_output2 = get_sem(processed_arrays[1])
    sem_output3 = get_sem(processed_arrays[2])
    sem_output4 = get_sem(processed_arrays[3])
