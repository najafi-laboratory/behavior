...
def cr_loops(trials, fec, fec_time, CR_stat, cr_interval_idx, bl_interval_idx, CR_interval_avg, base_line_avg,):
    ...
    for idx in fec:
        ...
        print(idx, "***********8")
        airpuff = trials[idx]['AirPuff'][0]
        print(airpuff)

        cr_idx = cr_onset_calc(fec[idx], fec_time[idx],15, airpuff, CR_stat[idx],
        cr_interval_idx[idx], bl_interval_idx[idx],
        CR_interval_avg[idx], base_line_avg[idx],
        veolcity_threshold_fraction, amp_threshold_fraction)
        # if cr_idx == 0 or cr_idx == len(fec_time[idx]):
        #     print('this is it', cr_idx)
        #     # breakpoint()
        #     continue

        if cr_idx == 0:
            cr_time = airpuff
            cr_index =  find_index(fec_time[idx], airpuff)


        try:

            cr_time[idx] = fec_time[idx][cr_idx]
            cr_index[idx] = cr_idx
    ...
    return cr_time, cr_index

def cr_onset_calc(fec, fec_time, window_size, air_puff, cr_stat) -> int:
    # doing a significant smoothing on the fec
    kernel = np.ones(window_size) / window_size
    fec = np.convolve(fec, kernel, mode='same')
    #calculating the velocity
    velocity = np.gradient(fec, fec_time)
    #assuming the wotst case in the beggining
    cr_idx = find_index(fec_time, air_puff)
    #calculating teh threshold for the velocity
    # threshold_v = velocity_threshold_fraction * (cr_interval_avg - base_line_avg)/ (fec_time[cr_interval_idx[0]] - fec_time[bl_interval_idx[1]])
    threshold_v = 0.001
    #calculating the threshold for the amplitude
    # threshold_a = amp_threshold_fraction * (cr_interval_avg - base_line_avg) + base_line_avg
    threshold_a = 0.01
    #goinga through time to find out the first CR onset like behaviour
    for t, time in enumerate(fec_time):
        if cr_stat ==1:
            if time > 50 and time < air_puff:
                if velocity[t] > threshold_v:
                    if fec[t] > threshold_a:
                        cr_idx = t
                        break
        else:
            if t == len(fec_time):
...
def CR_stat_indication(trials, fec, fec_time, static_threshold, AP_delay):
    ...
    for id in fec:
        ...
        # do a smoothing ovre the whole isi
        # if the value of the 
        # if isi_interval_avg[id] - base_line_avg[id] > poor_threshold:
