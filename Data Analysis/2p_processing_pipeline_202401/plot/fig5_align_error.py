...
def read_data(
    ...
    dff, _ = TracesSpikes.run(ops, fluo, neuropil)
    neural_trials = Trialization.run(ops, dff, raw_voltages)
    return [neural_trials, mask, label, raw_traces, raw_voltages]
...
def plot_fig5(
    ...
    try:
        [neural_trials, mask, label, raw_traces, raw_voltages] = read_data(ops)
        if len(neural_trials['jitter_flag']) <= 64:
            plot_omi(
                ops, neural_trials,
                mask, label,
                raw_traces, raw_voltages)
