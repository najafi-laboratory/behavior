...
def count_label(session_label, states, norm=False):
...
def plot_fig1_2(
    ...
    short_counts = np.zeros((len(session_id), len(states)))
    long_counts = np.zeros((len(session_id), len(states)))
    
    short_bottom = np.zeros((len(session_id), len(states)))
    long_bottom = np.zeros((len(session_id), len(states)))
    ...
    # if (short_delay_hit_rate > 0) and (long_delay_hit_rate > 0):    

        if usePercent:
            short_delay_hit_rate = session_data['session_short_delay_hit_rate'][i]
            long_delay_hit_rate = session_data['session_long_delay_hit_rate'][i]
            
            short_delay_pun_rate = session_data['session_short_delay_pun_rate'][i]
            long_delay_pun_rate = session_data['session_long_delay_pun_rate'][i]  
        
            print('short_delay_hit_rate', short_delay_hit_rate)
            print('short_delay_pun_rate', short_delay_pun_rate)
            print('short_delay_hit_rate + short_delay_pun_rate', round(short_delay_hit_rate + short_delay_pun_rate, 2))
            
            print('long_delay_hit_rate', long_delay_hit_rate)
            print('long_delay_pun_rate', long_delay_pun_rate)
            print('long_delay_hit_rate + long_delay_pun_rate', round(long_delay_hit_rate + long_delay_pun_rate, 2))
            
            short_counts[i, 0] = short_delay_hit_rate
            short_counts[i, 1] = short_delay_pun_rate
            
            short_bottom[i, 0] = 0
            short_bottom[i, 1] = short_delay_hit_rate
            
            long_counts[i, 0] = long_delay_hit_rate
            long_counts[i, 1] = long_delay_pun_rate
            
            long_bottom[i, 0] = 0
            long_bottom[i, 1] = long_delay_hit_rate            
            
        else: 
            short_delay_num_rew =  session_data['session_short_num_rew'][i]    
            short_delay_num_pun =  session_data['session_short_num_pun'][i]
            long_delay_num_rew =  session_data['session_long_num_rew'][i]
            long_delay_num_pun =  session_data['session_long_num_pun'][i]
            
            print('short_delay_num_rew', short_delay_num_rew)
            print('long_delay_num_rew', long_delay_num_rew)
            print('short_delay_num_rew + long_delay_num_rew', round(short_delay_num_rew + long_delay_num_rew, 2))
            
            print('short_delay_num_pun', short_delay_num_pun)
            print('long_delay_num_pun', long_delay_num_pun)
            print('short_delay_num_pun + long_delay_num_pun', round(short_delay_num_pun + long_delay_num_pun, 2))
            
         
            short_counts[i, 0] = short_delay_num_rew
            short_counts[i, 1] = short_delay_num_pun
            
            short_bottom[i, 0] = 0
            short_bottom[i, 1] = short_delay_num_rew
            
            long_counts[i, 0] = long_delay_num_rew
            long_counts[i, 1] = long_delay_num_pun
            
            long_bottom[i, 0] = 0
            long_bottom[i, 1] = long_delay_num_rew
...
# https://stackoverflow.com/questions/10369681/how-to-plot-bar-graphs-with-same-x-coordinates-side-by-side-dodged
    ...
    for i in range(len(states)):
        axs.bar(
            ...
            label=states[i])
        axs.bar(
            session_id + width, long_counts[:,i],
    ...
    fig.savefig(output_figs_dir + subject + '_Outcome_Short_Long_Delay.pdf', dpi=300)
    fig.savefig(output_imgs_dir + subject + '_Outcome_Short_Long_Delay.png', dpi=300)
