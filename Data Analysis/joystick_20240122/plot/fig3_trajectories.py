...
def plot_fig3(
    ...
    for i in range(start_idx, session_data['total_sessions']):
        ...
        for trial in range(min_trial, max_trial):
            ...
            # vis 1 aligned
            axs[0].plot(encoder_times_vis1, encoder_positions_aligned_vis1,'-', label='Average Trajectory')
            ...
            axs[1].plot(encoder_times_vis2, encoder_positions_aligned_vis2,'-', label='Average Trajectory')
            ...
            axs[2].plot(encoder_times_rew, encoder_positions_aligned_rew,'-', label='Average Trajectory')
            ...
            # save_image(filename)
            # fig.savefig('C:\\behavior\\joystick\\figures\\'+subject+'\\'+dates[i]+'\\fig3_'+subject+'_trajectory.png', dpi=300)
            # plt.close(fig)
