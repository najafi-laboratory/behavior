...
def Timing_epoch_add(
    ...
    for i in range(0 , len(session_id)):
        ...
        for trial in range(Block_size ,len(TrialOutcomes)):
            ...
            if TrialOutcomes[trial] == 'Reward':
                ...
                LeverRetract1 = int(trial_states['LeverRetract1'][1]*1000) # End of lever retract1
                # if trial_event['RotaryEncoder1_1'][0]:
                ...
                # Amplitude Max2
                amp2 = np.max(encoder_positions_aligned_vis1[punish1:punish2])
                # Onset2 to Amp2 (Time)
                on2_amp2 = np.abs((np.argmax(encoder_positions_aligned_vis1[punish1:punish2]) + punish1) - on2_vel)/1000 # type: ignore
                # Onset2 to target2 (Punish)
                on2_targ2 = np.abs(punish1 - on2_vel)/1000 # type: ignore
                # Onset Interval
                on_interval = np.abs(on2_vel - on1_vel)/1000 # type: ignore
                # Lever Retract1 End to Onset2
                levret1end_on2 = np.abs(on2_vel - LeverRetract1)/1000 # type: ignore
                ...
                base_amp2 = np.abs(amp2 - base_line)
                # Max velocity2
                if punish2 >= on2_vel:                 # type: ignore
                    max_vel2 = np.max(velocity[on2_vel:punish2])
                else:
                    max_vel2 = np.max(velocity[punish2:on2_vel])
                # Onset2 Velocity
                velaton2 = velocity[on2_vel]
            ...
            # For DidNotPress2 (WE have the press one only)
                ...
                if rotatory1 >100000:
                    print('trial lassts more than 100s',trial + 1)
                    continue
    ...
    pdf_path = os.path.join(output_figs_dir, subject + '_Learning_Epoch_time_amp_quantification_PLUS.pdf')
