...
def plot_average_outcomes_by_trial_type(sessions_outcome, sessions_trial_type, ax=None, outcome_colors=None):
    ...
    return ax
# ---------------------------------------------------------------------------------------
# plot outcome by blocks
def plot_average_outcomes_by_block_short(sessions_outcome, sessions_date, sessions_trial_type, sessions_block_type, ax=None, outcome_colors=None):
    ...
    bar_width = 0.2
    session_positions = np.arange(len(sessions_date)) * 2
    short_positions = session_positions - bar_width * 0.5
    long_positions = session_positions + bar_width * 0.5

    data = []
    for outcomes, trial_types, block_types in zip(sessions_outcome, sessions_trial_type, sessions_block_type):
        short_outcomes = [o for i, o in enumerate(outcomes) 
                          if i < len(trial_types) and i < len(block_types) 
                          and trial_types[i] == 1 and block_types[i] == 1 
                          and o not in ['RewardNaive', 'PunishNaive']]
        long_outcomes = [o for i, o in enumerate(outcomes) 
                        if i < len(trial_types) and i < len(block_types) 
                        and trial_types[i] == 2 and block_types[i] == 1 
                        and o not in ['RewardNaive', 'PunishNaive']]
        data.append({
            'short': short_outcomes,
            'long': long_outcomes,
            'total_short': len(short_outcomes),
            'total_long': len(long_outcomes)
        })
    ...
    for outcome in ordered_outcomes:
        ...
        ax.bar(short_positions, short_percentages, width=bar_width,
               bottom=short_bottoms, color=color, label=outcome)
        ax.bar(long_positions, long_percentages, width=bar_width,
               bottom=long_bottoms, color=color)
    ...
    ax.set_xlabel('Session Date')
    ax.set_ylabel('Percentage of Trials (%)')
    ax.set_title('Session Outcomes for Short Majority Blocks')
    ax.set_ylim(0, 100)
    ax.yaxis.set_major_formatter(plt.FuncFormatter(lambda y, _: f'{int(y)}%'))
...
# ---------------------------------------------------------------------------------------
def plot_pooled_outcomes_by_block_short(sessions_outcome, sessions_trial_type, sessions_block_type, ax=None, outcome_colors=None):
    if outcome_colors is None:
        outcome_colors = {
            'Reward': '#63f250',         # Green
            'Punish': '#e74c3c',         # Red
    ...
    # Pool all sessions for short and long majority blocks
    short_outcomes = []
    long_outcomes = []
    for outcomes, trial_types, block_types in zip(sessions_outcome, sessions_trial_type, sessions_block_type):
        short_outcomes.extend([o for i, o in enumerate(outcomes) 
                              if i < len(trial_types) and i < len(block_types) 
                              and trial_types[i] == 1 and block_types[i] == 1 
                              and o not in ['RewardNaive', 'PunishNaive']])
        long_outcomes.extend([o for i, o in enumerate(outcomes) 
                             if i < len(trial_types) and i < len(block_types) 
                             and trial_types[i] == 2 and block_types[i] == 1 
                             and o not in ['RewardNaive', 'PunishNaive']])
    ...
    for outcome in ordered_outcomes:
        short_count = short_outcomes.count(outcome)
        short_pct = (short_count / total_short * 100) if total_short > 0 else 0

        long_count = long_outcomes.count(outcome)
        long_pct = (long_count / total_long * 100) if total_long > 0 else 0

        color = outcome_colors.get(outcome, outcome_colors['Other'])

        # Assign label to both short and long bars to ensure legend includes all outcomes
        ax.bar(short_pos, short_pct, width=bar_width, bottom=short_bottom, 
               color=color, label=outcome)
        ax.bar(long_pos, long_pct, width=bar_width, bottom=long_bottom, 
               color=color, label=outcome)
    ...
    ax.set_ylabel('Percentage of Trials (%)')
    ax.set_title('Pooled Session Outcomes for Majaroty Short Blocks')
    ax.set_ylim(0, 100)
    ax.yaxis.set_major_formatter(plt.FuncFormatter(lambda y, _: f'{int(y)}%'))
...
# ---------------------------------------------------------------------------------------
# Grand average for short block majority
def plot_grand_average_outcomes_by_block_short(sessions_outcome, sessions_trial_type, sessions_block_type, ax=None, outcome_colors=None):
    if outcome_colors is None:
        outcome_colors = {
            'Reward': '#63f250',         # Green
            'Punish': '#e74c3c',         # Red
    ...
    short_percentages_by_outcome = {outcome: [] for outcome in ordered_outcomes}
    long_percentages_by_outcome = {outcome: [] for outcome in ordered_outcomes}
    ...
    for outcomes, trial_types, block_types in zip(sessions_outcome, sessions_trial_type, sessions_block_type):
        ...
        for outcome in ordered_outcomes:
            short_count = short_outcomes.count(outcome)
            short_pct = (short_count / total_short * 100) if total_short > 0 else 0
            short_percentages_by_outcome[outcome].append(short_pct)

            long_count = long_outcomes.count(outcome)
            long_pct = (long_count / total_long * 100) if total_long > 0 else 0
            long_percentages_by_outcome[outcome].append(long_pct)

    # Compute grand average percentages
    short_mean_percentages = []
    long_mean_percentages = []
    for outcome in ordered_outcomes:
        short_mean = np.mean(short_percentages_by_outcome[outcome]) if short_percentages_by_outcome[outcome] else 0
        long_mean = np.mean(long_percentages_by_outcome[outcome]) if long_percentages_by_outcome[outcome] else 0
        short_mean_percentages.append(short_mean)
        long_mean_percentages.append(long_mean)
    ...
    for outcome, short_pct, long_pct in zip(ordered_outcomes, short_mean_percentages, long_mean_percentages):
        color = outcome_colors.get(outcome, outcome_colors['Other'])
    ...
    ax.set_title('Grand Average Session Outcomes for Short Majority Blocks')
    ax.set_ylim(0, 100)
    ax.yaxis.set_major_formatter(plt.FuncFormatter(lambda y, _: f'{int(y)}%'))
...
# ---------------------------------------------------------------------------------------
def plot_average_outcomes_by_block_long(sessions_outcome, sessions_date, sessions_trial_type, sessions_block_type, ax=None, outcome_colors=None):
    ...
    for outcomes, trial_types, block_types in zip(sessions_outcome, sessions_trial_type, sessions_block_type):
        short_outcomes = [o for i, o in enumerate(outcomes) 
                          if i < len(trial_types) and i < len(block_types) 
                          and trial_types[i] == 1 and block_types[i] == 2 
                          and o not in ['RewardNaive', 'PunishNaive']]
        long_outcomes = [o for i, o in enumerate(outcomes) 
                        if i < len(trial_types) and i < len(block_types) 
                        and trial_types[i] == 2 and block_types[i] == 2 
                        and o not in ['RewardNaive', 'PunishNaive']]
        data.append({
            'short': short_outcomes,
            'long': long_outcomes,
    ...
    ax.set_xlabel('Session Date')
    ax.set_ylabel('Percentage of Trials (%)')
    ax.set_title('Session Outcomes for Long Majority Blocks')
    ax.set_ylim(0, 100)
    ax.yaxis.set_major_formatter(plt.FuncFormatter(lambda y, _: f'{int(y)}%'))
...
# ---------------------------------------------------------------------------------------
def plot_pooled_outcomes_by_block_long(sessions_outcome, sessions_trial_type, sessions_block_type, ax=None, outcome_colors=None):
    if outcome_colors is None:
        outcome_colors = {
            'Reward': '#63f250',         # Green
            'Punish': '#e74c3c',         # Red
    ...
    ax.set_ylabel('Percentage of Trials (%)')
    ax.set_title('Pooled Session Outcomes for Majaroty Long Blocks')
    ax.set_ylim(0, 100)
    ax.yaxis.set_major_formatter(plt.FuncFormatter(lambda y, _: f'{int(y)}%'))
...
# ---------------------------------------------------------------------------------------
# Grand average for short block majority
def plot_grand_average_outcomes_by_block_long(sessions_outcome, sessions_trial_type, sessions_block_type, ax=None, outcome_colors=None):
    if outcome_colors is None:
        outcome_colors = {
            'Reward': '#63f250',         # Green
            'Punish': '#e74c3c',         # Red
    ...
    ax.set_ylabel('Mean Percentage of Trials (%)')
    ax.set_title('Grand Average Session Outcomes for Long Majority Blocks')
    ax.set_ylim(0, 100)
    ax.yaxis.set_major_formatter(plt.FuncFormatter(lambda y, _: f'{int(y)}%'))
...
def plot_all_sessions_with_gridspec(sessions_outcome, sessions_date, sessions_trial_type, sessions_opto_tag, sessions_block_type, outcome_colors=None, figsize=(40, 30)):
    """
    ...
    gs = plt.GridSpec(7, 3,  height_ratios=[1, 1, 1, 1, 1, 1, 1], width_ratios=[10, 1, 1])  # Just one plot area
    ...
    # plot average outcomes by block type (short)
    ax = fig.add_subplot(gs[2, 0])
    plot_average_outcomes_by_block_short(sessions_outcome, sessions_date, sessions_trial_type, sessions_block_type, ax=ax, outcome_colors=None)

    # plot pooled outcomes by block type (short)
    ax = fig.add_subplot(gs[2, 1])
    plot_pooled_outcomes_by_block_short(sessions_outcome, sessions_trial_type, sessions_block_type, ax=ax, outcome_colors=None)

    # plot grand average outcomes by block type (short)
    ax = fig.add_subplot(gs[2, 2])
    plot_grand_average_outcomes_by_block_short(sessions_outcome, sessions_trial_type, sessions_block_type, ax=ax, outcome_colors=None)

    # plot average outcomes by block type (Long)
    ax = fig.add_subplot(gs[3, 0])
    plot_average_outcomes_by_block_long(sessions_outcome, sessions_date, sessions_trial_type, sessions_block_type, ax=ax, outcome_colors=None)

    # plot pooled outcomes by block type (Long)
    ax = fig.add_subplot(gs[3, 1])
    plot_pooled_outcomes_by_block_long(sessions_outcome, sessions_trial_type, sessions_block_type, ax=ax, outcome_colors=None)

    # plot grand average outcomes by block type (Long)
    ax = fig.add_subplot(gs[3, 2])
    plot_grand_average_outcomes_by_block_long(sessions_outcome, sessions_trial_type, sessions_block_type, ax=ax, outcome_colors=None)
