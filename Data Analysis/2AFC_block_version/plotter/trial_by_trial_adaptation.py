...
def plot_block_transitions(sessions_data, subject, data_paths, save_path=None):
    """
    Plot the fraction of Reward, Punish, DidNotChoose, and Rare trial outcomes aligned on block transitions
    (short-to-long and long-to-short) for multiple sessions, including pooled data, using GridSpec.
    Include the number of transitions in the plot titles. Save the plots to a PDF file.
    ...
    Args:
        ...
        save_path (str, optional): Path to save the output PDF file.

    Returns:
        None: Generates and saves plots to a PDF file.
    """
    def calculate_fractions(transitions, outcomes, rare_mask, window=10):
        """
        Calculate fractions and SEM of Reward, Punish, DidNotChoose, and Rare trials around transitions.
        
        Args:
            transitions (list): Indices of transition trials
            outcomes (np.array): Array of trial outcomes
            rare_mask (np.array): Boolean array indicating rare trials
    ...
    def plot_fractions(ax, fractions, fractions_sem, trial_positions, title, n_transitions):
        ...
        Args:
            ...
            n_transitions (int): Number of transitions contributing to the plot
        """
        ...
        ax.set_title(f"{title} ({n_transitions} Transitions)")
    ...
    trial_types_list = sessions_data.get('trial_types', [None] * len(dates))  # Assuming trial_types is in sessions_data
    n_sessions = len(dates)

    # Initialize figure with GridSpec
    fig = plt.figure(figsize=(16, 4 * (n_sessions + 1)))
    ...
    # Calculate rare mask for pooled data
    ...
    if short_to_long_transitions:
        fractions_s2l, sem_s2l, trial_positions = calculate_fractions(short_to_long_transitions, all_outcomes, rare_mask_pooled)
        plot_fractions(ax1, fractions_s2l, sem_s2l, trial_positions, 'Pooled Short to Long Transitions', len(short_to_long_transitions))
    else:
        ax1.text(0.5, 0.5, 'No Short to Long Transitions', ha='center', va='center')
        ax1.set_title('Pooled Short to Long Transitions (0 Transitions)')
        ax1.grid(False)
    ...
    if long_to_short_transitions:
        fractions_l2s, sem_l2s, trial_positions = calculate_fractions(long_to_short_transitions, all_outcomes, rare_mask_pooled)
        plot_fractions(ax2, fractions_l2s, sem_l2s, trial_positions, 'Pooled Long to Short Transitions', len(long_to_short_transitions))
    else:
        ax2.text(0.5, 0.5, 'No Long to Short Transitions', ha='center', va='center')
        ax2.set_title('Pooled Long to Short Transitions (0 Transitions)')
        ax2.grid(False)
        ax2.spines['right'].set_visible(False)
        ax2.spines['top'].set_visible(False)

    # Process individual sessions
    for i, (date, outcomes, block_types, trial_types) in enumerate(zip(dates, outcomes_list, block_types_list, trial_types_list)):
        short_to_long_transitions = []
        long_to_short_transitions = []
        ...
        # Calculate rare mask for this session
        ...
        if short_to_long_transitions:
            fractions_s2l, sem_s2l, trial_positions = calculate_fractions(short_to_long_transitions, outcomes, rare_mask)
            plot_fractions(ax1, fractions_s2l, sem_s2l, trial_positions, f'Short to Long Transitions - {date}', len(short_to_long_transitions))
        else:
            ax1.text(0.5, 0.5, 'No Short to Long Transitions', ha='center', va='center')
            ax1.set_title(f'Short to Long Transitions - {date} (0 Transitions)')
            ax1.grid(False)
            ax1.spines['right'].set_visible(False)
            ax1.spines['top'].set_visible(False)
        ...
        if long_to_short_transitions:
            fractions_l2s, sem_l2s, trial_positions = calculate_fractions(long_to_short_transitions, outcomes, rare_mask)
            plot_fractions(ax2, fractions_l2s, sem_l2s, trial_positions, f'Long to Short Transitions - {date}', len(long_to_short_transitions))
        else:
            ax2.text(0.5, 0.5, 'No Long to Short Transitions', ha='center', va='center')
            ax2.set_title(f'Long to Short Transitions - {date} (0 Transitions)')
            ax2.grid(False)
            ax2.spines['right'].set_visible(False)
            ax2.spines['top'].set_visible(False)
