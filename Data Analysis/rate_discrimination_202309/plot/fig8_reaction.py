...
def plot_curves(axs, session_data, max_sessions=5, max_time=7):
    ...
    start_idx = 0
    if max_sessions != -1 and len(reaction) > max_sessions:
        start_idx = len(reaction) - max_sessions
    reaction = reaction[start_idx:]
    group_reaction = [np.concatenate(r, axis=0) for r in reaction]
...
def plot_fig8(
    ...
    fig.savefig('./figures/fig8_reaction.pdf', dpi=300)
    fig.savefig('./figures/fig8_reaction.png', dpi=300)