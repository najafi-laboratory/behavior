...
session_data_path = 'C:\\Users\\Sana\\OneDrive\\Desktop\\PHD\\IntervalDiscrimination\\Data\\20240621\\'
...
def read_trials(subject , session_data_path):
    ...
    for f in tqdm(range(len(file_names))):
        ...
        for i in range(nTrials):
            ...
            if (not outcome_clean == 'EarlyLick' and not outcome_clean == 'earlyLickLimited' and not outcome_clean == 'Switching'):
            ...
            # lick events.
            if ('VisStimTrigger' in trial_states.keys() and
                not np.isnan(trial_states['VisStimTrigger'][1]) and not outcome_clean == 'EarlyLick' and not outcome_clean == 'Switching' and not outcome_clean == 'earlyLickLimited' ):
                licking_events = []
                direction = []
                correctness = []
                num_left = 0
