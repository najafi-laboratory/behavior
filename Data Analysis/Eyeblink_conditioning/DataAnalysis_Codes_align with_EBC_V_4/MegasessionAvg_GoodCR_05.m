...
for i = 1:length(data_files)
    ...
    for ctr_trial = 1:numTrials
       ...
       % Initialize a matrix to store interpolated FEC data
        ...
        % poor_CR_threshold = 0.03;
        % good_CR_threshold = 0.03;
...
for i = 1:length(data_files)
    ...
    title_text(1) = {' '};
    title_text(2) = {'Good CR trials average short-block above the baseline CR$_{\rm{th}}$ in $(T_{\rm{AirPuff-0.05}},T_{\rm{AirPuff}})$'};
...
title('Baseline-Adjusted Good CR Short Blocks', 'Interpreter', 'latex', 'FontSize', 14);
    if ~isempty(SD_1_FEC_norm_avg_short) && length(SD_1_FEC_norm_avg_short) == length(commonTime)
...
for i = 1:length(data_files)
    ...
    for ctr_trial = 1:numTrials
        ...
        % Skip trial if ISI does not exist (i.e., no LED_Puff_ISI field)
        if ~isfield(SessionData.RawEvents.Trial{1, ctr_trial}.States, 'LED_Puff_ISI') && SessionData.RawEvents.Trial{1, ctr_trial}.Data.IsProbeTrial  
            continue;
        end
                ...
                if isLongBlock
                    ...
                    elseif SessionData.SleepDeprived == 4
                        ...
                        N_C_long = N_C_long + 1;
                                   ...
                                   x_fill_long = [0.3999    0.4199    0.4199    0.3999];
...
hold on;
    ...
    title_text(1) = {' '};
    title_text(2) = {'Good CR trials average long-block above the baseline+CR$_{\rm{th}}$ in $(T_{\rm{AirPuff-0.05}},T_{\rm{AirPuff}})$'};
...
title('Baseline-Adjusted Good CR Long Blocks', 'Interpreter', 'latex', 'FontSize', 14);    
...
fill([commonTime, fliplr(commonTime)], ...
    ...
    if length(nameParts) >= 6
        ...
        newFilename = sprintf('%s_MegaSession_GoodCR_SD_Control', prefixPart); 
