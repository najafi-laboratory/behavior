...
def main():
    ...
    if len(data_files) == 0:
        print(f"No *_EBC_*.mat files found in {script_dir}. - Pooled_Line_CRamplitude_Dist_V_3.py:420")
        return
    ...
    for filepath in data_files:
        print(f"Processing: {filepath} - Pooled_Line_CRamplitude_Dist_V_3.py:501")
        S = loadmat(filepath, squeeze_me=True, struct_as_record=False)

        if "SessionData" not in S:
            print(f"Skipping {filepath}: SessionData not found - Pooled_Line_CRamplitude_Dist_V_3.py:505")
            continue
        ...
        if not np.isfinite(overall_max):
            print(f"Skipping {filepath}: could not compute overall_max - Pooled_Line_CRamplitude_Dist_V_3.py:514")
            continue
    ...
    print(f"Saved: {out_line} - Pooled_Line_CRamplitude_Dist_V_3.py:713")
    ...
    print(f"Saved: {out_cum} - Pooled_Line_CRamplitude_Dist_V_3.py:765")

    # --------------------------------------------------------
    # Summary print
    # --------------------------------------------------------
    print("\nCounts: - Pooled_Line_CRamplitude_Dist_V_3.py:770")
    for block in ["short", "long"]:
        for cat in ["Good CR", "Poor CR", "No CR"]:
            n_raw = len(dist[block][cat]["raw"])
            n_bs = len(dist[block][cat]["bs"])
            print(f"{block:>5s} | {cat:>7s} | raw={n_raw:4d} | bs={n_bs:4d} - Pooled_Line_CRamplitude_Dist_V_3.py:775")
