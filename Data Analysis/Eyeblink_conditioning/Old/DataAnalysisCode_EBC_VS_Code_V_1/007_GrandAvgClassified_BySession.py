...
def plot_panel_two_groups(ax, S_ctrl, S_chemo, cr_label, block_label):
    for S, color, label in [(S_ctrl, "k", "Control"), (S_chemo, "steelblue", "Chemo")]:
...
def main():
    ...
    _base_keys = ["short_good", "short_poor", "short_none", "long_good", "long_poor", "long_none"]
    ...
    # ---- baseline matching at session level: equalize pre-LED baseline distributions ----
    for _bk in _base_keys:
        _kc = f"{_bk}_ctrl"
        _kh = f"{_bk}_chemo"
        if not groups[_kc] or not groups[_kh]:
            continue
        _ic, _ih = match_baselines(groups[_kc], groups[_kh], t_grid)
        groups[_kc] = [groups[_kc][i] for i in _ic]
        groups[_kh] = [groups[_kh][i] for i in _ih]
        for _side, _idx in (("ctrl", _ic), ("chemo", _ih)):
            _k = f"{_bk}_{_side}"
            for _tk in ("led_s", "led_e", "puff_s", "puff_e", "cr_s", "cr_e"):
                timing[_k][_tk] = [timing[_k][_tk][i] for i in _idx]
    print("Baseline-matched (session means per CR category):")
    for _bk in _base_keys:
    ...
    plot_panel_two_groups(axs[0, 0], stats["short_good_ctrl"], stats["short_good_chemo"], "Good CR", "short")
    plot_panel_two_groups(axs[0, 1], stats["long_good_ctrl"],  stats["long_good_chemo"],  "Good CR", "long")

    plot_panel_two_groups(axs[1, 0], stats["short_poor_ctrl"], stats["short_poor_chemo"], "Poor CR", "short")
    plot_panel_two_groups(axs[1, 1], stats["long_poor_ctrl"],  stats["long_poor_chemo"],  "Poor CR", "long")

    plot_panel_two_groups(axs[2, 0], stats["short_none_ctrl"], stats["short_none_chemo"], "No CR", "short")
    plot_panel_two_groups(axs[2, 1], stats["long_none_ctrl"],  stats["long_none_chemo"],  "No CR", "long")
