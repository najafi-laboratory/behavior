...
% Initialize containers for session dates
sessionDates = [];
...
title('Mean Of Short Block Trials','Interpreter', 'latex', 'Units', 'normalized', 'Position', [0.5, 1.09, 0]);
% legend_handles = [];
legend_entries = {};
...
global_min_value = global_min_value - 0.02;
    ...
    h1 = plot(FEC_times_trimmed, FEC_norm_avg_short, 'b', 'LineWidth', 1.5);  % Blue for short blocks
    ...
    %     legend_entries_short = sprintf('%s_%s_Sessions', prefixPart, yearMonth);  % e.g., 'Session1_202310'
    % end
...
% Set the title with custom position
title('Mean Of Long Block Trials', 'Interpreter', 'latex','Units', 'normalized', 'Position', [0.5, 1.09, 0],'FontSize', 12);
...
for i = 1:length(data_files)
    ...
    % Plot the mean line for short blocks
    h2 = plot(FEC_times_trimmed, FEC_norm_avg_long, 'Color', [0 0.5 0], 'LineWidth', 1.5);  % Blue for short blocks
    ...
    %     % Extract only the year and month from datePart
    %     yearMonth = datePart(1:6);  % Extract first six characters representing 'YYYYMM'
    %     legend_entries_long = sprintf('%s_%s_Sessions', prefixPart, yearMonth);  % e.g., 'Session1_202310'
    % end
    % 
...
% Create legend entries with information directly, using line breaks
% legend(sprintf('Short Block (N = %d)\\newline(%s to %s)', N_short, ...
%     datestr(firstSessionDate, 'mm/dd/yyyy'), datestr(lastSessionDate, 'mm/dd/yyyy')), ...
%        sprintf('Long Block (N = %d)\\newline(%s to %s)', N_long, ...
%     datestr(firstSessionDate, 'mm/dd/yyyy'), datestr(lastSessionDate, 'mm/dd/yyyy')), ...
...
legend({[' (N = ', num2str(N_short), ')', newline, ...
    ...
    if length(nameParts) >= 6
        ...
        newFilename = sprintf('%s_AvgAllSessions_SEM', prefixPart); 
