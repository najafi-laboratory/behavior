...
title_text(1) = {'Average Of Epoch1 And Epoch2 Short Trials'};
...
for ctr_trial = 1:numTrials
       ...
       % Initialize a matrix to store interpolated FEC data
        ...
        % 
        %      if (first_epoch_end_boundari < ctr_trial)
        %          first_epoch_end_boundari = first_epoch_end_boundari + 50;
        %      end
        % 
        %     if ctr_trial <= (first_epoch_end_boundari+ctr_trial)
        %         if num_trials_first_epoch_short < epoch_length
        %             first_epoch_short_data = [first_epoch_short_data; FEC_norm_interp];
        %             num_trials_first_epoch_short = num_trials_first_epoch_short + 1;
        %             disp(ctr_trial)
        %         end 
        % 
        %     end
        %     if (last_epoch_start_boundari > ctr_trial)
        %             last_epoch_start_boundari = last_epoch_start_boundari - 50;
        %     end        
        %     if ctr_trial >= last_epoch_start_boundari 
        %         disp('inside elseif')
        %         disp(ctr_trial)
        %         % Last epoch in short block - Ensure only `epoch_length` trials are included
        %         if num_trials_last_epoch_short < epoch_length
        %             last_epoch_short_data = [last_epoch_short_data; FEC_norm_interp];
        %             num_trials_last_epoch_short = num_trials_last_epoch_short + 1;
        %             disp(ctr_trial)
        %         end
        % 
        %     end
        ...
        % end
        %     
    % Calculate the FEC normalized for each trial
    % FEC_led_aligned = SessionData.RawEvents.Trial{1, ctr_trial}.Data.FECTimes - LED_Puff_ISI_start;
    % FEC_norm = 1 - SessionData.RawEvents.Trial{1, ctr_trial}.Data.eyeAreaPixels / overallMax;
    % FEC_norm_interp = interp1(FEC_led_aligned, FEC_norm, commonTime, 'spline');
    ...
    if isShortBlock
        ...
        ctr_trial = current_block_start;
    end
end

    % Output the number of trials in each epoch for debugging
    fprintf('Short Block - Epoch 1 Trials: %d\n', num_trials_epoch1_short);
    fprintf('Short Block - Epoch 2 Trials: %d\n', num_trials_epoch2_short);


    average_epoch1_short = mean(epoch1_short_trs, 1, 'omitnan');
    average_epoch2_short = mean(epoch2_short_trs, 1, 'omitnan');
    ...
    % Compute baseline as the mean of the initial values across all trials in each epoch
    baseline_first_short = mean(epoch1_short_trs(:, 1), 'omitnan');
    baseline_last_short = mean(epoch2_short_trs(:, 1), 'omitnan');

    % Subtract the baseline from the entire trace
    % corrected_first_short = first_epoch_short_data - baseline_first_short;
    % corrected_last_short = last_epoch_short_data - baseline_last_short;

    % Initialize arrays for baseline-corrected data
    corrected_first_short = zeros(size(epoch1_short_trs));
    corrected_last_short = zeros(size(epoch2_short_trs));

    % Correct baseline for first_epoch_short_data
    for j = 1:size(epoch1_short_trs, 1)
        baseline_first_short = mean(epoch1_short_trs(j, 1:5)); % Baseline calculation using first 5 points
        corrected_first_short(j, :) = epoch1_short_trs(j, :) - baseline_first_short; % Subtract baseline
    end

     % Correct baseline for last_epoch_short_data
    for j = 1:size(epoch2_short_trs, 1)
        baseline_last_short = mean(epoch2_short_trs(j, 1:5));
        corrected_last_short(j, :) = epoch2_short_trs(j, :) - baseline_last_short;
    end

    % Now you can calculate the mean of the corrected data for plotting
    average_corrected_first_short = mean(corrected_first_short, 1, 'omitnan');
    average_corrected_last_short = mean(corrected_last_short, 1, 'omitnan');
    
    % Define a window size for smoothing (adjust as needed)
    windowSize = 5;
    
    % Apply moving average smoothing to the baseline-corrected means
    smoothed_average_corrected_first_short = movmean(average_corrected_first_short, windowSize);
    smoothed_average_corrected_last_short = movmean(average_corrected_last_short, windowSize);

    

    % Find the global minimum and maximum between both curves
    global_min_value = min([min(smoothed_average_corrected_first_short), min(smoothed_average_corrected_last_short)]);
    global_max_value = max([max(smoothed_average_corrected_first_short), max(smoothed_average_corrected_last_short)]);
    % Plot averages and SEM outside the trial loop
    hold on;
    ...
    % Plot the smoothed baseline-corrected data
    plot(commonTime, smoothed_average_corrected_first_short, 'Color', [0 1 1], 'LineWidth', 1.5);
    plot(commonTime, smoothed_average_corrected_last_short, 'Color', [0 0 1], 'LineWidth', 1.5);
    ...
    % Shading the SEM region around the mean
    fill([commonTime, fliplr(commonTime)], ...
         [smoothed_average_corrected_first_short + first_combined_sem_short, fliplr(smoothed_average_corrected_first_short - first_combined_sem_short)], ...
          [0 1 1], 'FaceAlpha', 0.25, 'EdgeColor', 'none', 'HandleVisibility', 'off');
    fill([commonTime, fliplr(commonTime)], ...
         [smoothed_average_corrected_last_short + last_combined_sem_short, fliplr(smoothed_average_corrected_last_short - last_combined_sem_short)], ...
...
title_text_long(1) = {'Average Of Epoche1 And Epoche2 Long Trials'}; 
...
for ctr_trial = 1:numTrials
   ...
   % 
   % 
   %      % First Epoch for Long Block
   %     if (first_epoch_end_boundari < ctr_trial)
   %         first_epoch_end_boundari = first_epoch_end_boundari + 50;
   %     end 
   %     if ctr_trial <= first_epoch_end_boundari
   %          if num_trials_first_epoch_long < epoch_length
   %              first_epoch_long_data = [first_epoch_long_data; FEC_norm_interp];
   %              num_trials_first_epoch_long = num_trials_first_epoch_long + 1;
   %              disp('when long block first epoch')
   %              disp(ctr_trial)
   %          end
   % 
   % 
   %     end
   %      % Last Epoch for Long Block
   %     if (last_epoch_start_boundari > ctr_trial)
   %         last_epoch_start_boundari = last_epoch_start_boundari - 50;
   %     end        
   % 
   %      if ctr_trial >= last_epoch_start_boundari
   %          if num_trials_last_epoch_long < epoch_length
   %              last_epoch_long_data = [last_epoch_long_data; FEC_norm_interp];
   %              num_trials_last_epoch_long = num_trials_last_epoch_long + 1;
   %              disp('when long block last epoch')
   %              disp(ctr_trial)
   %          end
   %      end
   %  end
    ...
    fprintf('Long Block - Epoch 1 Trials: %d\n', num_trials_epoch1_long);
    fprintf('Long Block - Epoch 2 Trials: %d\n', num_trials_epoch2_long);    % Calculate the average and SEM for each type of epoch


    % Calculate the average and SEM for each type of epoch
    average_epoch1_long = mean(epoch1_long_trs, 1, 'omitnan');
    average_epoch2_long = mean(epoch2_long_trs, 1, 'omitnan');
    ...
    % Plot for Long Block (dynamically assigned subplot position)
    % subplot(2, 2, 2);
    % hold on;


    % Plot first and last epochs for short blocks
    h3 = plot(commonTime, smoothed_average_epoch1_long, 'Color', lightgreen, 'LineWidth', 1.5); % First epoch, short blocks
    h4 = plot(commonTime, smoothed_average_epoch2_long, 'Color', dark_green, 'LineWidth', 1.5); % Last epoch, short blocks
    ...
    % Compute baseline as the mean of the initial values across all trials in each epoch
    baseline_first_long = mean(epoch1_long_trs(:, 1), 'omitnan');
    baseline_last_long = mean(epoch2_long_trs(:, 1), 'omitnan');

    % Initialize arrays for baseline-corrected data
    corrected_first_long = zeros(size(epoch1_long_trs));
    corrected_last_long = zeros(size(epoch2_long_trs));

    % Baseline  for each trace in each epoch
    for j = 1:size(epoch1_long_trs, 1)
        % Calculate the baseline as the mean of the initial few points for each trace
        baseline_first_long = mean(epoch1_long_trs(j, 1:5)); % Adjust the number of points as needed
        corrected_first_long(j, :) = epoch1_long_trs(j, :) - baseline_first_long;
    end

    for j = 1:size(epoch2_long_trs, 1)
        baseline_last_long = mean(epoch2_long_trs(j, 1:5));
        corrected_last_long(j, :) = epoch2_long_trs(j, :) - baseline_last_long;
    end

    % calculate the mean of baseline data for plotting
    average_corrected_first_long = mean(corrected_first_long, 1, 'omitnan');
    average_corrected_last_long = mean(corrected_last_long, 1, 'omitnan');
    
    % Define a window size for smoothing (adjust as needed)
    windowSize = 5;
    
    % Apply moving average smoothing to the baseline means
    smoothed_average_corrected_first_long = movmean(average_corrected_first_long, windowSize);
    smoothed_average_corrected_last_long = movmean(average_corrected_last_long, windowSize);
    ...
    % Find the global minimum and maximum between both curves
    global_min_value = min([min(smoothed_average_corrected_first_long), min(smoothed_average_corrected_last_long)]);
    global_max_value = max([max(smoothed_average_corrected_first_long), max(smoothed_average_corrected_last_long)]);
    % Plot averages and SEM outside the trial loop
    ...
    % Plot the smoothed baseline-corrected data
    plot(commonTime, smoothed_average_corrected_first_long, 'Color', lightgreen, 'LineWidth', 1.5);
    plot(commonTime, smoothed_average_corrected_last_long, 'Color',  dark_green, 'LineWidth', 1.5);


    % Shading the SEM region around the mean
    fill([commonTime, fliplr(commonTime)], [smoothed_average_corrected_first_long + first_combined_sem_long, fliplr(smoothed_average_corrected_first_long - first_combined_sem_long)], ...
    [0.3, 0.8, 0.6], 'FaceAlpha', 0.4, 'EdgeColor', 'none','HandleVisibility', 'off');

    fill([commonTime, fliplr(commonTime)], ...
         [smoothed_average_corrected_last_long + last_combined_sem_long, fliplr(smoothed_average_corrected_last_long - last_combined_sem_long)], ...
          [0 0.5 0], 'FaceAlpha', 0.4, 'EdgeColor', 'none','HandleVisibility', 'off');
    ...
    if length(nameParts) >= 5
        ...
        newFilename = sprintf('%s_AVG_EPOCHES_%s.pdf', prefixPart, datePart);
