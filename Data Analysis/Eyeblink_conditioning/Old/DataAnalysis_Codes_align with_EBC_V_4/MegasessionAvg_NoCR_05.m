...
for i = 1:length(data_files)
    ...
    title_text(1) = {' '};
    title_text(2) = {'No CR trials average short-block above the baseline CR$_{\rm{th}}$ in $(T_{\rm{LED}},T_{\rm{AirPuff}})$'};
...
title('Baseline-Adjusted No CR Short Blocks', 'Interpreter', 'latex', 'FontSize', 14);
    if ~isempty(SD_1_FEC_norm_avg_short) && length(SD_1_FEC_norm_avg_short) == length(commonTime)
...
hold on;
    ...
    title_text(1) = {' '};
    title_text(2) = {'No CR trials average long-block above the baseline+CR$_{\rm{th}}$ in $(T_{\rm{LED}},T_{\rm{AirPuff}})$'};
...
title('Baseline-Adjusted No CR Long Blocks', 'Interpreter', 'latex', 'FontSize', 14);    
...
fill([commonTime, fliplr(commonTime)], ...
    ...
    if length(nameParts) >= 6
        ...
        newFilename = sprintf('%s_MegaSession_NoCR_SD_Control', prefixPart); 
