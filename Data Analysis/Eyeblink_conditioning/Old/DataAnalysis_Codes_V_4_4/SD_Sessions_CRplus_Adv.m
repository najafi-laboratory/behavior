clc;close all;clear
 ...
 % data_files = dir('E3VT_EBC_V_3_11_20240914_163417.mat');
 % 
...
ylabel('Eyelid closure (norm)', 'interpreter', 'latex', 'fontsize', 14);
% Add legend
legend([SD_S_h, SD_L_h],{'Global Short Block Avg', 'Global Long Block Avg'}, 'Interpreter', 'latex', 'fontsize', 14,'Box', 'off', 'Location', 'bestoutside');


title_text(1) = {'\vspace{2cm} Global Average Across CR+ SD-Sessions \\'};
title_text(2) = {'CR$^{+}$short-block trials above the baseline+CR$_{\rm{th}}$ in $(T_{\rm{LED}},T_{\rm{AirPuff}})$'};
title_text(3) = {'CR$^{+}$long-block trials above the baseline+CR$_{\rm{th}}$ in $(T_{\rm{0.2(s)}},T_{\rm{AirPuff}})$'};
title_text(4) = {sprintf('${\\rm{CR}}_{\\rm{th}} = %.2f $',CR_threshold)};
title_text(5) = {' '}; 

title(title_text ,'interpreter', 'latex','fontname','Times New Roman','fontsize',12)
% 
% title('Global Average Across All SD-Sessions', 'Interpreter', 'latex', 'FontSize', 17);

hold off;
% Save the figure
newFilename = 'Avg_CRplus_SD_Sessions.pdf';
exportgraphics(gcf, newFilename, 'ContentType', 'vector');
