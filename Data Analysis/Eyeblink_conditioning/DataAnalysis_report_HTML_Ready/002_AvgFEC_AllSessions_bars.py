...
def main():
    ...
    if len(data_files) == 0:
        print(f"No *_EBC_*.mat files found in {script_dir}.  AvgFEC_AllSessions_bars.py:286 - 002_AvgFEC_AllSessions_bars.py:300")
        return
    print(f"Found {len(data_files)} .mat files in {script_dir} - 002_AvgFEC_AllSessions_bars.py:302")
    ...
    for filepath in data_files:
        print(f"Processing: {filepath}  AvgFEC_AllSessions_bars.py:356 - 002_AvgFEC_AllSessions_bars.py:384")
        ...
        if not include_sleep_deprived and np.any(sleep_deprived == 1):
            print(f"Skipping {os.path.basename(filepath)}: SD session  AvgFEC_AllSessions_bars.py:361 - 002_AvgFEC_AllSessions_bars.py:390")
            continue
        ...
        if not np.isfinite(overall_max):
            print(f"Skipping {os.path.basename(filepath)}: could not compute overall_max  AvgFEC_AllSessions_bars.py:369 - 002_AvgFEC_AllSessions_bars.py:398")
            continue
    ...
    if n_sessions == 0:
        print("No sessions with usable trials found.  AvgFEC_AllSessions_bars.py:511 - 002_AvgFEC_AllSessions_bars.py:551")
        return
    ...
    # chemo colors (red palette, matching BarPlotCR_Classification.py)
    chemo_color_good = "#8B0000"
    chemo_color_poor = "#D14949"
    chemo_color_no   = "#F2A3A3"
    ...
    print(f"Saved figure to: {outname}  AvgFEC_AllSessions_bars.py:578 - 002_AvgFEC_AllSessions_bars.py:644")
