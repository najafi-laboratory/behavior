...
def make_transition_clean_summary_pdf(path, x, chemo_s2l, chemo_l2s, control_s2l, control_l2s, tgrid, summary_text, meta_all):
    ...
    # CR-onset follows velocity and uses the exponential rise/decay fitter.
    draw_metric_dual(axs[3, 0], control_s2l["onset"], chemo_s2l["onset"], "S->L CR Onset latency (ms) — exponential fit", "s2l", True)
    draw_metric_dual(axs[3, 1], control_l2s["onset"], chemo_l2s["onset"], "L->S CR Onset latency (ms) — exponential fit", "l2s", True)
    draw_metric_dual(axs[4, 0], control_s2l["base"], chemo_s2l["base"], "S->L Baseline amplitude", "s2l")
    draw_metric_dual(axs[4, 1], control_l2s["base"], chemo_l2s["base"], "L->S Baseline amplitude", "l2s")
...
def make_transition_epoch_fec_pdf(path, control_s2l, control_l2s, chemo_s2l, chemo_l2s,
    ...
    control_epoch_specs = [
        ("traces_neg2", "pos_neg2", "Prev block late", "#000000", "--", 1.8),
        ("traces1", "pos1", "New block early", "#00AFCF", "-", 1.8),
        ("traces2", "pos2", "New block late", "#000000", "-", 2.0),
    ]
    chemo_epoch_specs = [
        ("traces_neg2", "pos_neg2", "Prev block late", "#000000", "--", 1.8),
        ("traces1", "pos1", "New block early", "#808080", "-", 1.8),
        ("traces2", "pos2", "New block late", "#000000", "-", 2.0),
    ]
...
def make_transition_raw_bl_metric_pdf(path, x, control_s2l, control_l2s, chemo_s2l, chemo_l2s,
    ...
    # Raw report source: keep only the raw FEC-at-200 row.  The baseline-
    # subtracted fits are presented in the separate baseline-matched figure.
    fig, axs = plt.subplots(1, 2, figsize=(13, 3.4), sharex=True)
    draw_metric_dual(axs[0], control_s2l["raw"], chemo_s2l["raw"], "S->L FEC @ 200 Raw", "s2l")
    draw_metric_dual(axs[1], control_l2s["raw"], chemo_l2s["raw"], "L->S FEC @ 200 Raw", "l2s")
...
def make_transition_blsub_baseline_matched_metric_pdf(path, x, control_s2l, control_l2s,
    ...
    # Match the raw metrics page dimensions so the two report composites align.
    fig, axs = plt.subplots(1, 2, figsize=(13, 3.4), sharex=True, sharey=False)
