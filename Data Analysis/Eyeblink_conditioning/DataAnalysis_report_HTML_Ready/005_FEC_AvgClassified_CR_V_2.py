...
def detect_onset_on_mean_trace(
    t_grid, mean_trace, block_label, smooth_window=3,
    vel_std_factor=2.0, vel_floor=0.01, min_sustain=2,
    min_onset_latency_s=0.050, short_end_s=0.212, long_end_s=0.412,
):
    """Velocity-based mean-trace CR onset used by the 010/011 analysis."""
    t = np.asarray(t_grid, dtype=float)
    signal = np.asarray(mean_trace, dtype=float)
    valid = np.isfinite(t) & np.isfinite(signal)
    baseline_idx = (t >= -0.2) & (t <= 0.0) & valid
    if np.count_nonzero(valid) < 2 or not np.any(baseline_idx):
        return np.nan

    signal_bs = signal - np.nanmean(signal[baseline_idx])
    signal_smooth = smooth_trace(signal_bs, window=smooth_window)
    velocity = np.gradient(signal_smooth, t)
    baseline_velocity = velocity[baseline_idx]
    baseline_velocity = baseline_velocity[np.isfinite(baseline_velocity)]
    if baseline_velocity.size > 1:
        velocity_threshold = (
            np.nanmean(baseline_velocity)
            + vel_std_factor * np.nanstd(baseline_velocity)
        )
    else:
        velocity_threshold = 0.0
    velocity_threshold = max(velocity_threshold, vel_floor)

    search_end = short_end_s if block_label.lower() == "short" else long_end_s
    search_idx = (t >= min_onset_latency_s) & (t <= search_end) & valid
    if not np.any(search_idx):
        return np.nan

    t_search = t[search_idx]
    velocity_search = velocity[search_idx]
    signal_search = signal_smooth[search_idx]
    if np.nanmax(signal_search) < 0.01:
        return np.nan

    run_start = None
    for i, (vel, amplitude) in enumerate(zip(velocity_search, signal_search)):
        if vel >= velocity_threshold and amplitude >= 0:
            if run_start is None:
                run_start = i
            if i - run_start + 1 >= min_sustain:
                return t_search[run_start]
        else:
            run_start = None
    return np.nan
...
def plot_panel(ax, S, cr_label, block_label):
    ...
    if np.isfinite(S["led_start"]) and np.isfinite(S["led_end"]):
        ax.text(S["led_start"] + 0.002, y_text, "LED",
                color="black", va="bottom", ha="left", fontsize=8)

    if np.isfinite(S["puff_start"]) and np.isfinite(S["puff_end"]):
        ax.text(S["puff_start"] + 0.002, y_text, "AirPuff",
                color=puff_text_color, va="bottom", ha="left", fontsize=8)

    if np.isfinite(S["cr_start"]) and np.isfinite(S["cr_end"]):
        ax.text(S["cr_start"] + 0.002, y_text - 0.06 * (ymax - ymin), "CR window",
                color="darkorange", va="bottom", ha="left", fontsize=7)

    ax.set_title(f"{cr_label} -- {block_label} (n={S['n']})", fontsize=10)
    ax.set_xlabel("Time from LED onset (s)", fontsize=9)
    ax.set_ylabel("FEC", fontsize=9)
    ax.grid(False)
...
def plot_panel_two_groups(ax, S_ctrl, S_chemo, cr_label, block_label):
    ...
    if np.any(np.isfinite(S_chemo["mean"])):
        ax.plot(S_chemo["t"], S_chemo["mean"], linewidth=1.8, color="#D14949", label="Chemo")
        ax.fill_between(
            S_chemo["t"],
    ...
    # Add velocity-detected CR onsets without replacing any event/window lines.
    # Do not report an onset when the estimate is not reliable: No-CR panels,
    # very small groups, or detections at/after the CR window (AirPuff response).
    min_onset_trials = 5

    def reliable_onset(stats):
        if cr_label == "No CR" or stats["n"] < min_onset_trials:
            return np.nan

        onset = detect_onset_on_mean_trace(stats["t"], stats["mean"], block_label)
        if not np.isfinite(onset):
            return np.nan

        if cr_label == "All":
            latest_valid_onset = stats["puff_start"]
        else:
            latest_valid_onset = stats["cr_start"]

        if np.isfinite(latest_valid_onset) and onset >= latest_valid_onset:
            return np.nan
        return onset

    onset_ctrl = reliable_onset(S_ctrl)
    onset_chemo = reliable_onset(S_chemo)
    if np.isfinite(onset_ctrl):
        ax.axvline(
            onset_ctrl, color="lightgreen", linestyle="--", linewidth=1.7,
            zorder=5, label=f"Ctrl onset: {onset_ctrl * 1000:.0f} ms",
        )
    if np.isfinite(onset_chemo):
        ax.axvline(
            onset_chemo, color="darkgreen", linestyle="--", linewidth=1.7,
            zorder=5, label=f"Chemo onset: {onset_chemo * 1000:.0f} ms",
        )
...
def plot_four_row_fec_figure(
    ...
    fig, axs = plt.subplots(4, 2, figsize=(11.5, 10.5))

    for r, (cat, label) in enumerate(row_defs):
        plot_panel_two_groups(
            axs[r, 0],
            stats_by_key[("short", cat, "ctrl")],
            stats_by_key[("short", cat, "chemo")],
            label,
            "Short",
        )
        plot_panel_two_groups(
            axs[r, 1],
            stats_by_key[("long", cat, "ctrl")],
            stats_by_key[("long", cat, "chemo")],
            label,
            "Long",
        )
    ...
    print(f"Saved figure to: {outname} - 005_FEC_AvgClassified_CR_V_2.py:735")
    plt.show()
...
def main():
    ...
    if len(data_files) == 0:
        print(f"No *_EBC_*.mat files found in {script_dir}.  FEC_AvgClassified_CR.py:425  FEC_AvgClassified_CR_V_2.py:436 - 005_FEC_AvgClassified_CR_V_2.py:748")
        return
    ...
    for filepath in data_files:
        print(f"Processing: {filepath}  FEC_AvgClassified_CR.py:512  FEC_AvgClassified_CR_V_2.py:523 - 005_FEC_AvgClassified_CR_V_2.py:815")
        ...
        if not np.isfinite(open_eye_area):
            print(f"Skipping {os.path.basename(filepath)}: could not estimate session openeye area  FEC_AvgClassified_CR_V_2.py - 005_FEC_AvgClassified_CR_V_2.py:839")
            continue
        ...
        for tr in trials:
            ...
            F_acc[all_key].append(Fq)
            puff_s_acc[all_key].append(puff_start_rel)
            puff_e_acc[all_key].append(puff_end_rel)
            cr_s_acc[all_key].append(np.nan)
            cr_e_acc[all_key].append(np.nan)
            led_s_acc[all_key].append(led_start_rel)
            led_e_acc[all_key].append(led_end_rel)

    raw_F_acc = {k: list(v) for k, v in F_acc.items()}
    raw_puff_s_acc = {k: list(v) for k, v in puff_s_acc.items()}
    raw_puff_e_acc = {k: list(v) for k, v in puff_e_acc.items()}
    raw_cr_s_acc = {k: list(v) for k, v in cr_s_acc.items()}
    raw_cr_e_acc = {k: list(v) for k, v in cr_e_acc.items()}
    raw_led_s_acc = {k: list(v) for k, v in led_s_acc.items()}
    raw_led_e_acc = {k: list(v) for k, v in led_e_acc.items()}
    ...
    print("Baselinematched (ctrl > chemo trials per category): - 005_FEC_AvgClassified_CR_V_2.py:979")
    for _bl in ("short", "long"):
        for _cat in ("all", "good", "poor", "no"):
            _nc = len(F_acc[(_bl, _cat, "ctrl")])
            _nh = len(F_acc[(_bl, _cat, "chemo")])
            print(f"{_bl:5s} {_cat:4s}: ctrl={_nc}, chemo={_nh} - 005_FEC_AvgClassified_CR_V_2.py:984")

    # ------------ pack stats ------------
    def ps_from(accs, bl, cat, grp):
        k = (bl, cat, grp)
        Fd, psd, ped, csd, ced, lsd, led = accs
        return pack_stats(
            t_grid, Fd[k],
            psd[k], ped[k],
            csd[k], ced[k],
    ...
    def pack_all(accs):
        return {
            (bl, cat, grp): ps_from(accs, bl, cat, grp)
            for bl in ("short", "long")
            for cat in ("all", "no", "poor", "good")
            for grp in ("ctrl", "chemo")
        }

    raw_stats = pack_all((
        raw_F_acc, raw_puff_s_acc, raw_puff_e_acc,
        raw_cr_s_acc, raw_cr_e_acc, raw_led_s_acc, raw_led_e_acc,
    ))
    matched_stats = pack_all((
        F_acc, puff_s_acc, puff_e_acc,
        cr_s_acc, cr_e_acc, led_s_acc, led_e_acc,
    ))

    raw_outname = f"PooledAvgFEC_Raw_AllAndCRclassified_ShortLong_{first_tok}_to_{last_tok}.pdf"
    plot_four_row_fec_figure(
        raw_stats,
        "Raw Pooled Avg FEC: All Trials and CR-classified Short/Long",
        raw_outname,
        first_date,
        last_date,
        n_ctrl_sessions,
    ...
    matched_outname = f"PooledAvgFEC_BaselineMatched_AllAndCRclassified_ShortLong_{first_tok}_to_{last_tok}.pdf"
    plot_four_row_fec_figure(
        matched_stats,
        "Baseline-matched Pooled Avg FEC: All Trials and CR-classified Short/Long",
        matched_outname,
        first_date,
        last_date,
        n_ctrl_sessions,
