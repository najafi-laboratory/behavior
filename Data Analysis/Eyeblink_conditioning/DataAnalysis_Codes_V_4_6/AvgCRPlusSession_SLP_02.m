...
color_map_short = [linspace(dimBlue(1), darkBlue(1), num_colors)', ...
    ...
    if ~isempty(short_block_data)
        FEC_norm_avg_short = mean(short_block_data, 1, 'omitnan');
        baseline_window_indices = (commonTime >= -0.2 & commonTime <= 0);
        baseline_short = mean(FEC_norm_avg_short(baseline_window_indices), 'omitnan');
        FEC_norm_avg_short_corrected = FEC_norm_avg_short - baseline_short;

        % Plot original and baseline-corrected
        h1 = plot(commonTime, FEC_norm_avg_short, 'Color', color_map_short(i, :), 'LineWidth', 1.2);
        plot(commonTime, FEC_norm_avg_short_corrected, 'Color', color_map_short(i, :), 'LineWidth', 1.2);
    end
...
color_map_long = [linspace(dimGreen(1), darkGreen(1), num_colors)', ...
    ...
    if ~isempty(long_block_data)
       FEC_norm_avg_long = mean(long_block_data, 1,'omitnan');
        baseline_window_indices = (commonTime >= -0.2 & commonTime <= 0);
        baseline_long = mean(FEC_norm_avg_long(baseline_window_indices), 'omitnan');
        FEC_norm_avg_long_corrected = FEC_norm_avg_long - baseline_long;

        % Plot original and baseline-corrected
        h2 = plot(commonTime, FEC_norm_avg_long, 'Color',color_map_long(i, :), 'LineWidth', 1.2); % Red for long blocks
        plot(commonTime, FEC_norm_avg_long_corrected, 'Color', color_map_long(i, :), 'LineWidth', 1.2);
 if i == 1 || i == num_colors
