...
all_trials_short_block_Control = [];
    ...
    avg_session_short_block_SD =[];
    avg_session_short_block_Control =[];
...
for i = 1:length(data_files)
        ...
        if isShortBlock 
            if is_CR_plus
                if SessionData.SleepDeprived
                    all_trials_short_block_SD = [all_trials_short_block_SD; FEC_trimmed];
                else
                    all_trials_short_block_Control = [all_trials_short_block_Control; FEC_trimmed];
                end
    ...
    avg_session_short_block_SD = [avg_session_short_block_SD ; mean(all_trials_short_block_SD, 1, 'omitnan')];
    avg_session_short_block_Control = [avg_session_short_block_Control ; mean(all_trials_short_block_Control, 1, 'omitnan')];
end
...
global_min_value = global_min_value - 0.02;
    ...
    h1 = plot(common_time_vector, SD_FEC_norm_avg_short, 'r', 'LineWidth', 1.5);
    h2 = plot(common_time_vector, C_FEC_norm_avg_short , 'b', 'LineWidth', 1.5);
...
for i = 1:length(data_files)
    ...
    title_text(1) = {' '};
    title_text(2) = {'CR$^{+}$ grand average short-block above the baseline+CR$_{\rm{th}}$ in $(T_{\rm{LED}},T_{\rm{AirPuff}})$'};
...
plot(common_time_vector, SD_FEC_norm_avg_short_corrected, 'r', 'LineWidth', 1.5);
plot(common_time_vector, C_FEC_norm_avg_short_corrected, 'b', 'LineWidth', 1.5);
    ...
    global_max_value_baseline = global_max_value_baseline + 0.03;
    global_min_value_baseline = global_min_value_baseline - 0.03;
...
avg_session_long_block_SD = [];
avg_session_long_block_Control = [];
...
for i = 1:length(data_files)
    ...
    for ctr_trial = 1:numTrials
        ...
        t1_CR = LED_Onset_Zero_Start;
        t2_CR = AirPuff_LED_Onset_Aligned_End;
        t1_B1 = t1_CR - 0.01;
        t2_B2 = t1_CR; 
        ...
        if isLongBlock 
            if is_CR_plus
            if SessionData.SleepDeprived
                all_trials_long_block_SD = [all_trials_long_block_SD; FEC_trimmed];
                % N_SD_long = N_SD_long + 1;
            else
                all_trials_long_block_Control = [all_trials_long_block_Control; FEC_trimmed];
                % N_C_long = N_C_long + 1;
            end
    ...
    avg_session_long_block_SD = [avg_session_long_block_SD ; mean(all_trials_long_block_SD, 1, 'omitnan')];
    avg_session_long_block_Control = [avg_session_long_block_Control ; mean(all_trials_long_block_Control, 1, 'omitnan')];
...
hold on;
    ...
    % Plot the mean line for short blocks
    h3 = plot(common_time_vector, SD_FEC_norm_avg_long,'r' , 'LineWidth', 1.5);  % Blue for short blocks
    h4 = plot(common_time_vector, C_FEC_norm_avg_long , 'b', 'LineWidth', 1.5);
...
plot(common_time_vector, SD_FEC_norm_avg_long_corrected, 'r', 'LineWidth', 1.5);
plot(common_time_vector, C_FEC_norm_avg_long_corrected, 'b', 'LineWidth', 1.5);
...
fill([common_time_vector, fliplr(common_time_vector)], ...
    ...
    if length(nameParts) >= 6
        ...
        newFilename = sprintf('%s_GrandAvgCR_SD_Control', prefixPart); 
