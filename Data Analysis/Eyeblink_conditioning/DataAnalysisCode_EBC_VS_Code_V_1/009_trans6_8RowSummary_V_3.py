...
def make_8row_pdf(path, x, s2l, l2s, tgrid, summary_text, meta_all):
    fig, axs = plt.subplots(10, 2, figsize=(13, 30), sharex=False)
    fig.suptitle(format_summary_title(summary_text), fontsize=12, y=0.992)

    draw_time_panel(axs[0, 0], s2l, "S->L FEC (Raw)", "s2l", tgrid, False)
    draw_time_panel(axs[0, 1], l2s, "L->S FEC (Raw)", "l2s", tgrid, False)

    draw_time_panel(axs[1, 0], s2l, "S->L FEC (BL-sub)", "s2l", tgrid, True)
    draw_time_panel(axs[1, 1], l2s, "L->S FEC (BL-sub)", "l2s", tgrid, True)
    ...
    draw_time_panel(axs[2, 0], wrap_vel(s2l), "S->L Velocity", "s2l", tgrid, False)
    draw_time_panel(axs[2, 1], wrap_vel(l2s), "L->S Velocity", "l2s", tgrid, False)

    draw_trial_panel(axs[3, 0], x, s2l["raw"], "S->L FEC @ 200 (Raw)", "s2l", False)
    draw_trial_panel(axs[3, 1], x, l2s["raw"], "L->S FEC @ 200 (Raw)", "l2s", False)

    draw_trial_panel(axs[4, 0], x, s2l["bs"], "S->L BL-Sub (Fit)", "s2l", True)
    draw_trial_panel(axs[4, 1], x, l2s["bs"], "L->S BL-Sub (Fit)", "l2s", True)

    draw_trial_panel(axs[5, 0], x, s2l["vel"], "S->L Velocity (Fit)", "s2l", True)
    draw_trial_panel(axs[5, 1], x, l2s["vel"], "L->S Velocity (Fit)", "l2s", True)

    draw_trial_panel(axs[6, 0], x, s2l["base"], "S->L Baseline amplitude", "s2l", False, color_override=COLOR_BASELINE)
    draw_trial_panel(axs[6, 1], x, l2s["base"], "L->S Baseline amplitude", "l2s", False, color_override=COLOR_BASELINE)

    draw_trial_panel(axs[7, 0], x, s2l["onset"], "S->L CR Onset latency (ms)", "s2l", False, color_override=COLOR_ONSET)
    draw_trial_panel(axs[7, 1], x, l2s["onset"], "L->S CR Onset latency (ms)", "l2s", False, color_override=COLOR_ONSET)

    draw_trial_panel(axs[8, 0], x, s2l["onset"], "S->L CR Onset latency (ms, Fit)", "s2l", True, color_override=COLOR_ONSET)
    draw_trial_panel(axs[8, 1], x, l2s["onset"], "L->S CR Onset latency (ms, Fit)", "l2s", True, color_override=COLOR_ONSET)

    draw_valid_transition_count_panel(
        axs[9, 0],
        meta_all,
        direction="s2l",
...
def run():
    ...
    for mouse, root in MOUSE_ROOTS.items():
        ...
        m_s2l = finalize(m_acc["s2l"])
        m_l2s = finalize(m_acc["l2s"])
        ...
        make_8row_pdf(
            ...
            x, m_s2l, m_l2s, tgrid_ref, summary, meta_all
        )
        ...
        for direction, src in [("s2l", m_s2l), ("l2s", m_l2s)]:
            for k in ["raw", "bs", "vel", "base", "onset"]:
                if src[k].size:
                    pool[direction][k].append(src[k])
            for k in trace_keys:
    ...
    make_8row_pdf(
        ...
        x, p_s2l, p_l2s, tg, pooled_summary, all_meta
    )
