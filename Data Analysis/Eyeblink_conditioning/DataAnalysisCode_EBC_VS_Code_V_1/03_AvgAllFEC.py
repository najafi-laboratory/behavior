...
def main():
    ...
    if len(data_files) == 0:
        print("No *_EBC_*.mat files found. - AvgAllFEC.py:347")
        return
    ...
    for filepath in data_files:
        print(f"Processing: {filepath} - AvgAllFEC.py:412")
        ...
        if not np.isfinite(overall_max):
            print(f"Skipping {os.path.basename(filepath)}: could not compute overall_max - AvgAllFEC.py:426")
            continue
    ...
    print(f"Saved figure to: {outname} - AvgAllFEC.py:558")
