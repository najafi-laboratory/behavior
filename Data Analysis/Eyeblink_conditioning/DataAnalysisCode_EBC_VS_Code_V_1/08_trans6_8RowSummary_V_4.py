...
def load_trials_from_mat(p: pathlib.Path) -> List[dict]:
    ...
    chemo_flag = int(get_ci(SD, "Chemogenetics", 0) or 0)
...
def make_block_count_lines_pdf(save_path, meta_all):
    ...
    with PdfPages(save_path) as pdf:
        pdf.savefig(fig)
    plt.close(fig)
    print(f"Saved: {save_path}  trans6_8RowSummary_V_3.py:323 - 08_trans6_8RowSummary_V_3.py:324")
...
def make_block_count_lines_pdf(save_path, meta_all, zoom_only=True, zoom_window=3):
    ...
    print(f"Saved: {save_path}  trans6_8RowSummary_V_3.py:732 - 08_trans6_8RowSummary_V_3.py:744")
...
def make_8row_pdf(path, x, chemo_s2l, chemo_l2s, control_s2l, control_l2s, tgrid, summary_text, meta_all):
    ...
    CTRL_COLORS  = ["#c0c0c0", "#888888", "#404040", "#000000"]  # light grey → black
    CHEMO_COLORS = ["#a8d4f8", "#5599ee", "#1a66cc", "#002f80"]  # light blue → dark blue

    def draw_time_single(ax, d, title, mode, group, bl_sub=False):
        colors = CTRL_COLORS if group == "ctrl" else CHEMO_COLORS
        specs = [
        ...
        for (tk, pk, lbl), col in zip(specs, colors):
            tr = d.get(tk, [])
            pos = d.get(pk, [])
            if tr:
                m, s = mean_sem_trace(tr, tgrid, bl_sub)
                ax.fill_between(tgrid, m - s, m + s, color=col, alpha=ALPHA_FILL, lw=0)
                ax.plot(tgrid, m, color=col, lw=1.4,
    ...
    with PdfPages(path) as pdf:
        pdf.savefig(fig)
    plt.close(fig)
    print(f"Saved: {path} - 08_trans6_8RowSummary_V_3.py:864")
...
def make_block_length_pairs_pdf(save_path, meta_all, summary_text=None):
    ...
    with PdfPages(save_path) as pdf:
        pdf.savefig(fig)
    plt.close(fig)
    print(f"Saved: {save_path}  trans6_8RowSummary_V_3.py:915 - 08_trans6_8RowSummary_V_3.py:979")
...
def make_session_block_timeline_pdf(save_path, meta_all):
    ...
    if not rows:
        print(f"No session block data for {save_path}  trans6_8RowSummary_V_3.py:1067 - 08_trans6_8RowSummary_V_3.py:1235")
        return
    ...
    with PdfPages(save_path) as pdf:
        pdf.savefig(fig)
    plt.close(fig)
    print(f"Saved: {save_path}  trans6_8RowSummary_V_3.py:1098 - 08_trans6_8RowSummary_V_3.py:1266")
...
def make_block_vs_trial_mean_pdf(save_path, meta_all):
    ...
    if not all_traces:
        print(f"No data for {save_path}  trans6_8RowSummary_V_3.py:1119 - 08_trans6_8RowSummary_V_3.py:1287")
        return
    ...
    with PdfPages(save_path) as pdf:
        pdf.savefig(fig)
    plt.close(fig)
    print(f"Saved: {save_path}  trans6_8RowSummary_V_3.py:1146 - 08_trans6_8RowSummary_V_3.py:1314")
...
def run():
    ...
    for mouse, root in MOUSE_ROOTS.items():
        print(f"Processing {mouse}...  trans6_8RowSummary_V_3.py:1379 - 08_trans6_8RowSummary_V_3.py:1561")
        ...
        for p in files:
            ...
            try:
                ...
                print(f"Skipped {p.name}: {e}  trans6_8RowSummary_V_3.py:1413 - 08_trans6_8RowSummary_V_3.py:1594")
    ...
    print("Processing pooled...  trans6_8RowSummary_V_3.py:1456 - 08_trans6_8RowSummary_V_3.py:1647")
