...
%%
% === Common binning settings ===
edges = 0:0.02:1;
bin_centers = edges(1:end-1) + diff(edges)/2;

% --- Set group data and color scheme ---
...
% cum_poor = cumsum(histcounts(CR_Poor_short_raw, edges)) / N_total;
% cum_no   = cumsum(histcounts(CR_No_short_raw, edges)) / N_total;
% cum_good = cumsum(histcounts(CR_Good_short_raw, edges)) / N_total;

cum_poor = cumsum(histcounts(CR_Poor_short_raw, edges)) / numel(CR_Poor_short_raw);
cum_no   = cumsum(histcounts(CR_No_short_raw, edges)) / numel(CR_No_short_raw);
cum_good = cumsum(histcounts(CR_Good_short_raw, edges)) / numel(CR_Good_short_raw);

plot(bin_centers, cum_good, '-', 'Color', color_good, 'LineWidth', 2); hold on;
plot(bin_centers, cum_poor, '-', 'Color', color_poor, 'LineWidth', 2);
plot(bin_centers, cum_no,   '-', 'Color', color_no,   'LineWidth', 2);
title('\textnormal{Short Block --- Raw}', 'Interpreter', 'latex');
xlabel('CR Magnitude', 'Interpreter', 'latex');
ylabel('Cumulative Fraction', 'Interpreter', 'latex');
...
% cum_poor = cumsum(histcounts(CR_Poor_short_bs, edges)) / N_total;
% cum_no   = cumsum(histcounts(CR_No_short_bs, edges)) / N_total;
% cum_good = cumsum(histcounts(CR_Good_short_bs, edges)) / N_total;

cum_poor = cumsum(histcounts(CR_Poor_short_bs, edges)) / numel(CR_Poor_short_bs);
cum_no   = cumsum(histcounts(CR_No_short_bs, edges)) / numel(CR_No_short_bs);
cum_good = cumsum(histcounts(CR_Good_short_bs, edges)) / numel(CR_Good_short_bs);
...
% cum_poor = cumsum(histcounts(CR_Poor_long_raw, edges)) / N_total;
% cum_no   = cumsum(histcounts(CR_No_long_raw, edges)) / N_total;
% cum_good = cumsum(histcounts(CR_Good_long_raw, edges)) / N_total;

cum_poor = cumsum(histcounts(CR_Poor_long_raw, edges)) / numel(CR_Poor_long_raw);
cum_no   = cumsum(histcounts(CR_No_long_raw, edges)) / numel(CR_No_long_raw);
cum_good = cumsum(histcounts(CR_Good_long_raw, edges)) / numel(CR_Good_long_raw);
...
% cum_poor = cumsum(histcounts(CR_Poor_long_bs, edges)) / N_total;
% cum_no   = cumsum(histcounts(CR_No_long_bs, edges)) / N_total;
% cum_good = cumsum(histcounts(CR_Good_long_bs, edges)) / N_total;

cum_poor = cumsum(histcounts(CR_Poor_long_bs, edges)) / numel(CR_Poor_long_bs);
cum_no   = cumsum(histcounts(CR_No_long_bs, edges)) / numel(CR_No_long_bs);
cum_good = cumsum(histcounts(CR_Good_long_bs, edges)) / numel(CR_Good_long_bs);
...
% --- Overall title ---
sgtitle(['Cumulative CR Distributions Pooled Control — ', group_name], 'FontSize', 13, 'Interpreter', 'latex');

% Optional: export
exportgraphics(gcf, ['Pooled Control_Cumulative_CR_Distributions_', group_name, '.pdf'], 'ContentType', 'vector');