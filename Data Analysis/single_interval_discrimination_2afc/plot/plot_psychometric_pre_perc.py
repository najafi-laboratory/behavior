...
def get_bin_stat(decision, isi='pre'):
    bin_size=100
    least_trials=5
...
def get_decision(subject_session_data):
    ...
    isi_pre_perc = subject_session_data['isi_pre_perc']
    isi_pre_perc = np.concatenate(isi_pre_perc).reshape(1,-1)
    isi_post_perc = subject_session_data['isi_post_perc']
    isi_post_perc = np.concatenate(isi_post_perc).reshape(1,-1)
    decision = np.concatenate([decision, jitter_flag, isi_pre_perc, isi_post_perc], axis=0)
...
def run(ax, subject_session_data):
    ...
    ax.plot(
        bin_isi_fix,
        bin_mean_fix,
        color='hotpink', marker='.', label='fix', markersize=4)
    ax.fill_between(
        bin_isi_fix,
    ...
    ax.plot(
        bin_isi_jitter,
        bin_mean_jitter,
        color='royalblue', marker='.', label='jitter', markersize=4)
    ax.fill_between(
        bin_isi_jitter,
    ...
    ax.set_xlabel('pre perturbation isi')
    ax.set_ylabel('prob. of choosing the right side (mean$\pm$sem)')
    ax.legend(loc='upper left', bbox_to_anchor=(1,1), ncol=1)
    ax.set_title('average psychometric function (percetual mean)')
