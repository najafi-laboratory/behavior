...
def make_block_count_lines_pdf(save_path, meta_all):
    ...
    with PdfPages(save_path) as pdf:
        pdf.savefig(fig)
    plt.close(fig)
    print(f"Saved: {save_path}  trans6_8RowSummary_V_3.py:323  08_trans6_8RowSummary_V_3.py:324 - 08_trans6_8RowSummary_V_4.py:346")
...
def per_trial_metrics(tr: dict, overall_max: float):
    ...
    # CR onset: first time (ms) FEC-baseline crosses threshold before puff
    cr_onset_t = np.nan
    if rel_t.size > 0 and np.isfinite(base):
        puff_rel = tr["AP_on"] - led
        search_end = puff_rel if np.isfinite(puff_rel) else 500.0
        fec_bl = fec - base
        mask = (rel_t >= 0.0) & (rel_t <= search_end)
        idx_cross = np.where(mask & (fec_bl >= CR_ONSET_THRESHOLD))[0]
        if idx_cross.size > 0:
            cr_onset_t = float(rel_t[idx_cross[0]])

    return amp_raw, amp_bs, vel_window, base, cr_onset_t, rel_t, fec
...
def make_block_count_lines_pdf(save_path, meta_all, zoom_only=True, zoom_window=3):
    ...
    print(f"Saved: {save_path}  trans6_8RowSummary_V_3.py:732  08_trans6_8RowSummary_V_3.py:744 - 08_trans6_8RowSummary_V_4.py:766")
...
def make_8row_pdf(path, x, chemo_s2l, chemo_l2s, control_s2l, control_l2s, tgrid, summary_text, meta_all):
    ...
    # Layout: rows 0-2 = Control traces, rows 3-5 = Chemo traces, rows 6-12 = superimposed metrics
    ...
    # Time-trace panels only:
    #   previous block = lighter color family
    #   new block      = darker color family
    #   early trials   = dashed
    #   late trials    = solid
    CTRL_STYLES = [
        ("#B8B8B8", (0, (4, 2)), 1.5),  # Prev early
        ("#6A6A6A", "-",          1.7),  # Prev late
        ("#4A4A4A", (0, (4, 2)), 1.9),  # New early
        ("#000000", "-",          2.1),  # New late
    ]
    CHEMO_STYLES = [
        ("#9ED0FF", (0, (4, 2)), 1.5),  # Prev early
        ("#4C9BFF", "-",          1.7),  # Prev late
        ("#1F66CC", (0, (4, 2)), 1.9),  # New early
        ("#003A8C", "-",          2.1),  # New late
    ]

    def draw_time_single(ax, d, title, mode, group, bl_sub=False):
        styles = CTRL_STYLES if group == "ctrl" else CHEMO_STYLES
        specs = [
            ("traces_neg1", "pos_neg1", "Prev block (early)"),
            ("traces_neg2", "pos_neg2", "Prev block (late)"),
            ("traces1",     "pos1",     "New block (early)"),
            ("traces2",     "pos2",     "New block (late)"),
        ]
        for (tk, pk, lbl), (col, ls, lw) in zip(specs, styles):
            tr = d.get(tk, [])
            pos = d.get(pk, [])
            if tr:
                m, s = mean_sem_trace(tr, tgrid, bl_sub)
                ax.fill_between(tgrid, m - s, m + s, color=col, alpha=0.14, lw=0)
                ax.plot(tgrid, m, color=col, linestyle=ls, lw=lw,
                        label=format_trial_position_label(lbl, pos))
        _shade_background(ax, mode)
        ax.axvline(0, color="k", ls="--")
    ...
    def draw_metric_dual(ax, stack_ctrl, stack_chemo, title, mode, do_fit=False):
        ax.axvline(0, color="k", ls="--")
        for stack, col, label in [(stack_ctrl, CONTROL_COLOR, "Control"), (stack_chemo, CHEMO_COLOR, "Chemo")]:
            if stack.size == 0:
                continue
            mu, se, cnt, frac = mean_sem_counts(stack)
            mask = frac >= MIN_FRAC
    ...
    def wv(d):
        return {
            ...
            "pos_neg1": d["pos_neg1"], "pos_neg2": d["pos_neg2"],
            "pos1": d["pos1"], "pos2": d["pos2"],
        }

    # ── Control Sessions (rows 0-2) ───────────────────────────
    draw_time_single(axs[0, 0], control_s2l, "S→L FEC Raw  [Control]",    "s2l", "ctrl")
    draw_time_single(axs[0, 1], control_l2s, "L→S FEC Raw  [Control]",    "l2s", "ctrl")
    draw_time_single(axs[1, 0], control_s2l, "S→L FEC BL-sub  [Control]", "s2l", "ctrl", True)
    draw_time_single(axs[1, 1], control_l2s, "L→S FEC BL-sub  [Control]", "l2s", "ctrl", True)
    draw_time_single(axs[2, 0], wv(control_s2l), "S→L Velocity  [Control]", "s2l", "ctrl")
    draw_time_single(axs[2, 1], wv(control_l2s), "L→S Velocity  [Control]", "l2s", "ctrl")

    # ── Chemo Sessions (rows 3-5) ─────────────────────────────
    draw_time_single(axs[3, 0], chemo_s2l, "S→L FEC Raw  [Chemo]",    "s2l", "chemo")
    draw_time_single(axs[3, 1], chemo_l2s, "L→S FEC Raw  [Chemo]",    "l2s", "chemo")
    draw_time_single(axs[4, 0], chemo_s2l, "S→L FEC BL-sub  [Chemo]", "s2l", "chemo", True)
    draw_time_single(axs[4, 1], chemo_l2s, "L→S FEC BL-sub  [Chemo]", "l2s", "chemo", True)
    draw_time_single(axs[5, 0], wv(chemo_s2l), "S→L Velocity  [Chemo]", "s2l", "chemo")
    draw_time_single(axs[5, 1], wv(chemo_l2s), "L→S Velocity  [Chemo]", "l2s", "chemo")

    # ── Superimposed metrics (rows 6-12) ──────────────────────
    ...
    draw_metric_dual(axs[9, 0],  control_s2l["cr_onset"], chemo_s2l["cr_onset"], "S→L CR Onset (ms)",    "s2l")
    draw_metric_dual(axs[9, 1],  control_l2s["cr_onset"], chemo_l2s["cr_onset"], "L→S CR Onset (ms)",    "l2s")
    draw_metric_dual(axs[10, 0], control_s2l["base"],     chemo_s2l["base"],     "S→L Baseline amp",     "s2l")
    draw_metric_dual(axs[10, 1], control_l2s["base"],     chemo_l2s["base"],     "L→S Baseline amp",     "l2s")

    draw_real_block_number_panel_dual(axs[11, 0], meta_all, "s2l", "S→L block # (ctrl=black, chemo=blue)")
    draw_real_block_number_panel_dual(axs[11, 1], meta_all, "l2s", "L→S block # (ctrl=black, chemo=blue)")
    ...
    with PdfPages(path) as pdf:
        pdf.savefig(fig)
    plt.close(fig)
    print(f"Saved: {path}  08_trans6_8RowSummary_V_3.py:864 - 08_trans6_8RowSummary_V_4.py:901")
...
def make_block_length_pairs_pdf(save_path, meta_all, summary_text=None):
    ...
    with PdfPages(save_path) as pdf:
        pdf.savefig(fig)
    plt.close(fig)
    print(f"Saved: {save_path}  trans6_8RowSummary_V_3.py:915  08_trans6_8RowSummary_V_3.py:979 - 08_trans6_8RowSummary_V_4.py:1016")
...
def draw_real_block_number_panel_dual(ax, meta_all, direction, title):
    ...
    chemo_rows = [r for r in rows if r.get("is_chemo", "Opto" in r.get("session_name", ""))]
    control_rows = [r for r in rows if not r.get("is_chemo", "Opto" in r.get("session_name", ""))]
...
def make_session_block_timeline_pdf(save_path, meta_all):
    ...
    if not rows:
        print(f"No session block data for {save_path}  trans6_8RowSummary_V_3.py:1067  08_trans6_8RowSummary_V_3.py:1235 - 08_trans6_8RowSummary_V_4.py:1272")
        return
    ...
    with PdfPages(save_path) as pdf:
        pdf.savefig(fig)
    plt.close(fig)
    print(f"Saved: {save_path}  trans6_8RowSummary_V_3.py:1098  08_trans6_8RowSummary_V_3.py:1266 - 08_trans6_8RowSummary_V_4.py:1303")
...
def make_block_vs_trial_mean_pdf(save_path, meta_all):
    ...
    if not all_traces:
        print(f"No data for {save_path}  trans6_8RowSummary_V_3.py:1119  08_trans6_8RowSummary_V_3.py:1287 - 08_trans6_8RowSummary_V_4.py:1324")
        return
    ...
    with PdfPages(save_path) as pdf:
        pdf.savefig(fig)
    plt.close(fig)
    print(f"Saved: {save_path}  trans6_8RowSummary_V_3.py:1146  08_trans6_8RowSummary_V_3.py:1314 - 08_trans6_8RowSummary_V_4.py:1351")
...
def collect_session(p):
    ...
    amps_raw, amps_bs, vels, baselines, cr_onsets, rel_ts, fecs = [], [], [], [], [], [], []
    blks = []

    for tr in trials:
        a, b, v, base, co, rt, y = per_trial_metrics(tr, overall)
    ...
    cr_onsets = np.asarray(cr_onsets, dtype=float)
    ...
    res = {
        "s2l": {"raw": [], "bs": [], "vel": [], "base": [], "cr_onset": [], "traces_neg1": [], "traces_neg2": [], "traces1": [], "traces2": [], "vtr_neg1": [], "vtr_neg2": [], "vtr1": [], "vtr2": [], "pos_neg1": [], "pos_neg2": [], "pos1": [], "pos2": []},
        "l2s": {"raw": [], "bs": [], "vel": [], "base": [], "cr_onset": [], "traces_neg1": [], "traces_neg2": [], "traces1": [], "traces2": [], "vtr_neg1": [], "vtr_neg2": [], "vtr1": [], "vtr2": [], "pos_neg1": [], "pos_neg2": [], "pos1": [], "pos2": []},
    }
...
def run():
    ...
    def finalize(d):
        out = {}
        for k in ["raw", "bs", "vel", "base", "cr_onset"]:
    ...
    for mouse, root in MOUSE_ROOTS.items():
        print(f"Processing {mouse}...  trans6_8RowSummary_V_3.py:1379  08_trans6_8RowSummary_V_3.py:1561 - 08_trans6_8RowSummary_V_4.py:1598")
        ...
        for p in files:
            ...
            try:
                ...
                print(f"Skipped {p.name}: {e}  trans6_8RowSummary_V_3.py:1413  08_trans6_8RowSummary_V_3.py:1594 - 08_trans6_8RowSummary_V_4.py:1631")
    ...
    print("Processing pooled...  trans6_8RowSummary_V_3.py:1456  08_trans6_8RowSummary_V_3.py:1647 - 08_trans6_8RowSummary_V_4.py:1684")
