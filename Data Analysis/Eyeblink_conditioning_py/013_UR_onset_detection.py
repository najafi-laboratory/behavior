...
def find_ur_onset_velocity_based(
    ...
    # Forward scan up to the peak: first sustained velocity run above threshold
    ...
    onset_idx = None
    run_start = None
    for i in range(len(pre_peak_vel)):
        if (np.isfinite(pre_peak_vel[i]) and pre_peak_vel[i] >= velocity_threshold
                and np.isfinite(pre_peak_sig[i]) and pre_peak_sig[i] >= min_signal_rise):
...
def detect_ur_onset_on_mean_trace(
    t_grid,
    mean_trace,
    t_puff,
    smooth_window=3,
    ...
    search_post_puff_s=0.150,
    baseline_window=(-0.2, 0.0),
    pre_puff_ref_s=0.050,
):
    """Velocity-based UR onset on the noise-averaged mean trace.

    Uses the 50 ms immediately before the puff as the velocity reference window
    (not the pre-LED baseline). This accounts for any ongoing CR-driven velocity
    so that onset is detected when post-puff velocity first EXCEEDS the pre-puff
    velocity level — the true UR signature.
    """
    t   = np.asarray(t_grid,     dtype=float)
    sig = np.asarray(mean_trace, dtype=float)

    # Baseline-subtract using pre-LED window
    bl_idx = (t >= baseline_window[0]) & (t <= baseline_window[1])
    baseline_amp = np.nanmean(sig[bl_idx])
    sig_bs = sig - baseline_amp

    sig_smooth = uniform_filter1d(sig_bs, size=smooth_window, mode="nearest")
    velocity   = np.gradient(sig_smooth, t)

    # Velocity threshold from the pre-puff reference window (accounts for CR)
    pre_puff_idx = (t >= t_puff - pre_puff_ref_s) & (t < t_puff)
    if np.any(pre_puff_idx):
        vel_ref = velocity[pre_puff_idx]
        vel_ref = vel_ref[np.isfinite(vel_ref)]
    else:
        vel_ref = np.array([])

    if vel_ref.size > 1:
        vel_threshold = np.nanmean(vel_ref) + vel_std_factor * np.nanstd(vel_ref)
    else:
        # fallback: use pre-LED baseline
        vel_base = velocity[bl_idx]
        vel_base = vel_base[np.isfinite(vel_base)]
        vel_threshold = (np.nanmean(vel_base) + vel_std_factor * np.nanstd(vel_base)
                         if vel_base.size > 1 else 0.0)
    vel_threshold = max(vel_threshold, vel_floor)

    # FEC level at puff onset — UR must rise above this
    puff_ref_idx = np.argmin(np.abs(t - t_puff))
    sig_at_puff  = float(sig_smooth[puff_ref_idx])

    # Search window: just after puff
    search_idx = (t >= t_puff + min_ur_latency_s) & (t <= t_puff + search_post_puff_s)
    if not np.any(search_idx):
        return np.nan
    t_s   = t[search_idx]
    vel_s = velocity[search_idx]
    sig_s = sig_smooth[search_idx]

    if np.nanmax(sig_s) <= sig_at_puff:
        return np.nan  # FEC never rises above puff level → no detectable UR

    onset_idx = None
    run_start = None
    for i in range(len(vel_s)):
        # Require velocity above pre-puff reference AND signal rising above puff level
        if vel_s[i] >= vel_threshold and sig_s[i] > sig_at_puff:
            if run_start is None:
                run_start = i
...
def plot_ur_onset_distribution(records, first_date, last_date, first_tok, last_tok):
    ...
    print(f"Saved {out_name} - 013_UR_onset_detection.py:263")
    plt.close(fig)
...
def plot_ur_average_traces(records, first_tok, last_tok):
    ...
    def passes_plot_baseline(record):
        baseline = record.get("baseline_amp", np.nan)
        return np.isfinite(baseline) and baseline <= PLOT_BASELINE_MAX_FEC

    groups = [
        ("Control", [r for r in records if not r.get("is_chemo", False) and passes_plot_baseline(r)]),
        ("Chemo", [r for r in records if r.get("is_chemo", False) and passes_plot_baseline(r)]),
    ]

    for row, (group_label, group_records) in enumerate(groups):
        for col, block_label in enumerate(["short", "long"]):
            ...
            all_block = [r for r in group_records if r["block_label"] == block_label]
            if all_block:
                t_grid_ref = all_block[0]["t_grid"]
                mean_all = np.nanmean(np.vstack([r["Fq"] for r in all_block]), axis=0)
                ax.plot(t_grid_ref, mean_all, color="royalblue", linewidth=2.8,
                        label=f"Grand mean — all trials (n={len(all_block)})", zorder=5)

                ur_onset_abs = detect_ur_onset_on_mean_trace(
                    t_grid_ref, mean_all, puff_x,
                    smooth_window=3, vel_std_factor=2.0, vel_floor=0.01,
                    min_sustain=2, min_ur_latency_s=0.005, search_post_puff_s=0.150,
                )
                if np.isfinite(ur_onset_abs):
                    ur_lat_ms = (ur_onset_abs - puff_x) * 1000
                    print(f"  [{group_label} | {block_label}] UR onset on grand mean: {ur_lat_ms:.1f} ms post-puff")
                    ax.axvline(ur_onset_abs, color="red", linestyle="--", linewidth=1.8,
                               label=f"UR onset  {ur_lat_ms:.0f} ms post-puff", zorder=6)
                    ax.axvline(ur_onset_abs - CR_BUFFER_S, color="green", linestyle=":",
                               linewidth=1.5,
                               label=f"CR window end  (-{CR_BUFFER_S*1000:.0f} ms buffer)",
                               zorder=6)
                else:
                    print(f"  [{group_label} | {block_label}] UR onset on grand mean: NOT DETECTED")
            else:
            ...
            for category, color in cat_colors.items():
                subset = [
                    ...
                    if r["block_label"] == block_label and r["category"] == category
                ]
                if not subset:
                    continue
                all_traces = np.vstack([r["Fq"] for r in subset])
                mean_trace = np.nanmean(all_traces, axis=0)
                ax.plot(subset[0]["t_grid"], mean_trace, color=color, linewidth=1.4,
                        alpha=0.75, label=f"{category} (n={len(subset)})", zorder=2)

            ax.set_title(f"{group_label} — {block_label.title()} block avg FEC by category")
            ax.set_xlim(-0.1, 0.65)
            ax.set_ylim(-0.05, 1.05)
            ax.set_xlabel("Time from LED onset (s)")
            ax.set_ylabel("FEC eyelid closure")
            ax.legend(loc="upper left", fontsize=7, ncol=1)
            ax.spines["top"].set_visible(False)
            ax.spines["right"].set_visible(False)

    fig.suptitle(
        f"Exact FEC eyelid closure traces with detected UR onset — Control vs Chemo\n"
        f"Trials with pre-LED baseline FEC > {PLOT_BASELINE_MAX_FEC:.2f} excluded",
        fontsize=13,
    )
    fig.tight_layout(rect=[0, 0, 1, 0.95])
    out_name = f"UR_onset_average_traces_{first_tok}_to_{last_tok}.pdf"
    fig.savefig(out_name, bbox_inches="tight")
    print(f"Saved {out_name} - 013_UR_onset_detection.py:314")
    plt.close(fig)
...
def plot_ur_summary_table(records, first_date, last_date, first_tok, last_tok):
    ...
    fig.savefig(out_name, bbox_inches="tight")
    print(f"Saved {out_name} - 013_UR_onset_detection.py:366")
    plt.close(fig)
...
def write_session_pdf(filepath, records):
    ...
    print(f"Saved {out_name}  ({len(records)} trials, UR detected in {n_ur}) - 013_UR_onset_detection.py:501")
...
def main():
    ...
    if not data_files:
        print("No *_EBC_*.mat files found in current folder. - 013_UR_onset_detection.py:510")
        return

    first_tok, last_tok, first_date, last_date = build_session_range(data_files)
    print(f"Processing {len(data_files)} session(s)  [{first_date}  {last_date}] - 013_UR_onset_detection.py:514")

    records = prepare_ur_records(data_files, onset_module)
    if not records:
        print("No usable trials found. - 013_UR_onset_detection.py:518")
        return

    n_total = len(records)
    n_ur = sum(1 for r in records if np.isfinite(r.get("ur_onset_latency", np.nan)))
    print(f"Trials: {n_total}  |  UR detected: {n_ur}  ({100 * n_ur / n_total:.1f}%) - 013_UR_onset_detection.py:523")
