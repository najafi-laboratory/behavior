...
def plot_curves(ax, subject, jitter_session, dates, decision, isi_post_emp):
    decision = np.concatenate([decision, isi_post_emp], axis=0)
    ...
    ax.set_title('psychometric function (empirical mean)')
...
def run(axs, subject_session_data):
    ...
    for i in range(len(dates)):
        plot_curves(
            ...
            jitter_session[i], dates[i], decision[i], isi_post_emp[i])
