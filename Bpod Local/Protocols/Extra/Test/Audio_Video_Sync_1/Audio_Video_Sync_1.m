...
InitCueSound = GenerateSineWave(SF, S.GUI.InitCueFreq_Hz, S.GUI.InitCueDuration_s)*S.GUI.MasterVolume_percent*S.GUI.InitCueVolume_percent; % Sampling freq (hz), Sine frequency (hz), duration (s)
InitCueSound = ApplySoundEnvelope(InitCueSound, Envelope);

GoCueSound = GenerateSineWave(SF, S.GUI.GoCueFreq_Hz, S.GUI.GoCueDuration_s)*S.GUI.MasterVolume_percent*S.GUI.GoCueVolume_percent; % Sampling freq (hz), Sine frequency (hz), duration (s)
GoCueSound = ApplySoundEnvelope(GoCueSound, Envelope);

IncorrectSound = GenerateWhiteNoise(SF, S.GUI.PunishSoundDuration_s, 1, 1)*S.GUI.MasterVolume_percent*S.GUI.IncorrectSoundVolume_percent; % white noise punish sound
IncorrectSound = ApplySoundEnvelope(IncorrectSound, Envelope);

% generate audio stim same duration as vis gratings
AudioStimSound = GenerateSineWave(SF, S.GUI.AudioStimFreq_Hz, S.GUI.GratingDur_s)*S.GUI.MasterVolume_percent*S.GUI.AudioStimVolume_percent; % Sampling freq (hz), Sine frequency (hz), duration (s)
...
for currentTrial = 1:MaxTrials
    ...
    if (S.GUI.GratingDur_s ~= LastGratingDuration) || ...
        ...
        % generate audio stim of 15s (longer than vis stim to go cue would
        % be) then turn sound off at beginning of next state after vis stim
        AudioStimSound = GenerateSineWave(SF, S.GUI.AudioStimFreq_Hz, GratingDur)*S.GUI.MasterVolume_percent*S.GUI.AudioStimVolume_percent; % Sampling freq (hz), Sine frequency (hz), duration (s) 
        AudioStimSound = ApplySoundEnvelope(AudioStimSound, Envelope);
    ...
    %     InitCueSound = GenerateSineWave(SF, S.GUI.InitCueFreq_Hz, S.GUI.InitCueDuration_s)*S.GUI.MasterVolume_percent; % Sampling freq (hz), Sine frequency (hz), duration (s)        
    %     H.load(1, InitCueSound);
    %     LastInitCueDuration = S.GUI.InitCueDuration_s;
    % end
    % if S.GUI.GoCueDuration_s ~= LastGoCueDuration        
    %     GoCueSound = GenerateSineWave(SF, S.GUI.GoCueFreq_Hz, S.GUI.GoCueDuration_s)*S.GUI.MasterVolume_percent; % Sampling freq (hz), Sine frequency (hz), duration (s)
    %     H.load(2, GoCueSound);
    %     LastGoCueDuration = S.GUI.GoCueDuration_s;
    % end
    if (S.GUI.PunishSoundDuration_s ~= LastPunishSoundDuration) || ...
        (S.GUI.IncorrectSoundVolume_percent ~= LastIncorrectSoundVolume)
        IncorrectSound = GenerateWhiteNoise(SF, S.GUI.PunishSoundDuration_s, 1, 2)*S.GUI.MasterVolume_percent*S.GUI.IncorrectSoundVolume_percent; % white noise punish sound
        IncorrectSound = ApplySoundEnvelope(IncorrectSound, Envelope);
        H.load(3, IncorrectSound);
        LastPunishSoundDuration = S.GUI.PunishSoundDuration_s;
        LastIncorrectSoundVolume = S.GUI.IncorrectSoundVolume_percent;
