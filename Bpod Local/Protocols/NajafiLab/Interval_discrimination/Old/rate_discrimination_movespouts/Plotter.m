...
function UpdateOutcomePlot(obj, BpodSystem, TrialTypes, isEndOfTrial)
    ...
    if isfield(Data, 'nTrials')
        ...
        for x = 1:Data.nTrials
            ...
            elseif ( isfield(Data.RawEvents.Trial{x}.States, 'HabituationReward') && ...
                    ~isnan(Data.RawEvents.Trial{x}.States.HabituationReward(1)))
                if (x == Data.nTrials && isEndOfTrial)  % only print outcome to console for the trial that just occured
                    disp('Outcome: HabituationReward');
                end
                Outcomes(x) = 1;    % draws green circle on outcome plot
            ...
            elseif ( isfield(Data.RawEvents.Trial{x}.States, 'ExtraStimDurPostRew_Naive') && ...
                    ~isnan(Data.RawEvents.Trial{x}.States.ExtraStimDurPostRew_Naive(1)))
                if (x == Data.nTrials && isEndOfTrial)  % only print outcome to console for the trial that just occured
            ...
            elseif ( isfield(Data.RawEvents.Trial{x}.States, 'DidNotChoose') && ...
                ...
                Outcomes(x) = 3;    % draws clear circle on outcome plot
            elseif ( isfield(Data.RawEvents.Trial{x}.States, 'DidNotConfirm') && ...
                    ~isnan(Data.RawEvents.Trial{x}.States.DidNotConfirm(1)))
                if (x == Data.nTrials && isEndOfTrial)  % only print outcome to console for the trial that just occured
