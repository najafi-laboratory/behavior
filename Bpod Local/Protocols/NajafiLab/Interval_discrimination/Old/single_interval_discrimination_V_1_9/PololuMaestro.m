...
classdef PololuMaestro < handle
    ...
    methods
        ...
        function [position] = getPosition(obj,MotorID, varargin)
            Output = [144 MotorID];
            obj.Port.write(Output, 'uint8');
            position = obj.Port.read(2, 'uint8');
            % [LowBits, HighBits]
            LowBits = position(1);
            HighBits = position(2);
            % Combine the upper and lower byte into a uint16
            combinedValue = uint16(HighBits) * 256 + uint16(LowBits);
            position = combinedValue / 4;
            % disp(position)
        end
        function [isAtPosition] = checkPosition(obj, MotorID, position, tolerance, varargin)
            isAtPosition = false;
            UpperPos = position + tolerance; 
            LowerPos = position - tolerance;            
            currPosition = obj.getPosition(MotorID);
            if (currPosition > LowerPos) && (currPosition < UpperPos);
                isAtPosition = true;
            end
        end
        function delete(obj)
