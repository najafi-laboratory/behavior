...
function [TrialTypes, BlockTypes] = GenTrials(obj, S)
    bt = zeros(1,1106);
    bt(1:S.GUI.NumBlockWarmup) = 0;
    if S.GUI.StartBlockType == 0
        bt(S.GUI.NumBlockWarmup+1) = randi([1,2]);
    else
        bt(S.GUI.NumBlockWarmup+1) = S.GUI.StartBlockType;
    end
    for i = S.GUI.NumBlockWarmup+2:1106
        if S.GUI.NumBlockType == 2
            c = setdiff([1,2], [bt(i-1)]);
        else
            c = setdiff([0,1,2], [bt(i-1)]);
        end
        bt(i) = c(randi(numel(c)));
    end
    % sample block lengths
    block_lengths = randi([S.GUI.BlockLength-S.GUI.BlockMargin, ...
                           S.GUI.BlockLength+S.GUI.BlockMargin], ...
                           1, numel(bt));
    % allocate trials dynamically
    TrialTypes = [];
    BlockTypes = [];
    p = [0.5, S.GUI.BlockSidePrior, 1-S.GUI.BlockSidePrior];
    for i = 1:numel(bt)
        L = block_lengths(i);
        prob = p(bt(i)+1);
        trial_vec = 1 + (rand(1,L) >= prob);
        if bt(i) > 0
            k = min(S.GUI.NumBlockChangeTrial,L);
            trial_vec(1:k) = bt(i);
            trial_vec(k+1:end) = trial_vec(k + randperm(L-k));
        end
        TrialTypes = [TrialTypes trial_vec];
        BlockTypes = [BlockTypes repmat(bt(i),1,L)];
    end
    % Warmup override
    warmup_len = sum(block_lengths(1:S.GUI.NumBlockWarmup));
    TrialTypes(1:warmup_len) = ceil(rand(1,warmup_len)*2);
end
        ...
        case 1 % normal contingency, short left
            switch TrialTypes(currentTrial)
                case 1 % trial is left with short ISI
                    ...
                    PostPertISI = DrawFromDiscrete(obj, S.GUI.ISIShortMin_s, S.GUI.ISIShortMax_s, 5);
                case 2 % trial is right with long ISI
                    PrePertISI = DrawFromUniform(obj, S.GUI.ISIShortMin_s, S.GUI.ISIShortMax_s);
                    PostPertISI = DrawFromDiscrete(obj, S.GUI.ISILongMin_s, S.GUI.ISILongMax_s, 5);
            end
        case 2 % reverse contingency, long left
            switch TrialTypes(currentTrial)
                case 1 % trial is right with short ISI
                    PrePertISI = DrawFromUniform(obj, S.GUI.ISIShortMin_s, S.GUI.ISIShortMax_s);
                    PostPertISI = DrawFromDiscrete(obj, S.GUI.ISILongMin_s, S.GUI.ISILongMax_s, 5);
                case 2 % trial is left with long ISI
                    PrePertISI = DrawFromUniform(obj, S.GUI.ISILongMin_s, S.GUI.ISILongMax_s);
                    PostPertISI = DrawFromDiscrete(obj, S.GUI.ISIShortMin_s, S.GUI.ISIShortMax_s, 5);
            end
    end
...
function [isi] = DrawFromDiscrete(obj, min, max, n)
        % Generate 5 evenly spaced values including min and max
        values = linspace(min, max, n);
