...
switch action
    ...
    case 'display'
        opto.Labels = {'Off', 'Stimulus', 'SpoutInDelay', 'SpoutIn', 'Choice', 'PreOutcome', 'Reward', 'PostReward', 'PunishITI'};
        opto.Colors = [0.70 0.70 0.70; 0.49 0.18 0.56; 0.95 0.45 0.20; 0.93 0.69 0.13; 0.54 0.40 0.78; 0.30 0.65 0.88; 0.20 0.50 0.88; 0.18 0.60 0.42; 0.84 0.27 0.22];
    otherwise
...
enabled = false(8, 1);
enabled(1:min(8, numel(optoType))) = optoType(1:min(8, numel(optoType))) ~= 0;
...
if enabled(4)
    opto.Timers(4) = gateSpec(timerIDs(4), S.GUI.PostLickDelay_s);
    opto.ChoiceStart = {'GlobalTimerTrig', timerIDs(4)};
end
if enabled(5)
    opto.Timers(5) = gateSpec(timerIDs(5), S.GUI.PreOutcomeDelay_s);
    opto.PreOutcomeStart = {'GlobalTimerTrig', timerIDs(5)};
end
if enabled(6)
    opto.Timers(6) = gateSpec(timerIDs(6), rewardValve_s);
    opto.RewardStart = {'GlobalTimerTrig', timerIDs(6)};
end
if enabled(7)
    opto.Timers(7) = gateSpec(timerIDs(7), S.GUI.PostRewardDelay_s);
    opto.PostRewardStart = {'GlobalTimerTrig', timerIDs(7)};
end
if enabled(8)
    opto.Timers(8) = gateSpec(timerIDs(8), punishITI_s);
    opto.PunishStart = {'GlobalTimerTrig', timerIDs(8)};
end
end
