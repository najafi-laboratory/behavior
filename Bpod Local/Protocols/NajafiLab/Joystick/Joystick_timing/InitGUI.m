...
function [S] = SetParams(obj, BpodSystem)
    ...
    if isempty(fieldnames(S))  % If settings file was an empty struct, populate struct with default settings
        ...
        S.GUI.Threshold = 10; % Threshold for completion of a lever press, units = degrees of shaft rotation        
        S.GUI.PressWindow_s = 1.0; % how long mouse has to press lever
        S.GUI.PressWindow_s2 = 1.0
        S.GUI.Reps = 1.0;   % number of required repeated lever presses in a trial
        S.GUIPanels.Joystick = {'Threshold', 'PressWindow_s', 'Reps'};
