            ...
            switch obj.EnableOpto
                ...
                case 1
                    if OptoTrialType == 2
                        switch Vis1Vis2
                            case 1
                                ...
                                elseif S.GUI.OptoVis1 && S.GUI.OptoWaitForPress1
                                    ...
                                    % AudStimOpto = [AudStimOpto , {'GlobalTimerTrig', '000100001'}];
                                    AudStimOpto = [AudStimOpto , {'GlobalTimerTrig', '000010001'}];
                                else
                                    % AudStimOpto = {'HiFi1', ['P', 6]};
                                end
                            case 2
                                ...
                                elseif S.GUI.OptoVis2 && S.GUI.OptoWaitForPress2
                                    ...
                                    AudStimOpto = [AudStimOpto , {'GlobalTimerTrig', '010000100'}];
                                else
                                    % AudStimOpto = {'HiFi1', ['P', 6]};
                                end
                        end
                        ...
                        % AudStimOpto = {'HiFi1', ['P', 6]};
                        % [WaitForPress2_OutputActions, {'GlobalTimerCancel', '001000100'}];
                    end
            end
        end
            ...
            if obj.EnableOpto 
                ...
                switch S.GUI.PulseType
                    case 1
                        ...
                        LEDWaitForPressOnDur = 30;
                        % WaitForPressLoopLED = 0;
                        

                        LEDOnsetDelay = PMTCloseDelay;

                        LEDWaitForPressOnsetDelay = 0;
                        PMTWaitForPressOnsetDelay = 0;
                    case 2
                        T = 1/S.GUI.PulseFreq_Hz;
                        OnDur = S.GUI.PulseOnDur_ms/1000;
                        LEDOffDur = abs(OnDur - T);

                        LoopLED = floor((VisStim.VisStimDuration - PMTCloseDelay)/(OnDur + LEDOffDur));

                        LEDWaitForPressOnDur = OnDur;
                    ...
                    case 3
                        ...
                        LEDWaitForPressOnDur = OnDur;
                        PMTWaitForPressDur = PMTCloseDur;

                        LEDOnsetDelay = PMTCloseDelay;

                        ContiguousSegmentDelayFactor = 0.05072;
                        LEDWaitForPressOnsetDelay = ContiguousSegmentDelayFactor + PMTCloseDelay;
                        PMTWaitForPressOnsetDelay = ContiguousSegmentDelayFactor;
                end 
                ...
                if ~S.GUI.OptoVis1 && S.GUI.OptoWaitForPress1
                    ...
                    % by vis1 LED/shutter
                    sma = SetGlobalTimer(sma, 'TimerID', 2, 'Duration', LEDWaitForPressOnDur, 'OnsetDelay', PMTCloseDelay,...
                        'Channel', 'PWM1', 'OnLevel', 255, 'OffLevel', 0,...
                        'Loop', LoopLED, 'SendGlobalTimerEvents', 0, 'LoopInterval', LEDOffDur,...
                        'GlobalTimerEvents', 0, 'OffsetValue', 0);                                       
                    ...
                    % wait1 shutter
                    sma = SetGlobalTimer(sma, 'TimerID', 6, 'Duration', PMTWaitForPressDur, 'OnsetDelay', 0,...
                        'Channel', 'BNC2', 'OnLevel', 1, 'OffLevel', 0,...
                        'Loop', LoopPMT, 'SendGlobalTimerEvents', 0, 'LoopInterval', PMTOffDur,...
                        'GlobalTimerEvents', 0, 'OffsetValue', 0);
                end
                ...
                if S.GUI.OptoVis1 && S.GUI.OptoWaitForPress1
                    ...
                    sma = SetGlobalTimer(sma, 'TimerID', 1, 'Duration', OnDur, 'OnsetDelay', PMTCloseDelay,...
                        ...
                        'GlobalTimerEvents', 0, 'OffsetValue', 0);
                    % vis1 shutter
                    % sma = SetGlobalTimer(sma, 'TimerID', 5, 'Duration', PMTCloseDur + PMTStartOpenDelay, 'OnsetDelay', 0,...
                    %     'Channel', 'BNC2', 'OnLevel', 1, 'OffLevel', 0,...
                    %     'Loop', 0, 'SendGlobalTimerEvents', 0, 'LoopInterval', 0,...
                    %     'GlobalTimerEvents', 0, 'OffsetValue', 0);
                    ...
                    % wait1 led
                    sma = SetGlobalTimer(sma, 'TimerID', 2, 'Duration', LEDWaitForPressOnDur, 'OnsetDelay', LEDWaitForPressOnsetDelay,...
                        'Channel', 'PWM1', 'OnLevel', 255, 'OffLevel', 0,...
                        'Loop', LoopLED, 'SendGlobalTimerEvents', 0, 'LoopInterval', LEDOffDur,...
                        'GlobalTimerEvents', 0, 'OffsetValue', 0);      

                    % wait1 shutter
                    sma = SetGlobalTimer(sma, 'TimerID', 6, 'Duration', PMTWaitForPressDur, 'OnsetDelay', PMTWaitForPressOnsetDelay,...
                        'Channel', 'BNC2', 'OnLevel', 1, 'OffLevel', 0,...
                        'Loop', LoopPMT, 'SendGlobalTimerEvents', 0, 'LoopInterval', PMTOffDur,...
                        'GlobalTimerEvents', 0, 'OffsetValue', 0);
                end
                ...
                if ~S.GUI.OptoVis2 && S.GUI.OptoWaitForPress2
                    %needs onset delay since not preceded
                    % by vis1 LED/shutter
                    sma = SetGlobalTimer(sma, 'TimerID', 4, 'Duration', LEDWaitForPressOnDur, 'OnsetDelay', PMTCloseDelay,...
                        'Channel', 'PWM1', 'OnLevel', 255, 'OffLevel', 0,...
                        'Loop', LoopLED, 'SendGlobalTimerEvents', 0, 'LoopInterval', LEDOffDur,...
                        'GlobalTimerEvents', 0, 'OffsetValue', 0); 
                    
                    % wait2 shutter
                    sma = SetGlobalTimer(sma, 'TimerID', 8, 'Duration', PMTWaitForPressDur, 'OnsetDelay', 0,...
                        'Channel', 'BNC2', 'OnLevel', 1, 'OffLevel', 0,...
                        'Loop', LoopPMT, 'SendGlobalTimerEvents', 0, 'LoopInterval', PMTOffDur,...
                        'GlobalTimerEvents', 0, 'OffsetValue', 0);
                end
                ...
                if S.GUI.OptoVis2 && S.GUI.OptoWaitForPress2
                    ...
                    % wait2 led
                    sma = SetGlobalTimer(sma, 'TimerID', 4, 'Duration', LEDWaitForPressOnDur, 'OnsetDelay', LEDWaitForPressOnsetDelay,...
                        'Channel', 'PWM1', 'OnLevel', 255, 'OffLevel', 0,...
                        'Loop', LoopLED, 'SendGlobalTimerEvents', 0, 'LoopInterval', LEDOffDur,...
                        'GlobalTimerEvents', 0, 'OffsetValue', 0);

                    % wait2 shutter
                    sma = SetGlobalTimer(sma, 'TimerID', 8, 'Duration', PMTWaitForPressDur, 'OnsetDelay', PMTWaitForPressOnsetDelay,...
                        'Channel', 'BNC2', 'OnLevel', 1, 'OffLevel', 0,...
                        'Loop', LoopPMT, 'SendGlobalTimerEvents', 0, 'LoopInterval', PMTOffDur,...
                        'GlobalTimerEvents', 0, 'OffsetValue', 0);                    
                end                
