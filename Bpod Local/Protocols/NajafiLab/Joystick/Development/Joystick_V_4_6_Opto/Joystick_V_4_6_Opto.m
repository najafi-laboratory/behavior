...
try
    ...
    if debug
        ...
        S.GUI.DoricSequence = 1;

        S.GUI.ContinuousCurrent = 30;
        S.GUI.ContinuousTimeOnMs = 3000;

        S.GUI.PulseCurrent = 30;
        S.GUI.PulseNbOfSeq = 1;
        S.GUI.PulseNbOfPulsesPerSeq = 100;
        S.GUI.PulsePeriodMs = 10;
        S.GUI.PulseTimeOnMs = 5;

        S.GUI.RampDownCurrent = 30;
        S.GUI.RampDownNbOfSeq = 1;
        S.GUI.RampDownNbOfPulsesPerSeq = 1;
        S.GUI.RampDownPeriodMs = 1000;
        S.GUI.RampDownTimeOnMs = 1000;

        S.GUI.RampUpCurrent = 30;
        S.GUI.RampUpNbOfSeq = 1;
        S.GUI.RampUpNbOfPulsesPerSeq = 1;
        S.GUI.RampUpPeriodMs = 3000;
        S.GUI.RampUpTimeOnMs = 3000;        

        S = BpodParameterGUI('sync', S);
    end    
     
    % update gui positions
    set(BpodSystem.ProtocolFigures.ParameterGUI, 'Position', [-325 53 2236 936]);
    ...
    % init Doric laser
    m_Opto = m_Opto.ConnectInitLaser(S);
    ...
    ExpNotes.ProtoVersion = 'Joystick_V_4_6_Opto'; % update to use current file name
    ...
    for currentTrial = 1:MaxTrials
            ...
            if (StartOfBlock(currentTrial)&&(currentTrial>1))
                ...
                elseif (sum(TempOutcome) >= 5)&&(LenBlock<MaxNumTrialsPerBlock)
                    ...
                    if (S.GUI.OptoTrialTypeSeq == 6)&&(m_Opto.EnableOpto)
                        if (OptoBlockTag > 1)
                            OptoTrailEpoch = randperm(int8(S.GUI.EpochTrialStop*S.GUI.OnFraction) , S.GUI.EpochTrialStop)+currentTrial ;
        ...
        if BpodSystem.Status.BeingUsed == 0 % If protocol was stopped, exit the loop
            ...
            % disconnect Doric laser
            m_Opto = m_Opto.DisconnectLaser(S);            
            return
        end
