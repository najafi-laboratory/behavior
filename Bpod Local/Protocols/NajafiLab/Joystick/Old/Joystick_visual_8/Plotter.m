...
function UpdateOutcomePlot(obj, BpodSystem, TrialTypes, isEndOfTrial)
    ...
    if isfield(Data, 'nTrials')
        ...
        for x = 1:Data.nTrials
            ...
            elseif ( isfield(Data.RawEvents.Trial{x}.States, 'DidNotPress1') && ...
                    ~isnan(Data.RawEvents.Trial{x}.States.DidNotPress1(1)))
                if (x == Data.nTrials && isEndOfTrial)  % only print outcome to console for the trial that just occured
                    disp('Outcome: DidNotPress1');
                end
                Outcomes(x) = 0;    % draws red circle on outcome plot
            elseif ( isfield(Data.RawEvents.Trial{x}.States, 'Reward2') && ...
                ~isnan(Data.RawEvents.Trial{x}.States.Reward2(1)) && (Data.TrialSettings(x).GUI.Reps == 2))
                if (x == Data.nTrials && isEndOfTrial)  % only print outcome to console for the trial that just occured
                    disp('Outcome: Reward2');
                end
                Outcomes(x) = 1;    % draws green circle on outcome plot
            elseif ( isfield(Data.RawEvents.Trial{x}.States, 'DidNotPress2') && ...
                    ~isnan(Data.RawEvents.Trial{x}.States.DidNotPress2(1)))
                if (x == Data.nTrials && isEndOfTrial)  % only print outcome to console for the trial that just occured
                    disp('Outcome: DidNotPress2');
                end
                Outcomes(x) = 0;    % draws red circle on outcome plot
            elseif ( isfield(Data.RawEvents.Trial{x}.States, 'Reward3') && ...
                ~isnan(Data.RawEvents.Trial{x}.States.Reward3(1)) && (Data.TrialSettings(x).GUI.Reps == 3))
                if (x == Data.nTrials && isEndOfTrial)  % only print outcome to console for the trial that just occured
                    disp('Outcome: Reward3');
                end
                Outcomes(x) = 1;    % draws green circle on outcome plot  
            elseif ( isfield(Data.RawEvents.Trial{x}.States, 'DidNotPress3') && ...
                    ~isnan(Data.RawEvents.Trial{x}.States.DidNotPress3(1)))
                if (x == Data.nTrials && isEndOfTrial)  % only print outcome to console for the trial that just occured
                    disp('Outcome: DidNotPress3');
                end
                Outcomes(x) = 0;    % draws red circle on outcome plot                
