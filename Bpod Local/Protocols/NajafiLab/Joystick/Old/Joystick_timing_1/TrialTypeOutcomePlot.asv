...
switch Action
    case 'init'
        ...
        % % joystick trial types
        MaxTrialType = 2;
        % 
        % if max(TrialTypeList == 1) && max(TrialTypeList == 2) % both trial types
        % 
        % elseif max(TrialTypeList == 1)
        %     YTick = [-1];
        %     YLim = [-1.5 -0.5];
        % else
        %     YTick = [-2];
    ...
    case 'update'
        ...
        set(AxesHandle,'YLim',YLim, 'YTick', YTick,'YTickLabel', BpodSystem.GUIHandles.TTOP_Ylabel);
        if CurrentTrial<1
            CurrentTrial = 1;
        end
