...
try
    ...
    pause(1.0); % matlab seems to require a pause here before clearing screen with play(0), 
                ...
                % might need longer delay if purple image hangs on Interval open
    ...
    for currentTrial = 1:MaxTrials
        ...
        switch TrialTypes(currentTrial)
            case 1
                PrePress2Delay_s = S.GUI.PressVisDelayShort_s;
                ExperimenterTrialInfo.TrialType = 'Short Pre Vis Delay';   % check variable states as field/value struct for experimenter info
            case 2
                PrePress2Delay_s = S.GUI.PressVisDelayLong_s;
                ExperimenterTrialInfo.TrialType = 'Long Pre Vis Delay';   % check variable states as field/value struct for experimenter info
        end

        ExperimenterTrialInfo.TrialType = 'Self Timed Interval';

        ExperimenterTrialInfo.PrePress2Delay_s = PrePress2Delay_s;   % check variable states as field/value struct for experimenter info        
        ...
        PrePress2Delay_StateChangeConditions = {};
        ...
        switch S.GUI.Reps
            ...
            case 2
                ...
                %     if PrePress2Delay_s == 0
                %         PrePress2Delay_StateChangeConditions = {'Tup', 'VisDetect2'};
                %     else
                %         PrePress2Delay_StateChangeConditions = {'RotaryEncoder1_2', 'EarlyPress', 'Tup', 'VisDetect2'};
                %     end                    
                % else
                    % LeverRetract1_StateChangeConditions = {'SoftCode1', 'WaitForPress2'};
                % if PrePress2Delay_s == 0
                    % PrePress2Delay_StateChangeConditions = {'Tup', 'WaitForPress2'};
                % else
        ...
        % For warmup trials, wait for press is extended by additional warmup param, after warmup wait for press is S.GUI.PressInterval_s
        ...
        PressInterval_s = S.GUI.PressInterval_s;
        if WarmupTrialsCounter > 0
            ...
            PressInterval_s = S.GUI.PressInterval_s + S.GUI.PressIntervalExtend_s;
        ...
        ExperimenterTrialInfo.PressInterval = PressInterval_s;
    ...
    %% reps complete
        ...
        if ~isempty(fieldnames(RawEvents)) % If trial data was returned (i.e. if not final trial, interrupted by user)
            ...
            BpodSystem.Data.EncoderData{currentTrial} = BpodSystem.PluginObjects.R.readUSBStream(); % Get rotary encoder data checkd since last call to R.readUSBStream()
