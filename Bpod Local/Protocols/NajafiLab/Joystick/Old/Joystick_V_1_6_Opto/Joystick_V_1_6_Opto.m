...
try
    ...
    %% Calibrate F2TTL
    % add some draw texture/flip with the bright/dark sync patches, then
    % draw background
    ...
    % ifi=Screen('GetFlipInterval', BpodSystem.PluginObjects.V.Window); % check via psychtoolbox
    % [resident texidresident] = Screen('PreloadTextures', BpodSystem.PluginObjects.V.Window);

    % PreviousEnableAutoDelay = 0;
    ...
    for currentTrial = 1:MaxTrials
        ...
        %% video for opto joystick stim
        % VideoOptoDelay = [GrayFrame_SyncW GrayFrame_SyncBlk GrayFrame_SyncBlk]; % 60 fps
        VideoOptoDelay = [GrayFrame_SyncW GrayFrame_SyncBlk]; % 60 fps, F1, F2

        % VideoOptoDelay = [GrayFrame_SyncW GrayFrame_SyncBlk GrayFrame_SyncBlk GrayFrame_SyncBlk GrayFrame_SyncBlk]; % 120 fps
        ...
        % load constructed video into the video object
        % BpodSystem.PluginObjects.V.Videos{7} = struct;
        % BpodSystem.PluginObjects.V.Videos{7}.nFrames = FullOptoVideoFrames; 
        % BpodSystem.PluginObjects.V.Videos{7}.Data = FullOptoVideo;

        BpodSystem.PluginObjects.V.Videos{5} = struct;
        BpodSystem.PluginObjects.V.Videos{5}.nFrames = FullOptoVideoFrames; 
        BpodSystem.PluginObjects.V.Videos{5}.Data = FullOptoVideo;
                    
        %% audio for opto joystick stim
        % generate audio stim same duration as vis gratings
        AudioStimSound = GenerateSineWave(SF, S.GUI.AudioStimFreq_Hz, S.GUI.GratingDur_s)*S.GUI.AudioStimVolume_percent; % Sampling freq (hz), Sine frequency (hz), duration (s)
        AudioStimSound = ApplySoundEnvelope(AudioStimSound, Envelope);
        ...
        VisDetectVariableOutputAction = {'RotaryEncoder1', ['E']};
        ...
        VisDetect1_StateChangeConditions = {'Tup', 'VisStimInterrupt', 'BNC1High', 'VisDetectVariable', 'RotaryEncoder1_2', 'EarlyPress1'};
        VisDetectVariable_StateChangeConditions = {'Tup', 'VisStimInterrupt', 'BNC1Low', 'VisualStimulus1', 'RotaryEncoder1_2', 'EarlyPress1'};
        ...
        sma = AddState(sma, 'Name', 'VisDetectVariable', ...
            'Timer', 0.100,...
            'StateChangeConditions', VisDetectVariable_StateChangeConditions,...
            'OutputActions', VisDetectVariableOutputAction);           

        sma = AddState(sma, 'Name', 'VisualStimulus1', ...
            'Timer', VisStim.VisStimOptoDuration,...
