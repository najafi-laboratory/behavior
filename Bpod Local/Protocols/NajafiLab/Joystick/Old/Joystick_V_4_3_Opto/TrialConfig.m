...
function [TrialTypes] = GenTrials(obj, S, MaxTrials, numTrialTypes, TrialTypes, currentTrial, updateTrialTypeSequence)
    ...
    NumWarmupTrials = S.GUI.NumEasyWarmupTrials;
    IsFirstBlock = 0;
    if NumWarmupTrials > 0
        IsFirstBlock = 1;
    end
    ...
    if S.GUI.EnableManualTrialType  
        ...
        switch S.GUI.ManualTrialType
            case 1
                TrialTypes(currentTrial:end) = [repmat(1, 1, MaxTrials - currentTrial + 1)];
            case 2
                TrialTypes(currentTrial:end) = [repmat(2, 1, MaxTrials - currentTrial + 1)];
        end                
    elseif updateTrialTypeSequence
        
        % block length - draw random integer from discrete uniform
        % distribution for each block length
        % {NumTrialsPerBlock - BlockLengthMargin, NumTrialsPerBlock + BlockLengthMargin}
        m = S.GUI.BlockLengthMargin;
        a = -m;
        b = m;
        N = MaxTrials;
        % if NumWarmupTrials > 0
        %     N = MaxTrials - 10;
        %     % N = MaxTrials - NumWarmupTrials;
        % end
        r = round(a + (b-a).*rand(N,1));
        NumTrialsPerBlock = S.GUI.NumTrialsPerBlock + r;
        NumTrialsPerBlockIdx = 1;
        switch S.GUI.TrialTypeSequence
            case 1 % interleaved - random - individual trials, block length = 1
                TrialTypes(currentTrial:end) = ceil(rand(1,MaxTrials - currentTrial + 1)*numTrialTypes);
            case 2 % interleaved - random first trial type                
                ...
                %     TrialTypesToAdd = [TrialTypesToAdd repmat(FirstTrialType, 1, NumTrialsPerBlock) repmat(SecondTrialType, 1, NumTrialsPerBlock)];
                %     numTrialsAddedToSequence = numTrialsAddedToSequence + 2*NumTrialsPerBlock; 
                % end
                % TrialTypes(currentTrial:end) = TrialTypesToAdd(1:length(TrialTypes) - currentTrial + 1);
                while numTrialsAddedToSequence < MaxTrials
                    TrialTypesToAdd = [TrialTypesToAdd repmat(FirstTrialType, 1, NumTrialsPerBlock(NumTrialsPerBlockIdx) + IsFirstBlock*NumWarmupTrials) repmat(SecondTrialType, 1, NumTrialsPerBlock(NumTrialsPerBlockIdx + 1))];                    
                    numTrialsAddedToSequence = numTrialsAddedToSequence + NumTrialsPerBlock(NumTrialsPerBlockIdx) + IsFirstBlock*NumWarmupTrials + NumTrialsPerBlock(NumTrialsPerBlockIdx + 1);
                    NumTrialsPerBlockIdx = NumTrialsPerBlockIdx + 2;
                    IsFirstBlock = 0;
                end
                TrialTypes(currentTrial:end) = TrialTypesToAdd(1:length(TrialTypes) - currentTrial + 1); 

            case 3 % interleaved - short first block, rcrsfty3rd
                % while numTrialsAddedToSequence < MaxTrials
                %     TrialTypesToAdd = [TrialTypesToAdd repmat(1, 1, NumTrialsPerBlock) repmat(2, 1, NumTrialsPerBlock)];
                %     numTrialsAddedToSequence = numTrialsAddedToSequence + 2*NumTrialsPerBlock; 
                % end
                % TrialTypes(currentTrial:end) = TrialTypesToAdd(1:length(TrialTypes) - currentTrial + 1);

                while numTrialsAddedToSequence < MaxTrials
                    TrialTypesToAdd = [TrialTypesToAdd repmat(1, 1, NumTrialsPerBlock(NumTrialsPerBlockIdx) + IsFirstBlock*NumWarmupTrials) repmat(2, 1, NumTrialsPerBlock(NumTrialsPerBlockIdx + 1))];
                    numTrialsAddedToSequence = numTrialsAddedToSequence + NumTrialsPerBlock(NumTrialsPerBlockIdx) + IsFirstBlock*NumWarmupTrials + NumTrialsPerBlock(NumTrialsPerBlockIdx + 1);
                    NumTrialsPerBlockIdx = NumTrialsPerBlockIdx + 2;
                    IsFirstBlock = 0;
                end
            ...
            case 4 % interleaved - long first block
                % while numTrialsAddedToSequence < MaxTrials
                %     TrialTypesToAdd = [TrialTypesToAdd repmat(2, 1, NumTrialsPerBlock) repmat(1, 1, NumTrialsPerBlock)];
                %     numTrialsAddedToSequence = numTrialsAddedToSequence + 2*NumTrialsPerBlock; 
                % end
                % TrialTypes(currentTrial:end) = TrialTypesToAdd(1:length(TrialTypes) - currentTrial + 1);

                while numTrialsAddedToSequence < MaxTrials
                    TrialTypesToAdd = [TrialTypesToAdd repmat(2, 1, NumTrialsPerBlock(NumTrialsPerBlockIdx) + IsFirstBlock*NumWarmupTrials) repmat(1, 1, NumTrialsPerBlock(NumTrialsPerBlockIdx + 1))];
                    numTrialsAddedToSequence = numTrialsAddedToSequence + NumTrialsPerBlock(NumTrialsPerBlockIdx) + IsFirstBlock*NumWarmupTrials + NumTrialsPerBlock(NumTrialsPerBlockIdx + 1);
                    NumTrialsPerBlockIdx = NumTrialsPerBlockIdx + 2;
                    IsFirstBlock = 0;
                end
...
function [PressVisDelay_s, ExperimenterTrialInfo] = GetPressVisDelay(obj, BpodSystem, S, TrialTypes, currentTrial, PressVisDelay_s)
    ...
    if S.GUI.EnableAutoLongDelay && (((TrialTypes(currentTrial) == 2)) || (S.GUI.SelfTimedMode == 1))
        ...
        if S.GUI.SelfTimedMode == 1
            ...
            S.GUI.PressVisDelayLong_s = PressVisDelay_s;
            BpodSystem.GUIData.ParameterGUI.LastParamValues.PressVisDelayLong_s = PressVisDelay_s;
        end
        % S.GUI.PressVisDelayLong_s = PressVisDelay_s;
        % S.GUI.PrePress2Delay_s = PressVisDelay_s;        
    end    
...
[wait_for_press_adjusted] = GetWaitForPress(obj, BpodSystem, S, wait_, currentTrial, VisStimDuration)
