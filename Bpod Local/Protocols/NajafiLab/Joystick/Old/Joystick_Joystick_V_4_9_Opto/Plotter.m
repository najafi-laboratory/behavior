...
function UpdateOutcomePlot(obj, BpodSystem, TrialTypes, OptoTrialTypes, ProbeTrialTypes, isEndOfTrial)
    ...
    if isfield(Data, 'nTrials')
        ...
        for x = 1:Data.nTrials
            ...
            elseif ( isfield(Data.RawEvents.Trial{x}.States, 'EarlyPressPunish') && ...
                    ~isnan(Data.RawEvents.Trial{x}.States.EarlyPressPunish(1)))
                if (x == Data.nTrials && isEndOfTrial)  % only print outcome to console for the trial that just occured
                    disp('Outcome: EarlyPress');
                end
                Outcomes(x) = 5;    % draws red circle on outcome plot                
            elseif ( isfield(Data.RawEvents.Trial{x}.States, 'EarlyPress1Punish') && ...
                    ~isnan(Data.RawEvents.Trial{x}.States.EarlyPress1Punish(1)))
                if (x == Data.nTrials && isEndOfTrial)  % only print outcome to console for the trial that just occured
                    disp('Outcome: EarlyPress1');
                end
                Outcomes(x) = 5;    % draws red circle on outcome plot
            elseif ( isfield(Data.RawEvents.Trial{x}.States, 'EarlyPress2Punish') && ...
                    ~isnan(Data.RawEvents.Trial{x}.States.EarlyPress2Punish(1)))
                if (x == Data.nTrials && isEndOfTrial)  % only print outcome to console for the trial that just occured
                    disp('Outcome: EarlyPress2');
                end
                Outcomes(x) = 5;    % draws blue circle on outcome plot 
            elseif ( isfield(Data.RawEvents.Trial{x}.States, 'LatePress2Punish') && ...
                    ~isnan(Data.RawEvents.Trial{x}.States.LatePress2Punish(1)))
                if (x == Data.nTrials && isEndOfTrial)  % only print outcome to console for the trial that just occured
                    disp('Outcome: LatePress2');
                end
                Outcomes(x) = 6;    % draws blue circle on outcome plot 
            else
                if (x == Data.nTrials && isEndOfTrial)  % only print outcome to console for the trial that just occured          
                    disp('Outcome: Other');
                end
