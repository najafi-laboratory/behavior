...
function [TrialTypes] = GenTrials(obj, S, MaxTrials, numTrialTypes, TrialTypes, currentTrial, updateTrialTypeSequence)
    ...
    elseif S.GUI.EnableManualTrialType  
        ...
        switch S.GUI.TrialTypeSequence
            ...
            case 2 % interleaved - random first trial type                
                ...
                while numTrialsAddedToSequence < MaxTrials
                    TrialTypesToAdd = [TrialTypesToAdd repmat(FirstTrialType, 1, S.GUI.NumTrialsPerBlock) repmat(SecondTrialType, 1, S.GUI.NumTrialsPerBlock)];
            ...
            case 3 % interleaved - short first block
                while numTrialsAddedToSequence < MaxTrials
                    TrialTypesToAdd = [TrialTypesToAdd repmat(1, 1, S.GUI.NumTrialsPerBlock) repmat(2, 1, S.GUI.NumTrialsPerBlock)];
                    numTrialsAddedToSequence = numTrialsAddedToSequence + 2*S.GUI.NumTrialsPerBlock; 
                end
                TrialTypes(currentTrial:end) = TrialTypesToAdd(1:length(TrialTypes) - currentTrial + 1);
            case 4 % interleaved - long first block
                while numTrialsAddedToSequence < MaxTrials
                    TrialTypesToAdd = [TrialTypesToAdd repmat(2, 1, S.GUI.NumTrialsPerBlock) repmat(1, 1, S.GUI.NumTrialsPerBlock)];
                    numTrialsAddedToSequence = numTrialsAddedToSequence + 2*S.GUI.NumTrialsPerBlock; 
                end
                TrialTypes(currentTrial:end) = TrialTypesToAdd(1:length(TrialTypes) - currentTrial + 1);
        end
