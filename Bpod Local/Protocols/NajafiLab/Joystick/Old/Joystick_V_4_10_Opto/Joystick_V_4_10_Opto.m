...
try
    ...
    if debug
        ...
        S.GUI.IncorrectSoundVolume_percent = 0.00;  % volume control

        S.GUI.NumTrialsPerBlock = 3;
        S.GUI.MaxNumTrialsPerBlock = 3;
        S.GUI.BlockLengthMargin = 0;
        ...
        S.GUI.OptoITICycleType = 1;

        S.GUI.SessionType = 1;
        S.GUI.OnFraction = 1;
        S.GUI.OptoTrialTypeSeq = 5;

        S.GUI.EpochTrialStop = 3;
        S.GUI.NumOptoTrialsPerBlock = 3;
    ...
    PreviousTrialTypeSequence = S.GUI.TrialTypeSequence;
    PreviousNumTrialsPerBlock = S.GUI.NumTrialsPerBlock;
    PreviousMaxNumTrialsPerBlock = S.GUI.MaxNumTrialsPerBlock;
    PreviousBlockLengthMargin = S.GUI.BlockLengthMargin;
    ...
    ExpNotes.StartTime = datetime('now', 'Format', 'HH:mm:ss');
    ExpNotes.EndTime = datetime('now', 'Format', 'HH:mm:ss');
    ExpNotes.numTrials = 0;

    ExpNotes.AssistedTrials = 0;
    ExpNotes.ExcludedTrials = 0;    
    if S.GUI.EnableSingleBlock
        ExpNotes.ExcludedTrials = 'Single Block';
    else
        ExpNotes.ExcludedTrials = 'Double Block';
    end
    ...
    if m_Opto.EnableOpto
        ...
        if S.GUI.OptoRewardITI
            ExpNotes.OptoRewardITI = 'On';
        end   
        if S.GUI.OptoEarlyPressITI
            ExpNotes.OptoEarlyPressITI = 'On';
        end
        if S.GUI.OptoLatePressITI
            ExpNotes.OptoLatePressITI = 'On';
        end
    ...
    ExpNotes.FinalPress2WindowShort_s = 0;
    ExpNotes.FinalPress2WindowLong_s = 0;
    ...
    ExpNotes.ProtoVersion = 'Joystick_V_4_10_Opto'; % update to use current file name
    ...
    for currentTrial = 1:MaxTrials
        ...
        if ~EnableRewardPulses     
            ...
            if (StartOfBlock(currentTrial)&&(currentTrial>1))
                if TrialTypes(currentTrial) == 1
                    ProbeTrialTypes(currentTrial:currentTrial+S.GUI.FirstEpochShortLen) = 0;
                else
                    ProbeTrialTypes(currentTrial:currentTrial+S.GUI.FirstEpochLongLen) = 0;
                end
            end
            BpodSystem.Data.ProbeTrial(currentTrial) = ProbeTrialTypes(currentTrial);
            ...
            LatePress2Punish_OutputActions = {} ;
            RewardITI_OutputActions = {} ;
            ...
            % TimerTrigger_V1W1 = {'GlobalTimerTrig', '00000000010001'};
            TimerCancel_V1W1 = {'GlobalTimerCancel', '000010001'};
            % TimerCancel_V1W1 = {'GlobalTimerCancel', '00000000010001'};
            ...
            if m_Opto.EnableOpto && (OptoTrialTypes(currentTrial) == 2)
                ...
                if S.GUI.OptoPrePressDelay || S.GUI.OptoVis2 || S.GUI.OptoWaitForPress2 || S.GUI.OptoPress2
                    ...
                    LatePress2_OutputActions = [LatePress2_OutputActions, TimerShutterReset, {'GlobalTimerCancel', '111111101'}];
                end
                ...
                if S.GUI.OptoLatePressITI
                    LatePress2Punish_OutputActions = [LatePress2Punish_OutputActions, {'GlobalTimerTrig', '10000000000000'}];
                end
            ...
            WaitForPress2_StateChangeConditions = {'Tup', 'WaitForLatePress2', 'RotaryEncoder1_3', 'Press2'};             
        ...
        % if ~S.GUI.EnableRewardPulses
            ...
            sma = AddState(sma, 'Name', 'VisualStimulus2', ...
                'Timer', VisStim.VisStimDuration + 0.020,...
                'StateChangeConditions', {'BNC1Low', 'WaitForPress2', 'RotaryEncoder1_1', 'EarlyPress2LeverRetract'},...
	        ...
	        % WaitForPress2_StateChangeConditions = {'Tup', 'DidNotPress2', 'RotaryEPreRewardDelayncoder1_3', 'Press2'};
            ...
            sma = AddState(sma, 'Name', 'Press2', ...
                'Timer', Press2Window_s,...
                'StateChangeConditions', {'Tup', 'WaitForLatePress2', 'GlobalTimer11_End', 'WaitForLatePress2', 'RotaryEncoder1_1', 'RewardLeverRetract'},...
            ...
            %%%% latepress2
            sma = AddState(sma, 'Name', 'WaitForLatePress2', ...
                'Timer', 1,...
                'StateChangeConditions', {'Tup', 'DidNotPress2', 'GlobalTimer11_End', 'DidNotPress2', 'RotaryEncoder1_1', 'LatePress2'},...
                'OutputActions', Press2_OutputActions);
            ...
            sma = AddState(sma, 'Name', 'LatePress2', ...
                'Timer', 0,...
                'StateChangeConditions', {'Tup', 'LatePress2LeverRetract'},...
                'OutputActions', LatePress2_OutputActions);	% {opto timers}
            
            sma = AddState(sma, 'Name', 'LatePress2LeverRetract', ...
                'Timer', 0.1,...
                'StateChangeConditions', {'Tup', 'LatePress2Punish'},...
                'OutputActions', {'SoftCode', 8});
            ...
            sma = AddState(sma, 'Name', 'LatePress2Punish', ...
                'Timer', S.GUI.EarlyPressPunishSoundDuration_s,...
                'StateChangeConditions', {'Tup', 'LeverRetractFinal'},...
                'OutputActions', LatePress2Punish_OutputActions);
            ...
            % 'OutputActions', {'SoftCode', 8, 'GlobalTimerTrig', '10000000000000', 'GlobalCounterReset', '00000111111111'});
        ...
        if BpodSystem.Status.BeingUsed == 0 % If protocol was stopped, exit the loop
            ...
            ExpNotes.FinalPress2WindowShort_s = S.GUI.Press2WindowShort_s;
            ExpNotes.FinalPress2WindowLong_s = S.GUI.Press2WindowLong_s;
            ...
            if S.GUI.EnableAlternatingReward
                ExpNotes.CenterValveAmountShort = S.GUI.ShortRewardAmount_uL;
                ExpNotes.CenterValveAmountLong = S.GUI.LongRewardAmount_uL;
            else
                ExpNotes.CenterValveAmount = S.GUI.CenterValveAmount_uL;
            end
            ExpNotes.RewardPulses = NumCompletedRewardPulses;            
...
function PrintInterruptLog(BpodSystem)
    ...
    LogFileDir = 'C:\Users\gtg424h\OneDrive - Georgia Institute of Technology\Najafi_Lab\2__Data_Analysis\z_InterruptLogs\';    
