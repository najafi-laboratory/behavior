    ...
    for currentTrial = 1:MaxTrials
        ...
        if S.GUI.EnableAutoLongDelay && (((TrialTypes(currentTrial) == 2)) || (S.GUI.SelfTimedMode == 1))
            ...
            if S.GUI.SelfTimedMode == 1
                % PressVisDelay_s = min(S.GUI.AutoDelayStart_s + NumDelaySteps * S.GUI.AutoDelayStep_s, S.GUI.AutoDelayMaxSelf_s);            
                % PressVisDelay_s = min(PrevPressVisDelay_s + S.GUI.AutoDelayStep_s, S.GUI.AutoDelayMaxSelf_s);            
                PressVisDelay_s = min(AutoPressVisDelay_s, S.GUI.AutoDelayMaxSelf_s);            
                S.GUI.PrePress2Delay_s = PressVisDelay_s;
                % ExperimenterTrialInfo.PrePress2Delay_s = PressVisDelay_s;
            else
                % PressVisDelay_s = min(S.GUI.AutoDelayStart_s + NumDelaySteps * S.GUI.AutoDelayStep_s, S.GUI.AutoDelayMaxVis_s);
                % PressVisDelay_s = min(PrevPressVisDelay_s + S.GUI.AutoDelayStep_s, S.GUI.AutoDelayMaxVis_s);            
