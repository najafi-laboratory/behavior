...
classdef PsychToolboxVideoPlayer < handle
    ...
    methods
        ...
        function loadVideo(obj, VideoIndex, VideoMatrix, varargin)
            ...
            FixedSignal = [];
            if nargin > 3
                FixedSignal = varargin{1};
                if ~ismember(FixedSignal, [0 1])
                    error('Sync patch state must be 0 or 1.')
                end
                SignalOn = FixedSignal;
            end
            for i = 1:nFrames
                switch MatrixType
                    case 1
                    ...
                    case 0 % Play video
                        if obj.Videos{StimulusIndex}.nFrames == 1
                            ...
                            elseif obj.TimerMode == 2
                                ...
                                % callback
                                if strcmp(obj.FrameDropTimer1.Running, 'on')
                                    stop(obj.FrameDropTimer1);
                                end
                                if strcmp(obj.FrameDropTimer2.Running, 'on')
