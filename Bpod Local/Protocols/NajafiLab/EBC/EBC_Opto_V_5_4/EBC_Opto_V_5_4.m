...
try
    ...
    % Add encoder var for session-wide data
    BpodSystem.Data.EncoderDataSession = struct();
   ...
   for currentTrial = 1:MaxTrials
            ...
            if currentTrial == numWarmupTrials
               ...
               % ========== Initialize probe trial indices ==========
                ...
                if S.GUI.UseProbeTrials == 1
                  ...
                  blockStartTrial = getBlockStartTrial(currentBlockIndex, blocks, numWarmupTrials);
    ...
    if currentTrialInBlock == 1  
        ...
        if S.GUI.OptoEnabled
            switch S.GUI.OptoSessionType
                case 1 % RandomTrial (pick trials within this block's absolute indices)
                    ...
                    Optoselected = [];
                    % keep picking until we hit numProbes (target) or run out of candidates
                    while numel(Optoselected) < numOpto && ~isempty(opto_candidate_trials)
                        % pick one candidate at random
        ...
        optoDurationThisTrial = S.GUI.OptoDuration;
        switch currentBlockType
            case 'single'
                optoDurationThisTrial = S.GUI.OptoDuration_SingleBlock;
            case 'short'
                optoDurationThisTrial = S.GUI.OptoDuration_DoubleBlockShort;
            case 'long'
                optoDurationThisTrial = S.GUI.OptoDuration_DoubleBlockLong;
        end
        ...
        if OptoTrigThisTrial
            sma = SetGlobalTimer(sma, 'TimerID', 4, ...
                'Duration', optoDurationThisTrial, ...
                'OnsetDelay', optoRelDelay, ...   % delay is relative to when we trigger it
                'Channel', 'BNC2', 'OnLevel', 1, 'OffLevel', 0, 'Loop', 0);
        end
        
        % Set zero position at beginning of start state on the first trial
        % of the session.  
        Start_OutputActions = {'GlobalTimerTrig', '0100', 'SoftCode', 1};
        if currentTrial == 1
            Start_OutputActions = [Start_OutputActions, 'RotaryEncoder1', ['Z#' 0]];
        else
            Start_OutputActions = [Start_OutputActions, 'RotaryEncoder1', ['#' 0]];
        end
        ...
        sma = AddState(sma, 'Name', 'Start', ...
            ...
            'OutputActions', Start_OutputActions);
        ...
        if isProbeTrial && OptoTrigThisTrial && strcmp(optoAnchor,'AIRPUFF')
        ...
        if ~isempty(fieldnames(RawEvents)) % If trial data was returned (i.e. if not final trial, interrupted by user)
            ...
            BpodSystem.Data.EncoderData{currentTrial}.nPositionsOrig = BpodSystem.Data.EncoderData{currentTrial}.nPositions;
            BpodSystem.Data.EncoderData{currentTrial}.TimesOrig = BpodSystem.Data.EncoderData{currentTrial}.Times;
            BpodSystem.Data.EncoderData{currentTrial}.PositionsOrig = BpodSystem.Data.EncoderData{currentTrial}.Positions;            
            ...
            BpodSystem.Data.EncoderData{currentTrial}.EventTimestampsTrialAligned = BpodSystem.Data.EncoderData{currentTrial}.EventTimestamps - BpodSystem.Data.EncoderData{currentTrial}.EventTimestamps(1); % Align event timestamps to state machine's trial time 0  
            ...
            if (S.GUI.WheelDirection == 2)
                BpodSystem.Data.EncoderData{currentTrial}.Positions = -BpodSystem.Data.EncoderData{currentTrial}.Positions;
                BpodSystem.Data.EncoderData{currentTrial}.PositionsOrig = -BpodSystem.Data.EncoderData{currentTrial}.PositionsOrig;
            end
            ...
            BpodSystem.Data.RawEvents.Trial{1, currentTrial}.Data.OptoDuration = optoDurationThisTrial;
            BpodSystem.Data.RawEvents.Trial{1, currentTrial}.Data.OptoAnchor = optoAnchor;
            BpodSystem.Data.RawEvents.Trial{1, currentTrial}.Data.OptoRelDelay = optoRelDelay;
            ...
            protocol_version = 'EBC_Opto_V_5_4';
            % fprintf('*******************************************************\n');
            ...
            elseif S.GUI.TrainingStage == 2  % Well-Trained → Use GUI selection
                trial_type_string = S.GUIMeta.TrialTypeSequence.String{S.GUI.TrialTypeSequence}; 
            ...
            fprintf('Opto Duration Current Trial: %.3f sec\n', optoDurationThisTrial);
        ...
        if BpodSystem.Status.BeingUsed == 0 % If protocol was stopped, exit the loop
            ...
            daqTimestampsAll = BpodSystem.Data.daqTimestampsAll;
            ...
            trialSyncBin = trialSyncChannel > 1.5;
            camStrobeBin = camStrobeChannel > 1.5;

            % probably need to binarize these
            trialSyncChannel_numRising = sum(diff(trialSyncBin > 0) == 1)
            camStrobeChannel_numRising = sum(diff(camStrobeBin > 0) == 1)

            fallIdx = find(diff(camStrobeBin > 0) == -1);
            ...
            %     fe_idx = fallIdx(nth_idx);
            % else
            %     error('Requested falling edge exceeds number of detected edges.');
            % end
            % 
            % check_idx = fe_idx;
    ...
    CrashFileDir = 'D:\behavior\error logs\';
    ...
    if ~exist(CrashFileDir, 'dir')
        [status, msg, msgID] = mkdir(CrashFileDir);
        if ~status
            error('Could not create crash log folder: %s. %s (%s)', CrashFileDir, msg, msgID);
        end
    end
...
function blockStartTrial = getBlockStartTrial(currentBlockIndex, blocks, numWarmupTrials)
    if currentBlockIndex <= 1
        completedBlockTrials = 0;
    else
        completedBlockTrials = sum(blocks(1:currentBlockIndex - 1));
    end
    blockStartTrial = numWarmupTrials + completedBlockTrials + 1;
end
