classdef EyelidAnalyzer < handle
    properties   
        ...
        tMinThresh
        totalEllipsePixels
        trialVideoDir = 'D:\behavior\video_data\';
        trialStartTime
        trialVidStartTime
    ...
    methods
        function obj = EyelidAnalyzer(subjectName, PupilColorForImaging)
            ...
            % exist
            if ~exist(obj.trialVideoDir, 'dir')
                [status, msg, msgID] = mkdir(obj.trialVideoDir);
                if ~status
                    error('Could not create video folder: %s. %s (%s)', obj.trialVideoDir, msg, msgID);
                end
            end 
        end
        ...
        function startSessionVideo(obj, subjectName)
            ...
            videoFileName = fullfile(obj.trialVideoDir, [subjectName, '_SessionVid_', datestr(now, 'yyyy-mm-dd_HHMMSS'), '_ID', num2str(obj.session_id), '.avi']);
            obj.EBC_vid_log_trial = VideoWriter(videoFileName, 'Grayscale AVI');
            obj.EBC_vid_log_trial.FrameRate = 160; %400; % err for framerate too high
        ...
        function startTrialsVideo(obj, currentTrial, subjectName)
            ...
            trialVideoFileName = fullfile(obj.trialVideoDir, [subjectName, '_TrialVid_', num2str(currentTrial), '_', datestr(now, 'yyyy-mm-dd_HHMMSS'), '.avi']);
            obj.EBC_vid_log_trial = VideoWriter(trialVideoFileName, 'Grayscale AVI');
            obj.EBC_vid_log_trial.FrameRate = 160; %400; % err for framerate too high
            % obj.EBC_vid_log_trial.Quality = 90;
            % obj.vid.DiskLogger = obj.EBC_vid_log_trial;
            open(obj.EBC_vid_log_trial);
