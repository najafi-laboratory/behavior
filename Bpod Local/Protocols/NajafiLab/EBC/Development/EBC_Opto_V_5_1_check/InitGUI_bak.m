...
function [S] = SetParams(obj, BpodSystem)
    ...
    if isempty(fieldnames(S))  % If settings file was an empty struct, populate struct with default settings
        ...
        S.GUIMeta.TrialTypeSequence.String = {'Random','Long','Short'};  
        
        
        % Define a checkbox for activating the airpuff pulse
        S.GUI.ActivateAirPuffPulse = false; % Default to false (unchecked)
        

        % Check if airpuff pulse should be active based on the checkbox state
        if S.GUI.ActivateAirPuffPulse
            % Set the airpuff pulse duration based on the default amount
            S.GUI.AirPuff_Pulse = true;
            S.GUI.AirPuff_Pulse_Dur = 0.02; % Default amount as specified
        else
            % Set the airpuff pulse to inactive
            S.GUI.AirPuff_Pulse = false;
        end
        ...
        S.GUIMeta.ActivateAirPuffPulse.Style = 'checkbox';
        S.GUIMeta.ActivateAirPuffPulse.String = 'Activate AirPuff Pulse';

        S.GUIPanels.EBC = {'ExperimenterInitials','SleepDeprived', 'ITI_Pre', 'ITI_Post', 'ITI_Extra','LED_OnsetDelay',...
            'TrialTypeSequence','LED_Dur_Short','LED_Dur_Long','ActivateAirPuffPulse', 'AirPuff_OnsetDelay', 'AirPuff_OnsetDelay_Short','AirPuff_OnsetDelay_Long', 'AirPuff_Dur', 'CheckEyeOpenAveragingBaseline','CheckEyeOpenTimeout','BlockLength','Margine'};
