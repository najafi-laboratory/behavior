classdef EBC_PostProcess_Compress_V_5_3 < handle
    properties
        % Data/state
        SessionData = []
        sessionDataPath = ''
    ...
    methods
        function obj = EBC_PostProcess_Compress_V_5_3()
        ...
        function loadSessionVideo(obj,~,~)
            ...
            % modify path for compressed filename
            [folder,name,ext] = fileparts(fullPath);          
            obj.outputVidPath = fullfile(folder, [name '_compressed', ext]);
            ...
            try
                ...
                % if compress enable
                writer = VideoWriter(obj.outputVidPath, 'Motion JPEG AVI');
                obj.sessionVideoWriter = writer;
                obj.sessionVideoWriter.FrameRate = reader.FrameRate;
                obj.sessionVideoWriter.Quality   = 90;   % 0–100, lower = smaller size; adjust as needed    
                open(obj.sessionVideoWriter);                
        ...
        function sameID = checkID(obj)
            obj.sessionDataPath            
            obj.sessionVideoPath

            % get session data ID
            toksessionDataPath = regexp(obj.sessionDataPath, 'ID(\d{6})', 'tokens');
            if isempty(toksessionDataPath)
                sameID = false;
                return;
            else
                obj.idsessionData = toksessionDataPath{1}{1}
            end

            % if length(toksessionDataPath{1}{1}) == 6
            %     obj.idsessionData = toksessionDataPath{1}{1};
            % else
            %     obj.idsessionData = '000000';
            % end

            % get video ID
            toksessionVideoPath = regexp(obj.sessionVideoPath, 'ID(\d{6})', 'tokens');
            if isempty(toksessionVideoPath)
                sameID = false;
                return;
            else
                obj.idsessionVideo = toksessionVideoPath{1}{1}; 
            end
            % if length(toksessionVideoPath{1}{1}) == 6

            % obj.idsessionVideo = toksessionVideoPath{1}{1};   

            % check ID
            sameID = all(obj.idsessionData == obj.idsessionVideo);
        end
        ...
        function runProcessing(obj, varargin)
            ...
            % if ~sameID
            %     uialert(obj.fig,'Select matching session data and video.','Process Error');
            %     return;
            % end
            % 
            % if ~sameID
            %     msg = sprintf([ ...
            %         'Select matching session data and video.\n\n' ...
            %         'SessionData ID: %s\n' ...
            %         'Video ID: %s'], ...
            %         string(obj.idsessionData), ...
            %         string(obj.idsessionVideo));
            % 
            %     uialert(obj.fig, msg, 'Process Error');
            %     return;
            % end            
            ...
            if ~sameID
                ...
                selection = uiconfirm(obj.fig, msg, 'Process Warning', ...
                    'Options', {'Cancel', 'Process Anyway'}, ...
                    'DefaultOption', 1, ...
                    'CancelOption', 1);
            ...
            obj.SessionData.eyeAreaSeries = obj.eyeAreaSeries;
            obj.SessionData.secondCount = obj.secondCount;
            obj.SessionData.cycleCount = obj.cycleCount;
            ...
            obj.SessionData.cycleSeconds = cycleSeconds;
            obj.SessionData.timestampSec = obj.timestampSec;
                    
            % binarize DAQ
            binTrial = obj.binarizeSignal(obj.daqTrialSync);
            binStrobe = obj.binarizeSignal(obj.daqCamStrobe);

            obj.SessionData.binTrial = binTrial;
            obj.SessionData.binStrobe = binStrobe;            
            ...
            obj.SessionData.AdjustedStrobeIdx = [];
            if strobeMaxDiff > 30
                strobeIdx = strobeIdx(1:strobeMaxDiffIdx);
                obj.SessionData.AdjustedStrobeIdx = strobeIdx;
            end
            ...
            obj.SessionData.tsAligned = tsAligned;
            ...
            obj.SessionData.trialSyncRiseTimes = obj.trialSyncRiseTimes;
            obj.SessionData.camStrobeRiseTimes = obj.camStrobeRiseTimes;
            ...
            obj.SessionData.droppedFrameNumbers = obj.droppedFrameNumbers;
            obj.SessionData.camStrobeRiseTimesAdjusted = obj.camStrobeRiseTimesAdjusted;

            % exposure center per image
            obj.FECTimes = obj.camStrobeRiseTimesAdjusted + 0.0015;            

            obj.findEyeOpenMax();
            obj.SessionData.minFur = obj.minFur;
