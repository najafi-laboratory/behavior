classdef EyelidAnalyzer < handle
    ...
    methods
        ...
        function trialVideoStop(obj, ~, ~) 
            stop(obj.vid);
        end

        function trialVideoProcess(obj, ~, ~)
            % open(obj.EBC_vid_log);            
            while obj.vid.FramesAvailable > 0
                ...
                % writeVideo(obj.EBC_vid_log, data);
                writeVideo(obj.EBC_vid_log, data);
                %  montage(data)
