...
function [S] = SetParams(obj, BpodSystem)
    ...
    if isempty(fieldnames(S))  % If settings file was an empty struct, populate struct with default settings
        ...
        S.GUI.TrainingStage = 1;  % Default to 'naive'
        S.GUIMeta.TrainingStage.Style = 'popupmenu';
        S.GUIMeta.TrainingStage.String = {'naive', 'well-trained'};


        % GUI Panels
        S.GUIPanels.EBC = {'ExperimenterInitials', 'SleepDeprived','TrainingStage' ,'probetrials_percentage_perBlock', 'num_initial_nonprobe_trials_perBlock' ,'num_warmup_trials', 'ITI_Pre', 'ITI_Post', 'ITI_Extra', ...
