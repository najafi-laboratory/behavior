classdef MouseEyeVideoAnalysis < handle
    ...
    methods
        ...
        function calculateFEC(obj)
            ...
            openPixels =  sum(obj.binFrame(:) == 1); % Assuming open eye corresponds to 1

            obj.fec = ((totalEllipsePixels - openPixels) / totalEllipsePixels) * 100; % FEC percentage
...
% fec over time
% function computeFEC(obj, binaryFrame)
%  totalPixels = numel(binaryFrame);
% closedPixels = sum(binaryFrame(:) == 0); % Assuming closed eye corresponds to 0
% fec = (closedPixels / totalPixels) * 100; % FEC percentage
% currentTime = now;
% obj.fecData = [obj.fecData, fec];
