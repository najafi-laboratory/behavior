   ...
   for currentTrial = 1:MaxTrials
        ...
        if isWarmupPhase
            ...
            if currentTrial == numWarmupTrials+1
               ...
               if S.GUI.TrainingStage == 2
                    numLeadIn = S.GUI.num_initial_nonprobe_trials_perBlock;
                    % blockLen = blocks(currentBlockIndex);  % This is randomized as BlockLength ± Margin
                    % % So it can range from 45 to 55 if BlockLength = 50 and Margin = 5
                    % % Exclude first N trials in the block
                    % minTrial = numLeadIn + 1;
                    % maxTrial = blockLen;
                    
                    if currentBlockIndex == 1
                        blockStartTrial = 1 + S.GUI.num_warmup_trials;
                    else
                        blockStartTrial = sum(blocks(1:currentBlockIndex - 1)) + S.GUI.num_warmup_trials + 1;
                    end
                    blockEndTrial = blockStartTrial + blocks(currentBlockIndex) - 1;
                    % Skip the first N lead-in trials in this block
                    ...
                    % Clamp for safety
                    numProbes = min(numProbes, numCandidateTrials);
                    % Randomly assign probes

                    probeTrialsThisBlock = randsample(probe_candidate_trials, numProbes);

                    fprintf('Block #%d: %d probe trials selected from trials %d–%d (excluding first %d trials)\n', ...
                        currentBlockIndex, numProbes, minTrial, maxTrial, numLeadIn);
           ...
           if S.GUI.OptoEnabled
                switch S.GUI.OptoSessionType
                    case 1  % RandomTrial
                        ...
                        nOpto = round(S.GUI.OptoFraction * numel(blockTrialRange));
                        nOpto = max(0, min(nOpto, numel(blockTrialRange)));

                        optoTrialsThisBlock = sort(randsample(blockTrialRange, nOpto));

                        fprintf('[Block %d] Random OPTO trials: %s\n', currentBlockIndex, mat2str(optoTrialsThisBlock));

                     case 2  % BlockTransition: first N of long after short
                        fprintf('This is first block, No Transition OPTO trials!!\n');
                     case 3  % ProbeTrialOpto (no pre-assign; set per-trial)
    ...
    if currentTrialInBlock == 1
        % Absolute range of trial numbers for *this* block
        if currentBlockIndex == 1
            blockStartTrial = numWarmupTrials + 1;
        else
            blockStartTrial = numWarmupTrials + 1 + sum(blocks(1:currentBlockIndex-1));
        end
        blockEndTrial   = blockStartTrial + blocks(currentBlockIndex) - 1;
        blockTrialRange = blockStartTrial:blockEndTrial;
        % ---------- PROBE selection (well-trained only) ----------
        % probeTrialsThisBlock = [];
        if S.GUI.TrainingStage == 2
            numLeadIn = min(S.GUI.num_initial_nonprobe_trials_perBlock, numel(blockTrialRange));
            if numLeadIn < numel(blockTrialRange)
                probeCandidates = blockTrialRange(numLeadIn+1:end);
            else
                probeCandidates = [];
            end
            probe_fraction = S.GUI.probetrials_percentage_perBlock / 100;
            nProbe = round(probe_fraction * numel(probeCandidates));
            nProbe = max(0, min(nProbe, numel(probeCandidates)));
            if nProbe > 0
                probeTrialsThisBlock = sort(randsample(probeCandidates, nProbe));
            end
            fprintf('########################################\n');
            fprintf('[Block %d] Probe trials for this block are: %s\n', ...
        ...
        if S.GUI.OptoEnabled
            switch S.GUI.OptoSessionType
                case 1 % RandomTrial (pick trials within this block's absolute indices)
                    ...
                    fprintf('########################################\n');
                    fprintf('[Block %d] Random OPTO trials are: %s\n', ...
                        currentBlockIndex, mat2str(optoTrialsThisBlock));
                    fprintf('########################################\n');
                case 2 % BlockTransition (first N trials of a long block if previous was short)
                    % Determine previous block type from the last trial of previous block, if any
                    if currentBlockIndex > 1
                        prevLastTrial    = blockStartTrial - 1;
                        previousBlockType = BlockTypes{prevLastTrial};  % <- you must store BlockTypes{t} each trial
                    else
                        previousBlockType = '';
                    end
                    if strcmp(previousBlockType,'short') && strcmp(currentBlockType,'long')
                        % Optional alternation across transitions
                        % if ~exist('TransitionCount','var'), TransitionCount = 0; end
                        TransitionCount = TransitionCount + 1;
                        allowThisTransition = true;
                        if S.GUI.OptoTransitionType == 1    % alternate
                            % use odd transitions; change to ==0 for even
                            allowThisTransition = (mod(TransitionCount,2) == 1);
                        end
                        if allowThisTransition
                            n = max(0, min(S.GUI.OptoTransitionTrials, numel(blockTrialRange)));
                            optoTrialsThisBlock   = blockTrialRange(1:n);
                            isTransitionOptoBlock = true;
                            fprintf('########################################\n');
                            fprintf('[Block %d] Transition %d short→long → OPTO on first %d trials: %s\n', ...
                                currentBlockIndex, TransitionCount, n, mat2str(optoTrialsThisBlock));
                            fprintf('########################################\n');
                        else
                            fprintf('########################################\n');
                            fprintf('[Block %d] Transition %d short→long → OPTO SKIPPED (alternating).\n', ...
                                currentBlockIndex, TransitionCount);
                            fprintf('########################################\n');
                        end
                    else
                        fprintf('########################################\n');
                        fprintf('[Block %d] No qualifying short→long transition → no transition‑opto.\n', ...
                            currentBlockIndex);
                        fprintf('########################################\n');
                    end
                case 3 % ProbeTrialOpto (no pre-assignment; handle per‑trial by tying opto to probe)
                    fprintf('########################################\n');
        ...
        sma = SetGlobalTimer(sma, 'TimerID', 4, 'Duration', S.GUI.OptoDuration, 'OnsetDelay', 0,...
        ...
        % ---------------------------------------------------
        % LED Onset State (triggers camera and LED)
        % ---------------------------------------------------
        ...
        if isProbeTrial && isOptoTrial
            fprintf('%%%%%%%%%%%%%%%%%%%%%%%%%%\n');
            fprintf('IS POROBE AND OPTO NEXT STATE\n'); 
            fprintf('%%%%%%%%%%%%%%%%%%%%%%%%%%\n');

            nextStateAfterLED = 'ProbeOpto';        % probe + opto

        elseif isProbeTrial

            fprintf('%%%%%%%%%%%%%%%%%%%%%%%%%%\n');
            fprintf('IS POROBE TRIAL NEXT STATE \n'); 
            fprintf('%%%%%%%%%%%%%%%%%%%%%%%%%%\n');

            nextStateAfterLED = 'ITI_Post';         % probe only (no puff, no opto)

        elseif isOptoTrial && allowThisTransition

            fprintf('%%%%%%%%%%%%%%%%%%%%%%%%%%\n');
            fprintf('IS OPTO AND BLOCK TRANITION NEXT STATE \n'); 
            fprintf('%%%%%%%%%%%%%%%%%%%%%%%%%%\n');

            nextStateAfterLED = 'TransitionOptoOnly'; % transition-only opto
        else

            fprintf('%%%%%%%%%%%%%%%%%%%%%%%%%%\n');
            fprintf('IS NORMAL TRIAL NEXT STATE AIRPUFF AFTER LED \n'); 
            fprintf('%%%%%%%%%%%%%%%%%%%%%%%%%%\n');

            nextStateAfterLED = 'AirPuff';          % normal trial
        end

        % ---------------------------------------------------
        % Delay after LED until AirPuff or Opto
        % ---------------------------------------------------
      
        if allowThisTransition
            fprintf('############################\n');
            fprintf('BLOCK TRANSITION OPTO \n'); 
            fprintf('############################\n');

            LED_Puff_ISI_Timer = S.GUI.AirPuff_OnsetDelay_Short;
        else

            fprintf('############################\n');
            fprintf('!!!!!!!  NO BLOCK TRANSITION OPTO !!!!!!! \n'); 
            fprintf('############################\n');

            LED_Puff_ISI_Timer = AirPuff_OnsetDelay;
        end
        
        sma = AddState(sma, 'Name', 'LED_Puff_ISI', ...
            'Timer', LED_Puff_ISI_Timer, ...
            'StateChangeConditions', {'Tup', nextStateAfterLED}, ...
            'OutputActions', {});

        % ---------------------------------------------------
        % AirPuff state (Skip if probe trial) (for regular trials with or without opto)
        % ---------------------------------------------------
        if ~isProbeTrial
            if isOptoTrial

                fprintf('############################\n');
                fprintf('NOT PROBE BUT IS OPTO \n'); 
                fprintf('############################\n');
               % 
               % sma = SetGlobalTimer(sma, 'TimerID', 1, 'Duration', S.GUI.OptoDuration, 'OnsetDelay', 0,...
               %  'Channel', 'PWM1', 'OnLevel', 255, 'OffLevel', 0,...
               %  'Loop', 0, 'SendGlobalTimerEvents', 0, 'LoopInterval', 0,...
               %  'GlobalTimerEvents', 0, 'OffsetValue', 0); 
               % 
               ...
               sma = AddState(sma, 'Name', 'AirPuff', ...
                'Timer', S.GUI.AirPuff_Dur, ...
                'StateChangeConditions', {'Tup', 'ITI_Post'}, ...
                'OutputActions', {'SoftCode', 4, 'GlobalTimerTrig', 4});  % trigger Opto here


            else
                fprintf('############################\n');
                fprintf('NOT PROBE and NOT OPTO \n'); 
                fprintf('############################\n');
        ...
        % ---------------------------------------------------
        % ProbeOpto state (for probe trials that get opto only)
        % ---------------------------------------------------

        if strcmp(nextStateAfterLED, 'ProbeOpto')
            
                  fprintf('############################\n');
                  fprintf('PROBE--OPTO BOTH NO AIRPUFF state and OPTO TURNS ON \n');
                  fprintf('############################\n');
                    % (REMOVE the SetGlobalTimer(...) you had here)
                  sma = AddState(sma, 'Name', 'ProbeOpto', ...
                        'Timer', 0, ...
                        'StateChangeConditions', {'Tup', 'ITI_Post'}, ...
                        'OutputActions', {'GlobalTimerTrig', '1000'});  % fire Opto (Timer 4) once
            ...
            %         'Channel', 'PWM1', 'OnLevel', 255, 'OffLevel', 0,...
            %         'Loop', 0, 'SendGlobalTimerEvents', 0, 'LoopInterval', 0,...
            %         'GlobalTimerEvents', 0, 'OffsetValue', 0); % Trigger opto pulse timer
            % 
            % 
            % % Trigger ONLY the opto timer at the US time (you already waited in LED_Puff_ISI)
            %   sma = AddState(sma, 'Name', 'ProbeOpto', ...
            %     'Timer', 0, ...
            %     'StateChangeConditions', {'Tup', 'ITI_Post'}, ...
            %     'OutputActions', {'GlobalTimerTrig', '1000'});  % <- triggers TimerID 4 only

        end
        % ---------------------------------------------------
        % TransitionOptoOnly: no airpuff, just opto at 200ms
        % ---------------------------------------------------
        if strcmp(nextStateAfterLED, 'TransitionOptoOnly')
                ...
                fprintf('%%%%%%%%%%%%%%%%%%%%%%%%%%\n');
                % (REMOVE the SetGlobalTimer(...) you had here)
                sma = AddState(sma, 'Name', 'TransitionOptoOnly', ...
                    'Timer', 0, ...
                    'StateChangeConditions', {'Tup', 'ITI_Post'}, ...
                    'OutputActions', {'GlobalTimerTrig', '1000'});  % fire Opto (Timer 4) once

            % fprintf('%%%%%%%%%%%%%%%%%%%%%%%%%%\n');
            % fprintf('BLOCK TRANITION OPTO SHOULD TURN ON For the first N trials AFTER transition of SORT To Long BLOCK\n'); 
            % fprintf('%%%%%%%%%%%%%%%%%%%%%%%%%%\n');
            % 
            % 
            % sma = SetGlobalTimer(sma, 'TimerID', 1, 'Duration', S.GUI.OptoDuration, 'OnsetDelay', 0,...
        ...
        % ---------------------------------------------------
        % if isOptoTrial || isTransitionOptoOnly
        %     cancelMask = '1011';
        % else
        %     cancelMask = '0011';
        % end
        ...
        if ~isempty(fieldnames(RawEvents)) % If trial data was returned (i.e. if not final trial, interrupted by user)
            ...
            protocol_version = 'EBC_Opto_V_4_4';
            % fprintf('*******************************************************\n');
