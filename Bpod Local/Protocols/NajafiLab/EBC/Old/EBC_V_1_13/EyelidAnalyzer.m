classdef EyelidAnalyzer < handle
    ...
    methods
        ...
        function createGUI(obj)
            ...
            % Create and start a timer to update the trial video display
            if isempty(obj.trialTimer) || ~isvalid(obj.trialTimer)
                obj.trialTimer = timer('TimerFcn',@(x,y)obj.updateTrialVideo(), 'ExecutionMode', 'fixedSpacing', 'Period', 0.03);
        ...
        function startGUIVideo(obj, ~, ~)          
            ...
            obj.trialStartTime = datetime('now');
...
% imshow(obj.frame, 'Parent', obj.axOriginal);
        ...
        function startTrialLog(obj)
            obj.vid.DiskLogger = VideoWriter([".\Videos\TrialVideos\vid" ],"Grayscale AVI");
            start(obj.trialTimer);
        end

        function stopTrialLog(obj)
        ...
        function updateTrialVideo(obj)
            % disp('update')
            
            if obj.vid.FramesAvailable > 0
                % obj.vid.FramesAvailable
                obj.counter = obj.counter + obj.vid.FramesAvailable;
                [data, time, metadata] = getdata(obj.vid, obj.vid.FramesAvailable);
                % obj.trialVid = [obj.trialVid data];
                % obj.trialTime = [obj.trialTime, time];
                % obj.trialMetadata = [obj.trialMetadata metadata];
        ...
        function calculateFEC(obj)
            ...
            % currentTime = seconds(datetime('now') - obj.startTime);
            currentTime = seconds(datetime('now') - obj.trialStartTime);
