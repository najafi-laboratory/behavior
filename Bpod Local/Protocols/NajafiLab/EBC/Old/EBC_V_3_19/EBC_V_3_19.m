...
try
    ...
    for currentTrial = 1:MaxTrials        
        ...
        % AirPuff Timer (GlobalTimer2) → Only for non-probe trials
        % if ~isProbeTrial
        ...
        % % Decide where to go after LED, based on trial type
        % if isProbeTrial
        %     nextStateAfterLED = 'ITI_Post';  % Skip AirPuff
        % else
        %     nextStateAfterLED = 'LED_Puff_ISI';  % Continue to AirPuff
        % end
        % 
        % sma = AddState(sma, 'Name', 'LED_Onset', ...
        %     'Timer', 0, ...
        %     'StateChangeConditions', {'GlobalTimer1_Start', nextStateAfterLED}, ...
        %     'OutputActions', {'GlobalTimerTrig', '011', 'SoftCode', 3});
        % 
        % % Add AirPuff sequence only if it's a normal trial
        % % if ~isProbeTrial
        ...
        %     'OutputActions', {'GlobalTimerCancel', '011'});  % Cancel LED (1) and Cam (3)
        % 
        ...
        %     'OutputActions', {'GlobalTimerCancel', '100'});  % Cancel only camera trigger (TimerID 3)
