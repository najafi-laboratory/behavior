classdef MouseEyeVideoAnalysis_V_2 < handle
    properties        
        axOriginal
        axThresholded
        axFEC
    ...
    methods
        function obj = MouseEyeVideoAnalysis_V_2()
            % Init Properties
        ...
        function selectROI(obj, ~, ~)
            ...
            disp(['ROI Selected']);
            % wait(obj.roiHandle); % Optional: wait for user to finalize the ellipse
            obj.pauseVideo;
        end
        ...
        function updateVideo(obj, ~, ~)
            ...
            % If Video Object running, get most recent image from buffer
            % then flush buffer
            if isrunning(obj.vid)
                ...
                % imshow(obj.frame, 'Parent', obj.axOriginal);
                % disp(['Video Display Updated'])
                flushdata(obj.vid);                
                % disp(['Video Input Buffer Flushed']);
            else
