classdef EyelidAnalyzer < handle
    ...
    methods
        ...
        function onGUIClose(obj, ~, ~)
            ...
            obj.stopPreSessionVideo(); % Ensure the live video is stopped
            % obj.saveSettings(); % Save settings before closing
            delete(obj.hFig); % Close the figure
            % delete(timerfindall)
        end     

        function stopPreSessionVideo(obj, ~, ~)
        ...
        function startPreSessionVideo(obj, ~, ~)
            if isempty(obj.vid) || ~isvalid(obj.vid)
            ...
            if isempty(obj.vidTimer) || ~isvalid(obj.vidTimer)
                obj.vidTimer = timer('TimerFcn',@(x,y)obj.updatePreSessionVideo(), 'ExecutionMode', 'fixedSpacing', 'Period', 0.030);
                start(obj.vidTimer);
        ...
        function updatePreSessionVideo(obj, ~, ~)
        ...
        function startTrialsVideo(obj, currentTrial, subjectName)
            ...
            % Create and start a timer to update the video display
            if isempty(obj.vidTimer) || ~isvalid(obj.vidTimer)
                obj.vidTimer = timer('TimerFcn',@(x,y)obj.updateTrialsVideo(), 'ExecutionMode', 'fixedSpacing', 'Period', 0.030);
                start(obj.vidTimer);
                disp(['Video Update Timer Started']);
            else
                start(obj.vidTimer);
