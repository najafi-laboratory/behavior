...
function LastTrialEncoderPlot(axes, op, varargin)
global BpodSystem
switch op
    case 'init'  
        % LEFT AXIS  (degrees)
        yyaxis left
        BpodSystem.GUIHandles.EncoderPlot = plot(axes, 0,0, 'k-', 'LineWidth', 6);
        hold(axes, 'on');
        BpodSystem.GUIHandles.EncoderPlotUnwrap = plot(axes, 0,0, 'r-', 'LineWidth', 4);
        ylabel('Angle (deg)', 'FontSize', 12);

        % RIGHT AXIS (meters)
        yyaxis right
        BpodSystem.GUIHandles.EncoderPlotLinear = plot(axes, 0,0, 'g-', 'LineWidth', 2);
        ylabel('Distance (m)', 'FontSize', 12);

        % legend('Degrees', 'Unwrapped Degrees', 'Distance (m)');

        legend([BpodSystem.GUIHandles.EncoderPlot, BpodSystem.GUIHandles.EncoderPlotUnwrap, BpodSystem.GUIHandles.EncoderPlotLinear], {'Degrees', 'Unwrapped Degrees', 'Distance (m)'}, 'Location', 'best')
        % 
    ...
    case 'update'
        ...
        set(BpodSystem.GUIHandles.EncoderPlotUnwrap, 'XData', EncoderData.Times,'YData', EncoderData.PositionsUnwrapped);
        set(BpodSystem.GUIHandles.EncoderPlotLinear, 'XData', EncoderData.Times,'YData', EncoderData.LinearPositions);
        % set(axes, 'ylim', [-1 choiceThreshold*4], 'xlim', [0 TrialDuration]);
        y_max_deg = max(max(EncoderData.Positions), max(EncoderData.PositionsUnwrapped));
        y_min_deg = min(min(EncoderData.Positions), min(EncoderData.PositionsUnwrapped));

        if y_max_deg <= y_min_deg || isnan(y_min_deg) || isnan(y_max_deg)
            y_min_deg = 0;
            y_max_deg = 1;
        end

        y_max_met = max(EncoderData.LinearPositions);
        y_min_met = min(EncoderData.LinearPositions);

        if y_max_met <= y_min_met || isnan(y_min_met) || isnan(y_max_met)
            y_min_met = 0;
            y_max_met = 1;
        end        

        yyaxis left
        set(axes, 'ylim', [y_min_deg y_max_deg], 'xlim', [0 TrialDuration]);

        yyaxis right
        set(axes, 'ylim', [y_min_met y_max_met], 'xlim', [0 TrialDuration]);
