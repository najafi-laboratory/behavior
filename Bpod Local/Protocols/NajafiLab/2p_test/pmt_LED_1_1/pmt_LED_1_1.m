...
try
    ...
    for currentTrial = 1:MaxTrials
        ...
        PMTLoopInterval = PMTOpenDelay + PMTOpenTransferDelay + Frame2Remainder + Frame3;
        ...
        OptoNumPulses = 2;
        OptoFramesPerPulseSequence = 3;

        % Pulse Sequence Counter - Run trial for a param-defined number of
        % opto pulses
        CounterNumber = 1;
        TargetEventName = 'BNC1High';
        CounterThreshold = OptoNumPulses * OptoFramesPerPulseSequence;

        disp(['Counter Threshold:', num2str(CounterThreshold)])
        
        % sma = SetGlobalCounter(sma, CounterNumber, 'BNC1High', CounterThreshold);


        sma = SetGlobalCounter(sma, 1, 'BNC1High', 12);
        ...
        if currentTrial == 1
            ...
            sma = AddState(sma, 'Name', 'PrePulseSequence', ...
                'Timer', 0,...
                'StateChangeConditions', {'BNC1Low', 'FrameDetect1'},...
                'OutputActions', {'GlobalCounterReset', 1}); % Reset Pulse Sequence Counter
        end
        %'StateChangeConditions', {'Tup', 'MissedETLSignal', 'BNC1Low', 'FrameDetect1'},...
        ...
        sma = AddState(sma, 'Name', 'FrameDetect1', ...
            'Timer', 0.5,...
            'StateChangeConditions', {'Tup', 'MissedETLSignal', 'BNC1High', 'Frame1'},...
            'OutputActions', {'BNC1', 1}); %        

        % 'OutputActions', {'BNC1', 1, 'GlobalTimerTrig', '11'}); %   
        ...
        sma = AddState(sma, 'Name', 'Frame3', ...
            'Timer', 0.5,...
            'StateChangeConditions', {'Tup', 'MissedETLSignal', 'BNC1Low', 'FrameDetect1', 'GlobalCounter1_End', 'PostPulseSequence'},...
            'OutputActions', {});
        
        % 'StateChangeConditions', {'Tup', 'MissedETLSignal', 'BNC1Low', 'ITI'},...
