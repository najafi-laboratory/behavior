...
function [AntiBiasVar, LeftValveAmount_uL, RightValveAmount_uL] = AntiBiasProbeTrialsWater( ...
        obj, BpodSystem, S, AntiBiasVar, currentTrial, TrialTypes, LeftValveAmount_uL, RightValveAmount_uL)    
    if S.GUI.ProbeWaterTrialsAct
        if (currentTrial > 1 && ...
            ~AntiBiasVar.IsProbeWaterTrial && ...
            ~strcmp(AntiBiasVar.ValveFlag, 'NoBias') && ...
            (isfield(BpodSystem.Data.RawEvents.Trial{currentTrial-1}.States, 'Punish') && ...
                        ~isnan(BpodSystem.Data.RawEvents.Trial{currentTrial-1}.States.Punish(1))))            
                AntiBiasVar.IsProbeWaterTrial = true;
                % AntiBiasVar.MoveCorrectSpout     = false;
                % AntiBiasVar.NumSpoutSelectTrials = 3;
                AntiBiasVar.NumProbeWaterTrials = S.GUI.NumProbeWaterTrials;    
        end
    
        if AntiBiasVar.IsProbeWaterTrial
            if AntiBiasVar.NumProbeWaterTrials > 0
                LeftValveAmount_uL = 0;
                RightValveAmount_uL = 0;
                AntiBiasVar.NumProbeWaterTrials = AntiBiasVar.NumProbeWaterTrials - 1;
            else
                AntiBiasVar.IsProbeWaterTrial = false;            
                AntiBiasVar.NumProbeWaterTrials = S.GUI.NumProbeWaterTrials;
                % AntiBiasVar.NumSpoutSelectTrials = 3;
                LeftValveAmount_uL = S.GUI.LeftValveAmount_uL;
                RightValveAmount_uL = S.GUI.RightValveAmount_uL;            
    ...
    disp('');
    disp(['IsProbeWaterTrial: ', num2str(AntiBiasVar.IsProbeWaterTrial)]);    
    disp(['ValveFlag: ', AntiBiasVar.ValveFlag]);  
    disp(['NumProbeWaterTrials: ', num2str(AntiBiasVar.NumProbeWaterTrials)]); 
end

% anti-bias probe trials 
function [AntiBiasVar] = AntiBiasProbeTrialsSpoutSelect( ...
        obj, BpodSystem, S, AntiBiasVar, currentTrial)    
    if S.GUI.ProbeSpoutSelectTrialsAct
        if (currentTrial > 1 && ...
            ~AntiBiasVar.IsProbeSpoutSelectTrial && ...
            ~strcmp(AntiBiasVar.ValveFlag, 'NoBias') && ...
            (isfield(BpodSystem.Data.RawEvents.Trial{currentTrial-1}.States, 'Punish') && ...
                        ~isnan(BpodSystem.Data.RawEvents.Trial{currentTrial-1}.States.Punish(1))))            
                AntiBiasVar.IsProbeSpoutSelectTrial = true;
                AntiBiasVar.MoveCorrectSpout     = false;
                % AntiBiasVar.NumSpoutSelectTrials = 3;
                AntiBiasVar.NumProbeSpoutSelectTrials = S.GUI.NumProbeSpoutSelectTrials;    
        end
    
        if AntiBiasVar.IsProbeSpoutSelectTrial
            if AntiBiasVar.NumProbeSpoutSelectTrials > 0
                AntiBiasVar.MoveCorrectSpout     = true;
                AntiBiasVar.NumProbeSpoutSelectTrials = AntiBiasVar.NumProbeSpoutSelectTrials - 1;
            else
                AntiBiasVar.IsProbeSpoutSelectTrial = false;
                AntiBiasVar.MoveCorrectSpout     = false;
                AntiBiasVar.NumProbeSpoutSelectTrials = S.GUI.NumProbeSpoutSelectTrials;
                % AntiBiasVar.NumSpoutSelectTrials = 3;
            end           
    ...
    disp('');
    disp(['IsProbeSpoutSelectTrial: ', num2str(AntiBiasVar.IsProbeSpoutSelectTrial)]);    
    disp(['ValveFlag: ', AntiBiasVar.ValveFlag]);  
    disp(['MoveCorrectSpout: ', num2str(AntiBiasVar.MoveCorrectSpout)]); 
    disp(['NumProbeSpoutSelectTrials: ', num2str(AntiBiasVar.NumProbeSpoutSelectTrials)]); 
end
