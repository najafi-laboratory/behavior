...
try
    ...
    BpodSystem.SoftCodeHandlerFunction = 'SoftCodeHandler_OptoLED';
    ...
    for currentTrial = 1:MaxTrials
        ...
        % 
        % 
        % % UPDATE - these will be different for opto only
        ...
        % UPDATE - different for opto only
        % write here then abstract opto function

        % sma = m_Opto.InsertGlobalTimer(sma, S, OptoPulseDur);
        Frame1Start = 0.023;
        Frame2Remainder = 0.006;
        Frame3 = 0.033;
        ...
        LEDOnsetDelay = Frame1Start + PMTCloseDelay;
        LEDOnDur = 0.0078; % use LEDOnDur = PMTOpenDelay? we'll see depending upon how pulsed opto criteria evolve, might even arrive at a formula
        LEDLoopInterval = PMTOpenTransferDelay + Frame2Remainder + Frame3;

        PMTOnsetDelay = Frame1Start;
        PMTDur = PMTCloseDelay;
        PMTLoopInterval = PMTOpenDelay;

        % LED
        sma = SetGlobalTimer(sma, 'TimerID', 1, 'Duration', LEDOnDur, 'OnsetDelay', LEDOnsetDelay,...
            'Channel', 'PWM1', 'OnLevel', 255, 'OffLevel', 0,...
            'Loop', 1, 'SendGlobalTimerEvents', 0, 'LoopInterval', LEDLoopInterval,...
            'GlobalTimerEvents', 0, 'OffsetValue', 0);

        % PMT
        sma = SetGlobalTimer(sma, 'TimerID', 2, 'Duration', PMTDur, 'OnsetDelay', PMTOnsetDelay,...
        'Channel', 'BNC2', 'OnLevel', 1, 'OffLevel', 0,...
        'Loop', 1, 'SendGlobalTimerEvents', 0, 'LoopInterval', PMTLoopInterval,...
        'GlobalTimerEvents', 0, 'OffsetValue', 0);
        ...
        CounterThreshold = OptoNumPulses * OptoFramesPerPulse;

        disp(['Counter Threshold:', num2str(CounterThreshold)])
        
        sma = SetGlobalCounter(sma, 1, 'BNC1High', 6);

        sma = AddState(sma, 'Name', 'Start', ...
            'Timer', 0.068,...
            'StateChangeConditions', {'Tup', 'PrePulseSequence'},...
            'OutputActions', {'BNC1', 1}); % Start state sync signal
        % 'OutputActions', {'BNC1', 1}); % Start state sync signal

        sma = AddState(sma, 'Name', 'PrePulseSequence', ...
            'Timer', 0.5,...
            'StateChangeConditions', {'Tup', 'FrameDetect1'},...
            'OutputActions', {}); % Reset Pulse Sequence Counter
        % 'OutputActions', {'GlobalCounterReset', 1}); % Reset Pulse Sequence Counter
        
        %% Frame1

        % Detect rising edge of scope imaging frame
        sma = AddState(sma, 'Name', 'FrameDetect1', ...
            'Timer', 0.5,...
            'StateChangeConditions', {'Tup', 'MissedETLSignal', 'BNC1High', 'Frame1', 'GlobalTimerTrig', '11'},...
            'OutputActions', {'BNC1', 1}); %        

        sma = AddState(sma, 'Name', 'Frame1', ...
            'Timer', 0.5,...
            'StateChangeConditions', {'Tup', 'MissedETLSignal', 'BNC1Low', 'FrameDetect2'},...
            'OutputActions', {}); %       
               
        %% Frame2

        % Detect rising edge of scope imaging frame
        sma = AddState(sma, 'Name', 'FrameDetect2', ...
            'Timer', 0.5,...
            'StateChangeConditions', {'Tup', 'MissedETLSignal', 'BNC1High', 'Frame2'},...
            'OutputActions', {'BNC1', 1}); %       

        sma = AddState(sma, 'Name', 'Frame2', ...
            'Timer', 0.5,...
            'StateChangeConditions', {'Tup', 'MissedETLSignal', 'BNC1Low', 'FrameDetect3'},...
            'OutputActions', {}); % 

        %% Frame3

        % Detect rising edge of scope imaging frame
        sma = AddState(sma, 'Name', 'FrameDetect3', ...
            'Timer', 0.5,...
            'StateChangeConditions', {'Tup', 'MissedETLSignal', 'BNC1High', 'Frame3'},...
            'OutputActions', {'BNC1', 1}); %      

        sma = AddState(sma, 'Name', 'Frame3', ...
            'Timer', 0.5,...
            'StateChangeConditions', {'Tup', 'MissedETLSignal', 'BNC1Low', 'PostPulseSequence'},...
            'OutputActions', {}); %  
       % 'StateChangeConditions', {'Tup', 'PostPulseSequence', 'BNC1Low', 'CheckPulseCounter', 'GlobalCounter1_End', 'PostPulseSequence'},...
       
       % sma = AddState(sma, 'Name', 'CheckPulseCounter', ...
        %     'Timer', 0.001,...
        %     'StateChangeConditions', {'Tup', 'FrameDetect1', 'GlobalCounter1_End', 'PostPulseSequence'},...
        %     'OutputActions', {}); % 

        % sma = AddState(sma, 'Name', 'Frame3', ...
        %     'Timer', 0.5,...
        %     'StateChangeConditions', {'Tup', 'PostPulseSequence', 'BNC1Low', 'FrameDetect1', 'GlobalCounter1_End', 'PostPulseSequence'},...
        %     'OutputActions', {}); %           

        %% PostPulseSequence

        % 
        sma = AddState(sma, 'Name', 'PostPulseSequence', ...
            'Timer', 0.5,...
            'StateChangeConditions', {'Tup', 'ITI'},...
            'OutputActions', {}); %     

        sma = AddState(sma, 'Name', 'MissedETLSignal', ...
            'Timer', 0,...
            'StateChangeConditions', {'Tup', 'ITI'},...
            'OutputActions', {}); %  
