...
function [VisStim] = GetVisStimImg( ...
    ...
    VisStim.GratingOmiss.Frames = GetFrames(obj, FPS, S.GUI.GratingDur_s);
    VisStim.GratingOmiss.Video  = GetUnitVideo(obj, VisStim.Img.GrayFrame_SyncW, VisStim.GratingOmiss.Frames);
    VisStim.GratingOmiss.Dur    = GetVideoDur(obj, FPS, VisStim.GratingOmiss.Video);    
end


function [VisStim] = GetVideoDataPre( ...
        obj, S, BpodSystem, FPS, VisStim, GrayPrePerturbISI, RandomISI)
    VideoSeq = [];
    PreISIinfo = [];
...
function [VisStim] = GetVideoDataPost( ...
        obj, S, BpodSystem, FPS, VisStim, GrayPerturbISI, RandomISI, OmissIdx, OddballISI)
    VideoSeq = [];
    PostISIinfo = [];
    ...
    for numImages = 1:PostPertFlashes
        ...
        if S.GUI.PrePertFlashRep > 0
            ...
            if numImages < PostPertFlashes
                if (numImages == OmissIdx) && (BpodSystem.Data.TrialTypes(BpodSystem.Data.CurrentTrial) == 1)
                    VisStim.Data.PostUnit.Video  = [VisStim.GratingOmiss.Video VisStim.GrayPost.Video];
                    Seq = [ones(1, length(VisStim.GratingOmiss.Video)) * VisStim.SampleGratingIdx zeros(1, length(VisStim.GrayPost.Video))];
                else
                    VisStim.Data.PostUnit.Video  = [VisStim.Grating.Video VisStim.GrayPost.Video];
...
function [FullAudio] = GenAudioStim( ...
        obj, S, VisStim, SF, Envelope, BpodSystem, OmissIdx)
    ...
    if ~isempty(VisStim.PostISIinfo)
        for i = 1 : length(VisStim.PostISIinfo)
            ...
            if S.GUI.PrePertFlashRep > 0
                ...
                if (i == OmissIdx) && (BpodSystem.Data.TrialTypes(BpodSystem.Data.CurrentTrial) == 1)
