...
if isempty(fieldnames(S))  % If settings file was an empty struct, populate struct with default settings
    ...
    S.GUIMeta.TrainingLevel.String = {'Naive', 'Mid 1 Trained', 'Mid 2 Trained', 'Well Trained'};
    ...
    S.GUI.CenterLickWindow_Trained_s = 0.5;   
...
for currentTrial = 1:MaxTrials
    ...
    switch S.GUI.TrainingLevel
        case 1 % naive       
            ...
            InitCue_Tup_NextState = 'InitWindow'; % on 8/30/2023 Farzaneh removed initiReward because mouse may get an inconsistent message in naive vs the rest of the stages.
            ...
            PunishSetup_Tup_NextState = 'Punish'; % Naive
        ...
        case 2 % Mid 1 Trained
            ...
            if ShowDebugOutput
                disp('Training Stage - Mid 1 Trained');
            end
            
            % mid 1: mouse receives water for having initiated the trial
           ...
           % PreGoCueDelay_OutputActions = {'SoftCode', 255}; % stop vis stim in PreGoCueDelay so its init and perturb segment durations are equal for well trained
            ...
            ExperimenterTrialInfo.TrainingLevel = 'Mid 1 Trained';


        case 3 % Mid 2 Trained
            
            if ShowDebugOutput
                disp('Training Stage - Mid 2 Trained');
            end
           ...
           % PreGoCueDelay_OutputActions = {'SoftCode', 255}; % stop vis stim in PreGoCueDelay so its init and perturb segment durations are equal for well trained
            ...
            ExperimenterTrialInfo.TrainingLevel = 'Mid 2 Trained';
    ...
    sma = AddState(sma, 'Name', 'InitWindow', ...
        ...
        'StateChangeConditions', {'Tup', 'InitCueAgain', 'Port2In', 'CenterRewardPostInit', 'Port1In', 'WrongInitiation', 'Port3In', 'WrongInitiation'},...         
        'OutputActions', {});    
   ...
   % Visual Stimulus Played before Audio; Switch to both after photodiode
   % reads a visual stimulus
    sma = AddState(sma, 'Name', 'VisStimTrigger', ...
        'Timer', S.GUI.VisStimTrigger_s,...
        'StateChangeConditions', {'BNC1', 'VisualStimulus'},...
        'OutputActions', {'SoftCode', 5}); 

    % VisualStimulus (plays the audio directly after video is noticed;
    % reduce delay)
    ...
    sma = AddState(sma, 'Name', 'WindowCenterLick', ...
        'Timer', S.GUI.CenterLickWindow_Trained_s,...
        'StateChangeConditions', {'Tup', 'DidNotLickCenter', 'Port2In', 'CenterRewardPostVisStim', 'Condition5', 'CenterRewardPostVisStim', 'Port1In', 'EarlyChoiceDuringWindCenter', 'Port3In', 'EarlyChoiceDuringWindCenter'},...
        'OutputActions', OutputActionsCenterLick); 

    sma = AddState(sma, 'Name', 'CenterRewardPostInit', ...
        'Timer', CenterValveTime,...
        'StateChangeConditions', {'Tup', 'PreVisStimDelay'},...
        'OutputActions', CenterReward_OutputActions);


    sma = AddState(sma, 'Name', 'CenterRewardPostVisStim', ...
        'Timer', CenterValveTime,...
    ...
    sma = AddState(sma, 'Name', 'PreGoCueDelay', ...
        ...
        'StateChangeConditions', {'Tup', 'GoCue', 'Port1In', 'EarlyChoiceDuringGoCueDelay', 'Port3In', 'EarlyChoiceDuringGoCueDelay'},...         
    ...
    sma = AddState(sma, 'Name', 'EarlyChoiceDuringWindCenter', ...
        'Timer', 0,...
        'StateChangeConditions', {'Tup', 'ITI'},...
        'OutputActions', {});
...
%'Timer', ValveTime,...
   ...
   %'Timer', S.GUI.WindowRewardGrabDuration_Naive_s,...
    ...
    % Notes for later: perhaps we want to make sure during RewardDelay they
    % don't lick the wrong side.
...
if isfield(Data, 'nTrials')
    ...
    for x = 1:Data.nTrials
        ...
        elseif ~isnan(Data.RawEvents.Trial{x}.States.EarlyChoice(1))
            ...
            Outcomes(x) = 3;    % draws clear circle on outcome plot
        elseif ~isnan(Data.RawEvents.Trial{x}.States.EarlyChoiceDuringWindCenter(1))
            if (x == Data.nTrials && isEndOfTrial)  % only print outcome to console for the trial that just occured
                disp(['Outcome: EarlyChoiceDuringWindCenter']);
            end
            Outcomes(x) = 3;    % draws clear circle on outcome plot
