...
% update total amount of water delivered
function [TotalRewardAmount_uL] = GetTotalRewardAmount( ...
        obj, BpodSystem, S, TotalRewardAmount_uL, ...
        currentTrial, TrialTypes, LeftValveAmount_uL, RightValveAmount_uL)
    if (currentTrial > 1)
        if (isfield(BpodSystem.Data.RawEvents.Trial{currentTrial-1}.States, 'RewardNaive') && ...
                ~isnan(BpodSystem.Data.RawEvents.Trial{currentTrial-1}.States.RewardNaive(1)))
            if (TrialTypes(currentTrial-1) == 1)
                TotalRewardAmount_uL = TotalRewardAmount_uL + LeftValveAmount_uL;
            elseif (TrialTypes(currentTrial-1) == 2)
                TotalRewardAmount_uL = TotalRewardAmount_uL + RightValveAmount_uL;
            end
