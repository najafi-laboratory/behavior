...
if isempty(fieldnames(S))  % If settings file was an empty struct, populate struct with default settings
    ...
    S.GUI.DefineContingencyForCase1 = 1;    % define contingency for case 1 (thus case 2 is the other)
                                            % 1 - 'Case 1 is short ISI (case 2 is long)', 2 - 'Case 1 is long ISI (case 2 is short)'
    S.GUIMeta.DefineContingencyForCase1.Style = 'popupmenu'; % the GUIMeta field is used by the ParameterGUI plugin to customize UI objects.
    S.GUIMeta.DefineContingencyForCase1.String = {'Short ISI', 'Long ISI'};
    S.GUI.RewardedSideForCase1 = 1; % set which side is reward for trial type case 1 (thus the other is punish side for case 2)
                             % 1 - 'Left', 2 - 'Right'
    S.GUIMeta.RewardedSideForCase1.Style = 'popupmenu'; % the GUIMeta field is used by the ParameterGUI plugin to customize UI objects.
    S.GUIMeta.RewardedSideForCase1.String = {'Left', 'Right'};
    S.GUI.Case1Fraction = 0.5;    % probability of selecting this trial as case 1
    S.GUI.ManualCaseSelect = 1;   % manual override to set contingency case for next trial
    S.GUIMeta.ManualCaseSelect.Style = 'checkbox';
    S.GUI.ManualCase = 1;
    S.GUIMeta.ManualCase.Style = 'popupmenu'; % the GUIMeta field is used by the ParameterGUI plugin to customize UI objects.
    S.GUIMeta.ManualCase.String = {'Case 1', 'Case 2'};
    S.GUIPanels.Reward = {'LeftValveTime_s', 'RightValveTime_s', 'CenterValveTime_s', 'WindowRewardGrabDuration_Naive_s', 'RewardDelay_s', 'DefineContingencyForCase1', 'RewardedSideForCase1', 'Case1Fraction', 'ManualCaseSelect', 'ManualCase'};
...
SideOutcomePlot(BpodSystem.GUIHandles.SideOutcomePlot,'init',2-TrialSidesRewarded);
...
for currentTrial = 1:MaxTrials
    ...
    %% account for anti-bias settings
    % set trial type for this trial if needed
    switch S.GUI.ManualCaseSelect
        case 0  % use case fraction settings if the input fraction is not 50%        
            if (S.GUI.Case1Fraction ~= 0.5)
                % define discrete case values in distribution
                Cases = [1, 2];
                % assigned probability weights for sampling from distribution
                CaseProbabilities = [S.GUI.Case1Fraction, 1-S.GUI.Case1Fraction];
                ...
                cp = [0, cumsum(CaseProbabilities)]; % cumulative probability -> use as interval to pick Easy OR Medium OR Hard

                r = rand; % get random scalar drawn from the uniform distribution in the interval (0,1).
                ind = find(r>cp, 1, 'last');  % get discrete index (1 or 2 for case 1 or case 2)
                TrialTypes(currentTrial) = Cases(ind); % get discrete value at the randomly (according to probability weights) selected index, in this case of 1 = Case 1, 2 = Case 2, it will be the same as the index.  This step is here in case more or fewer cases are added in the future, this gets used as example for drawing from weighted distribution later, or any other unforseen reason
            end
        case 1  % use manual case selection
            TrialTypes(currentTrial) = S.GUI.ManualCase;
    end

    switch TrialTypes(currentTrial)
        case 1
            ExperimenterTrialInfo.TrialCase = 'Case 1';   % capture variable states as field/value struct for experimenter info            
        case 2
            ExperimenterTrialInfo.TrialCase = 'Case 2';   % capture variable states as field/value struct for experimenter info
    end

    %% update rewarded side based on case and gui params
    switch S.GUI.RewardedSideForCase1
        case 1 % left side rewarded for case 1
            TrialSidesRewarded(currentTrial:end) = TrialTypes(currentTrial:end);
            ExperimenterTrialInfo.RewardedSideForCase1 = 'Left';   % capture variable states as field/value struct for experimenter info
        case 2 % right side rewarded for case 1
            TrialSidesRewarded(currentTrial:end) = (3-TrialTypes(currentTrial:end));
            ExperimenterTrialInfo.RewardedSideForCase1 = 'Right';   % capture variable states as field/value struct for experimenter info
    end
    ...
    %% define contigency
    % S.GUI.DefineContingencyForCase1
    % switch S.GUI.DefineContingencyForCase1
    %     case 1  % Case 1 is short ISI (case 2 is long)
    % 
    %     case 2  % Case 1 is long ISI (case 2 is short)
    % 
    % end

    % define reward according to contingency
    switch S.GUI.RewardedSideForCase1
        case 1 % Left is rewarded for case 1
        ...
        case 2 % Right is rewarded for case 1
            switch TrialTypes(currentTrial) % Determine trial-specific state matrix fields
                case 1 % right side is rewarded
                    CorrectLick = 'Port3In';
                    CorrectPort = 'Port3';
                    IncorrectLick = 'Port1In';
                    IncorrectPort = 'Port1';
    ...
    % S.GUI.Case1Fraction = 0.5;
    % 
    % 
    % 
    % 'RewardedSideForCase1', 'Case1Fraction', 'ManualCaseSelect', 'ManualCase'
    ...
    %         IncorrectPort = 'Port3';
    %         %StimulusOutput = {'SoftCode', 3};
    %         %RewardPortOut = 'Port1Out'; ValveTime = LeftValveTime; 
    %         Valve = 'Valve1';
    %         %CorrectWithdrawalEvent = 'Port1Out';
    %         %VisualGratingDuration = S.GUI.LDuration; % length of time to play visual stimulus when Left TrialType
    % 
    ...
    %         IncorrectPort = 'Port1';
    %         %StimulusOutput = {'SoftCode', 4};
    %         %RewardPortOut = 'Port3Out'; ValveTime = RightValveTime; 
    %         Valve = 'Valve3';
    %         %CorrectWithdrawalEvent = 'Port3Out';
    %         %VisualGratingDuration = S.GUI.RDuration; % length of time to play visual stimulus when Right TrialType
    % 
    ...
    %UpdateSideOutcomePlot(TrialTypes, BpodSystem.Data); % this will update the SideOutcomePlot to reflect the current trial type after accounting for any change due to anti-bias control 
    UpdateSideOutcomePlot(TrialSidesRewarded, BpodSystem.Data); % this will update the SideOutcomePlot to reflect the current trial type after accounting for any change due to anti-bias control 
    ...
    switch TrialTypes(currentTrial) % Determine trial-specific visual stimulus duration
        case 1 % trial is case 1
            ...
            switch S.GUI.DefineContingencyForCase1
                case 1
                    GrayPerturbDuration = CatBound - RandomPerturbationDur; % case 1 is short, case 2 is long
                    ExperimenterTrialInfo.ISIDurForCase1 = 'Short';   % capture variable states as field/value struct for experimenter info
                    ExperimenterTrialInfo.ISIDur = 'Short';   % capture variable states as field/value struct for experimenter info
                case 2
                    GrayPerturbDuration = CatBound + RandomPerturbationDur; % case 1 is long, case 2 is short
                    ExperimenterTrialInfo.ISIDurForCase1 = 'Long';   % capture variable states as field/value struct for experimenter info
                    ExperimenterTrialInfo.ISIDur = 'Long';   % capture variable states as field/value struct for experimenter info
            end
        case 2 % trial is case 2
    ...
    %GrayPerturbDuration = CatBound + RandomPerturbationDur; % random duration is subtracted or added according to the defined contingency
    ...
    % this used to be where left or right as correct incorrect was assigned
    % updated to be consistent with standards of defining contigency
...
%'Timer', ValveTime,...
   ...
   %'Timer', S.GUI.WindowRewardGrabDuration_Naive_s,...
    ...
    if ~isempty(fieldnames(RawEvents)) % If trial data was returned (i.e. if not final trial, interrupted by user)
        ...
        UpdateSideOutcomePlot(TrialSidesRewarded, BpodSystem.Data);        
