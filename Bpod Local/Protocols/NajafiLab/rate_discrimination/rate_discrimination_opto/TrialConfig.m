    ...
    elseif updateTrialTypeSequence
        switch S.GUI.TrialTypeSequence
            case 1 % interleaved - random
                numOnTrialsToAdd = [];
                TrialTypesToAdd = [repmat(1, 1, MaxTrials - currentTrial + 1)];                
                numOnTrialsToAdd = ceil(S.GUI.OnFraction * length(TrialTypesToAdd));                
                numOnTrialsToAdd_idxs = randperm(length(TrialTypesToAdd), numOnTrialsToAdd);
                TrialTypesToAdd(numOnTrialsToAdd_idxs) = 2;
                TrialTypes(currentTrial:end) = TrialTypesToAdd;                
