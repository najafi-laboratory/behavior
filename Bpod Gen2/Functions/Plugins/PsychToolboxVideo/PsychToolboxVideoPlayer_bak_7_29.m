...
classdef PsychToolboxVideoPlayer < handle
    ...
    methods
        ...
        function playNextFrame(obj, isTimer, timer1, timer2) 
            ...
            delayTime = obj.ifi - obj.toc1;
            if delayTime < 0    % need to drop frame
                framesToDrop = floor(abs(obj.ifi/delayTime));
                nextFrame = thisFrame+1+framesToDrop;

                r = rem(abs(delayTime), obj.ifi);
                %delayTime = obj.ifi + delayTime;
                delayTime = obj.ifi - r;
            else                
                nextFrame = thisFrame+1;
            end
            
            if delayTime < 0.001    % can't set sub-millisecond delay
                delayTime = 0;                
            end
            ...
            if nextFrame > obj.Videos{obj.StimulusIndex}.nFrames
                ...
                stop(obj.Timer2);
                set(obj.Timer2, 'TimerFcn', '');
                % added reset to blankscreen since sometimes frames at end
                % are dropped
                Screen('DrawTexture', obj.Window, obj.BlankScreen);
                Screen('Flip', obj.Window);
            end
            ...
            set(nextTimer,'StartDelay', round(delayTime, 3));
            start(nextTimer);
            obj.tic1 = tic;
        end        
    end
