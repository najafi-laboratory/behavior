...
for iCycle = 1:nCycles
    ...
    while GetSecs < tEnd
        ...
        rotatedGrating = imrotate(sinGrating, orientation, 'crop', 'bilinear');
        rotatedGratingTexture = Screen('MakeTexture', win, rotatedGrating);
        if FullScreen == true
            Screen('DrawTexture', win, rotatedGratingTexture, [], winRect);
        else
            Screen('DrawTexture', win, rotatedGratingTexture);
        end
        Screen('Close', rotatedGratingTexture);
        Screen('Flip', win);
    end
