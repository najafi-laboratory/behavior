...
MonitorID = 2;
BpodSystem.PluginObjects.V = PsychToolboxVideoPlayer(MonitorID, 0, [0 0], [180 180], 0, 1); % Assumes second monitor is screen #2. Sync patch = 180x180 pixels
...
VideoData = [repmat(VideoPrePerturbPattern, 1, S.GUI.NumISIOrigRep) GratingVideo GratingBlank]; % add one more grating video after initial repetitions
VideoDataIndex = 5;
...
BpodSystem.PluginObjects.V.Videos{5}.nFrames = length(VideoData); % + 1 for final frame
BpodSystem.PluginObjects.V.Videos{5}.Data = VideoData;

BpodSystem.PluginObjects.V.delete();
...
while isempty(T)
    ...
    try
        ...
        disp("could not connect to video server, yet...");
    end 
end
...
while ~KbCheck
    ...
    T.write([1 [length(VideoData) VideoDataIndex] VideoData]);
    disp(['i:', num2str(i)]);
    i = i + 1;

    T.write([2 1 VideoDataIndex]); % command, play, index

    pause(1.5);

    T.write([2 2]); % command, play, index
