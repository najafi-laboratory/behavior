...
if isempty(fieldnames(S))  % If settings file was an empty struct, populate struct with default settings
    ...
    S.GUI.TrainingLevel = 2; % Configurable training and test schemes.
                             % 1 - 'Naive', 2 - 'Trained'
    S.GUIMeta.TrainingLevel.Style = 'popupmenu'; % the GUIMeta field is used by the ParameterGUI plugin to customize UI objects.
    ...
    S.GUIPanels.Difficulty = {'PercentTrialsEasy', 'PercentTrialsMedium', 'PercentTrialsHard'};
    ...
    S.GUIPanels.VisStim = {'GratingDur_s', 'ISIOrig_s', 'ExtraStimDurPostRew_Naive_s', 'NumISIOrigRep', 'MinISIPerturb_ms', 'PreVisStimDelay_s', 'PreGoCueDelay_s'}; 
...
spatialFreq = .002; % Spatial frequency of grating in cycles per pixel
...
for currentTrial = 1:MaxTrials
    ...
    % full range of perturbation is calculated as the ISI time (category
    % boundary in this case) minus the selected minimum time separation from the category boundary and the grating
    ...
    disp('GrayPerturbDuration (perturb ISI):');
    disp(GrayPerturbDuration);
