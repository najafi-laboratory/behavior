...
classdef PsychToolboxVideoPlayer < handle
    ...
    methods
        % TCPmode 0 = server, 1 = client
        function obj = PsychToolboxVideoPlayer(MonitorID, ViewPortSize, ViewPortOffset, SyncPatchSize, SyncPatchYOffset, TCPMode)
            switch TCPMode
                case 0
        ...
        function play(obj, StimulusIndex)
            obj.tic1 = tic; % get tic prior to playing video to account for processing overhead when using TimerMode 2
            if StimulusIndex == 0
                Screen('DrawTexture', obj.Window, obj.BlankScreen);
                Screen('Flip', obj.Window);
            else
